...
- **Press vs Retreat Decision (Wave 24).** Before containment, the defender evaluates whether to pursue full-court press or fall back to halfcourt using `evaluatePressDecision()`. The decision weighs:
  - **Catchup potential**: Speed + turbo differential between defender and ball handler. Fast defender vs slow handler = more likely to press.
  - **Game situation**: Trailing teams press more (desperation mode at 10+ points behind). Teams with big leads may press too (low risk). Time pressure increases press likelihood when trailing.
  - **Threat assessment**: Good shooters (3PT > 7) discourage pressing since blowby = open three.
  - **Rubber banding**: Active boost increases press likelihood.
  - If `pressScore < 0.4`, defender retreats to ~25 tiles from basket (halfcourt) instead of chasing into backcourt.
- **Containment geometry.** When pressing, the defender parks `DEFENSE_ON_BALL.containDistance` tiles ahead of the handler along the vector to the defender's basket, adding a bit of jitter so it doesn't look perfectly robotic.
- **Court-position-aware reactions (Wave 24).** Defenders now react slower when far from their own basket, simulating the risk/reward of full-court pressure:
...
- **Momentum smoothing.** The defender's target position is now smoothed via `applyDefenderMomentum()` (same system help defense uses), preventing instant snapping to new positions.
