...
print("=== Fixing Nemesis/Abandoned Child Support Balances (v2) ===\n");
...
for (var playerId in players) {
    ...
    for (var i = 0; i < player.babyBallers.length; i++) {
        ...
        if (baby.isNemesis || baby.parentingMode === "abandon") {
            // Check if not already marked as abandoned OR has non-zero balance
            if (!baby.childSupport.isAbandoned || baby.childSupport.balance > 0) {
    ...
    if (needsUpdate) {
            ...
            for (var j = 0; j < player.babyMamas.length; j++) {
                ...
                for (var k = 0; k < player.babyBallers.length; k++) {
                    ...
                    if ((b.motherId === mama.id || b.motherName === mama.name) && 
                        (!b.childSupport || !b.childSupport.isAbandoned)) {
                        newBalance += (b.childSupport ? b.childSupport.balance : 0);
                    }
                }
        ...
        if (player.parentingStats) {
            ...
            for (var m = 0; m < player.babyBallers.length; m++) {
                ...
                if (!baby.childSupport || !baby.childSupport.isAbandoned) {
                    newTotalOwed += (baby.childSupport ? baby.childSupport.balance : 0);
                }
            }
        ...
        // CRITICAL: Force mark as updated and write back to database
        db.masterData.data.players[playerId] = player;
...
if (totalFixed > 0) {
    ...
    print("No fixes needed - all nemesis/abandoned children already properly marked\n");
}
