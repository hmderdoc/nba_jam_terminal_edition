...
// Load debugLog for diagnostic logging
...
(function() {
     ...
     *   - canLiveChallenge: boolean - whether opponent is online for live match
     *   - isSelf: boolean - whether this is the current player's own card
     * @returns {string|null} - "challenge" or "live_challenge" if user wants to challenge, null otherwise
     */
    function showForPlayer(player, options) {
        options = options || {};
        debugLog("[STATS_VIEW] showForPlayer called, player=" + (player ? player.name : "null") + ", options=" + JSON.stringify(options));
        if (RichView) {
    ...
    function showForPlayerRichView(player, options) {
        options = options || {};
        debugLog("[STATS_VIEW] showForPlayerRichView, canChallenge=" + options.canChallenge + ", isSelf=" + options.isSelf);
        var hLine = repeatChar(HLINE, 78);
        var showPvpStats = false;  // Toggle between career and PvP stats
        ...
        while (true) {
            ...
            if (footerFrame) {
                footerFrame.gotoxy(1, 1);
                var toggleLabel = showPvpStats ? "Career" : "PvP";
                var showGhost = options.canChallenge && !options.isSelf;
                var showLive = options.canLiveChallenge && !options.isSelf;
                var pvpHint = "\1h\1w[P]\1n\1c " + toggleLabel + "   ";
                if (showGhost && showLive) {
                    footerFrame.putmsg("  " + pvpHint + "\1h\1y[L]\1n\1w Live   \1h\1y[C]\1n\1w Ghost Match   \1h\1w[Q]\1n\1c Back\1n");
                } else if (showLive) {
                    footerFrame.putmsg("  " + pvpHint + "\1h\1y[L]\1n\1w Live Challenge   \1h\1w[Q]\1n\1c Back\1n");
                } else if (showGhost) {
                    footerFrame.putmsg("  " + pvpHint + "\1h\1y[C]\1n\1w Ghost Match   \1h\1w[Q]\1n\1c Back\1n");
            ...
            while (true) {
                var key = console.getkey(K_NOSPIN);
                debugLog("[STATS_VIEW] Key pressed: '" + key + "' (code=" + key.charCodeAt(0) + "), canChallenge=" + options.canChallenge + ", isSelf=" + options.isSelf);
                if (key === "q" || key === "Q" || key === "\x1B") {
                    debugLog("[STATS_VIEW] Returning null (quit)");
                    view.close();
                    return null;
                }
                if (key === "p" || key === "P") {
                ...
                if ((key === "l" || key === "L") && options.canLiveChallenge && !options.isSelf) {
                    debugLog("[STATS_VIEW] Returning live_challenge");
                    view.close();
                    return "live_challenge";
                }
                if ((key === "c" || key === "C") && options.canChallenge && !options.isSelf) {
                    debugLog("[STATS_VIEW] Returning challenge");
                    view.close();
                    return "challenge";
                }
            }
