...
(function() {
    ...
    // Load talk show host configuration
    var _talkShowConfigLoaded = false;
    try {
        load(LIB_PATH + "lorb/config/talk-shows.js");
        _talkShowConfigLoaded = (typeof LORB !== "undefined" && LORB.Config && LORB.Config.TalkShows);
        tvLog("TalkShows config loaded: " + _talkShowConfigLoaded);
    } catch (e) {
        tvLog("TalkShows config load FAILED: " + e);
    }
    
    // Helper to get host config (uses new config or falls back to defaults)
    function getHostConfig(hostKey) {
        if (_talkShowConfigLoaded && LORB.Config.TalkShows.getHost) {
            return LORB.Config.TalkShows.getHost(hostKey);
        }
        return null;
    }
    ...
    function getHostForEvent(eventType, eventSubtype) {
        if (_talkShowConfigLoaded && LORB.Config.TalkShows.getHostForEvent) {
            return LORB.Config.TalkShows.getHostForEvent(eventType, eventSubtype);
        }
        return null;
    }
    ...
    function present(opts) {
        ...
        if (shouldShowSplash) {
            ...
            if (!splashText && _talkShowConfigLoaded && opts.eventType) {
                splashText = LORB.Config.TalkShows.getSplashText(opts.eventType, opts.eventSubtype);
            }
