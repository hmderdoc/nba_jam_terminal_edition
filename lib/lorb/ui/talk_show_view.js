    ...
    function present(opts) {
        ...
        if (choices.length > 0) {
            ...
            if (typeof FrameLightbar !== "function") {
                ...
                render();
                var fallbackKey = console.getkey(K_NOSPIN).toUpperCase();
        ...
        // Cleanup - clear frames before closing to remove artifacts
        contentFrame.clear();
        artFrame.clear();
        headerFrame.clear();
        mainFrame.clear();
        mainFrame.cycle();  // Push cleared state to terminal
        ...
        return { choiceKey: selectedKey, show: show };
            ...
            tvLog("EXCEPTION: " + e);
            if (contentFrame) try { contentFrame.clear(); contentFrame.close(); } catch(x) {}
            if (artFrame) try { artFrame.clear(); artFrame.close(); } catch(x) {}
            if (headerFrame) try { headerFrame.clear(); headerFrame.close(); } catch(x) {}
            if (mainFrame) try { mainFrame.clear(); mainFrame.cycle(); mainFrame.close(); } catch(x) {}
