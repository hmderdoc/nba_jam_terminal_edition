...
(function() {
     ...
     * The y position represents the sprite's feet - sprite renders upward from there.
     */
    SpriteWanderer.prototype.isWalkable = function(x, y) {
        // Sprite occupies x to x+width-1
        // y is the feet position, sprite renders upward to y-height+1
        var right = x + this.spriteWidth - 1;
        
        // Feet (y position) must be inside at least one walkable zone
    ...
    /**
     * Apply edge transparency to a sprite frame.
     * For each bearing in the sprite sheet, checks all cells except protected "middle" 
     * cells and clears any that are "black on black" (empty/transparent in the source).
     * This allows the background art to show through around the sprite edges.
     * 
     * Protected cells (1-based, per bearing):
     *   Row 3: Cols 2, 3, 4
     *   Row 4: Cols 2, 4
     * 
     * @param {Frame} frame - The sprite frame with full sheet loaded
     * @param {Object} spriteData - Parsed sprite data with dimensions
     */
    SpriteWanderer.prototype.applyEdgeTransparency = function(frame, spriteData) {
        if (!frame || !spriteData) return;
        
        var width = spriteData.totalWidth;
        var bearingHeight = spriteData.height;
        
        // Protected cells within each bearing (0-based: row 2 cols 1-3, row 3 cols 1,3)
        // These are the "middle" cells that should never be transparent
        function isProtected(localRow, localCol) {
            // Row 3 (0-based: 2), Cols 2,3,4 (0-based: 1,2,3)
            if (localRow === 2 && localCol >= 1 && localCol <= 3) return true;
            // Row 4 (0-based: 3), Cols 2,4 (0-based: 1,3)
            if (localRow === 3 && (localCol === 1 || localCol === 3)) return true;
            return false;
        }
        
        // For each bearing in the sheet
        for (var bearingIdx = 0; bearingIdx < spriteData.bearings.length; bearingIdx++) {
            var baseY = bearingIdx * bearingHeight;
            
            // Check each cell of this bearing
            for (var localRow = 0; localRow < bearingHeight; localRow++) {
                var y = baseY + localRow;
                
                for (var x = 0; x < width; x++) {
                    // Skip protected middle cells
                    if (isProtected(localRow, x)) continue;
                    
                    var cell = frame.getData(x, y, false);
                    if (!cell) continue;
                    
                    // Check for black-on-black or empty space on black
                    var attr = cell.attr || 0;
                    var ch = cell.ch || ' ';
                    var fg = attr & 0x0F;        // Lower 4 bits = foreground
                    var bg = (attr >> 4) & 0x07; // Bits 4-6 = background
                    
                    // Clear if: black on black, OR space/empty on black bg
                    if ((fg === 0 && bg === 0) || 
                        ((ch === ' ' || ch === '\0' || ch === '') && bg === 0)) {
    ...
    /**
     * Apply jersey colors to entire sprite sheet (all bearings at once)
     * Called once after loading the sprite, before any scrolling.
     * 
     * Sprite layout per bearing (5x4):
     *   Row 0: head top
     *   Row 1: face/eyes  
     *   Row 2: jersey (cols 1,3 = number digits, col 2 = neck half-block)
     *   Row 3: legs/shorts (cols 1,3 = shorts half-blocks)
     * 
     * Bearings are stacked vertically, so we apply mask every 4 rows.
     * 
     * @param {Frame} frame - The sprite frame with full sheet loaded
     * @param {Object} jerseyConfig - { jerseyBg, accentFg, jerseyNumber }
     * @param {Object} spriteData - Parsed sprite data with dimensions and bearing count
     */
    SpriteWanderer.prototype.applyJerseyMask = function(frame, jerseyConfig, spriteData) {
        ...
        traceLog("applyJerseyMask: jerseyBg input=" + jerseyConfig.jerseyBg + " parsed=" + jerseyBg);
        if (jerseyBg === null) {
            traceLog("applyJerseyMask: No jersey color, skipping");
            return;  // No jersey color to apply
        }
        
        // Parse accent color (for number digits)
        var accentFg = this.parseColor(jerseyConfig.accentFg, "");
        traceLog("applyJerseyMask: accentFg input=" + jerseyConfig.accentFg + " parsed=" + accentFg);
        ...
        traceLog("applyJerseyMask: digitsAttr=" + digitsAttr + " (accentFg=" + accentFg + " | jerseyBg=" + jerseyBg + ")");
        var height = spriteData.height || 4;
        var bearingCount = spriteData.bearings ? spriteData.bearings.length : 1;
        var totalHeight = height * bearingCount;
        
        var topNeckChar = ascii(223);  // ▀ - upper half block for neck
        var shortsChar = ascii(220);   // ▄ - lower half block for shorts
        
        traceLog("applyJerseyMask: applying to " + bearingCount + " bearings, totalHeight=" + totalHeight);
        
        // Apply mask to every bearing (every 4 rows)
        for (var yOffset = 0; yOffset < totalHeight; yOffset += height) {
            ...
            // Middle neck (x=2): half-block with skin fg, jersey bg
            ...
            // Row 3 (shorts row): columns 1, 2, 3 (all three are shorts half-blocks)
            var shortsY = yOffset + 3;
            
            // Left leg at x=1: half-block with shoe fg, jersey bg
            ...
            // Middle (x=2): half-block with shoe/leg fg, jersey bg
            ...
            // Right leg at x=3: half-block with shoe fg, jersey bg
    ...
    SpriteWanderer.prototype.parseColor = function(colorStr, prefix) {
        traceLog("parseColor: input='" + colorStr + "' prefix='" + prefix + "'");
        if (colorStr === null || colorStr === undefined) {
            traceLog("parseColor: null/undefined, returning null");
            return null;
        }
        if (typeof colorStr === "number") {
            traceLog("parseColor: already a number, returning " + colorStr);
            return colorStr;
        }
        
        var str = String(colorStr).toUpperCase();
        traceLog("parseColor: normalized str='" + str + "'");
        
        // Color name to value map (foreground colors 0-15)
        var colors = {
        ...
        // Background versions (16, 32, 48, etc.)
        var bgColors = {
            "BG_BLACK": 0, "BG_BLUE": 16, "BG_GREEN": 32, "BG_CYAN": 48,
            "BG_RED": 64, "BG_MAGENTA": 80, "BG_BROWN": 96, "BG_LIGHTGRAY": 112
        };
        
        // If input already has BG_ prefix, it's explicitly a background color
        if (bgColors[str] !== undefined) {
            traceLog("parseColor: found in bgColors (explicit BG_), returning " + bgColors[str]);
            return bgColors[str];
        }
        
        // Check if it's a known foreground color name
        if (colors[str] !== undefined) {
            // If expecting background (prefix="BG_"), convert fg value to bg position
            if (prefix === "BG_") {
                var shifted = colors[str] << 4;
                traceLog("parseColor: shifting fg '" + str + "' to bg, returning " + shifted);
                return shifted;
            }
            // Otherwise return as foreground
            traceLog("parseColor: found in colors as fg, returning " + colors[str]);
            return colors[str];
        }
        
        traceLog("parseColor: not found, returning null");
        return null;
    };
    ...
    SpriteWanderer.prototype.init = function() {
        ...
        for (var i = 0; i < this.spriteConfigs.length; i++) {
            ...
            try {
                ...
                // Apply edge transparency (clear black-on-black border cells)
                this.applyEdgeTransparency(spriteFrame, spriteData);
                
                traceLog("Created child frame at (" + startX + "," + startY + "), parent=" + (spriteFrame.parent ? "exists" : "null"));
                
                // Build jersey config from sprite config and apply to full sheet BEFORE scrolling
                ...
                if (config.jerseyBg) {
                    ...
                    // Apply jersey mask to entire sprite sheet (all bearings)
                    this.applyJerseyMask(spriteFrame, jerseyConfig, spriteData);
                    traceLog("Applied jersey mask: bg=" + config.jerseyBg + " num=" + config.jerseyNumber);
                }
                
                // Scroll to initial bearing (after masking)
    ...
    SpriteWanderer.prototype.moveSprite = function(index) {
        ...
        if (s.bearing !== chosenDir.bearing) {
            ...
            // Scroll to show the new bearing (jersey already masked on full sheet)
