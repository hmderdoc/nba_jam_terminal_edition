...
(function () {
    ...
    LORB.View = {
        ...
        // Single key input - waits for one keypress and returns it immediately
        // Does not require Enter key. Good for Y/N, menu choices, etc.
        getKey: function (promptText) {
            ensureFrame();
            if (promptText) {
                mainFrame.putmsg(promptText);
                render();
            ...
            // Echo the key unless it's a control character
            if (key && key.charCodeAt(0) >= 32) {
                mainFrame.putmsg(key);
            }
            mainFrame.crlf();
            cursorY++;
            render();
        ...
        // Single key input from a set of valid keys (case-insensitive)
        // Returns the key pressed (uppercase) or null if invalid
        getKeys: function (promptText, validKeys) {
            ensureFrame();
            if (promptText) {
                mainFrame.putmsg(promptText);
                render();
            ...
            // console.getkeys takes a string of valid keys
            var key = console.getkeys(validKeys || "", 0);
