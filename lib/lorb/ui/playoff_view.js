...
(function() {
    ...
    // 2-team: simple finals only
    function renderBracket2(frame, rounds, bracket) {
        var finals = rounds["finals"] || [];
        var f = finals[0] || {};
        // Text row 5, line row 6
        frame.gotoxy(5, 5);
        frame.putmsg(getSlotText(f.player1, f, 0));
        frame.gotoxy(5, 6);
        frame.putmsg("\1h\1k" + hLine(20) + BOX.TR + "\1n");
        frame.gotoxy(26, 7);
        frame.putmsg("\1h\1k" + BOX.RT + "\1n");
        frame.gotoxy(5, 8);
        frame.putmsg(getSlotText(f.player2, f, 1));
        frame.gotoxy(5, 9);
        frame.putmsg("\1h\1k" + hLine(20) + BOX.BR + "\1n");
        // Winner
        if (f.winner) {
            frame.gotoxy(28, 7);
            frame.putmsg("\1h\1g" + (f.winner.shortNick || f.winner.name) + " WINS!\1n");
        }
    }
    
    // 4-team: semis + finals (generous spacing)
    function renderBracket4(frame, rounds, bracket) {
        var semis = rounds["semifinals"] || [];
        var finals = rounds["finals"] || [];
        var LW = 12; // line width
        
        // LEFT SEMI (slots 0,1) - template rows 2,4,6,8
        frame.gotoxy(1, 2);  frame.putmsg("\1h\1k" + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(1 + LW, 3); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(1 + LW, 4); frame.putmsg("\1h\1k" + BOX.RT + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(1 + LW, 5); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(1, 6);  frame.putmsg("\1h\1k" + hLine(LW) + BOX.BR + "\1n");
        
        // RIGHT SEMI (slots 2,3) - mirrored
        var RX = 53;
        frame.gotoxy(RX, 2);  frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + "\1n");
        frame.gotoxy(RX, 3);  frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(RX - LW - 1, 4); frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + BOX.LT + "\1n");
        frame.gotoxy(RX, 5);  frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(RX, 6);  frame.putmsg("\1h\1k" + BOX.BL + hLine(LW) + "\1n");
        
        // FINALS connector
        var FC = 26;
        for (var r = 5; r <= 10; r++) {
            frame.gotoxy(FC, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            frame.gotoxy(FC + 28, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        }
        frame.gotoxy(FC, 11); frame.putmsg("\1h\1k" + BOX.BL + hLine(27) + BOX.BR + "\1n");
        frame.gotoxy(35, 11); frame.putmsg("\1h\1yFINALS\1n");
        
        // TEXT above lines
        var s1 = semis[0] || {};
        frame.gotoxy(1, 1);  frame.putmsg(getSlotText(s1.player1, s1, 0));
        frame.gotoxy(1, 5);  frame.putmsg(getSlotText(s1.player2, s1, 1));
        frame.gotoxy(14, 3); frame.putmsg(getSlotText(s1.winner, s1, 0));
        
        var s2 = semis[1] || {};
        frame.gotoxy(RX + 1, 1); frame.putmsg(getSlotText(s2.player1, s2, 2));
        frame.gotoxy(RX + 1, 5); frame.putmsg(getSlotText(s2.player2, s2, 3));
        frame.gotoxy(RX - LW, 3); frame.putmsg(getSlotText(s2.winner, s2, 2));
    ...
    function renderBracket8(frame, rounds, bracket) {
        ...
        var semis = rounds["semifinals"] || [];
        var finals = rounds["finals"] || [];
        var LW = 10; // line width per round
        
        // === LEFT SIDE TEMPLATE ===
        // QF1 (rows 1-3), QF2 (rows 5-7) -> SF (row 4) -> Finals
        // QF matchup 1
        frame.gotoxy(1, 2);  frame.putmsg("\1h\1k" + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(1+LW, 3); frame.putmsg("\1h\1k" + BOX.RT + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(1, 4);  frame.putmsg("\1h\1k" + hLine(LW) + BOX.BR + "\1n");
        // QF matchup 2
        frame.gotoxy(1, 8);  frame.putmsg("\1h\1k" + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(1+LW, 9); frame.putmsg("\1h\1k" + BOX.RT + hLine(LW) + BOX.BR + "\1n");
        frame.gotoxy(1, 10); frame.putmsg("\1h\1k" + hLine(LW) + BOX.BR + "\1n");
        // SF connector
        for (var r = 4; r <= 8; r++) {
            frame.gotoxy(1+LW+LW+1, r);
            frame.putmsg("\1h\1k" + (r === 6 ? BOX.RT + hLine(LW) : BOX.V) + "\1n");
        }
        
        // === RIGHT SIDE TEMPLATE (mirrored) ===
        var RX = 57;
        frame.gotoxy(RX, 2);  frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + "\1n");
        frame.gotoxy(RX-LW-1, 3); frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + BOX.LT + "\1n");
        frame.gotoxy(RX, 4);  frame.putmsg("\1h\1k" + BOX.BL + hLine(LW) + "\1n");
        frame.gotoxy(RX, 8);  frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + "\1n");
        frame.gotoxy(RX-LW-1, 9); frame.putmsg("\1h\1k" + BOX.BL + hLine(LW) + BOX.LT + "\1n");
        frame.gotoxy(RX, 10); frame.putmsg("\1h\1k" + BOX.BL + hLine(LW) + "\1n");
        for (var r = 4; r <= 8; r++) {
            frame.gotoxy(RX-LW-LW-2, r);
            frame.putmsg("\1h\1k" + (r === 6 ? hLine(LW) + BOX.LT : BOX.V) + "\1n");
        }
        
        // FINALS connector
        var LC = 33, RC = 46;
        for (var r = 7; r <= 13; r++) {
            frame.gotoxy(LC, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            frame.gotoxy(RC, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        }
        frame.gotoxy(LC, 14); frame.putmsg("\1h\1k" + BOX.BL + hLine(RC-LC-1) + BOX.BR + "\1n");
        frame.gotoxy(37, 14); frame.putmsg("\1h\1yFINALS\1n");
        ...
        // Left QFs
        frame.gotoxy(1, 1);  frame.putmsg(getSlotText(q1.player1, q1, 0));
        frame.gotoxy(1, 3);  frame.putmsg(getSlotText(q1.player2, q1, 1));
        frame.gotoxy(12, 2); frame.putmsg(getSlotText(q1.winner, q1, 0));
        
        frame.gotoxy(1, 7);  frame.putmsg(getSlotText(q2.player1, q2, 2));
        frame.gotoxy(1, 9);  frame.putmsg(getSlotText(q2.player2, q2, 3));
        frame.gotoxy(12, 8); frame.putmsg(getSlotText(q2.winner, q2, 2));
        
        frame.gotoxy(23, 5); frame.putmsg(getSlotText(s1.winner, s1, 0));
        
        // Right QFs
        frame.gotoxy(RX+1, 1); frame.putmsg(getSlotText(q3.player1, q3, 4));
        frame.gotoxy(RX+1, 3); frame.putmsg(getSlotText(q3.player2, q3, 5));
        frame.gotoxy(RX-LW, 2); frame.putmsg(getSlotText(q3.winner, q3, 4));
        
        frame.gotoxy(RX+1, 7); frame.putmsg(getSlotText(q4.player1, q4, 6));
        frame.gotoxy(RX+1, 9); frame.putmsg(getSlotText(q4.player2, q4, 7));
        frame.gotoxy(RX-LW, 8); frame.putmsg(getSlotText(q4.winner, q4, 6));
        
        frame.gotoxy(RX-LW-LW-10, 5); frame.putmsg(getSlotText(s2.winner, s2, 4));
    ...
    // 16-team: compact - show R16 summary, then 8-team bracket
    function renderBracket16(frame, rounds, bracket) {
        ...
        // Render condensed 8-team below with Y offset
        var quarters = rounds["quarterfinals"] || [];
        var semis = rounds["semifinals"] || [];
        var finals = rounds["finals"] || [];
        var LW = 8;
        ...
        // Condensed left
        frame.gotoxy(1, Y+1); frame.putmsg("\1h\1k" + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(1+LW, Y+2); frame.putmsg("\1h\1k" + BOX.RT + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(1, Y+3); frame.putmsg("\1h\1k" + hLine(LW) + BOX.BR + "\1n");
        frame.gotoxy(1, Y+5); frame.putmsg("\1h\1k" + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(1+LW, Y+6); frame.putmsg("\1h\1k" + BOX.RT + hLine(LW) + BOX.BR + "\1n");
        frame.gotoxy(1, Y+7); frame.putmsg("\1h\1k" + hLine(LW) + BOX.BR + "\1n");
        for (var r = Y+3; r <= Y+5; r++) {
            frame.gotoxy(1+LW+LW+1, r);
            frame.putmsg("\1h\1k" + (r === Y+4 ? BOX.RT + hLine(6) : BOX.V) + "\1n");
        }
        
        // Condensed right
        var RX = 61;
        frame.gotoxy(RX, Y+1); frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + "\1n");
        frame.gotoxy(RX-LW-1, Y+2); frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + BOX.LT + "\1n");
        frame.gotoxy(RX, Y+3); frame.putmsg("\1h\1k" + BOX.BL + hLine(LW) + "\1n");
        frame.gotoxy(RX, Y+5); frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + "\1n");
        frame.gotoxy(RX-LW-1, Y+6); frame.putmsg("\1h\1k" + BOX.BL + hLine(LW) + BOX.LT + "\1n");
        frame.gotoxy(RX, Y+7); frame.putmsg("\1h\1k" + BOX.BL + hLine(LW) + "\1n");
        for (var r = Y+3; r <= Y+5; r++) {
            frame.gotoxy(RX-LW-LW-2, r);
            frame.putmsg("\1h\1k" + (r === Y+4 ? hLine(6) + BOX.LT : BOX.V) + "\1n");
        }
        
        // Finals
        var LC = 27, RC = 52;
        for (var r = Y+5; r <= Y+10; r++) {
            frame.gotoxy(LC, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            frame.gotoxy(RC, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        }
        frame.gotoxy(LC, Y+11); frame.putmsg("\1h\1k" + BOX.BL + hLine(RC-LC-1) + BOX.BR + "\1n");
        frame.gotoxy(37, Y+11); frame.putmsg("\1h\1yFINALS\1n");
        
        // Text
        var q1 = quarters[0]||{}, q2 = quarters[1]||{}, q3 = quarters[2]||{}, q4 = quarters[3]||{};
        frame.gotoxy(1, Y);   frame.putmsg(getSlotText(q1.player1, q1, 0));
        frame.gotoxy(1, Y+2); frame.putmsg(getSlotText(q1.player2, q1, 1));
        frame.gotoxy(1, Y+4); frame.putmsg(getSlotText(q2.player1, q2, 2));
        frame.gotoxy(1, Y+6); frame.putmsg(getSlotText(q2.player2, q2, 3));
        frame.gotoxy(RX+1, Y);   frame.putmsg(getSlotText(q3.player1, q3, 4));
        frame.gotoxy(RX+1, Y+2); frame.putmsg(getSlotText(q3.player2, q3, 5));
        frame.gotoxy(RX+1, Y+4); frame.putmsg(getSlotText(q4.player1, q4, 6));
        frame.gotoxy(RX+1, Y+6); frame.putmsg(getSlotText(q4.player2, q4, 7));
        
        var f = finals[0] || {};
        if (f.winner) {
            frame.gotoxy(35, Y+13);
            frame.putmsg("\1h\1y\xE0 " + (f.winner.name || f.winner.shortNick) + " \xE0\1n");
        }
    }
