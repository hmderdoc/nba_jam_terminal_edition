...
(function() {
    ...
    function buildWaitingMessage(roundInfo) {
        ...
        for (var i = 0; i < roundInfo.matches.length; i++) {
            ...
            // Case 2: Match is past soft deadline - grace period / force-sim scenario
                ...
                if (!blockingTimestamp || pm.hardDeadline < blockingTimestamp) {
                    ...
                    blockingReason = "have their match auto-simulated";
                }
            }
            // Case 3: Match has soft deadline approaching
                ...
                if (!blockingTimestamp || pm.softDeadline < blockingTimestamp) {
                    ...
                    blockingReason = "complete their match (or deadline expires)";
                }
            }
        ...
        if (!blockingMatch) {
            ...
            if (roundInfo.expiredCount > 0) {
                ...
                return {
                    message: "\1yWaiting for expired matches to be resolved.\1n",
                    detail: "\1w" + expiredPlayers.join(", ") + "\1n need to play or be auto-simmed.",
                    timestamp: null,
    ...
    function appendWaitingMessage(details, roundInfo) {
        ...
        if (waitMsg.canForceSim) {
            details.push("\1yYou can force-sim expired matches.\1n");
        }
    ...
    function getDetailedPlayoffStatus(status, ctx, playerId) {
        ...
        if (status.hasPendingMatch && status.pendingMatches && status.pendingMatches[0]) {
            ...
            if (isPastHardDeadline) {
                ...
                result.headline = "\1h\1r⚠ MATCH DEADLINE PASSED\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1rThis match will auto-simulate soon.\1n");
                // No action - auto-sim will handle it
                
            } else if (isPastSoftDeadline) {
                result.type = DETAILED_STATUS.PAST_SOFT_DEADLINE;
                result.artKey = "ghost_match";
                result.headline = "\1h\1y⚠ PAST DEADLINE\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1yOpponent missed the deadline.\1n");
                result.details.push("\1gYou can force a Ghost Match now!\1n");
                if (hardDeadline) {
                    var timeToHard = hardDeadline - now;
                    result.details.push("\1h\1kAuto-sim in: " + formatDuration(timeToHard) + "\1n");
                }
                result.action = {
        ...
        if (roundInfo.canForceSimCount > 0) {
            ...
            result.details.push("\1wYou can force-sim to advance the bracket.\1n");
            result.action = {
                text: "\1h\1yForce-Simulate Stalled Matches\1n",
                value: "force_sim",
                hotkey: "F"
            };
        }
     ...
     * - OVERDUE: hardDeadline (when auto-sim will trigger)
     * - EXPIRED: hardDeadline (auto-sim already triggered/imminent)
     */
    ...
    function showPlayoffHelp() {
        if (RichView) {
            ...
            view.line("\1w\1rHard Deadline (168h):\1n \1wMatch auto-simulates by CPU.\1n");
            ...
            LORB.View.line("Soft (72h): Can force ghost. Hard (168h): Auto-sim.");
            LORB.View.line("");
            LORB.View.line("Press any key...");
            console.getkey(K_NOSPIN);
        }
