...
(function() {
    ...
    // ========== DETAILED STATUS HELPER ==========
    
    /**
     * Status types for detailed playoff state display
     * Each type has a message template and recommended action
     */
    var DETAILED_STATUS = {
        NOT_IN_PLAYOFFS: "not_in_playoffs",
        CHAMPION: "champion",
        CHAMPION_RED_BULL_PENDING: "champion_red_bull",
        CHAMPION_RED_BULL_DONE: "champion_red_bull_done",
        ELIMINATED: "eliminated",
        WAITING_FOR_ROUND: "waiting_for_round",
        MATCH_UNSCHEDULED: "match_unscheduled",
        MATCH_SCHEDULED_FUTURE: "match_scheduled_future",
        MATCH_LIVE_GRACE: "match_live_grace",
        MATCH_GRACE_EXPIRED: "match_grace_expired",
        OPPONENT_ONLINE: "opponent_online",
        OPPONENT_OFFLINE_WAITING: "opponent_offline_waiting",
        PAST_SOFT_DEADLINE: "past_soft_deadline",
        PAST_HARD_DEADLINE: "past_hard_deadline",
        STALLED_MATCHES: "stalled_matches"
    };
    
    /**
     * Get info about all pending matches in a round (for "what are we waiting on" display)
     * @param {number} seasonNumber - Season to check
     * @param {string} roundName - Round name to filter by (optional - if null, shows all pending)
     * @param {string} excludePlayerId - Player ID to exclude from results (our own match)
     * @returns {Object} { matches: [], soonestDeadline: ms|null, expiredCount: number, canForceSimCount: number }
     */
    function getPendingRoundInfo(seasonNumber, roundName, excludePlayerId) {
        var result = {
            matches: [],
        ...
        if (!LORB.Playoffs) return result;
        
        var bracket = LORB.Playoffs.loadBracket ? LORB.Playoffs.loadBracket(seasonNumber) : null;
        if (!bracket || !bracket.matches) return result;
        
        var now = Date.now();
        
        for (var m = 0; m < bracket.matches.length; m++) {
            var bMatch = bracket.matches[m];
            
            // Skip completed/bye matches
            if (bMatch.status === "completed" || bMatch.status === "bye") continue;
            
            // Filter by round if specified
            if (roundName && bMatch.round !== roundName) continue;
            
            // Check if this match involves the excluded player
            var involvesExcluded = excludePlayerId && (
                (bMatch.player1 && bMatch.player1.playerId === excludePlayerId) ||
                (bMatch.player2 && bMatch.player2.playerId === excludePlayerId)
            );
            if (involvesExcluded) continue;
            
            // Get deadline status
            var isPastSoft = LORB.Playoffs.isMatchPastSoftDeadline 
                ? LORB.Playoffs.isMatchPastSoftDeadline(bMatch) : false;
            var isPastHard = LORB.Playoffs.isMatchPastHardDeadline
                ? LORB.Playoffs.isMatchPastHardDeadline(bMatch) : false;
            
            var p1Name = bMatch.player1 ? (bMatch.player1.shortNick || bMatch.player1.name) : "TBD";
            var p2Name = bMatch.player2 ? (bMatch.player2.shortNick || bMatch.player2.name) : "TBD";
            ...
            var matchInfo = {
                ...
                p1: p1Name,
                p2: p2Name,
                softDeadline: bMatch.softDeadline,
                hardDeadline: bMatch.hardDeadline,
                isPastSoft: isPastSoft,
                isPastHard: isPastHard,
                scheduling: bMatch.scheduling
            };
            
            result.matches.push(matchInfo);
            
            if (isPastSoft || isPastHard) {
                result.expiredCount++;
                
                // Check if player can force-sim this match
                if (excludePlayerId && LORB.Playoffs.canPlayerForceSimMatch &&
                    LORB.Playoffs.canPlayerForceSimMatch(excludePlayerId, bMatch)) {
                    result.canForceSimCount++;
                }
            }
            
            // Track soonest deadline (only for non-expired)
            if (bMatch.softDeadline && !isPastSoft) {
                if (!result.soonestDeadline || bMatch.softDeadline < result.soonestDeadline) {
                    result.soonestDeadline = bMatch.softDeadline;
                }
            }
        }
    ...
    /**
     * Build a clear, human-readable "waiting" message explaining what blocks progress.
     * Returns a structured object with the waiting reason and timestamp.
     * 
     * Message format: "Waiting until {timestamp} for {players} to {reason}"
     * 
     * @param {Object} roundInfo - Result from getPendingRoundInfo()
     * @returns {Object|null} { message: string, timestamp: number|null, players: string[], reason: string }
     */
    function buildWaitingMessage(roundInfo) {
        if (!roundInfo || roundInfo.matches.length === 0) {
            return null;
        }
        
        var now = Date.now();
        
        // Find the most relevant blocking match (soonest deadline or scheduled time)
        // Priority: 1) Scheduled matches (we know when they'll play)
        //           2) Matches with soft deadline approaching
        //           3) Expired matches (can be force-simmed)
        
        var blockingMatch = null;
        var blockingTimestamp = null;
        var blockingReason = null;
        
        for (var i = 0; i < roundInfo.matches.length; i++) {
            var pm = roundInfo.matches[i];
            
            // Case 1: Match has a scheduled time in the future
            if (pm.scheduling && pm.scheduling.scheduledStartUTC && pm.scheduling.scheduledStartUTC > now) {
                var schedTime = pm.scheduling.scheduledStartUTC;
                if (!blockingTimestamp || schedTime < blockingTimestamp) {
                    blockingMatch = pm;
                    blockingTimestamp = schedTime;
                    blockingReason = "play their match";
                }
            }
            // Case 2: Match is past soft deadline - can be force-simmed by waiting player
            else if (pm.isPastSoft && pm.hardDeadline && pm.hardDeadline > now) {
                if (!blockingTimestamp || pm.hardDeadline < blockingTimestamp) {
                    blockingMatch = pm;
                    blockingTimestamp = pm.hardDeadline;
                    blockingReason = "finish (deadline expired - can be simulated)";
                }
            }
            // Case 3: Match has soft deadline approaching
            else if (pm.softDeadline && pm.softDeadline > now) {
                if (!blockingTimestamp || pm.softDeadline < blockingTimestamp) {
                    blockingMatch = pm;
                    blockingTimestamp = pm.softDeadline;
                    blockingReason = "complete their match (deadline: " + formatDateTime(pm.softDeadline) + ")";
                }
            }
            // Case 4: Scheduled time passed but still waiting (overdue)
            else if (pm.scheduling && pm.scheduling.scheduledStartUTC && pm.scheduling.scheduledStartUTC <= now) {
                // Overdue - use grace end or soft deadline
                var overdueEnd = pm.scheduling.graceEndsUTC || pm.softDeadline;
                if (overdueEnd && overdueEnd > now) {
                    if (!blockingTimestamp || overdueEnd < blockingTimestamp) {
                        blockingMatch = pm;
                        blockingTimestamp = overdueEnd;
                        blockingReason = "play their overdue match";
                    }
                }
            }
        }
        ...
        if (!blockingMatch) {
            // All matches are past all deadlines - they should be force-simmable
            if (roundInfo.expiredCount > 0) {
                var expiredPlayers = [];
                for (var j = 0; j < roundInfo.matches.length && j < 3; j++) {
                    var em = roundInfo.matches[j];
                    if (em.isPastSoft || em.isPastHard) {
                        expiredPlayers.push(em.p1 + " & " + em.p2);
                    }
                }
                return {
                    message: "\1yStalled matches blocking bracket:\1n",
                    detail: "\1w" + expiredPlayers.join(", ") + "\1n",
                    extraDetail: "\1cYou can simulate these to advance.\1n",
                    timestamp: null,
                    canForceSim: roundInfo.canForceSimCount > 0
                };
            }
            return null;
        }
        ...
        var players = blockingMatch.p1 + " & " + blockingMatch.p2;
        var timestampStr = formatDateTime(blockingTimestamp);
        var durationStr = formatDuration(blockingTimestamp - now);
        
        return {
            message: "\1cWaiting until \1h\1w" + timestampStr + "\1n\1c for:\1n",
            detail: "\1w" + players + "\1n \1cto " + blockingReason + "\1n",
            countdown: "\1h\1k(" + durationStr + " from now)\1n",
            timestamp: blockingTimestamp,
            players: [blockingMatch.p1, blockingMatch.p2],
            reason: blockingReason,
            totalPending: roundInfo.matches.length
        };
    }
    
    /**
     * Append a clear waiting message to the details array
     * Uses buildWaitingMessage() to create a human-readable explanation
     * @param {Array} details - Array to append lines to
     * @param {Object} roundInfo - Result from getPendingRoundInfo()
     * @returns {boolean} True if a message was added
     */
    function appendWaitingMessage(details, roundInfo) {
        var waitMsg = buildWaitingMessage(roundInfo);
        if (!waitMsg) {
            // No pending matches found - bracket may be updating or all complete
            details.push("");
            details.push("\1cWaiting for bracket to update...\1n");
            return false;
        }
        
        details.push("");
        details.push(waitMsg.message);
        details.push(waitMsg.detail);
        if (waitMsg.extraDetail) {
            details.push(waitMsg.extraDetail);
        }
        if (waitMsg.countdown) {
            details.push(waitMsg.countdown);
        }
        
        // If there are more pending matches beyond the blocking one, mention it briefly
        if (waitMsg.totalPending && waitMsg.totalPending > 1) {
            details.push("\1h\1k(" + (waitMsg.totalPending - 1) + " other match" + 
                (waitMsg.totalPending > 2 ? "es" : "") + " also pending)\1n");
        }
     ...
     * @returns {string} Human-readable duration (e.g., "2h 15m", "3 days")
     */
     ...
     */
    function formatDateTime(utcMs) {
        if (!utcMs) return "unknown";
        ...
        hh = hh % 12;
        if (hh === 0) hh = 12;
        var timeStr = hh + ":" + (mm < 10 ? "0" : "") + mm + " " + ampm;
        return days[d.getDay()] + " " + months[d.getMonth()] + " " + d.getDate() + " @ " + timeStr;
    }
    
    /**
     * Get comprehensive detailed status for a player's playoff state.
     * Returns all information needed to display human-friendly status messages
     * and determine the appropriate action button.
     * 
     * @param {Object} status - Status object from getPlayerPlayoffStatus()
     * @param {Object} ctx - Player context
     * @param {string} playerId - Player's global ID
     * @returns {Object} Detailed status with:
     *   - type: DETAILED_STATUS enum value
     *   - headline: Main status message (colorized)
     *   - details: Array of detail lines (colorized)
     *   - action: { text, value, hotkey } for the primary action, or null
     *   - artKey: Key for PLAYOFF_ART
     *   - oppOnline: boolean
     *   - oppName: opponent name if applicable
     *   - match: pending match object if applicable
     *   - canForceSim: boolean - can force-sim stalled matches
     *   - stalledMatchCount: number of matches past deadline that can be force-simmed
     */
    function getDetailedPlayoffStatus(status, ctx, playerId) {
        var result = {
            type: DETAILED_STATUS.NOT_IN_PLAYOFFS,
            headline: "",
            details: [],
            action: null,
            artKey: "not_qualified",
            oppOnline: false,
            oppName: null,
            match: null,
            canForceSim: false,
        ...
        if (!status.inPlayoffs) {
            result.type = DETAILED_STATUS.NOT_IN_PLAYOFFS;
            if (status.seasonNumber) {
                result.headline = "\1wYou did not qualify for Season " + status.seasonNumber + " playoffs.\1n";
            } else {
                result.headline = "\1wNo playoffs are currently active.\1n";
            }
            result.details.push("Keep playing to qualify for next season!");
            return result;
        }
        
        // Champion
        if (status.champion) {
            result.artKey = "champion";
            
            // Check Red Bull challenge state
            var challengeState = null;
            if (LORB.Playoffs && LORB.Playoffs.getChampionChallengeState) {
                challengeState = LORB.Playoffs.getChampionChallengeState(status.seasonNumber, myPlayerId);
            }
            
            if (challengeState && challengeState.redBullDefeated) {
                result.type = DETAILED_STATUS.CHAMPION_RED_BULL_DONE;
                result.headline = "\1h\1y★ SEASON " + status.seasonNumber + " CHAMPION ★\1n";
                result.details.push("\1h\1g✓ You conquered the Red Bull!\1n");
                result.details.push("\1wYour legend echoes through Rim City.\1n");
            } else if (challengeState && challengeState.triesRemaining > 0) {
                result.type = DETAILED_STATUS.CHAMPION_RED_BULL_PENDING;
                result.headline = "\1h\1y★ SEASON " + status.seasonNumber + " CHAMPION ★\1n";
                result.details.push("\1wThe Red Bull awaits your challenge...\1n");
                result.details.push("\1c" + challengeState.triesRemaining + " attempt" + 
                    (challengeState.triesRemaining === 1 ? "" : "s") + " remaining.\1n");
                result.action = {
                    text: "\1h\1rChallenge the Red Bull\1n",
                    value: "red_bull_challenge",
                    hotkey: "R"
                };
            } else {
                result.type = DETAILED_STATUS.CHAMPION;
                result.headline = "\1h\1y★ SEASON " + status.seasonNumber + " CHAMPION ★\1n";
                if (challengeState && challengeState.triesRemaining === 0) {
                    result.details.push("\1h\1kNo challenge attempts remaining.\1n");
                }
            }
            return result;
        }
        ...
        if (status.eliminated) {
            result.type = DETAILED_STATUS.ELIMINATED;
            result.artKey = "eliminated";
            result.headline = "\1rEliminated from Season " + status.seasonNumber + " Playoffs\1n";
            if (status.currentRound) {
                result.details.push("\1wFell in: " + formatRoundName(status.currentRound) + "\1n");
            }
            result.details.push("Better luck next season!");
            return result;
        }
        
        // Has pending match - examine in detail
        if (status.hasPendingMatch && status.pendingMatches && status.pendingMatches[0]) {
            var match = status.pendingMatches[0];
            result.match = match;
            
            // Determine opponent
            var isPlayer1 = match.player1 && match.player1.playerId === myPlayerId;
            var opponent = isPlayer1 ? match.player2 : match.player1;
            result.oppName = opponent ? opponent.name : "Unknown";
            
            // Check opponent online status
            if (opponent && LORB.Persist && LORB.Persist.isPlayerOnline) {
                result.oppOnline = LORB.Persist.isPlayerOnline(opponent.playerId);
            }
            ...
            var softDeadline = match.softDeadline;
            var hardDeadline = match.hardDeadline;
            
            var Scheduling = LORB.Playoffs && LORB.Playoffs.Scheduling;
            var isPastSoftDeadline = LORB.Playoffs && LORB.Playoffs.isMatchPastSoftDeadline 
                ? LORB.Playoffs.isMatchPastSoftDeadline(match) : false;
            var isPastHardDeadline = LORB.Playoffs && LORB.Playoffs.isMatchPastHardDeadline 
                ? LORB.Playoffs.isMatchPastHardDeadline(match) : false;
            var isInGrace = Scheduling && Scheduling.isInGraceWindow 
                ? Scheduling.isInGraceWindow(match) : false;
            var isGraceExpired = Scheduling && Scheduling.isGraceExpired 
                ? Scheduling.isGraceExpired(match) : false;
            
            // Priority order for status determination:
            // 1. Past hard deadline (will auto-sim)
            // 2. Past soft deadline (ghost match available)
            // 3. In grace window (PLAY NOW!)
            // 4. Grace expired / missed scheduled time
            // 5. Opponent online (PvP available)
            // 6. Scheduled for future
            // 7. Unscheduled (no overlap found)
            // 8. Opponent offline, waiting
            
            if (isPastHardDeadline) {
                // Past hard deadline but user has an opponent - should play ghost match
                // (Auto-sim is for OTHER players' matches, not your own)
                result.type = DETAILED_STATUS.PAST_HARD_DEADLINE;
                result.artKey = "ghost_match";
                result.headline = "\1h\1r⚠ FINAL DEADLINE PASSED\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1yOpponent never showed up.\1n");
                result.details.push("\1gPlay a Ghost Match to advance!\1n");
                result.action = {
                    text: "\1h\1gPlay Ghost Match (vs AI)\1n",
                    value: "play",
                    hotkey: "P"
                };
                
            } else if (isPastSoftDeadline) {
                result.type = DETAILED_STATUS.PAST_SOFT_DEADLINE;
                result.artKey = "ghost_match";
                result.headline = "\1h\1y⚠ DEADLINE EXPIRED\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1yOpponent missed the deadline.\1n");
                result.details.push("\1gYou can play a Ghost Match now!\1n");
                if (hardDeadline) {
                    var timeToHard = hardDeadline - now;
                    result.details.push("\1h\1kFinal deadline in: " + formatDuration(timeToHard) + "\1n");
                }
                result.action = {
                    text: "\1h\1gPlay Ghost Match (vs AI)\1n",
                    value: "play",
                ...
                result.type = DETAILED_STATUS.MATCH_LIVE_GRACE;
                result.artKey = result.oppOnline ? "pvp_ready" : "ghost_match";
                result.headline = "\1h\1g⚡ MATCH IS NOW!\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                if (graceEnds) {
                    var graceLeft = graceEnds - now;
                    result.details.push("\1yGrace window closes in: " + formatDuration(graceLeft) + "\1n");
                }
                if (result.oppOnline) {
                    result.details.push("\1g✓ Opponent is ONLINE!\1n");
                    result.action = {
                        text: "\1h\1gCHALLENGE TO PVP NOW!\1n",
                        value: "play",
                        hotkey: "P"
                    };
                } else {
                    result.details.push("\1yo Opponent is offline\1n");
                    result.action = {
                        text: "\1h\1yPlay Now\1n",
                        value: "play",
                        hotkey: "P"
                    };
                }
                ...
                result.type = DETAILED_STATUS.MATCH_GRACE_EXPIRED;
                result.artKey = "waiting_opponent";
                result.headline = "\1h\1yMISSED SCHEDULED TIME\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1yThe scheduled time has passed.\1n");
                if (softDeadline) {
                    var timeToSoft = softDeadline - now;
                    if (timeToSoft > 0) {
                        result.details.push("\1wDeadline in: " + formatDuration(timeToSoft) + "\1n");
                    }
                }
                if (result.oppOnline) {
                    result.details.push("\1g✓ But opponent is online now!\1n");
                    result.action = {
                        text: "\1gChallenge to PvP\1n",
                        value: "play",
                        hotkey: "P"
                    };
                } else {
                    result.details.push("\1h\1kWaiting for opponent or deadline.\1n");
                    // Show action to set availability for rescheduling
                    result.action = {
            ...
            } else if (result.oppOnline) {
                result.type = DETAILED_STATUS.OPPONENT_ONLINE;
                result.artKey = "pvp_ready";
                result.headline = "\1h\1g✓ OPPONENT ONLINE!\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1gPvP match available right now!\1n");
                if (scheduledStart && scheduledStart > now) {
                    result.details.push("\1h\1k(Scheduled: " + formatDateTime(scheduledStart) + ")\1n");
                }
                result.action = {
                    text: "\1h\1gChallenge to PvP Match\1n",
                    value: "play",
                    hotkey: "P"
                };
                
            } else if (scheduledStart && scheduledStart > now) {
                result.type = DETAILED_STATUS.MATCH_SCHEDULED_FUTURE;
                result.artKey = "waiting_opponent";
                result.headline = "\1cMATCH SCHEDULED\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1cScheduled: " + formatDateTime(scheduledStart) + "\1n");
                var timeUntil = scheduledStart - now;
                result.details.push("\1wIn: " + formatDuration(timeUntil) + "\1n");
                result.details.push("\1h\1ko Opponent is offline\1n");
                // No urgent action - can browse or set availability
                result.action = {
                    text: "Set/Update Availability",
                    value: "availability",
                    hotkey: "A"
                };
                
            } else if (!scheduledStart) {
                result.type = DETAILED_STATUS.MATCH_UNSCHEDULED;
                result.artKey = "waiting_opponent";
                result.headline = "\1yMATCH NEEDS SCHEDULING\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1yNo overlapping availability found.\1n");
                result.details.push("\1wSet your availability to find a match time.\1n");
                if (softDeadline) {
                    var timeToDeadline = softDeadline - now;
                    result.details.push("\1h\1kDeadline: " + formatDuration(timeToDeadline) + "\1n");
                }
                result.action = {
                    text: "\1ySet Availability\1n",
                    value: "availability",
                    hotkey: "A"
                };
                
            } else {
                // Default: opponent offline, waiting
                result.type = DETAILED_STATUS.OPPONENT_OFFLINE_WAITING;
                result.artKey = "waiting_opponent";
                result.headline = "\1wWAITING FOR OPPONENT\1n";
                result.details.push("\1wvs " + result.oppName + "\1n");
                result.details.push("\1h\1ko Opponent is offline\1n");
                if (softDeadline) {
                    var timeLeft = softDeadline - now;
                    if (timeLeft > 0) {
                        result.details.push("\1wDeadline in: " + formatDuration(timeLeft) + "\1n");
                    }
                }
                result.action = {
                    text: "Set/Update Availability",
        ...
        // No pending match - check if waiting for round or can force-sim
        result.type = DETAILED_STATUS.WAITING_FOR_ROUND;
        result.artKey = "waiting_round";
        
        // Determine if we got here via bye or winning a match
        var advancedViaBye = false;
        if (LORB.Playoffs && LORB.Playoffs.loadBracket) {
            var bracket = LORB.Playoffs.loadBracket(status.seasonNumber);
            if (bracket && bracket.matches) {
                for (var bm = 0; bm < bracket.matches.length; bm++) {
                    var checkMatch = bracket.matches[bm];
                    if (checkMatch.status === "bye" && checkMatch.winner && 
                        checkMatch.winner.playerId === myPlayerId) {
                        advancedViaBye = true;
                        break;
                    }
                }
        ...
        if (advancedViaBye) {
            result.headline = "\1g✓ ADVANCED (BYE)\1n";
        } else {
            result.headline = "\1g✓ WON YOUR MATCH!\1n";
        }
        result.details.push("\1wRound: " + formatRoundName(status.currentRound || "current") + "\1n");
        
        // Use helper to get and display pending matches in this round
        var roundInfo = getPendingRoundInfo(status.seasonNumber, status.currentRound, myPlayerId);
        appendWaitingMessage(result.details, roundInfo);
        
        // Check for stalled matches that can be force-simmed
        // (These are OTHER players' matches that are blocking the bracket)
        if (roundInfo.canForceSimCount > 0) {
            result.type = DETAILED_STATUS.STALLED_MATCHES;
            result.canForceSim = true;
            result.stalledMatchCount = roundInfo.canForceSimCount;
            result.details.push("");
            result.details.push("\1yOther matches are past deadline.\1n");
            result.details.push("\1wSimulate them to advance the bracket.\1n");
            result.action = {
                text: "\1h\1ySimulate Stalled Matches\1n",
                value: "force_sim",
                hotkey: "S"
            };
        }
    ...
    // Match display states for bracket rendering - colors indicate urgency
    // All states show timestamps where available; color conveys meaning
    var MATCH_DISPLAY_STATES = {
        WAITING:     { code: "WAITING",     color: "\1w" },       // white - feeder match not done
        UNSCHEDULED: { code: "UNSCHEDULED", color: "\1w" },       // white - no scheduled time yet
        SCHEDULED:   { code: "SCHEDULED",   color: "\1c" },       // cyan - scheduled, upcoming
        LIVE:        { code: "LIVE",        color: "\1h\1g" },    // bright green - in grace window NOW
        MISSED:      { code: "MISSED",      color: "\1h\1y" },    // bright yellow - missed scheduled, deadline coming
        OVERDUE:     { code: "OVERDUE",     color: "\1h\1r" },    // bright red - past soft deadline
        EXPIRED:     { code: "EXPIRED",     color: "\1n\1r" }     // dark red - past hard deadline
    };
    
    /**
     * Format a timestamp as HH:MMMonDD (10 chars)
     * e.g., "14:30Dec23"
     */
     ...
     * Returns { state: MATCH_DISPLAY_STATES.*, text: string, timestamp: utcMs|null }
     * 
     * State-to-timestamp mapping:
     * - WAITING: "TBD" (no timestamp - waiting for feeder)
     * - UNSCHEDULED: softDeadline (deadline to schedule by)
     * - SCHEDULED: scheduledStartUTC (when match starts)
     * - LIVE: graceEndsUTC (grace window closes at)
     * - MISSED: softDeadline (deadline before ghost can be forced)
     * - OVERDUE: hardDeadline (when others can sim to advance bracket)
     * - EXPIRED: hardDeadline (others can now sim this match)
     */
    function getMatchDisplayState(match) {
        ...
        // Past hard deadline = EXPIRED - show hardDeadline timestamp in dark red
        if (Playoffs && Playoffs.isMatchPastHardDeadline && Playoffs.isMatchPastHardDeadline(match)) {
            var expiredText = hardDeadline ? formatBracketTimestamp(hardDeadline) : "EXPIRED";
            return { state: MATCH_DISPLAY_STATES.EXPIRED, text: expiredText, timestamp: hardDeadline };
        }
        
        // Past soft deadline = OVERDUE - show hardDeadline (when auto-sim triggers) in bright red
        if (Playoffs && Playoffs.isMatchPastSoftDeadline && Playoffs.isMatchPastSoftDeadline(match)) {
            var overdueText = hardDeadline ? formatBracketTimestamp(hardDeadline) : "OVERDUE";
            return { state: MATCH_DISPLAY_STATES.OVERDUE, text: overdueText, timestamp: hardDeadline };
        }
        
        // Grace expired (missed the scheduled window) = MISSED - show softDeadline in yellow
        if (Scheduling && Scheduling.isGraceExpired && Scheduling.isGraceExpired(match)) {
            var missedText = softDeadline ? formatBracketTimestamp(softDeadline) : "MISSED";
            return { state: MATCH_DISPLAY_STATES.MISSED, text: missedText, timestamp: softDeadline };
        }
        
        // In grace window = LIVE - show graceEnds timestamp in bright green
        if (Scheduling && Scheduling.isInGraceWindow && Scheduling.isInGraceWindow(match)) {
            var liveText = graceEnds ? formatBracketTimestamp(graceEnds) : "LIVE!";
            return { state: MATCH_DISPLAY_STATES.LIVE, text: liveText, timestamp: graceEnds };
        }
        
        // Has scheduled time in the future = SCHEDULED - show scheduledStart in cyan
        if (scheduledStart && scheduledStart > now) {
            var schedText = formatBracketTimestamp(scheduledStart);
            return { state: MATCH_DISPLAY_STATES.SCHEDULED, text: schedText, timestamp: scheduledStart };
        }
        
        // Both players ready but no scheduled time = UNSCHEDULED - show softDeadline in dim
        if (!scheduledStart) {
            var unschedText = softDeadline ? formatBracketTimestamp(softDeadline) : "UNSCHED";
            return { state: MATCH_DISPLAY_STATES.UNSCHEDULED, text: unschedText, timestamp: softDeadline };
        }
        
        // Fallback: show softDeadline
        var fallbackText = softDeadline ? formatBracketTimestamp(softDeadline) : "---";
        return { state: MATCH_DISPLAY_STATES.UNSCHEDULED, text: fallbackText, timestamp: softDeadline };
    }
    ...
    function getSlotText(player, match, slotIndex) {
        ...
        if (player.isBye) return "\1wBYE\1n";
        var nick = player.shortNick || (player.name || "???").substring(0, 5);
        var color = SLOT_COLORS[slotIndex % SLOT_COLORS.length];
        if (match && match.status === "completed" && match.winner && match.score) {
            // Show full score "21-15 Nick" format (winner-loser order)
            var w = match.score.winner;
            var l = match.score.loser;
            if (w !== undefined && l !== undefined) {
                return "\1w" + w + "-" + l + " " + color + nick + "\1n";
            }
        }
    ...
    function showBracketRichView(bracket, ctx) {
        ...
        // Legend for timestamp colors (only show if bracket not complete)
        if (bracket.status !== "completed") {
            frame.gotoxy(1, 21);
            frame.putmsg("\1wSCHED \1cUpcoming  \1h\1gLIVE  \1h\1yMissed  \1h\1rOverdue  \1n\1rExpired\1n");
        }
        
        frame.gotoxy(1, 22);
        frame.putmsg("\1wPress any key...\1n");
        view.render();
        console.getkey(K_NOSPIN);
        view.close();
    }
    ...
    function renderBracket4(frame, rounds, bracket) {
        ...
        // Finals - scheduling info on row 11, "FINALS" label on row 13
        if (f.winner) {
            frame.gotoxy(34, 15);
            frame.putmsg("\1h\1y\xE0 " + (f.winner.name || f.winner.shortNick) + " \xE0\1n");
        } else if (f.player1 && f.player2) {
            // Both finalists known - show scheduling state
            var finalsState = getMatchDisplayState(f);
            if (finalsState.state && finalsState.text) {
                frame.gotoxy(37, 11);
                frame.putmsg(finalsState.state.color + finalsState.text + "\1n");
            }
        }
        frame.gotoxy(36, 13); frame.putmsg("\1h\1yFINALS\1n");
    }
    ...
    function renderBracket8(frame, rounds, bracket) {
        ...
        // Bottom line - complete horizontal line
        ...
        // Finals - scheduling info on row 16, "FINALS" label on row 18
        if (f.winner) {
        ...
        frame.gotoxy(38, 18);  frame.putmsg("\1h\1yFINALS\1n");
    }
     ...
     * @param {Array|null} allBrackets - Pre-loaded brackets list (for arrow navigation context)
     */
    function showPlayerStatus(ctx, selectedSeason, allBrackets) {
        if (!LORB.Playoffs) {
        ...
        // Use provided brackets list or get from status
        var brackets = allBrackets || fullStatus.playerBrackets || [];
        ...
        if (selectedSeason && brackets.length > 1) {
        ...
        logPlayoffUI("showStatus", "status", "inPlayoffs=" + status.inPlayoffs + 
            ...
            " activeBrackets=" + brackets.length);
        
        if (RichView) {
            return showPlayerStatusRichView(status, ctx, playerId, brackets);
        } else {
    ...
    function showPlayerStatusRichView(status, ctx, playerId, allBrackets) {
        ...
        // Get comprehensive detailed status using the helper
        var detailed = getDetailedPlayoffStatus(status, ctx, myPlayerId);
        ...
        // Build figlet header text based on detailed status
        var figletText = "Playoffs";
        if (detailed.type === DETAILED_STATUS.CHAMPION || 
            detailed.type === DETAILED_STATUS.CHAMPION_RED_BULL_PENDING ||
            detailed.type === DETAILED_STATUS.CHAMPION_RED_BULL_DONE) {
            figletText = "Champion";
        } else if (detailed.type === DETAILED_STATUS.ELIMINATED) {
            figletText = "Eliminated";
        } else if (detailed.type === DETAILED_STATUS.MATCH_LIVE_GRACE) {
            figletText = "Match Now!";
        } else if (detailed.type === DETAILED_STATUS.OPPONENT_ONLINE) {
            figletText = "PvP Ready!";
        } else if (detailed.type === DETAILED_STATUS.PAST_SOFT_DEADLINE) {
            figletText = "Ghost Match";
        ...
        // Draw art in art zone
        var artFrame = view.getZone("art");
        if (artFrame) {
            drawPlayoffArt(artFrame, detailed.artKey, status.seasonNumber);
        }
        
        // Display headline and details from the detailed status
        view.blank();
        
        // Show season number context
        if (status.seasonNumber) {
            view.line("\1h\1kSeason " + status.seasonNumber + " Playoffs\1n");
            view.blank();
        }
        
        // Headline (main status message)
        if (detailed.headline) {
            view.line(detailed.headline);
        }
        
        // Detail lines
        for (var d = 0; d < detailed.details.length; d++) {
            view.line(detailed.details[d]);
        }
        
        view.blank();
        
        // Not in playoffs - just show message and exit
        if (!status.inPlayoffs) {
            view.line("\1h\1kPress any key...\1n");
            view.render();
            console.getkey(K_NOSPIN);
        ...
        // Primary action from detailed status (if any)
        if (detailed.action) {
            menuItems.push({
                text: detailed.action.text,
                value: detailed.action.value,
                hotkey: detailed.action.hotkey
            });
        }
        
        // Secondary play option if match is available but primary action was something else
        if (status.hasPendingMatch && !status.eliminated && !status.champion && 
            detailed.action && detailed.action.value !== "play") {
            // Add play as secondary option
            if (detailed.oppOnline) {
                menuItems.push({ text: "Challenge to PvP Match", value: "play", hotkey: "P" });
            } else if (detailed.type === DETAILED_STATUS.PAST_SOFT_DEADLINE) {
                menuItems.push({ text: "\1yPlay Ghost Match\1n", value: "play", hotkey: "P" });
            }
        }
        
        // Champion Red Bull Challenge - check both current and completed championships
        var hasChampionshipChallenge = (detailed.type === DETAILED_STATUS.CHAMPION_RED_BULL_PENDING);
        ...
        if (!hasChampionshipChallenge && status.championships && status.championships.length > 0) {
            for (var c = 0; c < status.championships.length; c++) {
                ...
                if (champ.challengeState.redBullDefeated) {
                    view.line("\1h\1g✓ Conquered the Red Bull!\1n");
                    ...
                    menuItems.push({ 
                        text: "\1h\1rS" + champ.seasonNumber + " Challenge the Red Bull\1n \1w" + triesTextC + "\1n", 
        ...
        // Only add if not already the primary action
        if (status.inPlayoffs && !status.eliminated && !status.champion &&
            (!detailed.action || detailed.action.value !== "availability")) {
            menuItems.push({ text: "Set/Update Availability", value: "availability", hotkey: "A" });
        }
        
        // Always show View Bracket
        menuItems.push({ text: "View Full Bracket", value: "bracket", hotkey: "B" });
        
        // Help section
        menuItems.push({ text: "\1h\1kHow Playoffs Work\1n", value: "help", hotkey: "?" });
        
        // Multi-bracket navigation
        if (allBrackets && allBrackets.length > 1) {
            menuItems.push({ text: "\1cJump to Season...\1n", value: "season_select", hotkey: "J" });
        }
        
        menuItems.push({ text: "Back", value: "back", hotkey: "Q" });
        
        // Show season navigation hint if multiple brackets
        if (allBrackets && allBrackets.length > 1) {
            view.blank();
            view.line("\1h\1k(\1w\x11 \x10\1h\1k to switch seasons)\1n");
        }
        
        // Use onLeftRight callback to handle arrow key navigation between seasons
        var choice = view.menu(menuItems, {
            onLeftRight: function(item, index, direction, richView, lb) {
                if (allBrackets && allBrackets.length > 1) {
                    return direction < 0 ? "prev_season" : "next_season";
                }
                return false;
            }
        });
    ...
    function run(ctx) {
        ...
        // Get the player's brackets list for navigation
        var playerId = ctx._globalId || ctx.name;
        var fullStatus = LORB.Playoffs ? LORB.Playoffs.getPlayerPlayoffStatus(playerId) : null;
        var allBrackets = (fullStatus && fullStatus.playerBrackets) ? fullStatus.playerBrackets : [];
        
        // Track selected season index for multi-bracket navigation
        var selectedIndex = 0;  // Start at first bracket (primary)
        
        // Helper to get current season number
        function getCurrentSeason() {
            if (allBrackets.length === 0) return null;
            return allBrackets[selectedIndex].seasonNumber;
        }
        
        while (true) {
            var selectedSeason = getCurrentSeason();
            var choice = showPlayerStatus(ctx, selectedSeason, allBrackets);
            ...
            switch (choice) {
                ...
                case "prev_season":
                    // Cycle to previous season (wrap around)
                    if (allBrackets.length > 1) {
                        selectedIndex = (selectedIndex - 1 + allBrackets.length) % allBrackets.length;
                        logPlayoffUI("run", "prev_season", "index=" + selectedIndex + " season=" + getCurrentSeason());
                    }
                    break;
                    
                case "next_season":
                    // Cycle to next season (wrap around)
                    if (allBrackets.length > 1) {
                        selectedIndex = (selectedIndex + 1) % allBrackets.length;
                        logPlayoffUI("run", "next_season", "index=" + selectedIndex + " season=" + getCurrentSeason());
                    }
                    break;
                    
                case "season_select":
                    // Show season selector submenu
                    var jumpSeason = showSeasonSelector(allBrackets, selectedIndex);
                    if (jumpSeason !== null) {
                        // Find the index for this season
                        for (var j = 0; j < allBrackets.length; j++) {
                            if (allBrackets[j].seasonNumber === jumpSeason) {
                                selectedIndex = j;
                                logPlayoffUI("run", "season_select", "jumped to index=" + selectedIndex + " season=" + jumpSeason);
                                break;
                            }
                        }
                    }
    ...
    /**
     * Show a season selector submenu
     * @param {Array} allBrackets - List of bracket info objects
     * @param {number} currentIndex - Currently selected index
     * @returns {number|null} Selected season number, or null if cancelled
     */
    function showSeasonSelector(allBrackets, currentIndex) {
        if (!allBrackets || allBrackets.length === 0) return null;
        ...
        if (RichView) {
            var view = new RichView({
                ...
                zones: [
                    { name: "header", x: 1, y: 1, width: 80, height: 2 },
                    { name: "content", x: 1, y: 3, width: 80, height: 22 }
                ]
            });
            ...
            if (headerFrame) {
                headerFrame.gotoxy(1, 1);
                headerFrame.putmsg("\1h\1cSELECT PLAYOFF SEASON\1n");
            }
            ...
            for (var i = 0; i < allBrackets.length; i++) {
                var b = allBrackets[i];
                var statusText = b.eliminated ? "\1r(Eliminated)\1n" : 
                               b.champion ? "\1h\1g(Champion!)\1n" : 
                               "\1y(" + formatRoundName(b.currentRound || "Active") + ")\1n";
                var prefix = (i === currentIndex) ? "\1h\1w\xFA \1n" : "  ";
                menuItems.push({
                    text: prefix + "\1wSeason " + b.seasonNumber + "\1n " + statusText,
                    value: b.seasonNumber,
                    hotkey: String(i + 1)
                });
            }
            menuItems.push({ text: "\1h\1kCancel\1n", value: null, hotkey: "Q" });
            
            var choice = view.menu(menuItems, { selected: currentIndex });
            view.close();
            ...
            // Legacy fallback
            LORB.View.clear();
            LORB.View.header("SELECT PLAYOFF SEASON");
            LORB.View.line("");
            
            for (var i = 0; i < allBrackets.length; i++) {
                var b = allBrackets[i];
                var statusText = b.eliminated ? "(Eliminated)" : 
                               b.champion ? "(Champion!)" : 
                               "(" + formatRoundName(b.currentRound || "Active") + ")";
                var marker = (i === currentIndex) ? "*" : " ";
                LORB.View.line(marker + " [" + (i + 1) + "] Season " + b.seasonNumber + " " + statusText);
            }
            
            LORB.View.line("[Q] Cancel");
            LORB.View.line("");
            ...
            if (idx >= 0 && idx < allBrackets.length) {
                return allBrackets[idx].seasonNumber;
            }
    ...
    /**
     * Show "How Playoffs Work" help section
     * Static help text explaining playoff mechanics
     */
    function showPlayoffHelp() {
        if (RichView) {
            var view = new RichView({
                theme: "lorb",
                zones: [
            ...
            if (headerFrame) {
                headerFrame.gotoxy(1, 1);
                headerFrame.putmsg("\1h\1c═══ HOW PLAYOFFS WORK ═══\1n");
            }
            
            view.line("\1h\1yQUALIFYING\1n");
            view.line("\1wAt the end of each 30-day season, the top players qualify\1n");
            view.line("\1wfor playoffs based on reputation. Play at least 3 games\1n");
            view.line("\1wand maintain positive rep to be eligible.\1n");
            view.blank();
            
            view.line("\1h\1ySCHEDULING\1n");
            view.line("\1wMatches are scheduled by finding overlap in both players'\1n");
            view.line("\1wavailability. Set your availability to help find a time.\1n");
            view.line("\1wYou'll see the scheduled time on the bracket.\1n");
            view.blank();
            
            view.line("\1h\1yPLAYING MATCHES\1n");
            view.line("\1w\1g* PvP Match:\1n \1wIf your opponent is online, challenge them!\1n");
            view.line("\1w\1y* Ghost Match:\1n \1wIf they're offline and past deadline, play vs AI.\1n");
            view.line("\1w  Ghost matches use your opponent's frozen Season stats.\1n");
            view.blank();
            
            view.line("\1h\1yDEADLINES\1n");
            view.line("\1w\1cSoft Deadline (72h):\1n \1wYou can force a ghost match.\1n");
            view.line("\1w\1rHard Deadline (168h):\1n \1wOthers can sim stalled matches.\1n");
            view.line("\1wDon't wait too long - play when you can!\1n");
            view.blank();
            
            view.line("\1h\1yCHAMPION'S CHALLENGE\1n");
            view.line("\1wWin the playoffs to unlock the \1h\1rRed Bull Challenge\1n\1w!\1n");
            view.line("\1wDefeat Jordan and the Red Bull to cement your legend.\1n");
            view.blank();
            
            view.line("\1h\1kPress any key to return...\1n");
            view.render();
            console.getkey(K_NOSPIN);
            view.close();
        } else {
            LORB.View.clear();
            LORB.View.header("HOW PLAYOFFS WORK");
            LORB.View.line("");
            LORB.View.line("QUALIFYING");
            LORB.View.line("Play 3+ games with positive rep to qualify.");
            LORB.View.line("");
            LORB.View.line("SCHEDULING");
            LORB.View.line("Set availability to schedule matches with opponents.");
            LORB.View.line("");
            LORB.View.line("MATCHES");
            LORB.View.line("PvP if opponent online, Ghost if offline + past deadline.");
            LORB.View.line("");
            LORB.View.line("DEADLINES");
            LORB.View.line("Soft (72h): Can force ghost. Hard (168h): Others can sim.");
            LORB.View.line("");
            LORB.View.line("Press any key...");
            console.getkey(K_NOSPIN);
        }
