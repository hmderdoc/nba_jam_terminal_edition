...
(function() {
     ...
     * @param {Object} player - Player object or null
     * @param {Object} match - Match object to check winner/loser
     * @param {string} playerId - Current player's ID for highlighting
     * @returns {string} Colored name string
     */
    function getPlayerDisplay(player, match, playerId) {
        if (!player) return "\1h\1k" + fitName("BYE", NAME_WIDTH) + "\1n";
        if (player.isBye) return "\1h\1k" + fitName("BYE", NAME_WIDTH) + "\1n";
        
        var name = fitName(player.name || player.shortNick || "???", NAME_WIDTH);
        ...
        if (match && (match.status === "completed" || match.status === "bye")) {
            if (match.winner && match.winner.playerId === player.playerId) {
                // Winner - bright green, or bright yellow if current player
        ...
        // Pending match
        if (isCurrentPlayer) {
            return "\1h\1y" + name + "\1n";  // Highlight current player
        }
        return "\1w" + name + "\1n";
    }
     ...
     * Seed5 ──┘                                    └── Seed6
     * 
     * For 16-team, we add another column of first round.
     */
    function showBracketRichView(bracket, ctx) {
        ...
        // Determine bracket structure based on size
        var bracketSize = bracket.bracketSize || 8;
        var teamsPerSide = bracketSize / 2;
        
        // Render based on bracket size
        if (bracketSize <= 4) {
            renderSmallBracket(bracketFrame, rounds, playerId, bracket);
        } else if (bracketSize <= 8) {
            render8TeamBracket(bracketFrame, rounds, playerId, bracket);
        } else {
            render16TeamBracket(bracketFrame, rounds, playerId, bracket);
        }
    ...
    /**
     * Render a 4-team bracket (2 per side) - simple finals view
     */
    function renderSmallBracket(frame, rounds, playerId, bracket) {
        ...
        // Left semifinal match (if exists)
        if (semis.length >= 1) {
            var m1 = semis[0];
            drawPlayerLine(frame, leftX, row1, m1.player1, m1, playerId, "right");
            drawConnectorRight(frame, leftX + NAME_WIDTH, row1, row2);
            drawPlayerLine(frame, leftX, row2, m1.player2, m1, playerId, "right");
        }
        
        // Right semifinal match (if exists)
        if (semis.length >= 2) {
            var m2 = semis[1];
            drawPlayerLine(frame, rightX, row1, m2.player1, m2, playerId, "left");
            drawConnectorLeft(frame, rightX - 1, row1, row2);
            drawPlayerLine(frame, rightX, row2, m2.player2, m2, playerId, "left");
        }
        
        // Finals in center
        if (finals.length >= 1) {
            var final = finals[0];
            frame.gotoxy(midX - 4, finalRow - 1);
            frame.putmsg("\1h\1y\xDB FINALS \xDB\1n");
            
            // Show finalists
            var f1Name = final.player1 ? final.player1.name : "TBD";
            var f2Name = final.player2 ? final.player2.name : "TBD";
            
            frame.gotoxy(midX - 6, finalRow);
            frame.putmsg(getPlayerDisplay(final.player1, final, playerId));
            frame.gotoxy(midX - 6, finalRow + 1);
            frame.putmsg("\1h\1w     vs     \1n");
            frame.gotoxy(midX - 6, finalRow + 2);
            frame.putmsg(getPlayerDisplay(final.player2, final, playerId));
            
            if (final.status === "completed" && final.winner) {
                frame.gotoxy(midX - 8, finalRow + 4);
                frame.putmsg("\1h\1y\xF0 " + final.winner.name + " WINS! \xF0\1n");
            }
        }
    }
    
    /**
     * Render an 8-team bracket (4 per side)
     */
    function render8TeamBracket(frame, rounds, playerId, bracket) {
        var finals = rounds["finals"] || [];
        var semis = rounds["semifinals"] || [];
        var quarters = rounds["quarterfinals"] || [];
        
        // Column positions
        var leftCol1 = 1;        // First round left
        var leftCol2 = 17;       // Semis left  
        var midCol = 35;         // Finals
        var rightCol2 = 48;      // Semis right
        var rightCol1 = 64;      // First round right
        
        // Vertical spacing: 4 rows per first-round match, 8 rows per semi
        // Rows: 1-4 (QF1), 5-8 (QF2), center for semis/finals
        
        // Left side quarterfinals (matches 1 and 2)
        if (quarters.length >= 1) {
            var q1 = quarters[0];
            drawPlayerLine(frame, leftCol1, 2, q1.player1, q1, playerId, "right");
            frame.gotoxy(leftCol1 + NAME_WIDTH, 2); frame.putmsg("\1h\1k" + BOX.H + BOX.TR + "\1n");
            frame.gotoxy(leftCol1 + NAME_WIDTH + 1, 3); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            frame.gotoxy(leftCol1 + NAME_WIDTH + 1, 4); frame.putmsg("\1h\1k" + BOX.RT + BOX.H + "\1n");
            drawPlayerLine(frame, leftCol1, 5, q1.player2, q1, playerId, "right");
            frame.gotoxy(leftCol1 + NAME_WIDTH, 5); frame.putmsg("\1h\1k" + BOX.H + BOX.BR + "\1n");
            
            // Show winner advancing
            var q1Winner = q1.winner ? q1.winner.name : "---";
            frame.gotoxy(leftCol2, 4);
            frame.putmsg(getPlayerDisplay(q1.winner, q1, playerId));
        }
        
        if (quarters.length >= 2) {
            var q2 = quarters[1];
            drawPlayerLine(frame, leftCol1, 9, q2.player1, q2, playerId, "right");
            frame.gotoxy(leftCol1 + NAME_WIDTH, 9); frame.putmsg("\1h\1k" + BOX.H + BOX.TR + "\1n");
            frame.gotoxy(leftCol1 + NAME_WIDTH + 1, 10); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            frame.gotoxy(leftCol1 + NAME_WIDTH + 1, 11); frame.putmsg("\1h\1k" + BOX.RT + BOX.H + "\1n");
            drawPlayerLine(frame, leftCol1, 12, q2.player2, q2, playerId, "right");
            frame.gotoxy(leftCol1 + NAME_WIDTH, 12); frame.putmsg("\1h\1k" + BOX.H + BOX.BR + "\1n");
            
            frame.gotoxy(leftCol2, 11);
            frame.putmsg(getPlayerDisplay(q2.winner, q2, playerId));
        }
        
        // Left semifinal connector and result
        if (semis.length >= 1) {
            var s1 = semis[0];
            // Draw connector from QF winners to semi
            frame.gotoxy(leftCol2 + NAME_WIDTH, 4); frame.putmsg("\1h\1k" + BOX.H + BOX.TR + "\1n");
            for (var r = 5; r < 8; r++) {
                frame.gotoxy(leftCol2 + NAME_WIDTH + 1, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            }
            frame.gotoxy(leftCol2 + NAME_WIDTH + 1, 8); frame.putmsg("\1h\1k" + BOX.RT + BOX.H + BOX.H + "\1n");
            for (var r = 9; r < 11; r++) {
                frame.gotoxy(leftCol2 + NAME_WIDTH + 1, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            }
            frame.gotoxy(leftCol2 + NAME_WIDTH, 11); frame.putmsg("\1h\1k" + BOX.H + BOX.BR + "\1n");
            
            // Semi winner going to finals
            frame.gotoxy(midCol - 5, 8);
            frame.putmsg(getPlayerDisplay(s1.winner, s1, playerId));
        }
        
        // Right side quarterfinals (matches 3 and 4, but displayed top and bottom)
        if (quarters.length >= 3) {
            var q3 = quarters[2];
            drawPlayerLine(frame, rightCol1, 2, q3.player1, q3, playerId, "left");
            frame.gotoxy(rightCol1 - 2, 2); frame.putmsg("\1h\1k" + BOX.TL + BOX.H + "\1n");
            frame.gotoxy(rightCol1 - 2, 3); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            frame.gotoxy(rightCol1 - 3, 4); frame.putmsg("\1h\1k" + BOX.H + BOX.LT + "\1n");
            drawPlayerLine(frame, rightCol1, 5, q3.player2, q3, playerId, "left");
            frame.gotoxy(rightCol1 - 2, 5); frame.putmsg("\1h\1k" + BOX.BL + BOX.H + "\1n");
            
            frame.gotoxy(rightCol2, 4);
            frame.putmsg(getPlayerDisplay(q3.winner, q3, playerId));
        }
        
        if (quarters.length >= 4) {
            var q4 = quarters[3];
            drawPlayerLine(frame, rightCol1, 9, q4.player1, q4, playerId, "left");
            frame.gotoxy(rightCol1 - 2, 9); frame.putmsg("\1h\1k" + BOX.TL + BOX.H + "\1n");
            frame.gotoxy(rightCol1 - 2, 10); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            frame.gotoxy(rightCol1 - 3, 11); frame.putmsg("\1h\1k" + BOX.H + BOX.LT + "\1n");
            drawPlayerLine(frame, rightCol1, 12, q4.player2, q4, playerId, "left");
            frame.gotoxy(rightCol1 - 2, 12); frame.putmsg("\1h\1k" + BOX.BL + BOX.H + "\1n");
            
            frame.gotoxy(rightCol2, 11);
            frame.putmsg(getPlayerDisplay(q4.winner, q4, playerId));
        }
        
        // Right semifinal connector and result
        if (semis.length >= 2) {
            var s2 = semis[1];
            frame.gotoxy(rightCol2 - 2, 4); frame.putmsg("\1h\1k" + BOX.TL + BOX.H + "\1n");
            for (var r = 5; r < 8; r++) {
                frame.gotoxy(rightCol2 - 2, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            }
            frame.gotoxy(rightCol2 - 4, 8); frame.putmsg("\1h\1k" + BOX.H + BOX.H + BOX.LT + "\1n");
            for (var r = 9; r < 11; r++) {
                frame.gotoxy(rightCol2 - 2, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            }
            frame.gotoxy(rightCol2 - 2, 11); frame.putmsg("\1h\1k" + BOX.BL + BOX.H + "\1n");
            
            frame.gotoxy(midCol + 6, 8);
            frame.putmsg(getPlayerDisplay(s2.winner, s2, playerId));
        }
        ...
        if (finals.length >= 1) {
            ...
            // Draw finals box
            frame.gotoxy(midCol - 2, 14);
            frame.putmsg("\1h\1y" + BOX.TL + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.TR + "\1n");
            frame.gotoxy(midCol - 2, 15);
            frame.putmsg("\1h\1y" + BOX.V + "\1n \1h\1wFINALS\1n   \1h\1y" + BOX.V + "\1n");
            frame.gotoxy(midCol - 2, 16);
            frame.putmsg("\1h\1y" + BOX.BL + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.H + BOX.BR + "\1n");
            
            // Connector lines from semis to finals
            frame.gotoxy(midCol - 5 + NAME_WIDTH, 8); frame.putmsg("\1h\1k" + BOX.H + BOX.H + BOX.TR + "\1n");
            for (var r = 9; r < 15; r++) {
                frame.gotoxy(midCol + NAME_WIDTH - 3, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            }
            
            frame.gotoxy(midCol + 6 - 1, 8); frame.putmsg("\1h\1k" + BOX.TL + BOX.H + "\1n");
            for (var r = 9; r < 15; r++) {
                frame.gotoxy(midCol + 5, r); frame.putmsg("\1h\1k" + BOX.V + "\1n");
            }
            
            // Show champion or finalists
            if (final.status === "completed" && final.winner) {
                frame.gotoxy(midCol - 4, 18);
                frame.putmsg("\1h\1y\xF0 CHAMPION \xF0\1n");
                frame.gotoxy(midCol - (final.winner.name.length / 2), 19);
                frame.putmsg("\1h\1g" + final.winner.name + "\1n");
            } else {
                frame.gotoxy(midCol - 6, 18);
                var f1 = final.player1 ? final.player1.name : "TBD";
                var f2 = final.player2 ? final.player2.name : "TBD";
                frame.putmsg("\1w" + f1 + " vs " + f2 + "\1n");
            }
        }
    }
    
    /**
     * Render a 16-team bracket (8 per side)
     * This is a condensed view showing round of 16 + quarters + semis + final
     */
    function render16TeamBracket(frame, rounds, playerId, bracket) {
        var finals = rounds["finals"] || [];
        var semis = rounds["semifinals"] || [];
        var quarters = rounds["quarterfinals"] || [];
        var r16 = rounds["round_of_16"] || [];
        
        // For 16 teams we need to show more compactly
        // Left: 4 R16 matches stacked, then 2 QF, then 1 SF
        // Right: mirror
        ...
        // Simplified: show R16 results, then bracket from QF
        frame.gotoxy(1, 1);
        frame.putmsg("\1h\1cROUND OF 16\1n");
        
        // Show R16 results in two columns
        var row = 2;
        for (var i = 0; i < r16.length && i < 4; i++) {
            var m = r16[i];
            var winner = m.winner ? m.winner.name : "---";
            frame.gotoxy(leftCol, row + i);
            frame.putmsg("\1w" + (i + 1) + ". " + fitName(winner, 10) + "\1n");
        }
        for (var i = 4; i < r16.length && i < 8; i++) {
            var m = r16[i];
            var winner = m.winner ? m.winner.name : "---";
            frame.gotoxy(rightCol, row + (i - 4));
            frame.putmsg("\1w" + (i + 1) + ". " + fitName(winner, 10) + "\1n");
        }
        
        // Then show QF onwards as 8-team bracket below
        frame.gotoxy(1, 7);
        frame.putmsg("\1h\1k" + repeatStr(BOX.H, 78) + "\1n");
        
        // Reuse 8-team logic with offset
        render8TeamBracketOffset(frame, quarters, semis, finals, playerId, 8);
    }
    
    /**
     * Helper to render 8-team bracket with Y offset
     */
    function render8TeamBracketOffset(frame, quarters, semis, finals, playerId, yOffset) {
        var leftCol1 = 1;
        var leftCol2 = 17;
        var midCol = 35;
        var rightCol2 = 48;
        ...
        if (quarters.length >= 1) {
            var q1 = quarters[0];
            frame.gotoxy(leftCol1, yOffset + 1);
            frame.putmsg(getPlayerDisplay(q1.player1, q1, playerId));
            frame.gotoxy(leftCol1, yOffset + 3);
            frame.putmsg(getPlayerDisplay(q1.player2, q1, playerId));
            frame.gotoxy(leftCol1 + NAME_WIDTH, yOffset + 2);
            frame.putmsg("\1h\1k" + BOX.RT + BOX.H + "\1n");
        }
        
        if (quarters.length >= 2) {
            var q2 = quarters[1];
            frame.gotoxy(leftCol1, yOffset + 5);
            frame.putmsg(getPlayerDisplay(q2.player1, q2, playerId));
            frame.gotoxy(leftCol1, yOffset + 7);
            frame.putmsg(getPlayerDisplay(q2.player2, q2, playerId));
            frame.gotoxy(leftCol1 + NAME_WIDTH, yOffset + 6);
            frame.putmsg("\1h\1k" + BOX.RT + BOX.H + "\1n");
        }
        ...
        if (semis.length >= 1) {
            var s1 = semis[0];
            frame.gotoxy(leftCol2, yOffset + 2);
            frame.putmsg(getPlayerDisplay(s1.winner || s1.player1, s1, playerId));
            frame.gotoxy(leftCol2, yOffset + 6);
            frame.putmsg(getPlayerDisplay(s1.winner ? null : s1.player2, s1, playerId));
        }
        
        // Right QF
        if (quarters.length >= 3) {
            var q3 = quarters[2];
            frame.gotoxy(rightCol1, yOffset + 1);
            frame.putmsg(getPlayerDisplay(q3.player1, q3, playerId));
            frame.gotoxy(rightCol1, yOffset + 3);
            frame.putmsg(getPlayerDisplay(q3.player2, q3, playerId));
        }
        
        if (quarters.length >= 4) {
            var q4 = quarters[3];
            frame.gotoxy(rightCol1, yOffset + 5);
            frame.putmsg(getPlayerDisplay(q4.player1, q4, playerId));
            frame.gotoxy(rightCol1, yOffset + 7);
            frame.putmsg(getPlayerDisplay(q4.player2, q4, playerId));
        }
        
        // Right SF
        if (semis.length >= 2) {
            var s2 = semis[1];
            frame.gotoxy(rightCol2, yOffset + 2);
            frame.putmsg(getPlayerDisplay(s2.winner || s2.player1, s2, playerId));
            frame.gotoxy(rightCol2, yOffset + 6);
            frame.putmsg(getPlayerDisplay(s2.winner ? null : s2.player2, s2, playerId));
        }
        
        // Finals
        if (finals.length >= 1) {
            var final = finals[0];
            frame.gotoxy(midCol - 4, yOffset + 4);
            frame.putmsg("\1h\1y FINALS \1n");
            
            if (final.status === "completed" && final.winner) {
                frame.gotoxy(midCol - 5, yOffset + 5);
                frame.putmsg("\1h\1g" + final.winner.name + " WINS!\1n");
            } else {
                var f1 = final.player1 ? final.player1.name : "TBD";
                var f2 = final.player2 ? final.player2.name : "TBD";
                frame.gotoxy(midCol - 6, yOffset + 5);
                frame.putmsg("\1w" + f1 + " v " + f2 + "\1n");
            }
        }
    }
    
    /**
     * Draw a player name line
     */
    function drawPlayerLine(frame, x, y, player, match, playerId, side) {
        frame.gotoxy(x, y);
        frame.putmsg(getPlayerDisplay(player, match, playerId));
    }
    
    /**
     * Draw right-side connector (bracket going right)
     */
    function drawConnectorRight(frame, x, y1, y2) {
        var midY = Math.floor((y1 + y2) / 2);
        frame.gotoxy(x, y1); frame.putmsg("\1h\1k" + BOX.H + BOX.TR + "\1n");
        for (var y = y1 + 1; y < midY; y++) {
            frame.gotoxy(x + 1, y); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        }
        frame.gotoxy(x + 1, midY); frame.putmsg("\1h\1k" + BOX.RT + BOX.H + "\1n");
        for (var y = midY + 1; y < y2; y++) {
            frame.gotoxy(x + 1, y); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        }
        frame.gotoxy(x, y2); frame.putmsg("\1h\1k" + BOX.H + BOX.BR + "\1n");
    }
    
    /**
     * Draw left-side connector (bracket going left)
     */
    function drawConnectorLeft(frame, x, y1, y2) {
        var midY = Math.floor((y1 + y2) / 2);
        frame.gotoxy(x, y1); frame.putmsg("\1h\1k" + BOX.TL + BOX.H + "\1n");
        for (var y = y1 + 1; y < midY; y++) {
            frame.gotoxy(x, y); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        }
        frame.gotoxy(x - 1, midY); frame.putmsg("\1h\1k" + BOX.H + BOX.LT + "\1n");
        for (var y = midY + 1; y < y2; y++) {
            frame.gotoxy(x, y); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        }
        frame.gotoxy(x, y2); frame.putmsg("\1h\1k" + BOX.BL + BOX.H + "\1n");
    }
