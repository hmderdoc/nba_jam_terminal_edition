...
(function() {
    ...
    function showPlayerStatusRichView(status, ctx, playerId, allBrackets) {
        ...
        // Multi-bracket navigation - single "Jump to Season" item if player is in multiple brackets
        ...
        if (allBrackets && allBrackets.length > 1) {
            view.blank();
            view.line("\1h\1kSeason " + status.seasonNumber + " (\1w\x11 \x10\1h\1k to switch)\1n");
        }
        
        // Use onLeftRight callback to handle arrow key navigation between seasons
        // Return the actual string value to exit menu with that choice
        var choice = view.menu(menuItems, {
            onLeftRight: function(item, index, direction, richView, lb) {
                if (allBrackets && allBrackets.length > 1) {
                    // Return the navigation action as the menu result
                    return direction < 0 ? "prev_season" : "next_season";
                }
                return false;  // Ignore arrow keys if single bracket
            }
        });
