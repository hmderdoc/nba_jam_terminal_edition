...
(function() {
     ...
     * 
     * With 20 rows and 8 teams (4 per side), each QF matchup gets ~5 rows:
     * - 2 teams × 2 rows each = 4 rows
     * - Plus connector space
     * 
     * Left side (cols 1-35), Right side (cols 45-80), Finals center
     */
    function renderBracket8(frame, rounds, playerId, bracket) {
        ...
        // Column positions
        var L1 = 1;    // Left QF teams
        var L2 = 17;   // Left SF winners
        var R2 = 49;   // Right SF winners  
        var R1 = 65;   // Right QF teams
        var MID = 33;  // Finals center
        ...
        // Left SF connector (vertical from row 5-9, output at row 7)
        frame.gotoxy(L2 + 15, 5);
        frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(L2 + 15, 6);
        frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(L2 + 15, 7);
        frame.putmsg("\1h\1k" + BOX.RT + "\1n");
        frame.gotoxy(L2 + 15, 8);
        frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(L2 + 15, 9);
        frame.putmsg("\1h\1k" + BOX.V + "\1n");
        
        // Left SF winner
        if (semis.length >= 1) {
            var s = semis[0];
            frame.gotoxy(MID - 14, 7);
            frame.putmsg(getPlayerDisplay(s.winner, s, playerId));
            frame.gotoxy(MID - 14, 8);
        ...
        if (quarters.length >= 3) {
            ...
            frame.putmsg(getPlayerDisplay(q.player1, q, playerId));
            frame.gotoxy(R1 - 2, 2);
            frame.putmsg("\1h\1k" + BOX.TL + hLine(14) + "\1n");
            frame.gotoxy(R1, 3);
            frame.putmsg(getPlayerDisplay(q.player2, q, playerId));
            frame.gotoxy(R1 - 2, 4);
            frame.putmsg("\1h\1k" + BOX.BL + hLine(14) + "\1n");
            frame.gotoxy(R1 - 3, 3);
        ...
        if (quarters.length >= 4) {
            ...
            frame.gotoxy(R1, 9);
            frame.putmsg(getPlayerDisplay(q.player2, q, playerId));
            frame.gotoxy(R1 - 2, 10);
            frame.putmsg("\1h\1k" + BOX.BL + hLine(14) + "\1n");
            frame.gotoxy(R1 - 3, 9);
            frame.putmsg("\1h\1k" + BOX.LT + "\1n");
            // QF4 winner
            frame.gotoxy(R2 - 2, 10);
            frame.putmsg("\1h\1k" + BOX.BL + hLine(14) + "\1n");
            frame.gotoxy(R2, 9);
            frame.putmsg(getPlayerDisplay(q.winner, q, playerId));
        }
        ...
        if (semis.length >= 2) {
            var s = semis[1];
            frame.gotoxy(MID + 3, 7);
            frame.putmsg(getPlayerDisplay(s.winner, s, playerId));
            frame.gotoxy(MID + 1, 8);
            frame.putmsg("\1h\1k" + BOX.TR + hLine(12) + "\1n");
        }
        
        // === FINALS ===
        // Vertical connectors from SFs to finals
        for (var r = 9; r <= 13; r++) {
            frame.gotoxy(MID - 2, r);
            frame.putmsg("\1h\1k" + BOX.V + "\1n");
            frame.gotoxy(MID + 13, r);
            frame.putmsg("\1h\1k" + BOX.V + "\1n");
        }
        
        // Finals box
        frame.gotoxy(MID - 2, 14);
        frame.putmsg("\1h\1k" + BOX.BL + hLine(3) + "\1n\1h\1y FINALS \1n\1h\1k" + hLine(3) + BOX.BR + "\1n");
        
        if (finals.length >= 1) {
            var f = finals[0];
            if (f.status === "completed" && f.winner) {
                frame.gotoxy(MID - 1, 16);
                frame.putmsg("\1h\1y\xF0 CHAMPION \xF0\1n");
                var champName = f.winner.name || "???";
                frame.gotoxy(MID + 5 - Math.floor(champName.length / 2), 17);
                frame.putmsg("\1h\1g" + champName + "\1n");
            } else {
                var n1 = f.player1 ? f.player1.name : "TBD";
                var n2 = f.player2 ? f.player2.name : "TBD";
                frame.gotoxy(MID - 2, 16);
