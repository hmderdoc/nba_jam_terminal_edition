...
(function() {
    ...
    var MATCH_DISPLAY_STATES = {
        WAITING:     { code: "WAITING",     color: "\1h\1k" },  // dim gray - feeder match not done
        UNSCHEDULED: { code: "UNSCHEDULED", color: "\1h\1k" },  // dim gray - no scheduled time yet
    ...
    function getSlotText(player, match, slotIndex) {
        ...
        if (match && match.status === "completed" && match.winner && match.score) {
            // match.score = { winner: X, loser: Y }
            var isWinner = match.winner.playerId === player.playerId;
            var playerScore = isWinner ? match.score.winner : match.score.loser;
            if (playerScore !== undefined && playerScore !== null) {
                return playerScore + " " + color + nick + "\1n";
            }
        }
