...
(function() {
    ...
    function getDetailedPlayoffStatus(status, ctx, playerId) {
        ...
        // Get pending matches in the same round to show what we're waiting on
        var pendingInRound = [];
        var soonestDeadline = null;
        var expiredCount = 0;
        
        if (LORB.Playoffs && status.seasonNumber) {
            var bracket = LORB.Playoffs.loadBracket ? LORB.Playoffs.loadBracket(status.seasonNumber) : null;
            if (bracket && bracket.matches) {
                for (var m = 0; m < bracket.matches.length; m++) {
                    var bMatch = bracket.matches[m];
                    // Find pending matches in same round (not ours, not completed)
                    if (bMatch.round === status.currentRound && 
                        bMatch.status !== "completed" && bMatch.status !== "bye") {
                        // Check if it involves us - if so, skip (we won ours)
                        var involvesUs = (bMatch.player1 && bMatch.player1.playerId === myPlayerId) ||
                                        (bMatch.player2 && bMatch.player2.playerId === myPlayerId);
                        ...
                        pendingInRound.push(matchInfo);
                        
                        if (isPastSoft || isPastHard) {
                            expiredCount++;
                        }
                        
                        // Track soonest deadline
                        if (bMatch.softDeadline && !isPastSoft) {
                            if (!soonestDeadline || bMatch.softDeadline < soonestDeadline) {
                                soonestDeadline = bMatch.softDeadline;
                            }
                        }
                    }
                }
        ...
        if (pendingInRound.length === 0) {
            result.details.push("\1cWaiting for bracket to update...\1n");
        } else {
            result.details.push("");
            result.details.push("\1h\1wWaiting on " + pendingInRound.length + " match" + 
                (pendingInRound.length === 1 ? "" : "es") + ":\1n");
            
            // Show each pending match with its status
            for (var p = 0; p < pendingInRound.length && p < 3; p++) {
                var pm = pendingInRound[p];
                var matchLine = "  \1w" + pm.p1 + " vs " + pm.p2 + "\1n";
                
                if (pm.isPastHard) {
                    matchLine += " \1r[AUTO-SIM SOON]\1n";
                    ...
                    if (timeToSoft > 0) {
                        matchLine += " \1h\1k[deadline: " + formatDuration(timeToSoft) + "]\1n";
                    }
                }
                result.details.push(matchLine);
            }
            
            if (pendingInRound.length > 3) {
                result.details.push("  \1h\1k...and " + (pendingInRound.length - 3) + " more\1n");
            }
        }
        ...
        result.details.push("");
        if (expiredCount > 0) {
            result.details.push("\1h\1y⚠ " + expiredCount + " match" + 
                (expiredCount === 1 ? " is" : "es are") + " past deadline!\1n");
        } else if (soonestDeadline) {
            var timeUntilDeadline = soonestDeadline - now;
            if (timeUntilDeadline > 0) {
                result.details.push("\1cNext deadline in: \1h\1w" + formatDuration(timeUntilDeadline) + "\1n");
            }
        }
        
        // Check for stalled matches that can be force-simmed
        if (LORB.Playoffs && LORB.Playoffs.getMatchesPastSoftDeadline) {
            var stalledMatches = LORB.Playoffs.getMatchesPastSoftDeadline();
            var canForceSimCount = 0;
            
            for (var i = 0; i < stalledMatches.length; i++) {
                if (LORB.Playoffs.canPlayerForceSimMatch && 
                    LORB.Playoffs.canPlayerForceSimMatch(myPlayerId, stalledMatches[i])) {
                    canForceSimCount++;
                }
            }
            
            if (canForceSimCount > 0) {
                result.type = DETAILED_STATUS.STALLED_MATCHES;
                result.canForceSim = true;
                result.stalledMatchCount = canForceSimCount;
