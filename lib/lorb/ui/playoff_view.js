...
(function() {
    ...
    /**
     * Append pending match info to a details array
     * Shows what matches are pending and when deadlines are
     * @param {Array} details - Array to append lines to
     * @param {Object} roundInfo - Result from getPendingRoundInfo()
     * @param {number} maxToShow - Max matches to display (default 3)
     */
    function appendPendingMatchDetails(details, roundInfo, maxToShow) {
        ...
        details.push("");
        details.push("\1h\1kOther matches in bracket:\1n");
        
        // Show each pending match with its status
        for (var p = 0; p < roundInfo.matches.length && p < maxToShow; p++) {
            var pm = roundInfo.matches[p];
            var matchLine = "  \1w" + pm.p1 + " vs " + pm.p2 + "\1n";
            
            if (pm.isPastHard) {
                matchLine += " \1r[AUTO-SIM]\1n";
            } else if (pm.isPastSoft) {
                matchLine += " \1h\1y[EXPIRED]\1n";
            } else if (pm.scheduling && pm.scheduling.scheduledStartUTC) {
                var schedTime = pm.scheduling.scheduledStartUTC;
                if (schedTime > now) {
                    matchLine += " \1c[" + formatDuration(schedTime - now) + "]\1n";
                } else {
                    matchLine += " \1y[overdue]\1n";
                }
            } else if (pm.softDeadline) {
                var timeToSoft = pm.softDeadline - now;
                if (timeToSoft > 0) {
                    matchLine += " \1h\1k[" + formatDuration(timeToSoft) + "]\1n";
                }
            }
            details.push(matchLine);
        }
        
        if (roundInfo.matches.length > maxToShow) {
            details.push("  \1h\1k+" + (roundInfo.matches.length - maxToShow) + " more\1n");
        }
        
        // Show deadline/expired summary
        if (roundInfo.expiredCount > 0) {
            details.push("\1h\1y⚠ " + roundInfo.expiredCount + " past deadline\1n");
        } else if (roundInfo.soonestDeadline) {
            var timeUntil = roundInfo.soonestDeadline - now;
            if (timeUntil > 0) {
                details.push("\1h\1kNext deadline: " + formatDuration(timeUntil) + "\1n");
            }
        }
    }
    ...
    function getDetailedPlayoffStatus(status, ctx, playerId) {
        ...
        if (status.hasPendingMatch && status.pendingMatches && status.pendingMatches[0]) {
            ...
            if (isPastHardDeadline) {
                ...
                if (result.oppOnline) {
                    ...
                    // Show what else is pending in this round
                    var graceRoundInfo = getPendingRoundInfo(status.seasonNumber, status.currentRound, myPlayerId);
                    appendPendingMatchDetails(result.details, graceRoundInfo);
                ...
                // Show what else is pending in this round
                var schedRoundInfo = getPendingRoundInfo(status.seasonNumber, status.currentRound, myPlayerId);
                appendPendingMatchDetails(result.details, schedRoundInfo);
                ...
                // Show what else is pending in this round
                var unschedRoundInfo = getPendingRoundInfo(status.seasonNumber, status.currentRound, myPlayerId);
                appendPendingMatchDetails(result.details, unschedRoundInfo);
                result.action = {
                ...
                // Show what else is pending in this round
                var offlineRoundInfo = getPendingRoundInfo(status.seasonNumber, status.currentRound, myPlayerId);
                appendPendingMatchDetails(result.details, offlineRoundInfo);
                result.action = {
                    text: "Set/Update Availability",
                    value: "availability",
                    hotkey: "A"
        ...
        appendPendingMatchDetails(result.details, roundInfo);
