...
(function() {
    ...
    function renderBracket4(frame, rounds, bracket) {
        ...
        frame.gotoxy(13, 4);  frame.putmsg("\1h\1k" + BOX.RT + hLine(12) + BOX.TR + "\1n");
        ...
        frame.gotoxy(53, 4);  frame.putmsg("\1h\1k" + BOX.TL + hLine(12) + BOX.LT + "\1n");
    ...
    function renderBracket8(frame, rounds, bracket) {
        ...
        // Column positions
        var L1 = 1, L2 = 12, L3 = 23;  // Left side column starts
        var R3 = 48, R2 = 59, R1 = 70; // Right side column starts
        var LW = 9;  // Line segment width
        
        // === LEFT SIDE TEMPLATE ===
        // QF1: text rows 1,3 -> line rows 2,4 -> winner row 3
        frame.gotoxy(L1, 2);   frame.putmsg("\1h\1k" + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(L1+LW, 3); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(L1+LW, 4); frame.putmsg("\1h\1k" + BOX.RT + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(L1+LW, 5); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(L1, 6);   frame.putmsg("\1h\1k" + hLine(LW) + BOX.BR + "\1n");
        
        // QF2: text rows 9,11 -> line rows 10,12
        frame.gotoxy(L1, 10);  frame.putmsg("\1h\1k" + hLine(LW) + BOX.TR + "\1n");
        frame.gotoxy(L1+LW, 11); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(L1+LW, 12); frame.putmsg("\1h\1k" + BOX.RT + hLine(LW) + BOX.BR + "\1n");
        frame.gotoxy(L1+LW, 13); frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(L1, 14);  frame.putmsg("\1h\1k" + hLine(LW) + BOX.BR + "\1n");
        
        // Left SF connector (vertical from row 5 to 12, tee at row 8)
        for (var r = 5; r <= 12; r++) {
            frame.gotoxy(L2+LW, r);
            if (r === 8) {
                frame.putmsg("\1h\1k" + BOX.RT + hLine(LW) + BOX.TR + "\1n");
            } else {
        ...
        frame.gotoxy(R1, 2);   frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + "\1n");
        frame.gotoxy(R1, 3);   frame.putmsg("\1h\1k" + BOX.V + "\1n");
        frame.gotoxy(R2, 4);   frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + BOX.LT + "\1n");
        ...
        frame.gotoxy(R2, 12);  frame.putmsg("\1h\1k" + BOX.BL + hLine(LW) + BOX.LT + "\1n");
        ...
        for (var r = 5; r <= 12; r++) {
            frame.gotoxy(R2, r);
            if (r === 8) {
                frame.putmsg("\1h\1k" + BOX.TL + hLine(LW) + BOX.LT + "\1n");
            } else {
                frame.putmsg("\1h\1k" + BOX.V + "\1n");
            }
        }
