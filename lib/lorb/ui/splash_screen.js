...
(function() {
    ...
    function showSplash() {
        ...
        if (typeof BinLoader !== "undefined" && file_exists(SPLASH_ART_PATH)) {
            try {
                var binData = BinLoader.load(SPLASH_ART_PATH);
                if (binData && binData.data) {
                    // Calculate centering
                    var artWidth = binData.width || 80;
                    var artHeight = binData.height || 20;
                    var offsetX = Math.floor((SCREEN_WIDTH - artWidth) / 2);
                    var offsetY = Math.floor((SCREEN_HEIGHT - artHeight - 2) / 2);  // Leave room for prompt
                    
                    // Blit the art to frame
                    BinLoader.blitToFrame(mainFrame, binData, offsetX, offsetY);
                    artLoaded = true;
                }
            } catch (e) {
        ...
        if (!artLoaded && file_exists(SPLASH_ANS_PATH)) {
            try {
                var ansFrame = new Frame(1, 1, SCREEN_WIDTH, SCREEN_HEIGHT - 2, BG_BLACK, mainFrame);
                ansFrame.load(SPLASH_ANS_PATH);
                ansFrame.open();
                artLoaded = true;
            } catch (e) {
