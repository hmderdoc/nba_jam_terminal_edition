...
(function() {
    ...
    function initializeMatchScheduling(match, player1Data, player2Data) {
        ...
        if (overlap) {
            ...
            log(LOG_INFO, "[PLAYOFF-SCHED] Scheduled match " + match.id + 
                " for " + new Date(overlap.startUTC).toISOString());
        } else {
            log(LOG_INFO, "[PLAYOFF-SCHED] No overlap found for match " + match.id + 
                " - will rely on opportunistic PvP");
        }
    }
    ...
    function rescheduleMatch(match, requestingPlayerId, player1Data, player2Data) {
        ...
        if (overlap) {
            ...
            log(LOG_INFO, "[PLAYOFF-SCHED] Rescheduled match " + match.id + 
                " for " + new Date(overlap.startUTC).toISOString() + 
                " (requested by " + requestingPlayerId + ")");
            ...
            log(LOG_INFO, "[PLAYOFF-SCHED] Reschedule for match " + match.id + 
                " found no overlap - cleared schedule");
