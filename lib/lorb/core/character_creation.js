...
(function() {
     ...
     * Shows live sprite preview on the right side of the screen
     */
    function customizeAppearance(ctx) {
        ...
        ctx.appearance.eyeColor = ctx.appearance.eyeColor || "WHITE";
        ...
        // Check if sprite preview is available
        var previewAvailable = (LORB.SpritePreview && typeof LORB.SpritePreview.update === "function");
        
        /**
         * Update the sprite preview with current appearance
         */
        function refreshPreview() {
            if (!previewAvailable) return;
            try {
                LORB.SpritePreview.update(ctx.appearance, ctx.nickname || ctx.name || "PLAYER");
            } catch (e) {
        ...
        while (true) {
            ...
            // Show current appearance options (left side)
            ...
            // Update sprite preview (right side)
            refreshPreview();
            ...
            if (choice === 1) {
                ...
                // Close preview temporarily for input
                if (previewAvailable) LORB.SpritePreview.close();
                LORB.View.line("");
