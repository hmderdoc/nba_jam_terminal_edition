    ...
    function getPlayerName(defaultName) {
        ...
        if (RichView && typeof BinLoader !== "undefined") {
            try {
                ...
                var footerFrame = view.getZone("footer");
                if (footerFrame) {
                    var promptText = "Drop your name (Enter=default): ";
                    footerFrame.gotoxy(2, 2);
                    footerFrame.putmsg("\1w" + promptText + "\1n");
                    view.renderForInput();
                    
                    // Position console cursor after the prompt text
                    console.gotoxy(footerFrame.x + 1 + promptText.length, footerFrame.y + 1);
                }
