...
(function() {
    ...
    function showIntro() {
        ...
        if (RichView && typeof BinLoader !== "undefined") {
            try {
                ...
                // Content zone - narrative
                view.setContentZone("content");
                view.setCursorY(0);
                
                view.line("\1cThe city never sleeps.\1n");
                view.line("\1cNeither do its courts.\1n");
                view.blank();
                view.line("Where ballers become legends.");
                view.line("Where the rims never rust.");
                view.line("Where Red Bull changes fate.");
                view.blank();
                view.line("At the center of it all...");
                view.line("\1rThe Court of Airness.\1n");
                view.line("Where \1rThe Red Bull\1n waits.");
                view.blank();
                view.line("No one has beaten it.");
                view.line("\1yNot yet.\1n");
                
                // Footer zone - prompt
                var footerFrame = view.getZone("footer");
                if (footerFrame) {
                    footerFrame.gotoxy(2, 2);
                    footerFrame.putmsg("\1h\1wBut first, prove you belong here.\1n");
                    footerFrame.gotoxy(2, 3);
                    footerFrame.putmsg("\1cPress any key to begin your journey...\1n");
                }
                ...
                log(LOG_WARNING, "[CHAR_CREATE] RichView intro failed: " + e);
            }
        }
    ...
    function getPlayerName(defaultName) {
        ...
        if (RichView && typeof BinLoader !== "undefined") {
            try {
                ...
                view.line("\1wEvery legend needs a name.\1n");
                view.line("What's yours?");
                view.blank();
                ...
                if (footerFrame) {
                    footerFrame.gotoxy(2, 2);
                    footerFrame.putmsg("\1wYour name (Enter for default): \1n");
                    view.render();
                }
                ...
                log(LOG_WARNING, "[CHAR_CREATE] RichView name entry failed: " + e);
            }
        }
        ...
        if (RichView && typeof BinLoader !== "undefined") {
            try {
                while (true) {
                    ...
                    var drawArchetypeDetail = function(arch) {
                        var footerFrame = view.getZone("footer");
                        ...
                        footerFrame.gotoxy(2, 3);
                        footerFrame.putmsg("\1ySpecial:\1n " + arch.special.name + " - " + arch.special.description);
                    ...
                    // Content zone - lightbar menu
                    view.setContentZone("content");
                    view.setCursorY(0);
                    
                    view.line("How do you dominate the court?");
                    view.blank();
                    
                    // Build menu items (just names)
                    var menuItems = [];
                    for (var i = 0; i < archetypes.length; i++) {
                        var arch = archetypes[i];
                        var pos = LORB.Data.getPositionFromArchetype(arch.id);
                        menuItems.push({
                            text: "\1w" + (i + 1) + ") \1y" + arch.name + "\1n  \1k(" + pos.name + ")\1n",
                            value: arch.id,
                            _archetype: arch
                        });
                    }
                    ...
                    drawArchetypeDetail(archetypes[0]);
                    ...
                    var choice = view.menu(menuItems, {
                        ...
                        onSelect: function(item, index, richView, lb) {
                            if (item._archetype) {
                                drawArchetypeDetail(item._archetype);
                            }
                        }
                    });
                    ...
                    if (choice) {
                        ...
                        if (selected) {
                            ...
                            LORB.View.line("");
                            LORB.View.line(selected.flavorText.replace(/\n/g, "\n"));
                            LORB.View.line("");
                ...
                log(LOG_WARNING, "[CHAR_CREATE] RichView archetype failed: " + e);
            }
        }
        ...
        if (RichView && typeof BinLoader !== "undefined") {
            try {
                while (true) {
                    ...
                    var drawBackgroundDetail = function(bg) {
                        var footerFrame = view.getZone("footer");
                        if (!footerFrame) return;
                        ...
                        footerFrame.gotoxy(2, 2);
                        footerFrame.putmsg("\1c" + bg.description + "\1n");
                        
                        // Show flavor text snippet on line 3
                        var flavorSnippet = bg.flavorText.split("\n")[0];
                        if (flavorSnippet.length > 74) flavorSnippet = flavorSnippet.substring(0, 74) + "...";
                        footerFrame.gotoxy(2, 3);
                    ...
                    if (headerFrame && LORB.Util && LORB.Util.FigletBanner && LORB.Util.FigletBanner.renderToFrame) {
                        ...
                        headerFrame.gotoxy(5, 2);
                        headerFrame.putmsg("\1h\1mWHERE DID YOU COME FROM?\1n");
                    }
                    ...
                    view.line("Every legend has an origin story.");
                    view.blank();
                    ...
                    drawBackgroundDetail(backgrounds[0]);
                    ...
                    var choice = view.menu(menuItems, {
                        ...
                        onSelect: function(item, index, richView, lb) {
                            if (item._background) {
                                drawBackgroundDetail(item._background);
                            }
                        }
                    });
                ...
                log(LOG_WARNING, "[CHAR_CREATE] RichView background failed: " + e);
            }
        }
    ...
    function showSummary(name, archetype, background, stats, ctx) {
        while (true) {
            ...
            LORB.View.line("\1wProceed into game? (Y/N/R=Restart)\1n");
