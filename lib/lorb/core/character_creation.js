...
(function() {
    ...
    /**
     * Show welcome screen with Barney graphic using RichView
     */
    function showBarneyWelcome(playerName) {
        // Try to show with RichView and Barney graphic
        if (RichView && typeof BinLoader !== "undefined") {
            try {
                ...
                // Try to load Barney art into art zone
                var artFrame = view.getZone("art");
                var barneyLoaded = false;
                
                if (artFrame) {
                    try {
                        var binData = BinLoader.loadBinFile(BARNEY_ART);
                        if (binData) {
                            // Draw border
                            artFrame.gotoxy(1, 1);
                            ...
                            // Render Barney art (40x20 .bin file)
                            // Each cell is 2 bytes: char + attr
                            var offset = 0;
                            var artStartY = 2;
                            var artStartX = 2;
                            var artWidth = Math.min(BARNEY_W, 35);
                            var artHeight = Math.min(BARNEY_H, 20);
                            
                            for (var ay = 0; ay < artHeight; ay++) {
                                for (var ax = 0; ax < BARNEY_W; ax++) {
                                    if (offset + 1 < binData.length) {
                                        var ch = binData.charAt(offset++);
                                        var attr = binData.charCodeAt(offset++);
                                        
                                        // Only render if within our display area
                                        if (ax < artWidth) {
                                            try {
                        ...
                        log(LOG_WARNING, "[CHAR_CREATE] Failed to load Barney art: " + e);
                    }
                }
                
                if (!barneyLoaded && artFrame) {
                    // Draw placeholder
                    artFrame.gotoxy(1, 1);
                    artFrame.putmsg("\1n\1h\1m\xDA" + repeatChar("\xC4", 36) + "\xBF\1n");
                    for (var y = 2; y < 23; y++) {
                    ...
                    artFrame.putmsg("\1m\1hBARNEY DINOSAUR\1n");
                ...
                view.blank();
                view.line("\1m\1hBarney Dinosaur\1n is waiting at the");
                view.line("courts. He's not much, but he's loyal.");
                view.blank();
                view.line("He'll be your first teammate on the");
        ...
        LORB.View.line("");
        LORB.View.line("\1mBarney Dinosaur\1n is waiting at the courts.");
        LORB.View.line("He's not much, but he's loyal.");
        LORB.View.line("");
    ...
    function runCharacterCreation(user, systemObj) {
        while (true) {
            ...
            if (confirmed) {
                ...
                // Show welcome message with Barney graphic
                showBarneyWelcome(name);
