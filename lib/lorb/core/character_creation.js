...
(function() {
    ...
    /**
     * Get string input with ESC to quit support
     * Returns QUIT_SIGNAL if user presses ESC, otherwise the input string
     */
    function getStrWithEscape(prompt, maxLen) {
        // For console.getstr we can't easily intercept ESC mid-input
        // So we check for ESC as first character or empty + ESC
        var result = console.getstr("", maxLen || 20, K_EDIT);
        // If they entered just ESC character or nothing and hit ESC
        if (result === ESC_KEY || result === "") {
            // Give them a chance to confirm quit with just ESC key
            return result;
        }
        return result;
    }
    ...
    function getPlayerName(defaultName) {
        ...
        if (RichView && typeof BinLoader !== "undefined") {
            try {
                ...
                if (footerFrame) {
                    footerFrame.gotoxy(2, 2);
                    footerFrame.putmsg("\1wDrop your name (\1h\1kESC\1n\1w=quit, Enter=default): \1n");
                    view.render();
                }
        ...
        var name = LORB.View.prompt("Drop your name (\1h\1kESC\1n=quit, Enter=default): ");
    ...
    function chooseArchetype() {
        ...
        while (true) {
            ...
            LORB.View.line("");
            var choice = LORB.View.prompt("Choose (1-" + archetypes.length + ", \1h\1kESC\1n=quit): ");
    ...
    function chooseBackground() {
        ...
        while (true) {
            ...
            var choice = LORB.View.prompt("Choose (1-" + backgrounds.length + ", \1h\1kESC\1n=quit): ");
