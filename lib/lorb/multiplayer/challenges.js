...
(function () {
    ...
    function pruneForPlayer(client, gid, ts, all) {
        if (!gid) return;
        if (!all) {
            all = client.read(DB_SCOPE, ROOT, LOCK_READ);
            if (typeof debugLog === "function") {
                var count = all ? Object.keys(all).length : 0;
                debugLog("[LORB:Challenges] readAll count=" + count + " for prune gid=" + gid);
            }
        }
        if (!all) return;
        for (var id in all) {
    ...
    function listIncoming(ctx) {
        ...
        return withClient(function (client) {
            cleanupExpired(client, ts);
            var all = client.read(DB_SCOPE, ROOT, LOCK_READ) || {};
            if (typeof debugLog === "function") {
                debugLog("[LORB:Challenges] readAll incoming count=" + Object.keys(all).length + " gid=" + gid);
            }
            pruneForPlayer(client, gid, ts, all);  // Also trims for this player
            var incoming = [];
            for (var id in all) {
                if (!all.hasOwnProperty(id)) continue;
                var ch = all[id];
    ...
    function listOutgoing(ctx) {
        ...
        return withClient(function (client) {
            ...
            var all = client.read(DB_SCOPE, ROOT, LOCK_READ) || {};
            if (typeof debugLog === "function") {
                debugLog("[LORB:Challenges] readAll outgoing count=" + Object.keys(all).length + " gid=" + gid);
            }
            pruneForPlayer(client, gid, ts, all);
            var outgoing = [];
            for (var id in all) {
                if (!all.hasOwnProperty(id)) continue;
                var ch = all[id];
