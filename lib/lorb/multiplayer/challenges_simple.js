/**
 * challenges_simple.js - DEPRECATED: Alternative challenge coordination
 * 
 * @deprecated Use challenges_pubsub.js instead. This file was an intermediate
 * implementation attempt. It is kept only for legacy tests and will be removed.
 *
 * The canonical implementation is challenges_pubsub.js which uses non-blocking pub/sub.
 * This file is NOT loaded in production (see boot.js).
 *
 * Modeled after oneliners/lib.js - a known working pattern.
 * Simple: connect, read, write, subscribe, cycle.
 * Uses shared JSONClient from json_client_helper.js for connection pooling.
 */
(function () {
    if (!this.LORB) return;
    if (!this.LORB.Multiplayer) this.LORB.Multiplayer = {};
    
    // Load json_client_helper if not already loaded
    if (!LORB.JsonClientHelper) {
        load(js.exec_dir + "lib/lorb/util/json_client_helper.js");
    }
    ...
    // Always unlock a path after operations to prevent blocking other users
    function safeUnlock(path) {
        try {
            var c = getClient();
            if (c && c.connected && c.unlock) {
                c.unlock(SCOPE, path);
            }
        } catch (e) {
    ...
    function extractTeammateData(ctx) {
        ...
        // getActiveTeammate already returns hydrated contact, no fallback needed
        ...
        log("extractTeammateData: returning teammate=" + result.name + 
            ", skin=" + result.skin + ", shortNick=" + (result.shortNick || "null"));
    ...
    /**
     * Get the shared JSONClient from json_client_helper.
     * Connection management (timeouts, backoff) is handled by the helper.
     */
    function connect() {
        if (!LORB.JsonClientHelper || !LORB.JsonClientHelper.ensureClient) {
            log("JsonClientHelper not available");
            return null;
        }
        return LORB.JsonClientHelper.ensureClient({ skipVersionCheck: true });
    }
    
    /**
     * Get current client without reconnecting - used for cycle() calls
     */
    function getClient() {
        if (!LORB.JsonClientHelper || !LORB.JsonClientHelper.getClient) {
            return null;
        }
        return LORB.JsonClientHelper.getClient();
    }
    
    function disconnect() {
        // Don't disconnect shared client - let json_client_helper manage it
    ...
    function setPresence(ctx) {
        ...
        var path = PRESENCE + "." + gid;
        try {
            // Fire-and-forget write to avoid blocking
            var oldTimeout = client.settings ? client.settings.TIMEOUT : undefined;
            if (client.settings) {
                client.settings.TIMEOUT = -1;
            }
            
            client.write(SCOPE, path, data);
            
            if (client.settings && oldTimeout !== undefined) {
                client.settings.TIMEOUT = oldTimeout;
            }
            
            safeUnlock(path);
            log("presence set for " + gid + " (fire-and-forget)");
            return true;
        } catch (e) {
            safeUnlock(path);
    ...
    function clearPresence(ctx) {
        ...
        var path = PRESENCE + "." + gid;
        try {
            // Fire-and-forget remove to avoid blocking on exit
            var oldTimeout = client.settings ? client.settings.TIMEOUT : undefined;
            if (client.settings) {
                client.settings.TIMEOUT = -1;
            }
            
            client.remove(SCOPE, path);
            ...
            safeUnlock(path);
            log("presence cleared for " + gid + " (fire-and-forget)");
            return true;
        } catch (e) {
            safeUnlock(path);
    ...
    /**
     * Get online players - uses cached data from ChallengeService if available,
     * falls back to direct read only if service not started
     */
    function getOnlinePlayers() {
        // Prefer cached data from service (no network call)
        if (LORB.Multiplayer && LORB.Multiplayer.ChallengeService && 
            LORB.Multiplayer.ChallengeService.isStarted && 
            LORB.Multiplayer.ChallengeService.isStarted()) {
            return LORB.Multiplayer.ChallengeService.getOnlinePlayers() || {};
        }
        
        // Fallback: direct read (only if service not available)
        var client = connect();
        if (!client) return {};
        cycle();
        
        try {
            var all = client.read(SCOPE, PRESENCE);
            safeUnlock(PRESENCE);
            ...
            log("getOnlinePlayers (fallback read): " + Object.keys(online).length + " online");
            return online;
        } catch (e) {
            safeUnlock(PRESENCE);
    ...
    function createChallenge(ctx, targetPlayer, meta, wagerOffer) {
        ...
        try {
            // Write to both player paths then unlock
            var path1 = CHALLENGES + "." + fromGid + "." + id;
            var path2 = CHALLENGES + "." + toGid + "." + id;
            client.write(SCOPE, path1, challenge);
            safeUnlock(path1);
            client.write(SCOPE, path2, challenge);
            safeUnlock(path2);
            log("challenge created: " + id);
            return challenge;
        } catch (e) {
            safeUnlock(CHALLENGES + "." + fromGid);
            safeUnlock(CHALLENGES + "." + toGid);
    ...
    function getChallengesForPlayer(gid) {
        ...
        var path = CHALLENGES + "." + gid;
        try {
            var bucket = client.read(SCOPE, path);
            safeUnlock(path);
    ...
    /**
     * List incoming challenges - uses cached data from service when available
     */
    function listIncoming(ctx) {
        ...
        if (LORB.Multiplayer && LORB.Multiplayer.ChallengeService && 
            LORB.Multiplayer.ChallengeService.isStarted && 
            LORB.Multiplayer.ChallengeService.isStarted()) {
            return LORB.Multiplayer.ChallengeService.getIncoming() || [];
        }
    ...
    /**
     * List outgoing challenges - uses cached data from service when available
     */
    function listOutgoing(ctx) {
        ...
        if (LORB.Multiplayer && LORB.Multiplayer.ChallengeService && 
            LORB.Multiplayer.ChallengeService.isStarted && 
            LORB.Multiplayer.ChallengeService.isStarted()) {
            return LORB.Multiplayer.ChallengeService.getOutgoing() || [];
        }
    ...
    function updateChallenge(id, fromGid, toGid, updates) {
        ...
        var readPath = CHALLENGES + "." + fromGid + "." + id;
        var path1 = CHALLENGES + "." + fromGid + "." + id;
        var path2 = CHALLENGES + "." + toGid + "." + id;
        
        try {
            // Read current then unlock
            var ch = client.read(SCOPE, readPath);
            safeUnlock(readPath);
            if (!ch) return null;
            ...
            // Write back to both paths then unlock
            client.write(SCOPE, path1, ch);
            safeUnlock(path1);
            client.write(SCOPE, path2, ch);
            safeUnlock(path2);
    ...
    function getChallenge(id, ctx) {
        ...
        var path = CHALLENGES + "." + gid + "." + id;
        try {
            var result = client.read(SCOPE, path);
            safeUnlock(path);
            return result;
        } catch (e) {
            safeUnlock(path);
    ...
    // =====================
    // DIRECT FETCH METHODS (for ChallengeService polling - bypass cache)
    // =====================
    
    /**
     * Fetch challenges directly from server (bypasses isStarted cache check)
     * Used by ChallengeService.poll() to get fresh data
     */
    function fetchChallengesForPlayer(gid) {
        var client = connect();
        if (!client) return { incoming: [], outgoing: [] };
        
        var path = CHALLENGES + "." + gid;
        try {
            var bucket = client.read(SCOPE, path);
            safeUnlock(path);
            if (!bucket || typeof bucket !== "object") return { incoming: [], outgoing: [] };
            ...
            for (var id in bucket) {
                if (!bucket.hasOwnProperty(id)) continue;
                var ch = bucket[id];
                if (!ch || !ch.expiresAt || ch.expiresAt <= now) continue;
                if (ch.status !== "pending" && ch.status !== "accepted") continue;
                
                if (ch.to && ch.to.globalId === gid) {
                    incoming.push(ch);
                }
                if (ch.from && ch.from.globalId === gid) {
                    outgoing.push(ch);
                }
            }
            
            log("fetchChallengesForPlayer: gid=" + gid + " incoming=" + incoming.length + " outgoing=" + outgoing.length);
            return { incoming: incoming, outgoing: outgoing };
        } catch (e) {
            safeUnlock(path);
            log("fetchChallengesForPlayer error: " + e);
            return { incoming: [], outgoing: [] };
        }
    }
    
    /**
     * Fetch presence directly from server (bypasses isStarted cache check)
     * Used by ChallengeService.poll() to get fresh data
     */
    function fetchOnlinePlayers() {
        ...
        try {
            ...
            log("fetchOnlinePlayers: " + Object.keys(online).length + " online");
            return online;
        } catch (e) {
            safeUnlock(PRESENCE);
            log("fetchOnlinePlayers error: " + e);
            return {};
        }
    }
    ...
    this.LORB.Multiplayer.Challenges = {
        ...
        // Direct fetch methods (for ChallengeService polling)
        fetchChallengesForPlayer: fetchChallengesForPlayer,
