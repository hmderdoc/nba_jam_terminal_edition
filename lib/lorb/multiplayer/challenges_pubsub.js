...
(function () {
    ...
    function fetchInitialData(gid) {
        ...
        // Use short timeout for initial fetch - fail fast, subscriptions will catch updates
        ...
        try {
            // Fetch my challenge bucket
            // NOTE: Using read WITHOUT lock to avoid blocking other users.
            // We're subscribed so we'll get updates anyway. Initial data may be slightly stale.
     ...
     * CRITICAL: Must be completely non-blocking to avoid cross-user lockups.
     * Presence will expire naturally after PRESENCE_TIMEOUT_MS anyway.
     */
    function disconnect() {
        if (client) {
            try {
                // Fire-and-forget presence removal - do NOT use locks here
                // as it can block if other users are subscribed to presence updates.
                // The presence entry will expire naturally after PRESENCE_TIMEOUT_MS.
                if (myGlobalId) {
                    var path = PRESENCE_ROOT + "." + myGlobalId;
                    try {
                        // Set fire-and-forget mode to avoid blocking
                        var oldTimeout = client.settings.TIMEOUT;
                        client.settings.TIMEOUT = -1;  // Fire and forget
                        
                        // Remove WITHOUT locking - if it fails, the presence expires naturally
                        client.remove(DB_SCOPE, path);
                        
                        client.settings.TIMEOUT = oldTimeout;
                    } catch (e) {
                        // Ignore errors - presence will expire naturally
                        logInfo("disconnect presence removal skipped: " + e);
                    }
                }
