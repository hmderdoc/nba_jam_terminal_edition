...
(function () {
     ...
     * Uses NBA_JAM.JsonClient singleton if available, with fallback to direct creation
     */
    function createClient(serverConfig) {
        var addr = serverConfig.addr || "localhost";
        var port = serverConfig.port || 10088;
        var client;
        
        // Use shared singleton if available
        if (typeof NBA_JAM !== "undefined" && NBA_JAM.JsonClient && NBA_JAM.JsonClient.get) {
            NBA_JAM.JsonClient.configure({ addr: addr, port: port });
            client = NBA_JAM.JsonClient.get();
        } else {
            ...
            // Set fire-and-forget mode for writes - we use sync loop for coordination
            // This prevents 30-second default timeout from blocking the BBS
            // See: JSONDB-ARCHITECTURE-AUDIT-v2.md
            if (client && client.settings) {
                client.settings.TIMEOUT = -1;  // Fire-and-forget writes
                client.settings.SOCK_TIMEOUT = 2000;  // 2s socket timeout for reads
            }
        }
        
        if (!client || !client.connected) {
            throw new Error("Failed to connect to " + addr + ":" + port);
        }
        return client;
    }
