...
(function () {
    ...
    /**
     * Synchronized game launch with proper two-way handshaking
     * 
     * PROTOCOL:
     * 1. Both players write their "ready" signal to sync path
     * 2. Both players poll until they see BOTH ready signals
     * 3. Coordinator writes startTime once both are ready
     * 4. Both players countdown to startTime
     * 5. Both players start game loop at same moment
     * 
    ...
    function showSynchronizedCountdown(client, challengeId, myGlobalId, isChallenger, countdownSeconds) {
        ...
        // =====================================================
        // PHASE 2: Wait for opponent's ready signal
        // =====================================================
        ...
        // =====================================================
        // PHASE 3: Coordinator sets start time, client waits for it
        // =====================================================
        ...
        if (isChallenger) {
            ...
            // I'm client - wait for coordinator to set start time
    ...
    function launchLorbMatch(challenge, myCtx, isChallenger) {
        ...
        // NOTE: For challenger, myCtx is full local ctx; opponentInfo is challenge.to
        //       For challengee, myCtx is full local ctx; opponentInfo is challenge.from
        ...
        try {
            ...
            if (isChallenger) {
                ...
                // Poll until session exists with correct coordinator (up to 10 seconds)
                ...
                while (!sessionReady && (Date.now() - waitStart) < maxWait) {
                    mswait(500);
                    var sessionCheck = client.read("nba_jam", "game." + challenge.id + ".meta", 1);
