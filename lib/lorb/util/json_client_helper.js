...
(function () {
    ...
    function checkVersionCompatibility(jsonClient, scope, serverName) {
        ...
        try {
            ...
            var isLocalhost = (cfg.addr === "localhost" || cfg.addr === "127.0.0.1");
            ...
            // If connecting to localhost, we are the authority - update server version to match ours
            if (isLocalhost && localVersion !== serverVersion) {
                logInfo("Localhost authority: updating server version from " + serverVersion + " to " + localVersion);
                jsonClient.write(scope, VERSION_PATH, {
                    commit: localVersion,
                    publishedAt: nowMs(),
                    publishedBy: system.qwk_id || system.name || "unknown"
