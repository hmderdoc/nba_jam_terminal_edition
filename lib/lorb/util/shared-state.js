...
(function () {
    ...
    function timeUntilNextDay(currentMs) {
        ...
        if (dayDuration === 86400000) {
            ...
            // Custom day duration - calculate relative to seasonStart
            var state = readState();
            var seasonStart = (state && state.seasonStart) ? state.seasonStart : currentMs;
            var elapsed = currentMs - seasonStart;
            var dayProgress = elapsed % dayDuration;
            return dayDuration - dayProgress;
        }
    }
