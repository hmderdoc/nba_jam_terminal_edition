 ...
 * Supports per-season tracking with ctx.seasonStats[seasonNumber].
 * 
 * Stats tracked:
 ...
 * 
 * Data structures:
 * - ctx.careerStats: All-time cumulative stats
 * - ctx.records: All-time single-game bests
 * - ctx.seasonStats[seasonNumber]: Per-season cumulative stats (same structure as careerStats)
 * - ctx.seasonRecords[seasonNumber]: Per-season single-game bests (same structure as records)
 */
(function() {
    ...
    /**
     * Initialize season stats structure for a specific season
     * @param {Object} ctx - Player context
     * @param {number} seasonNumber - Season number
     */
    function ensureSeasonStats(ctx, seasonNumber) {
        if (!ctx.seasonStats) {
            ctx.seasonStats = {};
        }
        if (!ctx.seasonStats[seasonNumber]) {
            ctx.seasonStats[seasonNumber] = {
                gamesPlayed: 0,
                wins: 0,
                losses: 0,
                totals: {}
            ...
            for (var i = 0; i < STAT_KEYS.length; i++) {
                ctx.seasonStats[seasonNumber].totals[STAT_KEYS[i]] = 0;
            }
        }
        return ctx.seasonStats[seasonNumber];
    }
    
    /**
     * Initialize season records structure for a specific season
     * @param {Object} ctx - Player context
     * @param {number} seasonNumber - Season number
     */
    function ensureSeasonRecords(ctx, seasonNumber) {
        if (!ctx.seasonRecords) {
            ctx.seasonRecords = {};
        }
        if (!ctx.seasonRecords[seasonNumber]) {
            ctx.seasonRecords[seasonNumber] = {};
            for (var i = 0; i < STAT_KEYS.length; i++) {
                ctx.seasonRecords[seasonNumber][STAT_KEYS[i]] = {
                    value: 0,
                    date: null,
                    opponent: null,
                    court: null
        ...
        return ctx.seasonRecords[seasonNumber];
    }
    
    /**
     * Get current season number from LORB.SharedState
     * @returns {number} Current season number, defaults to 1
     */
    function getCurrentSeason() {
        if (typeof LORB !== "undefined" && LORB.SharedState && LORB.SharedState.getInfo) {
            var info = LORB.SharedState.getInfo();
            return info.seasonNumber || 1;
        }
        return 1;
    }
    
    /**
     * Get list of all seasons a player has stats for
     * @param {Object} ctx - Player context
     * @returns {Array<number>} Array of season numbers, sorted ascending
     */
    function getPlayerSeasons(ctx) {
        var seasons = [];
        if (ctx.seasonStats) {
            for (var key in ctx.seasonStats) {
                if (ctx.seasonStats.hasOwnProperty(key)) {
    ...
    /**
     * Get season averages (per game) for a specific season
     * @param {Object} ctx - Player context
     * @param {number} seasonNumber - Season number to get averages for
     * @returns {Object|null} Averages for each stat, or null if no data for that season
     */
    function getSeasonAverages(ctx, seasonNumber) {
        if (!ctx.seasonStats || !ctx.seasonStats[seasonNumber]) {
            return null;
        }
        
        var season = ctx.seasonStats[seasonNumber];
        var games = season.gamesPlayed || 0;
        var averages = {};
        
        for (var i = 0; i < STAT_KEYS.length; i++) {
            var key = STAT_KEYS[i];
            var total = (season.totals && season.totals[key]) || 0;
            averages[key] = games > 0 ? (total / games) : 0;
        }
        
        // Calculate FG% and 3PT%
        var fgm = (season.totals && season.totals.fgm) || 0;
        var fga = (season.totals && season.totals.fga) || 0;
        var tpm = (season.totals && season.totals.tpm) || 0;
        var tpa = (season.totals && season.totals.tpa) || 0;
        
        averages.fgPct = fga > 0 ? Math.round((fgm / fga) * 1000) / 10 : 0;
        averages.tpPct = tpa > 0 ? Math.round((tpm / tpa) * 1000) / 10 : 0;
        averages.gamesPlayed = games;
        
        return averages;
    }
    
    /**
     * Get stats (career or season) based on season selector
     * @param {Object} ctx - Player context
     * @param {number|string} season - Season number or "all" for career
     * @returns {Object} Stats object with gamesPlayed and totals
     */
    function getStatsForSeason(ctx, season) {
        if (season === "all" || season === null || season === undefined) {
            return ensureCareerStats(ctx);
        }
        var seasonNum = parseInt(season, 10);
        if (ctx.seasonStats && ctx.seasonStats[seasonNum]) {
            return ctx.seasonStats[seasonNum];
        }
        // Return empty structure for seasons with no data
        return { gamesPlayed: 0, totals: {} };
    }
    
    /**
     * Get records (career or season) based on season selector
     * @param {Object} ctx - Player context
     * @param {number|string} season - Season number or "all" for career
     * @returns {Object} Records object
     */
    function getRecordsForSeason(ctx, season) {
        if (season === "all" || season === null || season === undefined) {
            return ensureRecords(ctx);
        }
        var seasonNum = parseInt(season, 10);
        if (ctx.seasonRecords && ctx.seasonRecords[seasonNum]) {
            return ctx.seasonRecords[seasonNum];
        }
        // Return empty structure for seasons with no data
        var empty = {};
        for (var i = 0; i < STAT_KEYS.length; i++) {
            empty[STAT_KEYS[i]] = { value: 0, date: null, opponent: null, court: null };
        }
        return empty;
    }
     ...
     * Also checks for new records (both career and season)
     * 
     * @param {Object} ctx - Player context
     * @param {Object} gameStats - Stats from this game
     * @param {Object} gameInfo - Optional game context (opponent, court, seasonNumber, etc.)
    ...
    function recordGame(ctx, gameStats, gameInfo) {
        ...
        // Get season number (from gameInfo, or from SharedState, or default to 1)
        var seasonNumber = info.seasonNumber || getCurrentSeason();
        var seasonStats = ensureSeasonStats(ctx, seasonNumber);
        var seasonRecords = ensureSeasonRecords(ctx, seasonNumber);
        
        career.gamesPlayed++;
        seasonStats.gamesPlayed++;
        
        var newRecords = [];
        var newSeasonRecords = [];
        var now = Date.now();
        ...
        for (var i = 0; i < STAT_KEYS.length; i++) {
            ...
            seasonStats.totals[key] = (seasonStats.totals[key] || 0) + value;
            ...
            var currentSeasonRecord = seasonRecords[key].value || 0;
            ...
            if (value > currentSeasonRecord) {
                ...
                // Only announce positive stat season records (if it's not also a career record)
                if (!isNegativeStat && value > 0 && value <= currentRecord) {
                    newSeasonRecords.push({
                        stat: key,
                        name: STAT_NAMES[key],
                        abbrev: STAT_ABBREVS[key],
                        value: value,
                        previousRecord: currentSeasonRecord,
        ...
        return {
            ...
            seasonGamesPlayed: seasonStats.gamesPlayed,
            seasonNumber: seasonNumber,
            newRecords: newRecords,
            newSeasonRecords: newSeasonRecords
        };
    }
    
    /**
     * Record a win or loss for the current season
     * This should be called when a game ends (in addition to recordGame for stats)
     * 
     * @param {Object} ctx - Player context
     * @param {boolean} isWin - True if player won, false if lost
     * @param {number} seasonNumber - Optional season number (defaults to current)
     */
    function recordWinLoss(ctx, isWin, seasonNumber) {
        seasonNumber = seasonNumber || getCurrentSeason();
        var seasonStats = ensureSeasonStats(ctx, seasonNumber);
        
        if (isWin) {
            seasonStats.wins = (seasonStats.wins || 0) + 1;
        } else {
            seasonStats.losses = (seasonStats.losses || 0) + 1;
        }
    ...
    LORB.Util.CareerStats = {
        ...
        recordWinLoss: recordWinLoss,
        getAverages: getAverages,
        calculateBonuses: calculateBonuses,
        
        // Season-specific functions
        ensureSeasonStats: ensureSeasonStats,
        ensureSeasonRecords: ensureSeasonRecords,
        getCurrentSeason: getCurrentSeason,
        getPlayerSeasons: getPlayerSeasons,
        getSeasonAverages: getSeasonAverages,
        getStatsForSeason: getStatsForSeason,
        getRecordsForSeason: getRecordsForSeason,
