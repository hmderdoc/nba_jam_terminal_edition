...
(function() {
    ...
    function formatNewsHeadline(entry) {
        ...
        if (entry.type === "red_bull_defeated") {
            ...
            lines.push("\1y" + entry.playerName + "\1n has \1gdefeated the Red Bull!\1n");
        ...
        if (entry.type === "red_bull_loss") {
            var scoreText = entry.playerScore && entry.bossScore 
                ...
                : "";
            lines.push("\1y" + entry.playerName + "\1n fell to the \1h\1rRed Bull\1n" + scoreText);
            return lines.join("\n");
        }
        
        if (entry.type === "championship") {
            lines.push("\1h\1y*** NEW CHAMPION! ***\1n");
            lines.push("\1y" + entry.playerName + "\1n wins the \1cSeason " + entry.seasonNumber + "\1n championship!");
            return lines.join("\n");
        }
        
        // Default: PvP match format
        if (entry.isTie) {
            lines.push("\1w" + entry.winnerName + "\1n and \1w" + entry.loserName + "\1n played to a \1y" + 
                      entry.winnerScore + "-" + entry.loserScore + "\1n draw!");
        } else {
            lines.push("\1c" + entry.winnerName + "\1n defeated \1r" + entry.loserName + 
                      "\1n \1w" + entry.winnerScore + "-" + entry.loserScore + "\1n");
        }
        
        // Add top scorer callout
        if (entry.topScorer && entry.topScorerPoints > 0) {
            lines.push("  \1y" + entry.topScorer + "\1n led scoring with \1h" + 
        ...
        if (entry.newRecords && entry.newRecords.length > 0) {
            for (var i = 0; i < entry.newRecords.length; i++) {
                ...
                lines.push("  \1g\1h* NEW PVP RECORD:\1n " + rec.name + ": " + rec.value);
            }
        }
