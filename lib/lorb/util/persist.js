...
(function () {
    ...
    /**
     * Update a player's cash and rep balance directly (for ghost match outcomes).
     * This is used when the affected player is offline.
     * @param {string} globalId - The player's global ID
     * @param {number} cashDelta - Amount to add/subtract from cash (can be negative)
     * @param {number} repDelta - Amount to add/subtract from rep (can be negative)
     * @param {string} reason - Reason for the update (for logging)
     * @returns {boolean} True if update succeeded
     */
    function updatePlayerBalance(globalId, cashDelta, repDelta, reason) {
        if (!globalId) return false;
        ...
        var data = read(path);
        if (!data) {
            logDb("updatePlayerBalance", path, "player_not_found", start);
            return false;
        }
        
        // Update balances (don't go below 0)
        var oldCash = data.cash || 0;
        var oldRep = data.rep || 0;
        
        data.cash = Math.max(0, oldCash + (cashDelta || 0));
        data.rep = Math.max(0, oldRep + (repDelta || 0));
        
        // Track ghost match history for the affected player
        if (!data.ghostMatchHistory) data.ghostMatchHistory = [];
        data.ghostMatchHistory.push({
            timestamp: Date.now(),
            cashDelta: cashDelta,
            repDelta: repDelta,
        ...
        // Keep only last 20 ghost match entries
        if (data.ghostMatchHistory.length > 20) {
            data.ghostMatchHistory = data.ghostMatchHistory.slice(-20);
        }
        
        // Also update their win/loss record if this was a ghost match
        if (reason === "ghost_match_win") {
            data.wins = (data.wins || 0) + 1;
            data.gamesPlayed = (data.gamesPlayed || 0) + 1;
        } else if (reason === "ghost_match_loss") {
            data.losses = (data.losses || 0) + 1;
            data.gamesPlayed = (data.gamesPlayed || 0) + 1;
        }
        
        data._lastSave = Date.now();
        
        var ok = write(path, data);
        logDb("updatePlayerBalance", path, ok ? "ok" : "error", start, 
              "cashDelta=" + cashDelta + " repDelta=" + repDelta + " reason=" + reason);
