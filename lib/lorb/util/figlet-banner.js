...
(function() {
     ...
     * Supports multi-color rendering with 2-3 team colors.
     */
    function renderToFrame(frame, text, fgAttr, bgAttr) {
        ...
        // Build 4-row gradient from 3 team colors (bg, bg_alt, accent)
        // Row 0: dim primary (e.g., BLUE)
        // Row 1: bright primary (e.g., LIGHTBLUE)
        // Row 2: bright secondary (e.g., LIGHTRED)
        // Row 3: bright accent (e.g., WHITE)
        var rowColors = [];  // 4 colors, one per row
        var fg = (typeof fgAttr === "number") ? fgAttr : ((typeof WHITE === "number") ? WHITE : 7);
        
        // Always try to get team colors for multi-color mode
        try {
            if (typeof LORB !== "undefined" && LORB.Cities) {
                ...
                if (city && LORB.Cities.getTeamColors) {
                    ...
                    if (colors) {
                        // Get the 3 source colors from .ini
                        var c1Raw = (typeof colors.fgFromBgAttr === "number") ? colors.fgFromBgAttr : -1;
                        var c2Raw = (typeof colors.fgFromBgAltAttr === "number") ? colors.fgFromBgAltAttr : -1;
                        var c3Raw = (typeof colors.fgAccentAttr === "number") ? colors.fgAccentAttr : -1;
                        
                        // Build 4-color gradient if we have at least 2 source colors
                        if (c1Raw >= 0 && c2Raw >= 0) {
                            // Row 0: dim version of primary (no high intensity)
                            var c1Dim = (c1Raw >= 8) ? c1Raw - 8 : c1Raw;
                            // Row 1: bright version of primary
                            var c1Bright = (c1Raw < 8) ? c1Raw + 8 : c1Raw;
                            // Row 2: bright version of secondary
                            var c2Bright = (c2Raw < 8) ? c2Raw + 8 : c2Raw;
                            // Row 3: bright version of accent (or secondary if no accent)
                            var c3Bright;
                            if (c3Raw >= 0) {
                                c3Bright = (c3Raw < 8) ? c3Raw + 8 : c3Raw;
                            } else {
                                c3Bright = c2Bright;  // fallback to secondary
                            }
                            
                            rowColors = [c1Dim, c1Bright, c2Bright, c3Bright];
                        }
                    }
                }
            }
        ...
        // Calculate content width for horizontal centering
        var contentWidth = 0;
        for (var i = 0; i < lines.length; i++) {
            var trimmed = (lines[i] || "").replace(/\s+$/, "");
            if (trimmed.length > contentWidth) contentWidth = trimmed.length;
        }
        var xOffset = Math.max(0, Math.floor((maxWidth - contentWidth) / 2));
            ...
            for (var col = 0; col < line.length && col < maxWidth; col++) {
                ...
                if (ch && ch !== " ") {
                    ...
                    if (rowColors.length === 4) {
                        // 4-color gradient mode, adapted to actual row count
                        // rowColors[0]=dim primary, [1]=bright primary, [2]=bright secondary, [3]=bright accent
                        // For shorter figlets, use appropriate slice:
                        //   4 rows: [0,1,2,3] → BLUE, LIGHTBLUE, LIGHTRED, WHITE
                        //   3 rows: [1,2,3]   → LIGHTBLUE, LIGHTRED, WHITE  
                        //   2 rows: [1,2]     → LIGHTBLUE, LIGHTRED
                        //   1 row:  [1]       → LIGHTBLUE
                        var colorIdx;
                        if (totalRows >= 4) {
                            colorIdx = row;
                        } else if (totalRows === 3) {
                            colorIdx = row + 1;  // skip index 0
                        } else if (totalRows === 2) {
                            colorIdx = row + 1;  // row 0→1 (LIGHTBLUE), row 1→2 (LIGHTRED)
                        } else {
                            colorIdx = 1;  // single row: LIGHTBLUE
                        }
                        charColor = rowColors[Math.min(colorIdx, 3)];
                    } else {
                        // Single color mode with gradient shading
                        charColor = getShadedColor(ch, fg, row, totalRows);
                    }
                    
                    var attr = charColor | bgMask;
                    try {
                        frame.setData(xOffset + col, row, ch, attr, false);
                    } catch (e) {}
                }
            }
        }
