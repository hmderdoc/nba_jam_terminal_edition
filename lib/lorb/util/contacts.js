...
(function() {
     ...
     * MINIMAL STORAGE: Only stores id, type, status, dateAcquired
     * All other data (name, team, stats, etc.) is hydrated from rosters.ini via LORB.Data.Roster
     * 
     ...
     * @returns {Object|null} - Minimal contact object or null if already exists
     */
    function createContact(nbaPlayer, ctx) {
        ...
        // MINIMAL STORAGE: Only store what can't be looked up
        // Everything else (name, team, stats, skin, jersey, rivals, etc.) 
        // comes from rosters.ini via hydrateContact()
        return {
            id: id,
            type: "nba",
    ...
    /**
     * Hydrate a minimal contact with full player data from rosters.ini
     * Merges stored contact data with looked-up roster data
     * 
     * @param {Object} contact - Minimal contact object { id, type, status, dateAcquired }
     * @returns {Object} Full contact with all fields populated
     */
    function hydrateContact(contact) {
        if (!contact || !contact.id) return contact;
        
        // Starter companions (legends) already have full data - no hydration needed
        if (contact.isStarter || contact.type === "legend") {
            return contact;
        }
        
        // Look up roster data for NBA players
        var rosterData = null;
        if (LORB.Data && LORB.Data.Roster) {
            rosterData = LORB.Data.Roster.getPlayer(contact.id);
        }
        
        if (!rosterData) {
            // Player not found in rosters - return with fallback data
            var fallbackName = contact.id.replace(/_/g, " ").replace(/\b\w/g, function(c) { return c.toUpperCase(); });
            return {
                id: contact.id,
                type: contact.type || "nba",
                name: contact.name || fallbackName,
                team: contact.team || "Free Agent",
                status: contact.status || "contact",
                signCost: contact.signCost || TIER_CONFIG.rookie.signCost,
                cutPercent: contact.cutPercent || TIER_CONFIG.rookie.cutPercent,
                tier: contact.tier || "rookie",
                stats: contact.stats || { speed: 5, threePt: 5, dunk: 5, block: 5, power: 5, steal: 5 },
                skin: contact.skin || "brown",
                jersey: contact.jersey || "0",
                shortNick: contact.shortNick || fallbackName.substring(0, 8).toUpperCase(),
                position: contact.position || "SF",
                positionName: contact.positionName || "Small Forward",
                dateAcquired: contact.dateAcquired || Date.now(),
                rivals: contact.rivals || []
            };
        }
        
        // Determine tier from roster stats
        var tier = determineTierFromStats(rosterData.stats);
        var config = TIER_CONFIG[tier];
        
        // Merge stored data with roster lookup
        return {
            // From stored contact
            id: contact.id,
            type: contact.type || "nba",
            ...
            teamKey: rosterData.teamKey,
            key: rosterData.key,
            position: rosterData.position,
            positionName: rosterData.positionName,
            positionCategory: rosterData.positionCategory,
            stats: rosterData.stats,
            skin: rosterData.skin,
            jersey: rosterData.jersey,
            shortNick: rosterData.shortNick,
            rivals: rosterData.rivals,
            // Derived from stats
            tier: tier,
    ...
    /**
     * Determine tier from stats object
     */
    function determineTierFromStats(stats) {
        if (!stats) return "rookie";
        
        var total = (stats.speed || 5) + (stats.threePt || 5) + (stats.dunk || 5) +
                    (stats.block || 5) + (stats.power || 5) + (stats.steal || 5);
        var avg = total / 6;
        
        if (avg >= 8) return "superstar";
        if (avg >= 6.5) return "star";
        if (avg >= 5) return "role";
        return "rookie";
    }
    ...
    /**
     * Get contact by ID - returns hydrated contact with full data
     */
    function getContact(ctx, contactId) {
        if (!ctx.contacts) return null;
        for (var i = 0; i < ctx.contacts.length; i++) {
            if (ctx.contacts[i].id === contactId) {
                return hydrateContact(ctx.contacts[i]);
            }
        }
        return null;
    }
    ...
    /**
     * Get raw (non-hydrated) contact by ID - for internal use
     */
    function getRawContact(ctx, contactId) {
        if (!ctx.contacts) return null;
        for (var i = 0; i < ctx.contacts.length; i++) {
            if (ctx.contacts[i].id === contactId) {
    ...
    /**
     * Get all crew members with their hydrated contact data
     * @returns {Array} Array of hydrated contact objects for crew members
     */
    ...
    /**
     * Get all contacts (hydrated)
     * @returns {Array} Array of hydrated contact objects
     */
    function getAllContacts(ctx) {
        if (!ctx.contacts) return [];
        
        var result = [];
        for (var i = 0; i < ctx.contacts.length; i++) {
            result.push(hydrateContact(ctx.contacts[i]));
        }
        return result;
    }
     ...
     * @returns {Object|null} - Hydrated contact if created, null if already had
     */
    function awardContactFromVictory(ctx, opponent) {
        ...
        if (contact && addContact(ctx, contact)) {
            // Hydrate the contact so it has full data (name, team, etc.)
            var hydratedContact = hydrateContact(contact);
            
            // Log to community Ballerdex news
            if (LORB.Locations && LORB.Locations.SportsAgent && LORB.Locations.SportsAgent.addContactNews) {
                var playerName = ctx.name || ctx.userHandle || "Someone";
                var contactName = hydratedContact.name || opponent.displayName || opponent.name || "Unknown";
                var contactTeam = hydratedContact.team || opponent.team || null;
                
                LORB.Locations.SportsAgent.addContactNews({
     ...
     * @param {Object} ctx - Player context
     * @param {Object} newContact - Contact to potentially add (will be hydrated if needed)
    ...
    function checkCrewRivalConflict(ctx, newContact) {
        ...
        // Ensure the new contact is hydrated for proper rival checking
        var hydratedNew = hydrateContact(newContact);
        
        var crew = getCrewWithContacts(ctx);
        for (var i = 0; i < crew.length; i++) {
            if (areRivals(hydratedNew, crew[i])) {
    ...
    /**
     * Get all rivals of a contact that are in the player's Ballerdex
     * @param {Object} ctx - Player context
     * @param {Object} contact - Contact to check
     * @returns {Array} Array of hydrated contact objects who are rivals
     */
    function getRivalsInBallerdex(ctx, contact) {
        if (!contact || !contact.rivals || contact.rivals.length === 0) return [];
        if (!ctx.contacts) return [];
        
        var rivals = [];
        for (var i = 0; i < ctx.contacts.length; i++) {
            var hydrated = hydrateContact(ctx.contacts[i]);
            if (hydrated.id !== contact.id && areRivals(contact, hydrated)) {
                rivals.push(hydrated);
            }
        }
        return rivals;
    }
    ...
    LORB.Util.Contacts = {
        ...
        getRivalsInBallerdex: getRivalsInBallerdex,
        getRivalsInRolodex: getRivalsInBallerdex,  // Backward compatibility alias
