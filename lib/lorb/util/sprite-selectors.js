...
(function() {
     ...
     * Mix of: current player, maybe 1 crew member, plus 1-2 random patrons
     * Total: 2-4 sprites
     */
    function getClub23Sprites(ctx) {
        var sprites = [];
        
        // 50% chance to include current player
        if (Math.random() < 0.5) {
            var playerSprite = getPlayerSprite(ctx);
            if (playerSprite) sprites.push(playerSprite);
        }
        
        // 30% chance to include a crew member (if they have any)
        if (Math.random() < 0.3) {
        ...
        // Fill remainder with random patrons (target 2-4 total)
