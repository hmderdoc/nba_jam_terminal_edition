...
(function() {
    ...
    function getCityRosterSprites(count) {
        ...
        if (!city || !city.id) return sprites;
        ...
        if (!teamKey) return sprites;
        
        // Get team data from roster
        if (typeof LORB === "undefined" || !LORB.Data || !LORB.Data.Roster) return sprites;
        
        var team = LORB.Data.Roster.getTeam(teamKey);
        if (!team || !team.roster || team.roster.length === 0) return sprites;
        ...
        for (var i = 0; i < Math.min(count, rosterCopy.length); i++) {
            var playerKey = teamKey + "." + rosterCopy[i];
            ...
            sprites.push({
                ...
                jerseyBg: team.colors.bg || null,  // Team jersey color
                jerseyNumber: player.jersey || null,
                accentFg: team.colors.fgAccent || team.colors.fg || null
            });
        }
