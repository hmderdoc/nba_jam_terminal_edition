...
(function () {
    ...
    function handlePregnancyReveal(ctx, pregnancy, currentCity) {
        ...
        LORB.View.clear();
        LORB.View.header("UNEXPECTED NEWS");
        LORB.View.line("");
        LORB.View.line("\1h\1mAs you arrive in " + (currentCity ? currentCity.cityName : "the city") + "...\1n");
        LORB.View.line("");
        LORB.View.line("Your phone buzzes. It's \1c" + pregnancy.npcName + "\1n.");
        LORB.View.line("");
        LORB.View.line("\1h\1m\"We need to talk. I'm pregnant.\"\1n");
        LORB.View.line("");
        
        if (pregnancy.count > 1) {
            var countWord = pregnancy.count === 2 ? "twins" : "triplets";
            LORB.View.line("\1h\1y(It's " + countWord + "!)\1n");
            LORB.View.line("");
        }
        
        LORB.View.line("\1wShe wants you to visit the doctor with her.\1n");
        LORB.View.line("");
        LORB.View.line("\1h\1y[D] \1wVisit the Doctor now\1n");
        LORB.View.line("\1h\1y[H] \1wProceed to Hub (deal with this later)\1n");
        LORB.View.line("");
        
        var key = console.getkey().toUpperCase();
        
        if (key === "D") {
            // Run doctor visit immediately
            ...
            // Player chose to proceed to hub
            LORB.View.line("");
            LORB.View.line("\1r" + pregnancy.npcName + " looks disappointed.\1n");
            LORB.View.line("\1y\"Don't forget about this. I need you.\"\1n");
            LORB.View.line("");
            LORB.View.line("\1h\1kPress any key...\1n");
            console.getkey();
            
            // Note: pregnancy stays at phase 1, will be prompted again later
            // Could add neglect/alignment penalty here for skipping
        }
