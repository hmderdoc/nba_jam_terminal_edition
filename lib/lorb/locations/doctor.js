...
(function() {
    ...
    var DOC_VITALE_BIRTH = [
        "\1w\"IT'S TIME BABY! Forget nine months - we do SPEED RUNS!\"\1n",
        "\1w\"Normally you'd wait nine months, BUT NOT IN MY CLINIC!\"\1n",
        "\1w\"GET YOUR DIAPERS READY - this baby's coming in HOT!\"\1n"
    ];
    
    var DOC_VITALE_REDBULL = [
        "\1w\"With prescription strength RED BULL, we skip the wait!\"\1n",
        "\1w\"One dose of RED BULL and BAM - instant baby!\"\1n",
        "\1w\"RED BULL GIVES BABIES WINGS, baby!\"\1n",
        "\1w\"Clinical trials? We got BULLS to birth!\"\1n"
    ];
    ...
    /**
     * Parse color name to attribute value
     * Matches the parseColor function in sprite-wanderer.js
     */
    function parseColorAttr(colorName) {
        ...
        var upper = String(colorName).toUpperCase();
        return (typeof COLOR_MAP[upper] !== "undefined") ? COLOR_MAP[upper] : null;
    }
    
    /**
     * Parse color name to background attribute value
     */
    function parseBgColorAttr(colorName) {
        if (!colorName) return null;
        var BG_MAP = {
            "BLACK": BG_BLACK, "BLUE": BG_BLUE, "GREEN": BG_GREEN, "CYAN": BG_CYAN,
            "RED": BG_RED, "MAGENTA": BG_MAGENTA, "BROWN": BG_BROWN, "LIGHTGRAY": BG_LIGHTGRAY
        };
        var upper = String(colorName).toUpperCase();
        return (typeof BG_MAP[upper] !== "undefined") ? BG_MAP[upper] : null;
    }

    /**
     * Draw a dynamic baby sprite preview (5x4 player sprite)
     * Uses BinLoader for direct pixel manipulation - renders into content zone
     * Positioned to the LEFT of the stats text for visual appeal
     */
    function drawBabySpritePreview(view, appearance) {
        ...
        try {
            ...
            var binPath = SPRITES_DIR + "player-" + skin + ".bin";
            ...
            // Parse first 5x4 frame from bin file
            var pixels = [];
            var offset = 0;
            ...
            // Get jersey colors from appearance
            var jerseyBg = parseBgColorAttr(appearance.jerseyColor) || BG_BLUE;
            var accentFg = parseColorAttr(appearance.jerseyLettering) || WHITE;
            var jerseyNum = String(appearance.jerseyNumber || "");
            
            // Jersey number digits
            ...
            // Apply jersey mask - row 2 (index 2), cols 1,2,3
            pixels[2][1] = { ch: leftDigit, attr: digitsAttr };
            var neckCell = pixels[2][2];
            var skinFg = neckCell.attr & 0x0F;  // Preserve skin color for neck
            pixels[2][2] = { ch: String.fromCharCode(223), attr: skinFg | jerseyBg };
            pixels[2][3] = { ch: rightDigit, attr: digitsAttr };
            
            // Shorts - row 3 (index 3), cols 1,3
            var shortsChar = String.fromCharCode(220);
            var leftLeg = pixels[3][1];
            var shoeFgL = leftLeg.attr & 0x0F;
            pixels[3][1] = { ch: shortsChar, attr: shoeFgL | jerseyBg };
            var rightLeg = pixels[3][3];
            var shoeFgR = rightLeg.attr & 0x0F;
            pixels[3][3] = { ch: shortsChar, attr: shoeFgR | jerseyBg };
            
            // Position: left side of content zone, vertically aligned with stats
            // Stats header is at row 1, so sprite at rows 3-6 looks good
            var spriteX = 2;   // Left padding
            var spriteY = 3;   // Below header line
            
            // Render sprite pixels to content frame using setData for proper attr
            for (var ry = 0; ry < SPRITE_HEIGHT; ry++) {
                for (var rx = 0; rx < SPRITE_WIDTH; rx++) {
                    ...
                    if (!ch || ch === '\0') ch = ' ';
                    try {
                        contentFrame.setData(spriteX + rx - 1, spriteY + ry - 1, ch, attr, false);
                    } catch (e) {
                        // Fallback to gotoxy/putmsg
                        contentFrame.gotoxy(spriteX + rx, spriteY + ry);
                        contentFrame.putmsg(ch);
                    }
                }
            }
            
            // Add jersey number label below sprite
            contentFrame.gotoxy(spriteX, spriteY + SPRITE_HEIGHT + 1);
            contentFrame.putmsg("\1w#\1h\1c" + jerseyNum + "\1n");
            return true;
        } catch (e) {
            return false;
        }
    ...
    // Stats text offset to make room for sprite on left
    var STATS_TEXT_OFFSET = 9;  // Sprite is 5 wide + padding
    
    /**
     * Draw stat bars with ANSI blocks (not unicode)
     * Offset to the right to make room for baby sprite on left
     */
    function drawStatsPreview(view, projection) {
        ...
        // Header row
        contentFrame.gotoxy(STATS_TEXT_OFFSET, 1);
        ...
        var row = 3;  // Start stats on row 3 (below header, aligned with sprite)
        for (var i = 0; i < STAT_NAMES.length; i++) {
            ...
            contentFrame.gotoxy(STATS_TEXT_OFFSET, row++);
        ...
        if (projection.monogamyBonus) {
            contentFrame.gotoxy(STATS_TEXT_OFFSET, row);
            contentFrame.putmsg("\1h\1g\xFE MONOGAMY BONUS! +1\1n");
        }
    }
    
    /**
     * Draw payment cost breakdown (with Red Bull flavor text)
     * Uses same offset as stats for alignment
     */
    function drawCostBreakdown(view, projection, ctx) {
        var contentFrame = view.getZone("content");
        if (!contentFrame) return;
        
        var row = 11;  // Start below stats section
        contentFrame.gotoxy(STATS_TEXT_OFFSET, row++);
        contentFrame.putmsg("\1h\1r=== RED BULL FUND ===\1n");
        contentFrame.gotoxy(STATS_TEXT_OFFSET, row++);
        contentFrame.putmsg("\1wWings Budget: \1y$" + projection.baseCost + "\1n");
        contentFrame.gotoxy(STATS_TEXT_OFFSET, row++);
        contentFrame.putmsg("\1wBaby Count:   \1yx" + projection.count + "\1n");
        
        if (projection.wedlockDiscount > 0) {
            contentFrame.gotoxy(STATS_TEXT_OFFSET, row++);
            contentFrame.putmsg("\1g\xFE Wedlock: -$" + projection.wedlockDiscount + "\1n");
        }
        
        row++;
        contentFrame.gotoxy(STATS_TEXT_OFFSET, row++);
        contentFrame.putmsg("\1h\1yTOTAL: \1w$" + projection.totalCost + "\1n");
        contentFrame.gotoxy(STATS_TEXT_OFFSET, row++);
        contentFrame.putmsg("\1k\1h(Red Bull ain't cheap, baby!)\1n");
    }
    ...
    function createBabies(ctx, pregnancy, babyNames, opts) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            // CHECKPOINT SAVE - Baby creation is significant, persist immediately
            // This prevents data loss if the BBS crashes or user disconnects
            // See: JSONDB-REMEDIATION-PLAN-v2.md Phase 5
            if (LORB.Persist && LORB.Persist.save) {
                try {
                    ...
                    if (typeof debugLog === "function") {
                        debugLog("[DOCTOR] Checkpoint save failed: " + saveErr);
                    }
                }
            }
    ...
    function presentPaymentOptions(view, ctx, projection, vars) {
        ...
        contentFrame.putmsg("\1wRed Bull Fund: \1h\1y$" + projection.totalCost + "\1n\r\n\r\n");
        contentFrame.putmsg("\1h\1wHow will you fund those wings?\1n\r\n");
        contentFrame.putmsg("\1g[L]\1w Pay Now ($" + projection.totalCost + ")" + (cash < projection.totalCost ? " \1r(Can't afford)\1n" : "\1n") + "\r\n");
        contentFrame.putmsg("\1y[I]\1w Installments ($" + Math.floor(projection.totalCost / 10) + "/mo)\r\n");
        contentFrame.putmsg("\1r[A]\1w Abandon (No wings for this kid)\r\n");
        
        view.render();
        
        while (true) {
            var ch = console.getkey(K_NOSPIN).toUpperCase();
    ...
    function promptForBabyNames(view, pregnancy, count) {
        ...
        // Track line position within the content frame
        var lineY = countWord ? 5 : 3; // Starting line after header text
        
        for (var i = 0; i < count; i++) {
            var promptText = "Baby #" + (i + 1) + " full name: ";
            contentFrame.putmsg("\1c" + promptText + "\1w");
            view.renderForInput();
            
            // Position console cursor at the input location
            // contentFrame position + current line + prompt length
            console.gotoxy(contentFrame.x + promptText.length, contentFrame.y + lineY - 1);
            ...
            var nickPrompt = "Baby #" + (i + 1) + " nickname (5 chars): ";
            contentFrame.putmsg("\1c" + nickPrompt + "\1w");
            view.renderForInput();
            
            // Position console cursor at the input location
            console.gotoxy(contentFrame.x + nickPrompt.length, contentFrame.y + lineY - 1);
    ...
    */
    function showBirthNews(newsEntries) {
        ...
        if (RichView) {
            ...
            if (LORB.Util && LORB.Util.FigletBanner && LORB.Util.FigletBanner.renderToFrame) {
                LORB.Util.FigletBanner.renderToFrame(headerFrame, "Doc Vitale", LIGHTGREEN | HIGH);
            } else {
                headerFrame.gotoxy(2, 2);
                headerFrame.putmsg("\1h\1g=== DOC VITALE ===\1n");
            }
            ...
            contentFrame.putmsg("\1h\1yBABY WIRE\1n");
            cy++;
            ...
            for (var i = 0; i < newsEntries.length; i++) {
                ...
                contentFrame.gotoxy(1, cy++);
                contentFrame.putmsg(nameStr + "  " + birthdayStr);
                contentFrame.gotoxy(1, cy++);
                contentFrame.putmsg("  \1wParents: " + (bn.parentName || "Unknown") + " + " + (bn.mother || "Unknown") + "\1n");
                var extra = bn.extra;
                ...
                if (extra) {
                    ...
                    contentFrame.gotoxy(1, cy++);
                    contentFrame.putmsg("  " + noteColor + extra + "\1n");
                }
            }
    ...
    /**
     * Show empty state when no baby news exists - explains how to get baby ballers
     */
    function showEmptyBirthNews() {
        // RichView presentation
        if (RichView) {
            var view = new RichView({
                zones: [
            ...
            cy++;
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1h\1w--- NO BABIES YET ---\1n");
            cy++;
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1cThe nursery is empty,");
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1cbaby! IT'S QUIET TIME!\1n");
            cy++;
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1wWanna change that?\1n");
            cy++;
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1g\xFE \1nHit up the \1h\1mClub\1n and");
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("   romance someone special");
            cy++; 
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1g\xFE \1nStart a relationship");
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("   and nature takes over!");
            cy++;
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1kYour babies inherit YOUR\1n");
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1kstats and become ballers!\1n");
            cy++;
            contentFrame.gotoxy(1, cy++);
            contentFrame.putmsg("\1h\1kPress any key...\1n");
        ...
        LORB.View.header("DOC VITALE'S BABY WIRE");
        LORB.View.line("");
        LORB.View.line("\1h\1wNo babies yet!\1n");
        LORB.View.line("");
        LORB.View.line("Hit up the Club, find someone special,");
        LORB.View.line("and nature will take its course!");
        LORB.View.line("");
        LORB.View.line("Your babies inherit YOUR stats");
        LORB.View.line("and become playable ballers!");
        LORB.View.line("");
        LORB.View.line("Press any key...");
        console.getkey(K_NOSPIN);
        return true;
    ...
    function runDoctorVisit(ctx, pregnancy) {
        ...
        // Doc Vitale comments on stats (positioned below the stats/cost sections)
        contentFrame.gotoxy(1, 19);
        ...
        // ========== PHASE 5: BIRTH EVENT (RED BULL INSTANT DELIVERY) ==========
        
        view.clear("content");
        
        var birthLine = pick(DOC_VITALE_BIRTH);
        var redbullLine = pick(DOC_VITALE_REDBULL);
        showNarrative(view, [
            "\1h\1cDoc Vitale:\1n " + birthLine,
            "",
            "\1h\1cDoc Vitale:\1n " + redbullLine,
            "",
            "\1wDoc Vitale pulls out a glowing can...\1n",
            "",
            "\1h\1r*** PRESCRIPTION RED BULL ***\1n",
            "",
            "\1wThe room rumbles. Lightning flashes.\1n",
            "",
            "\1h\1g" + (projection.count === 1 ? "A baby's cry echoes!" : "Multiple cries fill the room!") + "\1n"
        ], vars, 1000);
    ...
    LORB.Locations.Doctor = {
        ...
        showEmptyBirthNews: showEmptyBirthNews,
