...
try {
    ...
    log(LOG_WARNING, "[SPORTS_AGENT] Failed to load RichView: " + e);
}
...
if (typeof BinLoader === "undefined") {
    try {
        ...
        log(LOG_WARNING, "[SPORTS_AGENT] Failed to load bin-loader.js: " + e);
    }
}
...
(function() {
    ...
    var LARRY_RETURNING = [
        "\1h\1yWelcome back!\1n",
        "",
        "\1wLet me catch you up on who's been",
        "\1wbuilding their Ballerdex lately...\1n"
    ];
    ...
    function formatNewsEntry(entry) {
        ...
        var phrases = [
            "\1c{contact}\1n gave \1w{player}\1n their number!",
            "\1w{player}\1n got \1c{contact}\1n's digits!",
            "\1c{contact}\1n joined \1w{player}\1n's Ballerdex!",
            "\1w{player}\1n connected with \1c{contact}\1n!"
        ];
        ...
        if (entry.contactTeam) {
            lines.push("  \1k(" + entry.contactTeam + ")\1n");
        }
    ...
    function showRichView(ctx) {
        ...
        if (isFirstTime) {
            // First-time tutorial
            view.line("\1h\1m" + getRandomGreeting() + "\1n");
            view.blank();
            
            for (var i = 0; i < LARRY_FIRST_TIME.length; i++) {
                view.line(LARRY_FIRST_TIME[i]);
            }
