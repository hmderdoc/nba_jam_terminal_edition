...
(function() {
    ...
    function runBabyMamasAndKids(ctx) {
        ...
        while (true) {
            ...
            if (headerFrame) {
                headerFrame.putmsg("\1h\1c=== BABY MAMAS & KIDS ===\1n\r\n");
                headerFrame.putmsg("\1wManage your children and child support\1n");
            }
            ...
            for (var i = 0; i < babies.length; i++) {
                if (babies[i].childSupport && !babies[i].childSupport.isPaidOff) {
                    totalOwed += babies[i].childSupport.balance;
                } else {
                    independentCount++;
                }
            }
            ...
            view.setContentZone("content");
            view.line("\1h\1yFamily Overview\1n");
            view.line("");
            view.line("\1wTotal Children: \1c" + babies.length + "\1n");
            view.line("\1wIndependent: \1g" + independentCount + "\1n");
            view.line("\1wDependent: \1r" + (babies.length - independentCount) + "\1n");
            view.line("\1wTotal Support Owed: \1y$" + totalOwed + "\1n");
            view.line("\1wYour Cash: \1y$" + (ctx.cash || 0) + "\1n");
            view.blank();
            view.info("[Arrows] Select [ENTER] Confirm");
            
            var menuItems = [
                { text: "View Children", value: "children", hotkey: "1" },
                { text: "View Baby Mamas", value: "mamas", hotkey: "2" },
                { text: "Family Tree", value: "tree", hotkey: "3" },
                { text: "Make Payment", value: "pay", hotkey: "4" },
    ...
    function viewChildren(ctx) {
        ...
        while (true) {
            ...
            var choice = view.menu(menuItems, { y: 1, showDetail: function(idx) {
                if (idx === "back" || idx === null || idx >= babies.length) return;
                showChildDetail(view, babies[idx]);
            }});
            ...
            // Selected a child - show full detail
            showChildFullView(ctx, babies[choice]);
        }
    }
    ...
    function showChildDetail(view, baby) {
        ...
        if (baby.childSupport.isPaidOff) {
            ...
            if (baby.childSupport.isOverdue) {
                ...
                var dStatus = LORB.Data.BabyBallers.getDeadlineStatus(baby, gameDay);
                if (dStatus.status === "warning") {
                    view.line("\1h\1y⚠ " + dStatus.daysRemaining + " days until deadline!\1n");
                } else if (dStatus.status === "due_today") {
     ...
     */
    function showChildFullView(ctx, baby) {
        while (true) {
            var view = new RichView({
                zones: [
                    { name: "header", x: 1, y: 1, width: 80, height: 3 },
            ...
            var menuItems = [
                ...
                { text: "Change Nickname", value: "nickname", hotkey: "N" },
                { text: "Back", value: "back", hotkey: "Q" }
            ];
    ...
    function makeChildSupportPayment(ctx) {
        ...
        // Filter to unpaid babies only
        var unpaidBabies = [];
        for (var i = 0; i < babies.length; i++) {
            if (!babies[i].childSupport.isPaidOff) {
                unpaidBabies.push(babies[i]);
            }
        }
        ...
        while (true) {
            ...
            for (var j = 0; j < unpaidBabies.length; j++) {
                ...
                menuItems.push({
                    text: baby.nickname + " \1r$" + baby.childSupport.balance + "\1n",
                    value: j,
                    hotkey: String(j + 1)
                });
            }
    ...
    function showPaymentOptions(ctx, baby) {
        ...
        if (cash >= lumpSum) {
            ...
            menuItems.push({ text: "\1k(Can't afford lump sum)\1n", value: null });
        }
    ...
    function runRichView(ctx) {
        while (true) {
            ...
            var menuItems = [
                { text: "Ballerdex", value: "contacts", hotkey: "1" },
                { text: "Your Crew", value: "crew", hotkey: "2" },
                { text: "Your Appearance", value: "appearance", hotkey: "3" },
                { text: "Stats & Records", value: "stats", hotkey: "4" },
                { text: "My Schedule", value: "schedule", hotkey: "5" },
                { text: "Baby Mamas & Kids", value: "babies", hotkey: "6" },
                { text: "Traveling Companion", value: "companion", hotkey: "7" },
            ...
            if (LORB.Config.DEBUG_ENABLE_BOSS_TEST) {
                menuItems.splice(5, 0, { text: "\1h\1rChampion's Challenge\1n", value: "boss_test", hotkey: "B" });
            }
            ...
            switch (choice) {
                ...
                case "stats":
                    ...
                    break;
                case "schedule":
                    // Show match availability settings (reuse from PlayoffView)
                    if (LORB.UI && LORB.UI.PlayoffView && LORB.UI.PlayoffView.showAvailabilitySettings) {
                        ...
                        view.line("\1rScheduling not available.\1n");
                        console.getkey();
                    }
                    break;
                case "babies":
