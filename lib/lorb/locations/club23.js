...
(function() {
    ...
    function getSpriteWanderer() {
        ...
        try {
            ...
            if (typeof LORB !== "undefined" && LORB.UI && LORB.UI.SpriteWanderer) {
                ...
                log(LOG_DEBUG, "[CLUB23] SpriteWanderer loaded successfully");
            } else {
                log(LOG_WARNING, "[CLUB23] SpriteWanderer: LORB.UI.SpriteWanderer not found after load");
            }
        } catch (e) {
            log(LOG_WARNING, "[CLUB23] SpriteWanderer load failed: " + e);
        }
    ...
    function runRichView(ctx) {
        ...
        log(LOG_DEBUG, "[CLUB23] SpriteWanderer class=" + !!SpriteWanderer);
        if (SpriteWanderer) {
            try {
                var artFrame = view.getZone("art");
                log(LOG_DEBUG, "[CLUB23] artFrame=" + !!artFrame);
                if (artFrame) {
                    ...
                    log(LOG_DEBUG, "[CLUB23] Creating wanderer with " + patronSprites.length + " sprites");
                    ...
                    wanderer.start();
                    log(LOG_DEBUG, "[CLUB23] Wanderer started, sprites=" + wanderer.sprites.length);
                }
            } catch (e) {
                log(LOG_WARNING, "[CLUB23] Wanderer init failed: " + e);
                wanderer = null;
            }
        } else {
            log(LOG_DEBUG, "[CLUB23] No SpriteWanderer available");
        }
