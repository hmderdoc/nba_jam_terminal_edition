...
(function() {
    ...
    var STATUS_ADVANCE_LINES = {
        ...
        fiance: ["{npcName} tells you she feels something real."]
    };
    ...
    function flirtWithSomeone(view, ctx, city, vars, gameDay) {
        ...
        // If at fiance status, offer proposal
        if (newStatus === "fiance" && result.outcome === "success") {
            view.blank();
            view.line("\1h\1wShe looks at you with something special in her eyes...\1n");
            view.blank();
            view.line("\1h\1yP) Propose Marriage    Q) Not yet\1n");
            view.render();
            
            var key = console.getkey().toUpperCase();
            if (key === "P") {
                flirtPropose(view, ctx, npc, city.id, vars);
                return;
            }
        }
        ...
        if (rels.length === 0) {
            ...
            for (var i = 0; i < Math.min(8, rels.length); i++) {
                ...
                var statusColor = r.status === "spouse" ? "\1h\1m" : 
                                  r.status === "fiance" ? "\1h\1r" :
                                  r.status === "partner" ? "\1h\1y" : "\1w";
                view.line("  " + statusColor + r.name + "\1n - " + 
