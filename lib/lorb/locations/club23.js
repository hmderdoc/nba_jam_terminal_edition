...
(function() {
    ...
    function loadClubHeader(frame) {
        ...
        // Try figlet rendering first
        if (LORB.Util && LORB.Util.FigletBanner) {
            var success = LORB.Util.FigletBanner.renderToFrame(
                frame,
                clubName,
                null,  // Use font's own colors
                null,
                ART_HEADER_W,
                ART_HEADER_H
            );
        ...
        // Fallback: try loading static .bin file
        if (typeof BinLoader !== "undefined" && file_exists(ART_HEADER)) {
            BinLoader.loadIntoFrame(frame, ART_HEADER, ART_HEADER_W, ART_HEADER_H, 1, 1);
            return;
        }
        
        // Final fallback: plain centered text
        frame.clear();
        var padding = Math.floor((ART_HEADER_W - clubName.length) / 2);
        frame.gotoxy(padding + 1, 2);
        frame.putmsg("\1h\1c" + clubName + "\1n");
    }
    ...
    function placeBet(view, ctx) {
        ...
        while (true) {
            ...
            var matchupLine = team1Color + game.team1.abbr + "\1n \1wvs\1n " + team2Color + game.team2.abbr + "\1n";
    ...
    function showBettingDetail(view, ctx, game) {
        ...
        while (true) {
            ...
            view.blank();
            view.line(team1Color + game.team1.name + "\1n vs " + team2Color + game.team2.name + "\1n");
            view.blank();
