...
(function() {
    ...
    // Lazy-loaded SpriteSelectors reference
    ...
    function flirtWithSomeone(view, ctx, city, vars, gameDay) {
        ...
        if (result.outcome === "success" && Romance.shouldOfferCribVisit(rel)) {
            ...
            var cribKey = console.getkey(K_NOSPIN).toUpperCase();
            if (cribKey === "Y") {
     ...
     */
    function runTravelingCompanion(view, ctx) {
        var Companion = LORB.Data && LORB.Data.Companion;
        if (!Companion) {
            view.clearZone("content");
            view.setContentZone("content");
            view.line("\1r\1hERROR: Companion system not available.\1n");
            view.render();
            console.getkey(K_NOSPIN);
            return;
        }
        ...
        while (true) {
            ...
            view.line("  \1h\1m=== TRAVELING COMPANION ===\1n");
            view.line("  \1wFly a special someone to your city\1n");
            view.blank();
            ...
            if (companion) {
                ...
                view.line("  \1w- Take them on dates\1n");
            ...
            if (choice === "clear") {
                ...
                showCompanionMessage(view, companion.name + " has been sent home.", "info");
            } else if (choice === "invite") {
                runInviteCompanion(view, ctx);
            }
        }
    }
     ...
     */
    function runInviteCompanion(view, ctx) {
        var Companion = LORB.Data.Companion;
        var eligible = Companion.getEligibleCompanions(ctx);
        
        if (eligible.length === 0) {
            showCompanionMessage(view, "No eligible partners to invite!", "error");
            return;
        }
        ...
        view.line("  \1h\1m=== INVITE COMPANION ===\1n");
        view.line("  \1wChoose who to fly to your city\1n");
        view.blank();
        view.line("\1wYour Cash: \1y$" + (ctx.cash || 0) + "\1n");
        view.blank();
        ...
        view.render();
        var choice = view.menu(menuItems, { y: 7 });
        ...
        if (choice.indexOf("partner_") === 0) {
            ...
            if (selected) {
                ...
                if (result.success) {
                    showCompanionMessage(view, selected.name + " is flying to your city!\nYou paid $" + result.cost + " for their flight.", "success");
                } else {
                    showCompanionMessage(view, result.message || "Failed to purchase flight.", "error");
                }
            }
        }
    }
     ...
     */
    function showCompanionMessage(view, message, type) {
        view.clearZone("content");
        view.setContentZone("content");
        view.setCursorY(0);
    ...
    function writeGraffiti(view, ctx, maxLines, maxLineLen) {
        ...
        for (var i = 0; i < maxLines; i++) {
            // Calculate where input cursor should be positioned
            var inputY = view.getCursorY() + 1; // Next line where text will be written
            var promptText = "  Line " + (i + 1) + ": ";
            
            // Write prompt without auto-render, then render for input
            view.lineNoRender("  \1yLine " + (i + 1) + ":\1n ");
            view.renderForInput();
            
            // Position cursor after the prompt text for input
            view.positionCursor(null, promptText.length + 1, inputY);
        ...
        var confirm = console.getkey(K_NOECHO | K_NOSPIN);
     ...
     * Wave 24: Also regenerate if totals are out of expected range (stale cached data)
     */
    function ensureDailyMatchups(ctx) {
        var currentDay = ctx.day || 1;
        
        // Check if we need to generate new matchups
        var needsRegeneration = !ctx.dailyBetting || ctx.dailyBetting.day !== currentDay;
        
        // Wave 24: Force regeneration if totals are outside expected range for short games
        // This handles stale cached data from before the totals adjustment
        if (!needsRegeneration && ctx.dailyBetting && ctx.dailyBetting.matchups && ctx.dailyBetting.matchups.length > 0) {
            var firstGame = ctx.dailyBetting.matchups[0];
            if (firstGame && firstGame.total && firstGame.total > 50) {
                // Old totals format detected - force regeneration
    ...
    function placeBet(view, ctx) {
        ...
        while (true) {
            ...
            // Row 3: Centered matchup "DET vs PHX"
            var matchupLine = team1Color + game.team1.abbr + "\1n \1h\1kvs\1n " + team2Color + game.team2.abbr + "\1n";
            view.line("              " + matchupLine);
            // Row 4: blank
            view.blank();
            
            // Rows 5-6: Player names in 2 columns
            var t1p1 = truncateName(game.team1.players[0].name, 18);
            var t1p2 = truncateName(game.team1.players[1].name, 18);
            var t2p1 = truncateName(game.team2.players[0].name, 18);
            var t2p2 = truncateName(game.team2.players[1].name, 18);
            
            view.line(team1Color + padRight(t1p1, 19) + "\1n " + team2Color + t2p1 + "\1n");
            view.line(team1Color + padRight(t1p2, 19) + "\1n " + team2Color + t2p2 + "\1n");
            
            // Rows 7-10: Player sprites (4 across, 5w x 4h each)
            // Use SpriteWanderer pattern for proper rendering
            var spriteCleanup = drawMatchupSprites(view, game);
            ...
            // Footer at bottom: rows 18-19 (stacked, no margin)
            view.setCursorY(18);
            view.line("\1h\1gSelect a game and press B to bet\1n");
            ...
            // Clean up sprites before next iteration or exit
            if (spriteCleanup) spriteCleanup();
            ...
            if (key === KEY_LEFT || key === "," || key === "<" || key === "[") {
                ...
                // Wave 24: Use bookie UI for betting (unified experience with arcade mode)
                var bookieResult = launchBookieBetting(view, ctx, game);
                if (bookieResult && bookieResult.placed) {
                    // View was re-created after game, use the new one
                    view = bookieResult.view;
    ...
    /**
     * Wave 24: Convert LORB game data to bookie matchupData format
     * This allows Club 23 to use the arcade bookie UI for betting
     * @param {Object} game - LORB game object from generateDailyMatchups
     * @returns {Object} - matchupData for bookie.showBettingInterface
     */
    function buildBookieMatchupData(game) {
        // Build player arrays in bookie format
        var teamAPlayers = [];
        var teamBPlayers = [];
        
        for (var i = 0; i < game.team1.players.length && i < 2; i++) {
            var p = game.team1.players[i];
            teamAPlayers.push({
                name: p.name,
                speed: p.stats.speed || 5,
                threePoint: p.stats.threePt || 5,
                dunk: p.stats.dunk || 5,
                power: p.stats.power || 5,
                steal: p.stats.steal || 5,
                block: p.stats.block || 5
            });
        }
        
        for (var j = 0; j < game.team2.players.length && j < 2; j++) {
            var q = game.team2.players[j];
            teamBPlayers.push({
                name: q.name,
                speed: q.stats.speed || 5,
                threePoint: q.stats.threePt || 5,
                dunk: q.stats.dunk || 5,
                power: q.stats.power || 5,
                steal: q.stats.steal || 5,
                block: q.stats.block || 5
            });
        }
        
        // Build team color info for bookie (expects fg_code as Ctrl-A string, fg as numeric)
        var team1ColorCode = getTeamColorCode(game.team1.colors);
        var team2ColorCode = getTeamColorCode(game.team2.colors);
        
        // Convert LORB odds format to bookie format
        // LORB uses { team1: number, team2: number }
        // Bookie uses { teamAOdds: number, teamBOdds: number }
        var bookieOdds = {
            teamAOdds: game.odds.team1,
            teamBOdds: game.odds.team2
        };
        
        // Convert LORB spread format to bookie format
        // LORB uses { favorite: "team1"|"team2", spread: number }
        // Bookie uses { favorite: "teamA"|"teamB", spread: number }
        var bookieSpread = {
            favorite: game.spread.favorite === "team1" ? "teamA" : "teamB",
            spread: game.spread.spread
        };
        
        return {
            teamATeam: game.team1.name,
            teamBTeam: game.team2.name,
            ...
            total: game.total,
            teamColors: {
                teamA: { fg_code: team1ColorCode },
                teamB: { fg_code: team2ColorCode }
            },
            sprites: null  // No sprites in LORB betting (could add later)
        };
    }
    
    /**
     * Wave 24: Convert bookie slip bets to LORB wager format and add to ctx
     * @param {Object} slip - BettingSlip from bookie.showBettingInterface
     * @param {Object} game - LORB game object
     * @param {Object} ctx - Player context
     * @returns {Array} - Array of wager objects added to ctx
     */
    function convertSlipToWagers(slip, game, ctx) {
        if (!slip || !slip.bets || slip.bets.length === 0) return [];
        ...
        for (var i = 0; i < slip.bets.length; i++) {
            ...
            // Convert bookie bet format to LORB wager format
            var wager = {
                gameId: game.id,
            ...
            if (bet.type === "moneyline") {
                wager.pick = bet.selection === "teamA" ? "team1" : "team2";
            } else if (bet.type === "spread") {
                wager.pick = bet.selection.team === "teamA" ? "team1" : "team2";
                wager.line = bet.selection.spread;
            } else if (bet.type === "total") {
                wager.pick = bet.selection; // "over" or "under"
                wager.line = bet.odds; // Total line stored in odds for total bets
            } else if (bet.type === "leader") {
                wager.pick = bet.selection; // { stat: string, player: string }
            }
        ...
        // Wave 24: Persist wagers immediately after placement so they survive session exit
        if (wagers.length > 0 && typeof LORB.Persist !== "undefined" && LORB.Persist.save) {
    ...
    /**
     * Wave 24: Convert game result to bookie format for grading
     * @param {Object} gameResult - LORB game result
     * @param {Object} game - LORB game object  
     * @returns {Object} - gameResults for BettingSlip.gradeBets
     */
    function convertResultForBookie(gameResult, game) {
        return {
            teamAScore: gameResult.score1,
            teamBScore: gameResult.score2,
            winner: gameResult.winner === "team1" ? "teamA" : "teamB",
            teamATeam: game.team1.name,
            teamBTeam: game.team2.name,
            leaders: null  // TODO: Could add stat leaders if tracked
        };
    }
    
    /**
     * Wave 24: Launch bookie UI for betting, then run game and show results
     * This replaces the old showBettingDetail flow with the arcade bookie UI
     * @param {Object} view - RichView instance
     * @param {Object} ctx - Player context
     * @param {Object} game - LORB game object
     * @returns {Object|boolean} - { placed: true, view: RichView } or false if cancelled
     */
    function launchBookieBetting(view, ctx, game) {
        // Close the RichView before launching bookie UI (bookie uses raw console)
        view.close();
        
        // Build matchup data for bookie
        var matchupData = buildBookieMatchupData(game);
        
        // Show bookie betting interface with player's current cash
        // showBettingInterface is global from lib/bookie/bookie.js
        var slip = null;
        if (typeof showBettingInterface === "function") {
            slip = showBettingInterface(matchupData, ctx.cash || 0);
        }
        
        // If no bets placed, return to game browser
        if (!slip || !slip.bets || slip.bets.length === 0) {
            // Re-create view and return
            var newView = createView();
            swapArt(newView, ART_BOOKIE, ART_SIDE_W, ART_SIDE_H);
            return { placed: false, view: newView };
        }
        
        // Convert slip bets to LORB wagers and deduct cash
        var wagers = convertSlipToWagers(slip, game, ctx);
        game.wager = wagers[0];  // Store primary wager on game for compatibility
        
        // Run the actual game
        var gameResult = runBettingGame(game, ctx);
        
        // Handle forfeit case
        if (gameResult.forfeited) {
            // Re-create view
            var forfeitView = createView();
            swapArt(forfeitView, ART_BOOKIE, ART_SIDE_W, ART_SIDE_H);
            
            // Show forfeit message using RichView (all bets lost)
            forfeitView.clearZone("content");
            forfeitView.setContentZone("content");
            forfeitView.setCursorY(0);
            forfeitView.header("BET FORFEITED");
            forfeitView.blank();
            forfeitView.line("\1h\1rYou quit the game.\1n");
            forfeitView.line("All bets are lost.");
            forfeitView.blank();
            forfeitView.line("Total lost: \1h\1r$" + slip.getTotalRisk() + "\1n");
            forfeitView.blank();
            forfeitView.line("\1wCash: $" + (ctx.cash || 0) + "\1n");
            forfeitView.blank();
            
            // Mark all wagers as resolved (lost)
            for (var w = 0; w < wagers.length; w++) {
                wagers[w].resolved = true;
                wagers[w].result = { won: false, push: false, forfeited: true };
            }
            
            // Save immediately
            if (typeof LORB.Persist !== "undefined" && LORB.Persist.save) {
                LORB.Persist.save(ctx);
            }
            
            forfeitView.info("Press any key...");
            forfeitView.render();
            console.getkey(K_NOSPIN);
            
            return { placed: true, view: forfeitView };
        }
        
        // Grade bets using bookie's slip
        var bookieResult = convertResultForBookie(gameResult, game);
        var gradeResults = slip.gradeBets(bookieResult);
        
        // Update LORB wagers with results
        for (var i = 0; i < wagers.length && i < slip.bets.length; i++) {
            var bet = slip.bets[i];
            wagers[i].resolved = true;
            wagers[i].result = {
                won: bet.result === "win",
        ...
        // Apply net profit to cash
        ctx.cash += gradeResults.totalPayout;
        
        // Store result on game
        game.result = gameResult;
        
        // Wave 24: Show results using RichView instead of bookie's console-based showBettingResults
        // This avoids artifacting from raw console output
        var resultView = createView();
        swapArt(resultView, ART_BOOKIE, ART_SIDE_W, ART_SIDE_H);
        
        showBookieResults(resultView, slip, bookieResult, gradeResults);
        ...
        return { placed: true, view: resultView };
    }
    
    /**
     * Wave 24: Show betting results using RichView instead of bookie's console-based display
     * Avoids artifacting from raw console output
     * @param {Object} view - RichView instance
     * @param {Object} slip - BettingSlip with graded bets
     * @param {Object} bookieResult - Game result in bookie format
     * @param {Object} gradeResults - Results from slip.gradeBets()
     */
    function showBookieResults(view, slip, bookieResult, gradeResults) {
        view.clearZone("content");
        view.setContentZone("content");
        view.setCursorY(0);
        
        view.header("BETTING RESULTS");
        view.blank();
        
        // Final score
        var winnerColor = bookieResult.winner === "teamA" ? "\1h\1r" : "\1h\1c";
        var loserColor = bookieResult.winner === "teamA" ? "\1c" : "\1r";
        
        var teamAColor = bookieResult.winner === "teamA" ? winnerColor : loserColor;
        var teamBColor = bookieResult.winner === "teamB" ? winnerColor : loserColor;
        
        view.line(teamAColor + bookieResult.teamATeam + ": " + bookieResult.teamAScore + "\1n");
        view.line(teamBColor + bookieResult.teamBTeam + ": " + bookieResult.teamBScore + "\1n");
        view.blank();
        ...
        for (var i = 0; i < slip.bets.length; i++) {
            ...
            var betDesc = "";
            if (bet.type === "moneyline") {
                var teamName = bet.selection === "teamA" ? bookieResult.teamATeam : bookieResult.teamBTeam;
                betDesc = "ML: " + teamName;
            } else if (bet.type === "spread") {
                var tName = bet.selection.team === "teamA" ? bookieResult.teamATeam : bookieResult.teamBTeam;
                var sign = bet.selection.spread > 0 ? "+" : "-";
                betDesc = "Spread: " + tName + " " + sign + Math.abs(bet.selection.spread);
            } else if (bet.type === "total") {
                betDesc = bet.selection.toUpperCase() + " " + bet.odds;
            } else if (bet.type === "leader") {
                betDesc = bet.selection.player + " (" + bet.selection.stat + ")";
            }
            
            if (isWin) {
                view.line(resultColor + resultText + "\1n \1w" + betDesc + "\1n \1g+$" + Math.round(bet.payout - bet.risk) + "\1n");
            } else {
                view.line(resultColor + resultText + "\1n \1w" + betDesc + "\1n \1r-$" + bet.risk + "\1n");
            }
        }
        ...
        // Summary
        view.line("\1wRecord: \1h\1g" + wins + "W\1n - \1h\1r" + losses + "L\1n");
        
        var profitColor = gradeResults.netProfit >= 0 ? "\1h\1g" : "\1h\1r";
        var profitSign = gradeResults.netProfit >= 0 ? "+" : "";
        view.line("\1wNet: " + profitColor + profitSign + "$" + Math.round(gradeResults.netProfit) + "\1n");
        
        view.blank();
        
        if (gradeResults.netProfit > 0) {
            view.line("\1h\1gNice win!\1n");
        } else if (gradeResults.netProfit < 0) {
            view.line("\1yBetter luck next time.\1n");
        } else {
            view.line("\1wBroke even.\1n");
        }
        ...
        if (typeof runExternalGame === "function") {
            var config = {
                ...
                options: {
                    ...
                    isBettingGame: true  // Wave 24: Enable pause/quit confirmation with forfeit warning
                },
                lorbContext: {
                    betting: true,
                    game: game
            ...
            } else if (result && result.quitForfeited) {
                // Wave 24: Player quit and confirmed forfeit - bet is lost
                gameResult = {
                    score1: result.score.teamA,
                    score2: result.score.teamB,
                    winner: null,
    ...
    function showBetResult(view, ctx, game, wager, gameResult) {
        ...
        // Wave 24: Handle forfeit case
        if (gameResult.forfeited) {
            view.header("BET FORFEITED");
            view.blank();
            view.line("\1h\1rYou quit the game.\1n");
            view.line("Your bet of \1y$" + wager.amount + "\1n is lost.");
            view.blank();
            view.line("\1wCash: $" + (ctx.cash || 0) + "\1n");
            view.blank();
            
            // Mark wager as resolved (lost)
            wager.resolved = true;
            wager.result = { won: false, push: false, forfeited: true };
            
            // Wave 24: Save immediately after forfeit
            if (typeof LORB.Persist !== "undefined" && LORB.Persist.save) {
                LORB.Persist.save(ctx);
            }
        ...
        // Wave 24: Save immediately after bet resolution to prevent loss on disconnect
        if (typeof LORB.Persist !== "undefined" && LORB.Persist.save) {
            LORB.Persist.save(ctx);
        }
    ...
    function runRichView(ctx) {
        ...
        if (SpriteWanderer) {
            try {
                var artFrame = view.getZone("art");
                if (artFrame) {
                    // Get bar patrons using sprite selectors (includes player/crew with nametags)
                    var patronSprites;
                    var walkableZone = { x: 2, y: 12, width: 36, height: 7 };
                    
                    if (SpriteSelectors) {
                        // Use smart selection: mix of player, crew, and random patrons
                        patronSprites = SpriteSelectors.getClub23Sprites(ctx);
                        SpriteSelectors.applyPositions(patronSprites, walkableZone);
                        // Debug: log what sprites were selected
                        for (var si = 0; si < patronSprites.length; si++) {
                            var sp = patronSprites[si];
                        }
                    } else {
        ...
        while (true) {
            ...
            var menuItems = [
                ...
                { text: "Traveling Companion", value: "companion", hotkey: "6" },
            ...
            switch (choice) {
                ...
                case "companion":
                    if (wanderer) wanderer.stop();
                    runTravelingCompanion(view, ctx);
                    loadArt(view);
                    if (wanderer) { wanderer.start(); view.render(); }
                    break;
                case "leave":
    ...
    // ========================================================================
    // Sprite preview helpers for game browser
    // ========================================================================
    
    var NBA_JAM_ROOT = js.exec_dir.replace(/lib[\/\\]lorb[\/\\]locations[\/\\]?$/, "");
    
    /**
     * Parse a color string to attribute value for jersey masking
     * Returns the raw value - caller decides if it's fg or bg
     */
    function parseJerseyColor(colorStr) {
        ...
        // Foreground color names (return as-is, 0-15)
        var fgColors = {
        ...
        if (fgColors[str] !== undefined) return fgColors[str];
        
        return null;
    }
    
    /**
     * Apply jersey colors to a sprite frame (all bearings)
     * Based on sprite-wanderer.js applyJerseyMask
     */
    function applyMatchupJerseyMask(frame, jerseyBg, accentFg, jerseyNumber, spriteHeight, bearingCount) {
        if (!frame || jerseyBg === null) return;
        
        // Default accent to white if not provided
        if (accentFg === null || accentFg === undefined) accentFg = 15;
        // If it's a bg value (>= 16), convert to fg
        if (accentFg >= 16) accentFg = accentFg >> 4;
        ...
        var totalHeight = spriteHeight * bearingCount;
        var topNeckChar = ascii(223);  // ▀
        var shortsChar = ascii(220);   // ▄
        
        // Apply mask to every bearing (every spriteHeight rows)
        for (var yOffset = 0; yOffset < totalHeight; yOffset += spriteHeight) {
            // Row 2 (jersey row): columns 1, 2, 3
            var jerseyY = yOffset + 2;
            
            frame.setData(1, jerseyY, leftDigit, digitsAttr);
            
            var neckData = frame.getData(2, jerseyY, false);
            var skinFg = (neckData && typeof neckData.attr === "number") ? (neckData.attr & 0x0F) : 7;
            frame.setData(2, jerseyY, topNeckChar, skinFg | jerseyBg);
            
            frame.setData(3, jerseyY, rightDigit, digitsAttr);
            
            // Row 3 (shorts row): columns 1, 2, 3
            var shortsY = yOffset + 3;
            
            var leftLeg = frame.getData(1, shortsY, false);
            var leftFg = (leftLeg && typeof leftLeg.attr === "number") ? (leftLeg.attr & 0x0F) : 7;
            frame.setData(1, shortsY, shortsChar, leftFg | jerseyBg);
            
            var midLeg = frame.getData(2, shortsY, false);
            var midFg = (midLeg && typeof midLeg.attr === "number") ? (midLeg.attr & 0x0F) : 7;
            frame.setData(2, shortsY, shortsChar, midFg | jerseyBg);
            
            var rightLeg = frame.getData(3, shortsY, false);
            var rightFg = (rightLeg && typeof rightLeg.attr === "number") ? (rightLeg.attr & 0x0F) : 7;
            frame.setData(3, shortsY, shortsChar, rightFg | jerseyBg);
        }
    }
    
    /**
     * Draw 4 player sprites for a matchup game in the browser view
     * Uses direct Frame child creation (like sprite-wanderer.js) instead of Sprite.Aerial
     * 
     * @param {Object} view - The RichView instance
     * @param {Object} game - Matchup game object with team1, team2 players
     * @returns {Function} Cleanup function to close sprite frames
     */
    function drawMatchupSprites(view, game) {
        var frames = [];
        
        try {
            var parentFrame = view.getContentFrame ? view.getContentFrame() : null;
            if (!parentFrame) {
                return function() {};
            }
            
            var FrameClass = parentFrame.constructor;
            var SPRITE_WIDTH = 5;
            var SPRITE_HEIGHT = 4;
            var SPRITE_ROW = 14;
            
            // Layout: content zone is 40 wide
            // Team 1 on left half (cols 0-19), Team 2 on right half (cols 20-39)
            // Sprites are 5 wide, so position them with spacing
            var positions = [
                { x: 42,  team: 1, idx: 0 },   // Team1 P1: cols 2-6
                { x: 49,  team: 1, idx: 1 },   // Team1 P2: cols 9-13
                { x: 62, team: 2, idx: 0 },   // Team2 P1: cols 22-26
                { x: 69, team: 2, idx: 1 }    // Team2 P2: cols 29-33
            ];
            
            for (var i = 0; i < positions.length; i++) {
                var pos = positions[i];
                var team = pos.team === 1 ? game.team1 : game.team2;
                var player = team.players[pos.idx];
                if (!player) continue;
                
                var skin = (player.skin || "brown").toLowerCase();
                var binPath = NBA_JAM_ROOT + "sprites/player-" + skin + ".bin";
                
                // Check if sprite exists
                var testFile = new File(binPath);
                if (!testFile.exists) {
                    binPath = NBA_JAM_ROOT + "sprites/player-brown.bin";
                }
                ...
                try {
                    ...
                    spriteFrame.load(binPath, SPRITE_WIDTH, SPRITE_HEIGHT * 8);  // 8 bearings
                    
                    // Apply jersey colors and number
                    // Jersey background from ansi_bg_alt, accent from ansi_fg_accent
                    var jerseyBg = parseJerseyColor(team.colors.bg_alt);
                    var accentFg = parseJerseyColor(team.colors.fg_accent);
                    applyMatchupJerseyMask(spriteFrame, jerseyBg, accentFg, player.jersey, SPRITE_HEIGHT, 8);
                    
                    spriteFrame.scrollTo(0, SPRITE_HEIGHT * 4);  // South bearing
                    spriteFrame.open();
