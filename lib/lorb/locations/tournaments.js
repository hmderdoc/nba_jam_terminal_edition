/**
 * tournaments.js - Action-focused Tournaments Hub
 * 
 * Provides:
 * - Player Standings (with challenge capability)
 * - Playoffs bracket and matches
 * - Set Availability for playoff scheduling
 * - Live and Ghost match challenges
 * 
 * For info-focused views (Season Leaders, Records, PvP Stats, 
 * Ballerdex, Crew Strength), see stats_records.js
 */
...
// Simple file-based logging that actually works
function tourneyLog(msg) {
    var f = new File("/sbbs/xtrn/nba_jam/data/tourney-debug.log");
    if (f.open("a")) {
...
(function() {
    ...
    // View modes (only rankings used now - leaders/records/pvp moved to stats_records.js)
    ...
    // =====================================================
    // TOURNAMENTS HUB - Action-focused navigation menu
    // Standings and info views are now in stats_records.js
    // This hub focuses on: Playoffs, Availability, Challenge initiation
    // =====================================================
    
    // Hub menu items with tooltips and art references
    var HUB_MENU_ITEMS = [
        { text: "Live Players", value: "live_players", tooltip: "See who's online and challenge them" },
    ...
    // =====================================================
    // LIVE PLAYERS VIEW - Online players with PvP stats and challenge
    // =====================================================
    
    /**
     * Calculate crew strength (sum of all contact stat points)
     */
    ...
    /**
     * Show live players view - online players filtered with PvP record and crew strength
     * @param {Object} ctx - Player context
     */
    function showLivePlayersView(ctx) {
        // Cycle network service to update presence
        if (ctx._networkService) {
            ctx._networkService.cycle();
        }
        
        // Get online players from network service
        var onlinePlayers = {};
        ...
        // Get all players to cross-reference
        var allPlayers = [];
        ...
        // Build list of online players with their full data
        var onlineList = [];
        for (var i = 0; i < allPlayers.length; i++) {
            var p = allPlayers[i];
            if (onlinePlayers[p.globalId]) {
                // Merge presence data with player data
                var presence = onlinePlayers[p.globalId];
                p.isOnline = true;
                p.presenceData = presence;
                onlineList.push(p);
            }
        }
        
        // Sort by crew strength (highest first)
        onlineList.sort(function(a, b) {
            var strengthA = calculateCrewStrength(a.contacts);
            var strengthB = calculateCrewStrength(b.contacts);
        ...
        if (headerFrame) {
            ...
            headerFrame.putmsg("\1h\1w  LIVE PLAYERS \1h\1g" + onlineList.length + " online now\1n");
            headerFrame.gotoxy(1, 3);
            headerFrame.putmsg("\1h\1g" + hLine + "\1n");
            headerFrame.gotoxy(1, 4);
            headerFrame.putmsg("  \1cPlayer                  \1n\xB3\1c PvP W-L  \1n\xB3\1c Win%  \1n\xB3\1c Crew Str \1n\xB3\1c BBS\1n");
        }
        ...
        if (footerFrame) {
            ...
            footerFrame.putmsg(" \1h\1w[ENTER]\1n\1c Challenge \1h\1w[R]\1n\1c Refresh \1h\1w[Q]\1n\1c Back");
        }
        ...
        if (onlineList.length === 0) {
            ...
            if (tableFrame) {
                ...
                tableFrame.putmsg("\1kNo other players online right now.\1n");
                tableFrame.gotoxy(3, 7);
                tableFrame.putmsg("\1wCheck back later or set your availability\1n");
                tableFrame.gotoxy(3, 8);
                tableFrame.putmsg("\1wfor playoff matches!\1n");
            }
            view.render();
        ...
        for (var j = 0; j < onlineList.length; j++) {
            var player = onlineList[j];
            var isSelf = (player.globalId === myGlobalId);
            
            // Get PvP stats
            var pvp = player.pvpStats || {};
            var pvpWins = pvp.wins || 0;
            var pvpLosses = pvp.losses || 0;
            var pvpGames = pvp.gamesPlayed || 0;
            var winPct = pvpGames > 0 ? Math.round((pvpWins / pvpGames) * 100) : 0;
            var wlStr = pvpWins + "-" + pvpLosses;
            
            // Get crew strength
            var crewStrength = calculateCrewStrength(player.contacts);
            
            var displayName = player.name || player.globalId;
            if (isSelf) displayName += " \1c(YOU)\1n";
            
            var bbsName = truncate(player.bbsName || "Unknown", 10);
            
            var row = " \1g\u2022\1w " + padRight(truncate(displayName, 20), 20) + "\1n \xB3\1c " +
                      padRight(wlStr, 8) + "\1n \xB3\1c " +
                      padRight(winPct + "%", 5) + "\1n \xB3\1c " +
                      padRight(String(crewStrength), 8) + "\1n \xB3 \1w" +
                      bbsName + "\1n";
        ...
        var choice = view.menu(menuItems, { y: 1, hotkeys: "R", onIdle: onIdleCallback });
        view.close();
        
        // Handle refresh
        if (choice === "R") {
            // Recurse to refresh
            return showLivePlayersView(ctx);
        }
        ...
        // Selected a player - show challenge options
        var selectedPlayer = onlineList[choice];
        var isSelfSelected = (selectedPlayer && selectedPlayer.globalId === myGlobalId);
        
        if (selectedPlayer && !isSelfSelected) {
            var action = showPlayerOptions(ctx, selectedPlayer, false);
            
            if (action === "challenge") {
                // Ghost match challenge
                challengePlayer(ctx, selectedPlayer);
            } else if (action === "live_challenge") {
                // Live challenge
                startLiveChallenge(ctx, selectedPlayer);
            }
            
            // After challenge attempt, loop back to view
            return showLivePlayersView(ctx);
        }
        
        // If self selected or other issue, just return to view
        if (isSelfSelected) {
            return showLivePlayersView(ctx);
        }
    }
    ...
    function runRichView(ctx, initialViewMode) {
         ...
         * Uses cached data from network service or subscription callbacks.
         * No network calls, no locks, no cycle calls - instant return.
         */
        function refreshOnlinePlayers() {
            var t0 = Date.now();
            
            // Use network service if available
            if (ctx._networkService && ctx._networkService.isActive()) {
                var t1 = Date.now();
                onlinePlayers = ctx._networkService.getOnlinePlayers() || {};
                logTiming("refreshOnlinePlayers: networkService path took " + (Date.now() - t1) + "ms, result count=" + Object.keys(onlinePlayers).length);
            } else if (LORB.Persist && LORB.Persist.getOnlinePlayers) {
                var t2 = Date.now();
                onlinePlayers = LORB.Persist.getOnlinePlayers() || {};
    ...
    function showRankingsView(ctx, players, onlinePlayers, myGlobalId, sortMode, seasonFilter, formatSeasonLabel) {
        ...
        // Create onIdle callback to cycle network service while waiting for input
        var onIdleCallback = null;
        if (ctx._networkService) {
            onIdleCallback = function() {
                ctx._networkService.cycle();
        ...
        tourneyLog("[GHOST_CHALLENGE] view.menu returned choice=" + choice + " (type=" + typeof choice + ")");
        view.close();
        ...
        var isSelf = (player && player.globalId && myGlobalId && player.globalId === myGlobalId);
        tourneyLog("[GHOST_CHALLENGE] Player selected: " + (player ? player.name : "null") + ", isSelf=" + isSelf + ", myGlobalId=" + myGlobalId + ", playerGlobalId=" + (player ? player.globalId : "null"));
        if (player) {
            var action = showPlayerOptions(ctx, player, isSelf);
            tourneyLog("[GHOST_CHALLENGE] showPlayerOptions returned: " + action);
            if (action === "live_challenge" && !isSelf) {
                startLiveChallenge(ctx, player);
            } else if (action === "challenge" && !isSelf) {
                tourneyLog("[GHOST_CHALLENGE] Calling challengePlayer...");
                challengePlayer(ctx, player);
                tourneyLog("[GHOST_CHALLENGE] challengePlayer returned");
            } else {
                tourneyLog("[GHOST_CHALLENGE] No action taken - action=" + action + ", isSelf=" + isSelf);
            }
        }
    ...
    function showPlayerOptions(ctx, player, isSelf) {
        tourneyLog("[GHOST_CHALLENGE] showPlayerOptions called, player=" + (player ? player.name : "null") + ", isSelf=" + isSelf);
        ...
        if (LORB.UI && LORB.UI.StatsView && LORB.UI.StatsView.showForPlayer) {
            tourneyLog("[GHOST_CHALLENGE] Using StatsView.showForPlayer");
            var result = LORB.UI.StatsView.showForPlayer(player, {
                canChallenge: true,
                canLiveChallenge: isOnline && !isSelf,
                isSelf: isSelf
            });
            tourneyLog("[GHOST_CHALLENGE] StatsView.showForPlayer returned: " + result);
            return result;  // Returns "challenge", "live_challenge", or null
        } else {
            // Fallback to legacy view with options
            tourneyLog("[GHOST_CHALLENGE] Using legacy showPlayerOptionsLegacy");
            return showPlayerOptionsLegacy(ctx, player, isSelf);
        }
    }
     ...
     */
    function showPlayerOptionsLegacy(ctx, player, isSelf) {
        LORB.View.clear();
        ...
        if (isSelf) {
            ...
            LORB.View.line("\1h\1y[C]\1n Ghost Match (select court tier)");
        }
    ...
    function startLiveChallenge(ctx, player) {
        ...
        if (ctx._networkService && ctx._networkService.isActive()) {
            var onlinePlayers = ctx._networkService.getOnlinePlayers() || {};
    ...
    // =====================================================
    // GHOST MATCH COURT TIER HELPERS
    // =====================================================
    
    /**
     * Get the maximum court tier a player can access based on their rep
     * @param {number} rep - Player's reputation
     * @returns {string} Court ID of highest unlocked court
     */
    function getMaxCourtTierForRep(rep) {
        rep = rep || 0;
        var ghostConfig = LORB.Config && LORB.Config.GHOST_MATCH;
        if (!ghostConfig || !ghostConfig.COURTS || !ghostConfig.COURT_ORDER) {
            // Fallback if config not loaded
            if (rep >= 300) return "nba";
            if (rep >= 150) return "college";
            if (rep >= 75) return "aau";
            if (rep >= 25) return "high_school";
            return "middle_school";
        }
        
        var courts = ghostConfig.COURTS;
        var order = ghostConfig.COURT_ORDER;
        var maxCourt = order[0];
        
        for (var i = 0; i < order.length; i++) {
            var courtId = order[i];
            var court = courts[courtId];
            if (court && rep >= court.repRequired) {
                maxCourt = courtId;
            }
        }
        return maxCourt;
    ...
    /**
     * Get the tier number for a court ID (1-5)
     */
    function getCourtTierNumber(courtId) {
        var ghostConfig = LORB.Config && LORB.Config.GHOST_MATCH;
        if (ghostConfig && ghostConfig.COURTS && ghostConfig.COURTS[courtId]) {
            return ghostConfig.COURTS[courtId].tier;
        }
        // Fallback
        var tierMap = { middle_school: 1, high_school: 2, aau: 3, college: 4, nba: 5 };
        return tierMap[courtId] || 1;
    }
    
    /**
     * Get available court tiers for a ghost match between two players
     * Returns courts that BOTH players have unlocked, filtered by opponent's cash
     * @param {Object} ctx - Challenger's context
     * @param {Object} opponent - Opponent's data
     * @returns {Array} Array of available court objects
     */
    function getAvailableGhostCourts(ctx, opponent) {
        var ghostConfig = LORB.Config && LORB.Config.GHOST_MATCH;
        if (!ghostConfig || !ghostConfig.COURTS || !ghostConfig.COURT_ORDER) {
            return [];
        }
        ...
        var myRep = ctx.rep || 0;
        var theirRep = opponent.rep || 0;
        var theirCash = opponent.cash || 0;
        var myCash = ctx.cash || 0;
        
        var courts = ghostConfig.COURTS;
        var order = ghostConfig.COURT_ORDER;
        var minCash = ghostConfig.MIN_OPPONENT_CASH || {};
        ...
        for (var i = 0; i < order.length; i++) {
            ...
            // Check if BOTH players have enough rep
            var myUnlocked = (myRep >= court.repRequired);
            var theirUnlocked = (theirRep >= court.repRequired);
            
            // Check if opponent has enough cash to lose at this tier
            var minOpponentCash = minCash[courtId] || 0;
            var theyHaveCash = (theirCash >= minOpponentCash);
            
            // Check if challenger has enough cash to lose at this tier
            var iHaveCash = (myCash >= court.cashReward);
            
            available.push({
                court: court,
                unlocked: myUnlocked && theirUnlocked && theyHaveCash && iHaveCash,
                reason: !myUnlocked ? "You need " + court.repRequired + " rep" :
                        !theirUnlocked ? opponent.name + " only has " + theirRep + " rep" :
                        !theyHaveCash ? opponent.name + " doesn't have $" + minOpponentCash :
                        !iHaveCash ? "You need $" + court.cashReward : null
            });
        }
    ...
    /**
     * Ghost challenge flow - now uses court tier selection instead of wager
     */
    function challengePlayer(ctx, player) {
        tourneyLog("[GHOST_CHALLENGE] challengePlayer ENTERED, player=" + (player ? player.name : "null"));
        LORB.View.clear();
        LORB.View.header("CHALLENGE " + (player.name || player.globalId).toUpperCase());
        LORB.View.line("");
        
        // Check street turns
        if ((ctx.streetTurns || 0) <= 0) {
            LORB.View.warn("No street turns remaining!");
            LORB.View.line("");
            LORB.View.line("Ghost matches cost 1 street turn.");
            LORB.View.line("");
            LORB.View.line("Press any key...");
            console.getkey(K_NOSPIN);
            return;
        ...
        LORB.View.line("Your rep: " + (ctx.rep || 0) + " | Street turns: " + ctx.streetTurns);
        LORB.View.line("");
        
        // Get available courts
        var courtOptions = getAvailableGhostCourts(ctx, player);
        var hasAnyUnlocked = false;
        
        LORB.View.line("\1h\1wSelect a court to challenge on:\1n");
        LORB.View.line("");
        
        var validChoices = [];
        for (var i = 0; i < courtOptions.length; i++) {
            var opt = courtOptions[i];
            var court = opt.court;
            var num = i + 1;
            
            if (opt.unlocked) {
                hasAnyUnlocked = true;
                validChoices.push(court.id);
                LORB.View.line("\1h\1y[" + num + "]\1n " + court.name + 
                    " - \1g$" + court.cashReward + "\1n / \1c+" + court.repReward + " rep\1n");
            } else {
                LORB.View.line("\1k[" + num + "] " + court.name + " - LOCKED (" + opt.reason + ")\1n");
            }
        }
        
        LORB.View.line("");
        LORB.View.line("\1w[Q] Cancel\1n");
        LORB.View.line("");
        
        if (!hasAnyUnlocked) {
            LORB.View.warn("No courts available for this matchup!");
            LORB.View.line("You or your opponent may need more rep or cash.");
            LORB.View.line("");
            LORB.View.line("Press any key...");
            console.getkey(K_NOSPIN);
            return;
        ...
        if (isNaN(choiceNum) || choiceNum < 1 || choiceNum > courtOptions.length) {
            LORB.View.line("Invalid choice.");
            console.getkey(K_NOSPIN);
            return;
        }
        
        var selected = courtOptions[choiceNum - 1];
        if (!selected.unlocked) {
            LORB.View.warn("That court is locked: " + selected.reason);
            console.getkey(K_NOSPIN);
            return;
        }
        
        var selectedCourt = selected.court;
        
        // Confirm
        LORB.View.line("");
        LORB.View.line("Challenge " + player.name + " at \1h" + selectedCourt.name + "\1n?");
        LORB.View.line("Stakes: \1g$" + selectedCourt.cashReward + "\1n / \1c" + selectedCourt.repReward + " rep\1n");
        LORB.View.line("Cost: \1y1 street turn\1n");
        LORB.View.line("");
        
        if (LORB.View.confirm("Confirm (Y/N): ")) {
            // Deduct street turn
            ctx.streetTurns--;
            runGhostMatch(ctx, player, selectedCourt);
        } else {
            LORB.View.line("Challenge cancelled.");
            console.getkey(K_NOSPIN);
        }
     ...
     * Now uses court tier for stakes instead of custom wager
     * @param {Object} ctx - Player context
     * @param {Object} opponent - Opponent data
     * @param {Object} court - Court tier object with cashReward, repReward, difficulty
     */
    function runGhostMatch(ctx, opponent, court) {
        var cashStakes = court.cashReward || 50;
        var repStakes = court.repReward || 2;
        var xpReward = court.xpReward || 10;
        var difficulty = court.difficulty || 1;
        
        LORB.View.clear();
        LORB.View.header("GHOST MATCH - " + court.name.toUpperCase());
        LORB.View.line("");
        LORB.View.line("vs " + (opponent.name || opponent.globalId));
        LORB.View.line("Stakes: \1g$" + cashStakes + "\1n / \1c" + repStakes + " rep\1n");
        LORB.View.line("");
        ...
        if (realEngine) {
            ...
            var config = {
                ...
                options: {
                    ...
                    // Use court tier difficulty for AI behavior
            ...
            if (result && result.completed) {
                ...
                // Player quit - refund street turn, don't count as loss
                ctx.streetTurns = (ctx.streetTurns || 0) + 1;
                LORB.View.clear();
                LORB.View.header("MATCH ABANDONED");
                LORB.View.line("");
                LORB.View.warn("You quit the match. Street turn refunded.");
                LORB.View.line("");
                LORB.View.line("Press any key...");
                console.getkey(K_NOSPIN);
                return;
        ...
        if (won) {
            // Challenger wins: gains cash/rep, opponent loses it
            ctx.cash = (ctx.cash || 0) + cashStakes;
            ctx.rep = (ctx.rep || 0) + repStakes;
            ctx.xp = (ctx.xp || 0) + xpReward;
            ...
            LORB.View.line("Cash: \1g+$" + cashStakes + "\1n");
            LORB.View.line("Rep: \1c+" + repStakes + "\1n");
            LORB.View.line("XP: \1w+" + xpReward + "\1n");
            
            // Update opponent's balance (they lose the stakes)
            // This requires persisting the change to their data
            if (LORB.Persist && LORB.Persist.updatePlayerBalance) {
                LORB.Persist.updatePlayerBalance(opponent.globalId, -cashStakes, -repStakes, "ghost_match_loss");
            }
        } else {
            // Challenger loses: loses cash/rep, opponent gains it
            ctx.cash = Math.max(0, (ctx.cash || 0) - cashStakes);
            ctx.rep = Math.max(0, (ctx.rep || 0) - repStakes);
            ...
            LORB.View.line("Cash: \1r-$" + cashStakes + "\1n");
            LORB.View.line("Rep: \1r-" + repStakes + "\1n");
            
            // Update opponent's balance (they gain the stakes)
            if (LORB.Persist && LORB.Persist.updatePlayerBalance) {
                LORB.Persist.updatePlayerBalance(opponent.globalId, cashStakes, repStakes, "ghost_match_win");
            }
        }
        ...
        if (playerGameStats && LORB.Util && LORB.Util.CareerStats && LORB.Util.CareerStats.recordGame) {
            try {
                ...
                LORB.Util.CareerStats.recordGame(ctx, playerGameStats, {
                    ...
                    courtName: court.name || "Ghost Match",
    ...
    LORB.Locations.Tournaments = {
        run: run,
        // Export challenge functions for use by stats_records.js
        showPlayerChallenge: showPlayerOptions,
        startLiveChallenge: startLiveChallenge,
        challengePlayer: challengePlayer,
        // Export ghost match helpers for other modules
        getAvailableGhostCourts: getAvailableGhostCourts,
        getMaxCourtTierForRep: getMaxCourtTierForRep
    };
