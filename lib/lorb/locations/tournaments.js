...
(function() {
    ...
    function startLiveChallenge(ctx, player) {
        ...
        var challenge = LORB.Multiplayer.Challenges.createChallenge(ctx, player, { mode: "live" });
        if (!challenge) {
        ...
        if (LORB.Multiplayer.ChallengeLobbyUI && LORB.Multiplayer.ChallengeLobbyUI.showOutcome) {
            ...
            if (result.status === "ready") {
                LORB.View.line("\1gOpponent accepted. Launch your multiplayer match now.\1n");
        ...
        if (result.status !== "ready" && LORB.Multiplayer.Challenges && LORB.Multiplayer.Challenges.markCancelled) {
            LORB.Multiplayer.Challenges.markCancelled(challenge.id);
        }
    }
