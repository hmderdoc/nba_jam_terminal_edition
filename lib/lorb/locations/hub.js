...
(function() {
    ...
    /**
     * Handle a defer request - use defer token and reschedule the match.
     * @param {Object} match - The playoff match
     * @param {Object} ctx - Player context
     */
    function handleDeferRequest(match, ctx) {
        ...
        // Get player contexts for rescheduling
        // Note: We need to use the snapshot data which has availability
    ...
    /**
     * Show notification for missed scheduled matches.
     * Offers reschedule option if player has defer token remaining.
     * @param {Object} ctx - Player context
     */
    function showMissedMatchNotification(ctx) {
        ...
        view.header("\1r⚠ MISSED PLAYOFF MATCH ⚠\1n");
        view.blank();
        view.line("\1yA scheduled playoff match was missed.\1n");
        view.blank();
        view.line("\1wOpponent: \1h" + oppName + "\1n");
        view.line("\1wScheduled for: \1c" + missedTimeStr + "\1n");
        view.blank();
        ...
        if (result.canReschedule) {
            view.line("\1gYou can reschedule this match.\1n");
            view.blank();
            menuItems.push({ text: "\1yReschedule Match\1n", value: "reschedule", hotkey: "R" });
        } else {
            view.line("\1h\1kYou've already used your reschedule for this match.\1n");
            view.blank();
        }
    ...
    function checkPregnancyEvents(ctx, city) {
        ...
        for (var i = 0; i < pregnancies.length; i++) {
            ...
            if (pregnancy.phase === 1) {
                // Phase 1 → Phase 2: Doctor discovery
                // Advance to phase 2 and trigger doctor visit
                Romance.advancePregnancyPhase(ctx, pregnancy.npcId);
                
                log(LOG_DEBUG, "[HUB] Pregnancy Phase 1→2 for " + pregnancy.npcId + " in " + city.id);
                
                // Run doctor visit scene (if Doctor location exists)
                if (Doctor && Doctor.runDoctorVisit) {
                    var visitResult = Doctor.runDoctorVisit(ctx, pregnancy);
                    
                    // Store payment choice on pregnancy record
                    if (visitResult && visitResult.paymentChoice) {
                        pregnancy.paymentChoice = visitResult.paymentChoice;
                        pregnancy.projectedStats = visitResult.projectedStats;
                        pregnancy.projectedAppearance = visitResult.projectedAppearance;
                        pregnancy.projectedCost = visitResult.projectedCost;
                    }
                }
                
                return true; // Only process one event per hub entry
                
            } else if (pregnancy.phase === 2) {
                // Phase 2 → Phase 3: Birth event
                Romance.advancePregnancyPhase(ctx, pregnancy.npcId);
                
                log(LOG_DEBUG, "[HUB] Pregnancy Phase 2→3 (birth) for " + pregnancy.npcId + " in " + city.id);
                
                // Run birth event
                if (Doctor && Doctor.runBirthEvent) {
                    Doctor.runBirthEvent(ctx, pregnancy);
                }
                
                // Remove the completed pregnancy record
                Romance.removeCompletedPregnancy(ctx, pregnancy.npcId);
    ...
    function runRichView(ctx) {
        ...
        while (true) {
            ...
            // Show child support notification if player has unpaid balances
            var totalChildSupport = 0;
            var dependentKids = 0;
            var overdueKids = 0;
            var warningKids = 0;
            if (ctx.babyBallers && ctx.babyBallers.length > 0) {
                for (var bi = 0; bi < ctx.babyBallers.length; bi++) {
                    var bb = ctx.babyBallers[bi];
                    if (bb.childSupport && !bb.childSupport.isPaidOff) {
                        totalChildSupport += bb.childSupport.balance;
                        dependentKids++;
                        if (bb.childSupport.isOverdue) {
                            overdueKids++;
                        } else if (LORB.Data && LORB.Data.BabyBallers && LORB.Data.BabyBallers.getDeadlineStatus) {
                            var status = LORB.Data.BabyBallers.getDeadlineStatus(bb, gameDay);
                            if (status.status === "warning" || status.status === "due_today") {
                                warningKids++;
                            }
                        }
                    }
                ...
                if (totalChildSupport > 0) {
                    if (overdueKids > 0) {
                        // Critical - past due!
                        view.line("\1h\1r⚠⚠ PAST DUE:\1n \1h\1r$" + totalChildSupport + "\1n \1h\1wowed (" + overdueKids + " ABANDONED!)\1n");
                    } else if (warningKids > 0) {
                        // Warning - deadline approaching
                        view.line("\1h\1y⚠ DEADLINE SOON:\1n \1y$" + totalChildSupport + "\1n \1wowed (" + dependentKids + " kids)\1n");
                    } else {
                        // Normal
                        view.line("\1r\1h⚠ CHILD SUPPORT:\1n \1y$" + totalChildSupport + "\1n \1wowed (" + dependentKids + " kids)\1n");
                    }
                }
            }
