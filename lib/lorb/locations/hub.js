...
(function() {
     ...
     * (or null). Uses new NetworkService if available, falls back to ChallengeService.
     */
    function getFirstIncomingChallenge(ctx, networkService) {
        if (!LORB.Config || !LORB.Config.ENABLE_LIVE_CHALLENGES) return null;
        
        // Prefer new NetworkService if provided
        if (networkService && networkService.isActive()) {
            networkService.cycle();
            var incoming = networkService.getIncomingChallenges() || [];
            if (incoming.length > 0) return incoming[0];
            return null;
        }
        
        // Fallback to legacy ChallengeService
     ...
     * Uses new NetworkService if available, falls back to legacy.
     * @param {Object} ctx - Player context
     * @param {Object} networkService - Optional network service instance
     * @returns {number} Count of other online players
     */
    function getOtherOnlinePlayerCount(ctx, networkService) {
        ...
        // Prefer new NetworkService if provided
        if (networkService && networkService.isActive()) {
            onlinePlayers = networkService.getOnlinePlayers() || {};
        }
        // Fallback to legacy ChallengeService
        else if (LORB.Multiplayer && LORB.Multiplayer.ChallengeService && 
                 LORB.Multiplayer.ChallengeService.getOnlinePlayers) {
            onlinePlayers = LORB.Multiplayer.ChallengeService.getOnlinePlayers(ctx) || {};
        }
    ...
    function run(ctx, options) {
        ...
        if (RichView) {
            return runRichView(ctx, options);
        } else {
            return runLegacy(ctx, options);
        }
    }
    
    function runRichView(ctx, options) {
        options = options || {};
        var networkService = options.networkService || null;
        ...
        // Auto-save timer - save every 5 minutes to prevent data loss
        // See: JSONDB-REMEDIATION-PLAN-v2.md Phase 5
        var lastAutoSave = Date.now();
        var AUTO_SAVE_INTERVAL_MS = 5 * 60 * 1000;  // 5 minutes
        
        while (true) {
            // Auto-save check (runs at top of loop before any blocking operations)
            var now = Date.now();
            ...
            // Cycle network service (processes incoming packets, heartbeat)
            // This is the ONLY place we cycle it during the hub loop
            if (networkService) {
                networkService.cycle();
            }
            
            var incomingChallenge = getFirstIncomingChallenge(ctx, networkService);
            ...
            var otherPlayersOnline = getOtherOnlinePlayerCount(ctx, networkService);
            ...
            // Create onIdle callback to cycle network service and update live player count
            var onIdleCallback = null;
            
            // Prefer new NetworkService over legacy ChallengeService
            if (networkService) {
                onIdleCallback = function(richView, lb) {
                    networkService.cycle();
                    
                    // Check if player count changed
                    var currentCount = getOtherOnlinePlayerCount(ctx, networkService);
            ...
            // Legacy fallback
            else if (LORB.Multiplayer && LORB.Multiplayer.ChallengeService && LORB.Multiplayer.ChallengeService.cycle) {
                onIdleCallback = function(richView, lb) {
                    LORB.Multiplayer.ChallengeService.cycle();
                    
                    // Check if player count changed
                    var currentCount = getOtherOnlinePlayerCount(ctx, null);
                    if (currentCount !== lastOnlineCount) {
                        lastOnlineCount = currentCount;
                        var newLabel = buildTournamentsLabel(currentCount);
                        lb.updateItemText(tournamentsItemIndex, newLabel);
    ...
    function runLegacy(ctx, options) {
        options = options || {};
        var networkService = options.networkService || null;
        
        while (true) {
            // Cycle network service if available
            if (networkService) {
                networkService.cycle();
            }
