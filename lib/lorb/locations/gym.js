...
try {
    ...
    log(LOG_WARNING, "[GYM] Failed to load RichView: " + e);
}
...
if (typeof BinLoader === "undefined") {
    try {
        ...
        log(LOG_WARNING, "[GYM] Failed to load bin-loader.js: " + e);
    }
}
...
(function() {
    ...
    function buildStatBar(currentValue, maxValue) {
        var filled = "";
        var empty = "";
        for (var b = 0; b < currentValue; b++) filled += "Û";
        for (var b = currentValue; b < maxValue; b++) empty += "Û";
    ...
    function runRichView(ctx) {
        ...
        while (true) {
            ...
            for (var i = 0; i < statOrder.length; i++) {
                ...
                menuItems.push({
                    text: padRight(stat.name, 7) + bar + "\1c " + currentValue + " " + costStr + "\1n",
                    value: statKey,
                    hotkey: stat.key,
                    disabled: disabled,
                    _statKey: statKey
