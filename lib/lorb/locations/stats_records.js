...
(function() {
    ...
    var ART_WIDTH = 40;
    var ART_HEIGHT = 20;
    var ART_PATH = "/sbbs/xtrn/nba_jam/assets/lorb/hall_of_fame.bin";
    var FALLBACK_ART = "/sbbs/xtrn/nba_jam/assets/lorb/hub_art.bin";
    
    // Walkable zone for sprite wandering (within art frame coordinates)
    // Art is 40x20, sprites wander in bottom portion: lines 12-20, cols 1-40
    var STATS_WALKABLE_ZONE = { x: 1, y: 12, width: 40, height: 8 };
    ...
    function getSpriteSelectors() {
        ...
        try {
            ...
            // SpriteSelectors is optional
        }
        return _spriteSelectorsModule;
    }
    
    /**
     * Get sprites for Hall of Fame display - players who achieved greatness
     * 
     * Criteria for inclusion (any of):
     * - Won a season championship (appears in hallOfFame)
     * - Holds a high score record
     * - Is the current rep leader
     * 
     * TODO: Expand criteria as data grows:
     * - Top 3 in any leaderboard category
     * - PvP win leaders
     * - Multiple championship winners
     * 
     * @param {Object} ctx - Current player context
     * @returns {Array} Array of sprite configs for SpriteWanderer
     */
    function getHallOfFameSprites(ctx) {
        var sprites = [];
        var seenPlayers = {}; // Track by globalId to avoid duplicates
        
        // Helper to add a sprite if not already present
        function addSprite(playerData, source) {
            if (!playerData) return;
            var id = playerData.globalId || playerData.name || playerData.shortNick;
            if (!id || seenPlayers[id]) return;
            seenPlayers[id] = true;
            
            sprites.push({
                skin: playerData.skin || playerData.appearance && playerData.appearance.skin || "brown",
                shortNick: playerData.shortNick || playerData.name || null,
                jerseyBg: playerData.jerseyColor || playerData.appearance && playerData.appearance.jerseyColor || null,
                jerseyNumber: playerData.jerseyNumber || playerData.appearance && playerData.appearance.jerseyNumber || null,
                bearing: ["n", "s", "e", "w"][Math.floor(Math.random() * 4)]
            });
        }
        
        // 1. Season champions from Hall of Fame
        if (LORB.Persist && LORB.Persist.lorb && LORB.Persist.lorb.sharedState) {
            var hallOfFame = LORB.Persist.lorb.sharedState.hallOfFame;
            if (hallOfFame && Array.isArray(hallOfFame)) {
                for (var i = 0; i < hallOfFame.length && sprites.length < 5; i++) {
                    var champ = hallOfFame[i];
                    if (champ && champ.winner) {
                        addSprite({
                            skin: champ.winner.skin || "brown",
                            shortNick: champ.winner.shortNick || champ.winner.name,
                            globalId: champ.winner.globalId
                        }, "champion");
                    }
                }
            }
        ...
        // 2. Get all players for leaderboard checks
        var allPlayers = [];
        if (LORB.Persist && LORB.Persist.getAllPlayers) {
            try {
                allPlayers = LORB.Persist.getAllPlayers() || [];
            } catch (e) {
                allPlayers = [];
            }
        }
        
        // 3. Rep leader (highest rep)
        if (allPlayers.length > 0 && sprites.length < 5) {
            var repLeader = null;
            var maxRep = -1;
            for (var i = 0; i < allPlayers.length; i++) {
                var p = allPlayers[i];
                if (p && typeof p.rep === "number" && p.rep > maxRep) {
                    maxRep = p.rep;
                    repLeader = p;
                }
            }
            if (repLeader) {
                addSprite(repLeader, "rep_leader");
            }
        }
        ...
        if (allPlayers.length > 0 && sprites.length < 5) {
            ...
            for (var i = 0; i < allPlayers.length; i++) {
                var p = allPlayers[i];
                if (p && typeof p.wins === "number" && p.wins > maxWins) {
                    maxWins = p.wins;
                    winsLeader = p;
                }
            }
            if (winsLeader) {
                addSprite(winsLeader, "wins_leader");
            }
        }
        
        // 5. Fallback: include current player if no other sprites
        if (sprites.length === 0 && ctx) {
            addSprite({
                skin: ctx.appearance && ctx.appearance.skin || "brown",
                shortNick: ctx.shortNick || ctx.name,
                globalId: ctx.globalId,
                jerseyColor: ctx.appearance && ctx.appearance.jerseyColor,
                jerseyNumber: ctx.appearance && ctx.appearance.jerseyNumber
            }, "current_player");
        }
        
        // Limit to 4 sprites max for visual clarity
        if (sprites.length > 4) {
            sprites = sprites.slice(0, 4);
        }
    ...
    function runStatsHub(ctx) {
        // Lazy-load sprite modules (once per hub visit)
        ...
        while (true) {
            ...
            // Initialize wandering sprites in the art zone
            if (SpriteWanderer) {
                try {
                    var artFrame = view.getZone("art");
                    if (artFrame) {
                        var hofSprites = getHallOfFameSprites(ctx);
                        
                        // Apply positions within walkable zone
                        if (SpriteSelectors && SpriteSelectors.applyPositions) {
                            SpriteSelectors.applyPositions(hofSprites, STATS_WALKABLE_ZONE);
                        } else {
                            // Manual position fallback
                            for (var i = 0; i < hofSprites.length; i++) {
                                var spacing = Math.floor(STATS_WALKABLE_ZONE.width / (hofSprites.length + 1));
                                hofSprites[i].x = STATS_WALKABLE_ZONE.x + spacing * (i + 1);
                                hofSprites[i].y = STATS_WALKABLE_ZONE.y + Math.floor(Math.random() * Math.max(1, STATS_WALKABLE_ZONE.height - 2));
                            }
                        }
                        
                        if (hofSprites.length > 0) {
                            wanderer = new SpriteWanderer({
                                parentFrame: artFrame,
                                sprites: hofSprites,
                                walkableZones: [STATS_WALKABLE_ZONE],
                                options: {
                                    speed: 400,        // Move every 400ms
                                    pauseChance: 0.40, // 40% chance to idle
                                    showNametags: true // Show nametags for hall of famers
                                }
                            });
                            wanderer.start();
                            view.render();
            ...
            var choice = view.menu(HUB_MENU_ITEMS, { y: 3, onSelect: onSelectCallback, onIdle: onIdleCallback });
            
            // Clean up wanderer before closing view
            if (wanderer) {
                try { wanderer.stop(); } catch (e) {}
                wanderer = null;
            }
            view.close();
    ...
    function showStandingsView(ctx, players, onlinePlayers, myGlobalId, sortMode, seasonFilter) {
        ...
        if (player && !isSelf && LORB.Locations && LORB.Locations.Tournaments) {
            ...
            if (LORB.Locations.Tournaments.showPlayerChallenge) {
                var action = LORB.Locations.Tournaments.showPlayerChallenge(ctx, player, isSelf);
                // Handle the returned action
                if (action === "challenge") {
                    if (LORB.Locations.Tournaments.challengePlayer) {
                        LORB.Locations.Tournaments.challengePlayer(ctx, player);
                    }
                } else if (action === "live_challenge") {
                    if (LORB.Locations.Tournaments.startLiveChallenge) {
