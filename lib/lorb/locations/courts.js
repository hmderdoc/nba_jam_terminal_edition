...
(function() {
     ...
     * Falls back to hub_art.bin if no court-specific art available
     */
    function loadCourtArt(view, courtId) {
        ...
        // Try court-specific art first
        var artFile = COURT_ART_FILES[courtId];
        var artPath = artFile ? (COURT_ART_PATH + artFile) : null;
        
        if (artPath && file_exists(artPath)) {
            try {
        ...
        if (file_exists(fallbackPath)) {
            try {
                ...
                log(LOG_DEBUG, "[COURTS] Failed to load fallback art: " + e);
            }
        }
    ...
    function runRichView(ctx) {
        ...
        // Create sprite wanderer for ambient animation on art panel
        var wanderer = null;
        ...
        while (true) {
            ...
            // Draw initial tooltip for first unlocked court
            var firstUnlocked = null;
            for (var i = 0; i < items.length; i++) {
                if (items[i]._court && items[i]._unlocked) {
                    firstUnlocked = items[i];
                    break;
                }
            }
            if (firstUnlocked) {
                drawCourtTooltip(view, firstUnlocked._court, firstUnlocked._unlocked);
            }
            ...
            if (court) {
                ...
                loadCourtArt(view, null);  // Reload art after returning
