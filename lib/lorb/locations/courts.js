...
(function() {
    ...
    function getNBAOpponent() {
        if (typeof LORB === "undefined" || typeof LORB.getRandomOpponent !== "function") return null;
        try {
            var opp = LORB.getRandomOpponent();
    ...
    function drawStreetballSprite(view, opponent) {
        ...
        try {
            ...
            if (!skipJersey) {
                // Apply uniform colors
                var jerseyColors = [BG_RED, BG_BLUE, BG_GREEN, BG_CYAN, BG_MAGENTA];
                var jerseyBg = jerseyColors[Math.floor(Math.random() * jerseyColors.length)];
    ...
    function drawCourtPreview(view, court, unlocked) {
        view.updateZone("art", function(frame) {
            ...
            frame.gotoxy(2, 5);
            frame.putmsg("Difficulty: \1r" + stars + "\1n");
            ...
            frame.gotoxy(2, 10);
            frame.putmsg("NBA Chance: \1m" + nba + "%\1n");
    ...
    function showResult(view, won, scoreDiff, rewards, ctx, opponent) {
        cleanupSprite(view);
        view.clearZone("art");
        view.clearZone("content");
        view.setContentZone("content");
        ...
        if (won) {
            ...
            view.blank();
            view.line("You win by " + Math.abs(scoreDiff));
            view.blank();
            view.line("\1yRewards:\1n");
            
            // Show gross and net if there were crew cuts
            if (rewards.crewCuts && rewards.crewCuts.length > 0 && rewards.grossCash) {
                view.line("  Winnings: \1w$" + rewards.grossCash + "\1n");
                for (var i = 0; i < rewards.crewCuts.length; i++) {
                    var cut = rewards.crewCuts[i];
                    view.line("  \1r- " + cut.name + " (" + cut.percent + "%): -$" + cut.amount + "\1n");
                }
                view.line("  \1gYou keep: +$" + rewards.cash + "\1n");
            } else {
                view.line("  Cash: \1g+$" + rewards.cash + "\1n");
            }
            view.line("  Rep:  \1c+" + rewards.rep + "\1n");
            view.line("  XP:   +" + rewards.xp);
            
            if (opponent.isNBA) {
                view.blank();
                view.line("\1mYou beat an NBA player!\1n");
                
                // Show new contact if acquired
                if (rewards.newContact) {
                    view.line("\1h\1c" + rewards.newContact.name + " gave you their number!\1n");
                    view.line("\1kCheck your Crib > Contacts\1n");
                }
            }
        } else {
            view.line("\1h\1rYOU LOSE\1n");
            view.blank();
            view.line("Lost by " + Math.abs(scoreDiff));
            view.blank();
            view.line("The streets are unforgiving.");
        }
            ...
            if (choice === "play") {
                ...
                if (realEngine) {
                    ...
                    var config = {
                        ...
                        teamB: { name: court.name + " Crew", abbr: "OPP", players: [opp1, opponentToPlayer(opp2)], colors: { fg: "WHITE", bg: "BG_BLUE" } },
                        options: { gameTime: 90, mode: "play", humanTeam: "teamA", humanPlayerIndex: 0, showMatchupScreen: true, showGameOverScreen: false },
                        lorbContext: { court: court.id, difficulty: court.difficulty, opponent: opponent }
                    };
                ...
                if (won) {
                    ...
                    // Apply crew cuts if any temp members
                    var cutResult = null;
                    ...
                    showResult(view, true, scoreDiff, rewards, ctx, opponent);
                    checkLevelUp(ctx, view);
                } else {
                    ctx.losses = (ctx.losses || 0) + 1;
                    ctx.dayStats.losses++;
                    showResult(view, false, scoreDiff, {}, ctx, opponent);
                }
