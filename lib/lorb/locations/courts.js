...
(function() {
    ...
    function playAtCourt(court, ctx) {
        ...
        if (LORB.Core.Battle && typeof LORB.Core.Battle.runWithOpponent === "function") {
            // Use real NBA JAM engine
            LORB.View.line("");
            LORB.View.line("\1h\1cTip-off!\1n");
            console.getkey();
            
            var result = LORB.Core.Battle.runWithOpponent(opponent, ctx);
            won = result === true || (result && result.won);
            scoreDiff = (result && result.scoreDiff) ? result.scoreDiff : (won ? 10 : -5);
        } else {
            // Fallback simulation
            LORB.View.line("");
