        ...
        if (SpriteWanderer) {
            try {
                ...
                if (artFrame) {
                    ...
                    // Court walkable zones: trapezoid (top) + rectangle (bottom key area)
                    // Matches the perspective court art
                    var walkableZones = [
                        // Trapezoid approximation (stacked rectangles getting wider)
                        { x: 14, y: 13, width: 12, height: 2 },  // narrow top (just below net)
                        { x: 10, y: 15, width: 20, height: 2 },  // mid trapezoid
                        { x: 6,  y: 17, width: 28, height: 2 },  // wider
                        // Bottom rectangle (the key/paint area)
                        { x: 4,  y: 19, width: 32, height: 2 }   // widest at bottom
                    ];
                    
                    // Use first (widest) zone for initial positioning
                    var primaryZone = walkableZones[3];
