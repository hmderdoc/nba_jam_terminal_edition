...
(function() {
     ...
     */
    function getBabyBallerChance(courtTier) {
        // 5% at tier 1, up to 15% at tier 5
    ...
    function getBabyBallerOpponent(court, ctx) {
        ...
        // These have priority - the abandoned child is hunting their parent!
        if (ctx.babyBallers && ctx.babyBallers.length > 0) {
            var nemesisAtCourt = [];
            for (var i = 0; i < ctx.babyBallers.length; i++) {
                var baby = ctx.babyBallers[i];
                if (baby.currentCourt === courtTier && baby.isNemesis) {
                    nemesisAtCourt.push(baby);
                }
            }
            // 50% chance for nemesis to appear if any exist at this tier
            if (nemesisAtCourt.length > 0 && Math.random() < 0.5) {
        ...
        if (worldBabies && worldBabies.length > 0) {
            // Filter out player's own children (already handled above)
            var otherBabies = [];
    ...
    function processBabyBallerMatch(ctx, opponent, playerWon, winnings) {
        ...
        if (playerWon) {
            ...
            if (result.isOwnChild) {
                ...
                // Beat someone else's kid - just a normal win
    ...
    function generateOpponent(court, ctx) {
        ...
        // Nemesis children have high priority - they're hunting their deadbeat parent
        var babyChance = getBabyBallerChance(courtTier);
        if (Math.random() < babyChance) {
            var babyOpp = getBabyBallerOpponent(court, ctx);
            if (babyOpp) return babyOpp;
        }
        
        // Standard NBA player encounters
    ...
    function playAtCourtRichView(view, court, ctx) {
        ...
        // This is the "find opponent" moment - perfect time for drama to interrupt
        if (LORB.Data && LORB.Data.BabyEvents && LORB.Data.BabyEvents.checkAndShowEvent) {
            ...
            if (eventResult) {
                // Event was shown - if it triggered a match, use that opponent
                if (eventResult.triggerMatch && eventResult.matchOpponent) {
                    opponent = eventResult.matchOpponent;
                }
                // Otherwise continue to show the regular opponent
            }
        }
