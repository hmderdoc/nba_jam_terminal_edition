...
function getSpriteWanderer() {
    ...
    try {
        ...
        log(LOG_DEBUG, "[COURTS] SpriteWanderer not available: " + e);
    }
    return _spriteWandererClass;
}
...
function getSpriteSelectors() {
    ...
    try {
        ...
        log(LOG_DEBUG, "[COURTS] SpriteSelectors not available: " + e);
    }
    return _spriteSelectorsModule;
}
...
(function() {
    ...
    /**
     * Path to court art assets (when available)
     */
    var COURT_ART_PATH = NBA_JAM_ROOT + "assets/lorb/";
    
    /**
     * Map court IDs to art file names (stubs for now)
     */
    var COURT_ART_FILES = {
        "middle_school": "courts_middle_school.bin",
        "high_school": "courts_high_school.bin",
        "aau": "courts_aau.bin",
        "college": "courts_college.bin",
        "nba": "courts_nba.bin"
    };
    
    /**
     * Load court-specific art into the art zone
     * Returns true if art was loaded, false if no art available
     */
    function loadCourtArt(view, courtId) {
        ...
        var artFile = COURT_ART_FILES[courtId];
        if (!artFile) return false;
        
        var artPath = COURT_ART_PATH + artFile;
        if (!file_exists(artPath)) return false;
        
        try {
            BinLoader.loadIntoFrame(artFrame, artPath, 40, 20, 0, 0);
            return true;
        } catch (e) {
            log(LOG_DEBUG, "[COURTS] Failed to load court art: " + e);
            return false;
        }
    }
    
    /**
     * Load the header figlet/banner
     */
    function loadHeader(view) {
        ...
        // Try to load figlet header if available
        var headerPath = COURT_ART_PATH + "courts_header.bin";
        if (file_exists(headerPath) && typeof BinLoader !== "undefined") {
            try {
                BinLoader.loadIntoFrame(headerFrame, headerPath, 80, 4, 0, 0);
                return;
            } catch (e) {}
        }
        ...
        headerFrame.putmsg("\1h\1c╔══════════════════════════════════════════════════════════════════════════════╗\1n");
        headerFrame.gotoxy(1, 2);
        headerFrame.putmsg("\1h\1c║\1n\1h                           H I T   T H E   C O U R T S                          \1h\1c║\1n");
    ...
    function buildCourtMenuItems(ctx) {
        ...
        for (var i = 0; i < courts.length; i++) {
            ...
            // Build text - boss courts get a star marker
            var text = court.name;
            if (court.boss) text += " *";
            
            // Single detail line: status + difficulty stars
            var stars = "";
            for (var s = 0; s < court.difficulty; s++) stars += "*";
            
            var detail;
            if (c.unlocked) {
                detail = stars + " - " + court.tagline;
            } else {
                detail = "Locked (need " + c.repNeeded + " rep)";
            }
    ...
    /**
     * Draw court info overlay on the art panel
     * Does NOT clear the frame - preserves background art and sprites
     * Draws info in the top-left corner
     */
    function drawCourtPreview(view, court, unlocked) {
        var artFrame = view.getZone("art");
        if (!artFrame) return;
        
        // Draw info box in top-left corner (with dark background for readability)
        var boxY = 1;
        
        artFrame.gotoxy(1, boxY);
        artFrame.putmsg("\1n\1h\1w" + court.name + "\1n");
        
        artFrame.gotoxy(1, boxY + 1);
        artFrame.putmsg(unlocked ? "\1g[UNLOCKED]\1n" : "\1r[LOCKED]\1n");
        
        var stars = "";
        for (var i = 0; i < court.difficulty; i++) stars += "\1r*";
        artFrame.gotoxy(1, boxY + 3);
        artFrame.putmsg("\1kDifficulty:\1n " + stars + "\1n");
        
        artFrame.gotoxy(1, boxY + 4);
        artFrame.putmsg("\1kRewards:\1n \1y$" + court.rewards.cashBase + "\1n \1c+" + court.rewards.repBase + " rep\1n");
        
        var nba = Math.floor((court.nbaChance || 0.2) * 100);
        artFrame.gotoxy(1, boxY + 5);
        artFrame.putmsg("\1kNBA Chance:\1n \1m" + nba + "%\1n");
        
        artFrame.gotoxy(1, boxY + 7);
        artFrame.putmsg("\1c" + court.tagline + "\1n");
    }
    ...
    function runRichView(ctx) {
        ...
        // Create sprite wanderer for ambient animation
        var wanderer = null;
        var SpriteWanderer = getSpriteWanderer();
        var SpriteSelectors = getSpriteSelectors();
        var currentCourtId = null;  // Track which court's art is loaded
        
        // Initialize wanderer with court sprites
        if (SpriteWanderer) {
            try {
                var artFrame = view.getZone("art");
                if (artFrame) {
                    var courtSprites;
                    var walkableZone = { x: 2, y: 10, width: 36, height: 8 };
                    
                    if (SpriteSelectors) {
                        // Get city NBA players with art, baby ballers, street ballers
                        courtSprites = SpriteSelectors.getCourtSelectSprites(ctx);
                        SpriteSelectors.applyPositions(courtSprites, walkableZone);
                        log(LOG_DEBUG, "[COURTS] SpriteWanderer: " + courtSprites.length + " sprites selected");
                    } else {
                ...
                log(LOG_DEBUG, "[COURTS] Wanderer init failed: " + e);
                wanderer = null;
            }
        }
        
        /**
         * Draw the art panel for a selected court
         * Shows court art (if available) with wandering sprites
         */
        function drawCourtArtPanel(rv, courtId) {
            var artFrame = rv.getZone("art");
            if (!artFrame) return;
            
            // Only reload art if court changed
            if (courtId !== currentCourtId) {
                // Stop wanderer before clearing
                if (wanderer) wanderer.stop();
                
                artFrame.clear();
                
                // Try to load court-specific art
                var artLoaded = loadCourtArt(rv, courtId);
                
                // If no art, draw a simple background
                if (!artLoaded) {
                    artFrame.gotoxy(2, 2);
                    artFrame.putmsg("\1k\1h[Court Preview]\1n");
                }
                
                // Restart wanderer with new parent state
                if (wanderer) {
                    wanderer.start();
                }
        ...
        while (true) {
            ...
            // Left panel: player info and instructions
            view.setContentZone("content");
            view.setCursorY(0);
            ...
            view.line("\1kNBA players can appear anywhere!\1n");
            
            // Build menu items
            var items = buildCourtMenuItems(ctx);
            
            // Draw initial art for first unlocked court
            var firstUnlocked = null;
            for (var i = 0; i < items.length; i++) {
                if (items[i]._court && items[i]._unlocked) {
                    firstUnlocked = items[i]._court.id;
                    break;
                }
            }
            if (firstUnlocked && currentCourtId === null) {
                drawCourtArtPanel(view, firstUnlocked);
                drawCourtPreview(view, items[0]._court, items[0]._unlocked);
            }
            ...
            var choice = view.menu(items, {
                ...
                onSelect: function(item, index, rv) {
                    if (item._court) {
                        drawCourtArtPanel(rv, item._court.id);
                        drawCourtPreview(rv, item._court, item._unlocked);
                    }
                    rv.render();
                },
            ...
            if (court) {
                ...
                currentCourtId = null;  // Force art reload
                if (wanderer) wanderer.start();
            }
        }
    }
