...
(function() {
    ...
    function buildChallengerTeam(ctx) {
        ...
        if (ctx.appearance && ctx.appearance.jerseyColor) {
            teamColors.bg = "BG_" + ctx.appearance.jerseyColor.toUpperCase();
            teamColors.bg_alt = teamColors.bg;
        }
        if (ctx.appearance && ctx.appearance.jerseyLettering) {
            teamColors.fg_accent = ctx.appearance.jerseyLettering.toUpperCase();
        }
    ...
    function launchRedBullChallenge(ctx, options) {
        ...
        var gameConfig = {
            ...
            teamB: redBullTeam,
            options: {
                gameTime: 180,  // 3-minute game for the final boss
                mode: "play",
                humanTeam: "teamA",
                humanPlayerIndex: 0,
                showMatchupScreen: true,
