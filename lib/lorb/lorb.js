...
(function () {
    ...
    // Attached to ctx so all views can access it via ctx._networkService.
    // Views call ctx._networkService.cycle() in their loops.
    ctx._networkService = null;
    if (LORB.Services && LORB.Services.NetworkService) {
        try {
            ctx._networkService = new LORB.Services.NetworkService(ctx, LORB.Config);
            ctx._networkService.start();
