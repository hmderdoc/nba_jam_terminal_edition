...
(function () {
    ...
    LORB.Config = {
        ...
        // When true: data arrives via callback, cycle() just drains queue (non-blocking)
        // When false: uses blocking pollOnDemand() - causes input loop freezes!
        // See: JSONDB-ARCHITECTURE-AUDIT-v2.md for full explanation
        ...
        // ========== STREETBALL OPPONENT STATS ==========
        // Per-tier stat ranges for procedurally generated streetball opponents.
        // Each tier defines min/max for all 6 stats (speed, 3point, dunk, power, steal, block).
        // One random stat is designated as a "weakness" and re-rolled in the range [1, min] instead.
        // Lower tiers = lower stats = AI takes worse shots (natural difficulty scaling).
        STREETBALL_STATS_BY_TIER: {
            1: { min: 2, max: 5 },   // Middle School: weakness range 1-2
            2: { min: 3, max: 7 },   // High School: weakness range 1-3
            3: { min: 4, max: 8 },   // AAU Circuit: weakness range 1-4
            4: { min: 5, max: 9 },   // College: weakness range 1-5
            5: { min: 6, max: 10 }   // The League: weakness range 1-6
        },
        
        // ========== GHOST MATCH SYSTEM ==========
        // Ghost matches (challenging offline players) use court tiers instead of custom wagers.
        // This prevents exploitation by limiting stakes to court-based rewards.
        // Available court tier = min(challenger's unlocked courts, opponent's unlocked courts)
        // Costs 1 street turn to challenge.
        GHOST_MATCH: {
            // Court tier definitions with rep requirements and rewards
            // Mirrors street courts but specifically for ghost matches
            COURTS: {
                middle_school: {
                    id: "middle_school",
                    tier: 1,
            ...
            // Order of court IDs for menu display (lowest to highest)
            COURT_ORDER: ["middle_school", "high_school", "aau", "college", "nba"],
            
            // Minimum cash required to challenge at a court tier
            // If opponent has less than this, they can't be challenged at that tier
            // (prevents taking money from someone who has none)
            MIN_OPPONENT_CASH: {
                middle_school: 25,    // Must have at least $25 to lose
                high_school: 50,      // Must have at least $50 to lose
                aau: 100,             // Must have at least $100 to lose
                college: 200,         // Must have at least $200 to lose
                nba: 500              // Must have at least $500 to lose
            }
        },
        ...
        BABY_BALLERS: {
            ...
            // === Random Encounters ===
            // Narrative events (spouse retaliation) - limited per day, trigger at town/login
            MAX_RANDOM_EVENTS_PER_DAY: 1,           // Cap on NARRATIVE events per day
            SPOUSE_RETALIATION_CHANCE: 0.20,        // 20% chance for angry spouse event when cheating
            
            // Anti-spam events (Find Another consequences) - NO daily limit
            ANTI_SPAM_EVENT_CHANCE: 0.15,           // 15% chance per "Find Another" reroll
            BABY_MAMA_EVENT_CHANCE: 0.20,           // 20% per baby mama when anti-spam fires
            CHILD_CHALLENGE_CHANCE: 0.15,           // 15% chance to encounter own child
            
            // === TEST MODE FLAGS ===
            // Set FORCE_EVENTS to true to guarantee events trigger (ignores chance rolls)
            // REMEMBER TO SET BACK TO false AFTER TESTING!
            FORCE_EVENTS: false,                    // Master switch: forces all event types
            FORCE_BABY_MAMA_EVENT: false,           // Force baby mama events specifically
            FORCE_BABY_BALLER_EVENT: false,         // Force baby baller events specifically  
            FORCE_SPOUSE_RETALIATION: false,        // Force spouse retaliation events
