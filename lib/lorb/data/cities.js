...
(function () {
    ...
    /**
     * Get the mall name for a city.
     */
    function getMallName(city) {
        if (!city) return "The Mall";
    ...
    function getTeamColors(city) {
        ...
        // Convert BG_* colors to foreground equivalents (e.g., BG_MAGENTA -> MAGENTA)
        var bgColorName = teamColors.ansi_bg || "";
        var fgFromBgName = bgColorName.replace(/^BG_/, "");
        var fgFromBg = ANSI_TO_CTRL[fgFromBgName] || defaults.fg;
        
        var bgAltColorName = teamColors.ansi_bg_alt || "";
        var fgFromBgAltName = bgAltColorName.replace(/^BG_/, "");
        var fgFromBgAlt = ANSI_TO_CTRL[fgFromBgAltName] || defaults.fg;
        ...
        return {
            ...
            fgFromBg: fgFromBg,        // Foreground version of bg color
            fgFromBgAlt: fgFromBgAlt,  // Foreground version of bg_alt color
            ...
            // Numeric attributes for derived foreground colors (for figlet etc)
            fgFromBgAttr: getFgAttrFromName(fgFromBgName),
            fgFromBgAltAttr: getFgAttrFromName(fgFromBgAltName)
        };
    }
    ...
    function getCityTheme(city) {
        ...
        // For hfg (highlight foreground), we need high intensity for readability
        // If fgAccentAttr is a base color (0-7), add 8 to make it bright
        var hfgAttr = teamColors.fgAccentAttr;
        if (hfgAttr >= 0 && hfgAttr <= 7) {
            hfgAttr = hfgAttr + 8;  // Add HIGH bit for brightness
        }
        
        return {
            fg: teamColors.fgAttr,           // Normal text: team primary color (e.g., LIGHTRED)
            bg: 0,                            // Normal bg: BG_BLACK (dark background)
            hfg: hfgAttr,                    // Highlight text: bright accent color
