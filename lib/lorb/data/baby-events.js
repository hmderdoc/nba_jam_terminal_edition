...
(function() {
    ...
    var SPOUSE_RETALIATION_EVENTS = {
        ...
        sabotage: {
            weight: 25,
            dialogue: [
                "Your next opponent walks up with {spouse}. \"She told me all your weaknesses. Thanks, champ!\"",
                "{spouse} was seen at your rival's practice. \"Just helping them beat you,\" she says coldly.",
                "You lose the game and find {spouse} celebrating with your opponent. \"Consider that payback.\""
            ],
            effects: {
                repLoss: [25, 50],
    ...
    function checkSpouseRetaliationEvent(ctx, gameDay) {
        ...
        if (ctx.lastSpouseRetaliationDay && gameDay - ctx.lastSpouseRetaliationDay < 5) {
            return null;  // At least 5 days between spouse events
        }
        
        // Chance of spouse retaliation event
        var retaliationChance = getConfig("SPOUSE_RETALIATION_CHANCE", 0.20);
    ...
    function showEventRichView(eventData, ctx, gameDay) {
        var RichView = null;
        try {
            if (typeof _hubRichView !== "undefined") {
                RichView = _hubRichView;
            } else {
                load("/sbbs/xtrn/nba_jam/lib/ui/rich-view.js");
                RichView = this.RichView;
            }
        } catch (e) {
            // Fall back to legacy
            return showEventLegacy(eventData, ctx, gameDay);
        }
        ...
        var view = new RichView({
            zones: [
                ...
                { name: "content", x: 1, y: 4, width: 80, height: 16 },
                { name: "footer", x: 1, y: 20, width: 80, height: 5 }
            ],
            theme: "lorb"
        });
        ...
        if (headerFrame) {
            ...
            headerFrame.putmsg("  \1h\1r\x10 " + title + " \x11\1n");
        ...
        if (contentFrame) {
            ...
            for (var i = 0; i < words.length; i++) {
                if (line.length + words[i].length + 1 > 76) {
                    contentFrame.gotoxy(2, y++);
                    contentFrame.putmsg("\1w" + line + "\1n");
                    line = words[i];
                } else {
                    line += (line ? " " : "") + words[i];
                }
            ...
            // Show current cash if relevant
            if (eventData.amount > 0 || eventData.event.choices.some(function(c) { return c.cost; })) {
                y += 2;
                contentFrame.gotoxy(2, y);
                contentFrame.putmsg("\1cYour cash: \1h\1y$" + (ctx.cash || 0) + "\1n");
            }
        }
        ...
        if (footerFrame) {
            ...
            footerFrame.putmsg("\1h\1y--- What do you do? ---\1n");
            
            var choices = eventData.event.choices;
            for (var i = 0; i < choices.length; i++) {
                footerFrame.gotoxy(2, 2 + i);
                var choiceText = choices[i].text;
                if (choices[i].cost) {
                    choiceText += " \1y($" + choices[i].cost + ")\1n";
                } else if (choices[i].effect === "pay" && eventData.amount) {
                    choiceText += " \1y($" + eventData.amount + ")\1n";
                } else if (choices[i].effect === "half" && eventData.amount) {
                    choiceText += " \1y($" + Math.floor(eventData.amount / 2) + ")\1n";
                }
                footerFrame.putmsg("\1h\1w[" + choices[i].key + "]\1n " + choiceText);
            }
        }
     ...
     */
    function showEventResult(result, eventData, choice) {
        if (typeof LORB !== "undefined" && LORB.View) {
            LORB.View.clear();
            LORB.View.header("RESULT");
            LORB.View.line("");
            
            if (!result.success) {
                LORB.View.line("\1r" + result.message + "\1n");
            } else {
                if (result.message) {
                    LORB.View.line("\1w" + result.message + "\1n");
                    LORB.View.line("");
                }
                
                if (result.cashChange !== 0) {
                    var cashColor = result.cashChange < 0 ? "\1r" : "\1g";
                    LORB.View.line(cashColor + "Cash: " + (result.cashChange < 0 ? "" : "+") + "$" + result.cashChange + "\1n");
                }
                
                if (result.alignmentChange !== 0) {
                    var alignColor = result.alignmentChange < 0 ? "\1r" : "\1g";
                    LORB.View.line(alignColor + "Karma: " + (result.alignmentChange < 0 ? "" : "+") + result.alignmentChange + "\1n");
                }
                
                if (result.relationshipChange !== 0) {
                    var relColor = result.relationshipChange < 0 ? "\1r" : "\1g";
                    var who = eventData.type === "baby_mama" ? eventData.mama.name : (eventData.child.name || "Your kid");
                    LORB.View.line(relColor + who + " relationship: " + (result.relationshipChange < 0 ? "" : "+") + result.relationshipChange + "\1n");
                }
                
                if (result.repChange !== 0) {
                    LORB.View.line("\1r" + "Rep: " + result.repChange + "\1n");
                }
                
                if (result.xpChange !== 0 && eventData.child) {
                    LORB.View.line("\1c" + (eventData.child.name || "Your kid") + " XP: +" + result.xpChange + "\1n");
                }
            }
