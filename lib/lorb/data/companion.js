...
(function() {
    ...
    function takeToDinner(ctx, gameDay) {
        ...
        // Check if intimate encounter should be offered (based on relationship status)
    ...
    function takeToClub(ctx, gameDay) {
        ...
        // Check if intimate encounter should be offered (higher chance at club)
        var offersIntimacy = shouldOfferIntimacy(ctx, true);  // true = at club (higher odds)
     ...
     * Based on relationship status and random chance
     * @param {Object} ctx - Player context
     * @param {boolean} atClub - If true, higher chance (club dates are spicier)
     * @returns {boolean} - Whether intimacy should be offered
     */
    function shouldOfferIntimacy(ctx, atClub) {
        if (!ctx.travelingCompanion || !ctx.travelingCompanion.npcId) {
            return false;
        }
        ...
        // Get relationship from ctx.romance.relationships
        var relationship = null;
        if (ctx.romance && ctx.romance.relationships && ctx.romance.relationships[npcName]) {
            relationship = ctx.romance.relationships[npcName];
        }
        ...
        // Only offer for partner+ relationships (crush is too early)
        var eligibleStatuses = ["partner", "fiance", "spouse"];
        if (eligibleStatuses.indexOf(relationship.status) === -1) {
            return false;
        }
        
        // Check if already pregnant - don't offer again
        if (ctx.pregnancies) {
            for (var i = 0; i < ctx.pregnancies.length; i++) {
                if (ctx.pregnancies[i].npcId === npcName || ctx.pregnancies[i].npcName === npcName) {
                    return false;  // Already pregnant or waiting for reveal
                }
            }
        }
        
        // Base chance depends on status (higher = more likely)
        var baseChance = 0.30;  // partner
        if (relationship.status === "fiance") baseChance = 0.45;
        if (relationship.status === "spouse") baseChance = 0.60;
        
        // Club dates are spicier
        if (atClub) {
            baseChance += 0.15;
        }
        
        // Affection bonus (higher affection = more willing)
        var affectionBonus = (relationship.affection || 0) / 200;  // up to +50% at 100 affection
        
        return Math.random() < (baseChance + affectionBonus);
    }
    ...
    function rollForMultiples() {
        var twinChance = getBabyConfig("MULTIPLE_CHANCE_TRAVELING_COMPANION", 0.20);
        var tripletChance = twinChance * 0.25;  // 25% of twin chance = triplet
