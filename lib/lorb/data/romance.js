...
(function() {
    ...
    // Note: No "fiance" status - proposal is an action, not a state
    var STATUS_THRESHOLDS = {
        stranger: 0,
        acquaintance: 5,
        crush: 20,
        partner: 40
        // spouse is set only via successful proposal
    };
    
    // Proposal success thresholds
    var PROPOSAL_THRESHOLDS = {
        minAffection: 50,      // Minimum affection to attempt proposal
        guaranteedSuccess: 90  // Auto-success at this level
    };
     ...
     * Note: "spouse" is only set via successful proposal, not by affection.
     */
     ...
     * Success is based on affection level, not a fixed status.
     * Returns { success: boolean, accepted: boolean, reason: string, affectionLoss: number }
     */
    function proposeMarriage(ctx, npcName, cityId) {
        if (!ctx || !ctx.romance || !npcName) {
            return { success: false, accepted: false, reason: "Invalid state", affectionLoss: 0 };
        }
        
        // Check if player is already married
        if (ctx.romance.spouseName) {
            return { success: false, accepted: false, reason: "You are already married to " + ctx.romance.spouseName, affectionLoss: 0 };
        }
        
        // Check relationship exists and has enough affection
        var rel = getRelationship(ctx, npcName);
        if (!rel) {
            return { success: false, accepted: false, reason: "You don't know this person", affectionLoss: 0 };
        }
        
        // Need minimum affection to even attempt
        if (rel.affection < PROPOSAL_THRESHOLDS.minAffection) {
            return { success: false, accepted: false, reason: "You need to build a stronger relationship first (" + rel.affection + "/" + PROPOSAL_THRESHOLDS.minAffection + " affection)", affectionLoss: 0 };
        }
        
        // Calculate success chance based on affection
        // 50 affection = 25% chance, scales up to 100% at 90+
        var affectionRange = PROPOSAL_THRESHOLDS.guaranteedSuccess - PROPOSAL_THRESHOLDS.minAffection;
        var playerProgress = rel.affection - PROPOSAL_THRESHOLDS.minAffection;
        var successChance = Math.min(1.0, 0.25 + (playerProgress / affectionRange) * 0.75);
        
        // Roll for success
        var roll = Math.random();
        if (roll > successChance) {
            // Proposal rejected - lose some affection
            var affectionLoss = Math.floor(5 + Math.random() * 10); // 5-15 loss
            rel.affection = Math.max(0, rel.affection - affectionLoss);
            updateStatus(rel);
            return { success: true, accepted: false, reason: "She's not ready for that kind of commitment yet.", affectionLoss: affectionLoss };
        }
        ...
        if (isNPCMarried(npcName, registry)) {
            ...
            return { success: false, accepted: false, reason: npcName + " is already committed to someone else", affectionLoss: 0 };
        }
        ...
        if (!saveMarriageRegistry(registry)) {
            return { success: false, accepted: false, reason: "Failed to save marriage (try again)", affectionLoss: 0 };
        }
        ...
        return { success: true, accepted: true, reason: "accepted", affectionLoss: 0 };
    }
    ...
    function checkForPregnancy(ctx, npc, cityId, isTravelingCompanion) {
        ...
        if (isTravelingCompanion) {
            pregnancyChance = getPregnancyConfig("PREGNANCY_CHANCE_INTIMATE", 0.35);
        } else if (status === "partner" || status === "spouse") {
    ...
    function shouldOfferCribVisit(relationship) {
        ...
        // Only offer for partner or spouse relationships
        var eligibleStatuses = ["partner", "spouse"];
        if (eligibleStatuses.indexOf(relationship.status) === -1) return false;
        
        // Base chance scales with affection: 30% at 40 affection, up to 70% at 100
        var baseChance = 0.30;
        var affectionBonus = Math.min(0.40, (relationship.affection - 40) / 150); // Max +40% at 100 affection
        var chance = baseChance + affectionBonus;
        
        // Spouse always higher chance
        if (relationship.status === "spouse") chance = Math.max(chance, 0.70);
