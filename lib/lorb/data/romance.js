...
(function() {
    ...
    // Daily flirt limit (can be overridden by LORB.Config)
    var MAX_FLIRTS_PER_DAY = 3;
    
    // Rarity weights for NPC selection
    var RARITY_WEIGHTS = {
        "common": 5,
        "uncommon": 3,
        "rare": 1
    ...
    function canFlirt(ctx) {
        ...
        var maxFlirts = (LORB.Config && LORB.Config.MAX_FLIRTS_PER_DAY) 
            ? LORB.Config.MAX_FLIRTS_PER_DAY 
            : MAX_FLIRTS_PER_DAY;
        
        return ctx.romance.dailyFlirtCount < maxFlirts;
    }
    ...
    function getRemainingFlirts(ctx) {
        ...
        return Math.max(0, maxFlirts - ctx.romance.dailyFlirtCount);
    }
    ...
    function pickRandomNPC(eligibleNPCs) {
        ...
        for (var i = 0; i < eligibleNPCs.length; i++) {
            ...
            var weight = RARITY_WEIGHTS[npc.rarity] || RARITY_WEIGHTS.common;
