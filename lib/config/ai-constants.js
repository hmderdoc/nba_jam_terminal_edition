...
var AI_CONSTANTS = {
    ...
    DEFENSE_ON_BALL: {
        ...
        // Press vs Retreat decision - should defender pursue full court or fall back?
        PRESS_DECISION: {
            // Base threshold - pressScore must exceed this to attempt full court press
            pressThreshold: 0.4,
            // Retreat position when not pressing (tiles from own basket toward midcourt)
            retreatDistance: 25,              // Fall back to ~25 tiles from basket (near halfcourt)
            retreatJitter: 4,                 // Random offset to avoid robotic positioning
            
            // Catchup potential weights (speed/turbo advantage)
            catchupWeight: 0.15,              // Weight for speed+turbo differential
            catchupTurboScale: 0.01,          // How much each turbo point matters (turbo is 0-100)
            catchupSpeedScale: 0.08,          // How much each speed attribute point matters (1-10)
            
            // Game situation weights
            scoreDifferentialWeight: 0.03,    // Per-point weight for score differential
            trailingUrgencyBonus: 0.25,       // Bonus when trailing (encourages press when behind)
            leadingComfortBonus: 0.10,        // Small bonus when leading big (low-risk gambling)
            leadingComfortThreshold: 15,      // Point lead needed for "comfortable" status
            desperationThreshold: 10,         // Points behind to trigger desperation mode
            desperationBonus: 0.35,           // Big bonus when in desperation
            
            // Time pressure
            timeUrgencyThreshold: 30,         // Seconds remaining to trigger time urgency
            timeUrgencyWeight: 0.02,          // Per-second-under-threshold weight
            criticalTimeThreshold: 15,        // Seconds for critical time bonus
            criticalTimeBonus: 0.20,          // Bonus when time is critical AND trailing
            
            // Threat assessment (ball handler danger)
            shooterThreatWeight: 0.04,        // Per-point of 3PT rating (discourages pressing good shooters)
            shooterThreatThreshold: 7,        // 3PT rating above which threat penalty applies
            
            // Rubber banding integration
            rubberBandBonus: 0.15,            // Bonus when rubber banding boost is active
            
            // Minimum distance to even consider pressing (don't press near your own basket)
            minPressDistance: 30              // Only consider press if ball handler is 30+ tiles from defender's basket
        },
