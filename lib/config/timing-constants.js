...
var TIMING_CONSTANTS = {
    ...
    SHOVE: {
        ...
        DIRECTION: {
            // Weight for pushing away from victim's target basket (primary goal)
            awayFromBasketWeight: 0.6,
            // Weight for pushing toward nearest sideline (secondary goal - clear the lane)
            towardSidelineWeight: 0.4,
            // Minimum sideline push component (ensures some lateral movement)
            minSidelineComponent: 0.25,
            // Random jitter to add unpredictability (0-1 scale)
            randomJitter: 0.15
        },
        VICTIM_RECOVERY: {
            // Base frames victim is stunned after being shoved (AI skipped)
            baseFrames: 6,
            // Additional frames per unit of push distance (push 12-25)
            framesPerPushUnit: 0.3,
            // Turbo drained from victim on successful shove
            turboDrain: 15,
            // Speed multiplier during recovery (0.5 = 50% speed)
            speedPenalty: 0.5,
            // Victim cannot use turbo for this many frames after recovery ends
    ...
    CLOCK: {
        SECOND_MS: 1000,
        DEFAULT_CLOCK_SPEED_MULTIPLIER: 1,
    ...
    LORB_CHALLENGES: {
        // Grace period before a pending challenge is considered timed out by the lobby waiter
        pendingAcceptanceGraceMs: 45000,
        // Overall lobby wait timeout while both players confirm
        lobbyTimeoutMs: 120000,
        // Poll cadence while waiting in the lobby
        pollTickMs: 2000
    },
    STAT_TRAIL: {
        LIFETIME_FRAMES: 96,
    ...
    SUBSTITUTION: {
        // Turbo threshold below which CPU will consider substituting (0-150 scale)
        CPU_TURBO_THRESHOLD: 30,
        // Minimum turbo a fresh player should have when entering (percentage of capacity)
        FRESH_PLAYER_TURBO_PERCENT: 0.9,
        // If true, human must have at least one substitute available to use S key
        REQUIRE_AVAILABLE_SUB: true,
        // Time allowed for substitution screen before auto-continue (ms)
