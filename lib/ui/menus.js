...
function mainMenu() {
    ...
    while (true) {
        ...
        if (keyUpper === 'Q') {
            ...
            return "lorb"; // LORB - first option for ENTER key mashers
        } else if (key === '2') {
            return arcadeSubmenu(); // Arcade submenu
        }
...
/**
 * Arcade mode submenu - Single Player, Multiplayer, CPU Demo, Back
 */
function arcadeSubmenu() {
    ...
    while (true) {
        ...
        if (keyUpper === 'B' || keyUpper === 'Q') {
            return mainMenu(); // Back to main menu
        } else if (key === '1') {
            return "play"; // Single player
        } else if (key === '2') {
...
function showMatchupScreen(allowBetting, systems, mpScreenCoordinator, myPlayerId) {
    ...
    var bettingPromptEnabled = !!(allowBetting && BETTING_PROMPTS_ENABLED);
    var bettingHotkeyEnabled = !!(allowBetting && BETTING_HOTKEY_ENABLED);
    var isMultiplayer = !!(mpScreenCoordinator);
    var isCoordinator = !!(mpScreenCoordinator && mpScreenCoordinator.coordinator && mpScreenCoordinator.coordinator.isCoordinator);
