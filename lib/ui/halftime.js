...
function showHalftimeScreen(systems, mpScreenCoordinator, myPlayerId, coordinator, playerClient) {
    ...
    // NON-COORDINATOR: Fetch and apply remote substitutions before halftime ends
    // The coordinator broadcasts subs in state.g.subs. We need to force a reconcile
    // to get the latest state packet, then extract subs from it.
    if (!isCoordinator && playerClient) {
        debugLog("[HALFTIME] Non-coordinator: checking for remote substitutions");
        // Force a state sync to get latest state packet
        if (typeof playerClient.reconcile === 'function') {
            playerClient.reconcile();
        }
        // Check lastState for pending subs from coordinator
        var lastState = playerClient.lastState;
        var pendingSubs = (lastState && lastState.g && lastState.g.subs) ? lastState.g.subs : null;
        if (pendingSubs && pendingSubs.length > 0) {
            debugLog("[HALFTIME] Applying " + pendingSubs.length + " remote substitution(s) from coordinator state");
            if (typeof playerClient.applyRemoteSubstitutions === 'function') {
                playerClient.applyRemoteSubstitutions(pendingSubs);
            }
        } else {
            debugLog("[HALFTIME] No remote substitutions to apply (lastState.g.subs=" + JSON.stringify(pendingSubs) + ")");
        }
    }
