...
function showSubstitutionScreen(systems) {
    ...
    function renderSubstitutionScreen() {
        ...
        // Note: In LORB, slot 0 is the human player who typically shouldn't be subbed
        ...
        if (phase === "select_active") {
            courtFrame.center("\1h\1y[UP/DOWN]\1n Select player to substitute out\r\n");
            courtFrame.center("\1h\1y[ENTER]\1n Confirm selection    \1h\1r[Q]\1n Cancel\r\n");
        } else {
            courtFrame.center("\1h\1y[UP/DOWN]\1n Select substitute    \1h\1y[ENTER]\1n Confirm\r\n");
            courtFrame.center("\1h\1r[ESC/BACKSPACE]\1n Go back\r\n");
        }
    ...
    while (true) {
        ...
        if (key === ' ') {
        ...
        } else if (keyCode === 27 || keyCode === 8) {  // ESC or Backspace
            if (phase === "select_bench") {
                phase = "select_active";
                renderSubstitutionScreen();
            }
        } else if (keyUpper === 'KEY_UP' || key === '\x1b[A' || keyCode === 72) {  // Up arrow
            if (phase === "select_active") {
            ...
            renderSubstitutionScreen();
        } else if (keyUpper === 'KEY_DOWN' || key === '\x1b[B' || keyCode === 80) {  // Down arrow
            if (phase === "select_active") {
                selectedActiveSlot = (selectedActiveSlot === 0) ? 1 : 0;
            } else {
