/**
 * NBA JAM - Score Display Module
 * 
 * Handles all score board rendering including:
 * - Main score display with team names, scores, and shot clock
 * - Big digit score rendering with TD fonts
 * - Turbo bars
 * - Player names and controller labels
 * - Score flash animations
 * - Frame management for score panels
 * 
 * NOTE: Uses view model from lib/game-logic/score-calculator.js
 * All game logic moved there - this module only renders.
 */

/**
 * Get width needed for turbo bar display
 */
function getTurboBarWidth(player) {
    if (!player || !player.playerData) return 0;
    var jerseyDisplay = calculateJerseyDisplay(player);
...
/**
 * Draw turbo bar for a player at current cursor position
 */
...
/**
 * Ensure a score frame exists at the specified position
 */
...
/**
 * Remove font control codes from a line
 */
...
/**
 * Fill frame background with specified attribute
 */
...
/**
 * Render score using big TD font digits
 */
...
/**
 * Render score as fallback text when font rendering fails
 */
...
/**
 * Main score display function - renders complete scoreboard
 */
function drawScore(systems) {
    ...
    var teamBTeamName = (teamNames.teamB || "TEAM B").toUpperCase();
    var teamATeamName = (teamNames.teamA || "TEAM A").toUpperCase();
    var teamAbbrs = stateManager.get('teamAbbrs');
    var teamBTeamAbbr = (teamAbbrs && teamAbbrs.teamB) ? String(teamAbbrs.teamB).toUpperCase() : "TEMB";
    var teamATeamAbbr = (teamAbbrs && teamAbbrs.teamA) ? String(teamAbbrs.teamA).toUpperCase() : "TEAM A";
    ...
    var teamBNameColor = (teamColors.teamB ? teamColors.teamB.fg : LIGHTBLUE) | teamBBg;
    var teamANameColor = (teamColors.teamA ? teamColors.teamA.fg : LIGHTRED) | teamABg;
