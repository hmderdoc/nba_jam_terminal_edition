...
function showGameOver(isDemoMode, systems, mpScreenCoordinator, myPlayerId, coordinator) {
    ...
    if (isDemoMode) {
        ...
        // WAVE 24: Simplified multiplayer game-over - just show box score and quit to lobby
        // No voting needed - players can restart from lobby
        var lastRender = 0;
        var gameOverTimeout = 60000; // 60 seconds max display time
        var gameOverStart = Date.now();
        
        while (true) {
            // Check for dismissal signal from coordinator
            ...
            if (mpScreenAction && mpScreenAction.action === "dismiss" && mpScreenAction.screen === "game_over") {
                debugLog("[GAME OVER] Received dismissal signal");
                stateManager.set("mpScreenAction", null, "game_over_dismiss_handled");
                return "quit";
            }

            // Coordinator: Check if can dismiss (all ready or timeout)
            ...
            // Update countdown display in scoreboard frame
            if (Date.now() - lastRender >= 500) {
                var scoreboardFrame = (typeof FrameManager !== "undefined") ? FrameManager.get("scoreboard") : null;
                if (scoreboardFrame) {
                    var elapsed = Date.now() - gameOverStart;
                    var remaining = Math.max(0, Math.ceil((gameOverTimeout - elapsed) / 1000));
                    scoreboardFrame.clear();
                    scoreboardFrame.gotoxy(1, 2);
                    scoreboardFrame.center("\1h\1y[ENTER]\1n Quit to Lobby");
                    scoreboardFrame.gotoxy(1, 3);
                    scoreboardFrame.center("\1h\1c" + remaining + "s\1n");
            ...
            if (Date.now() - gameOverStart >= gameOverTimeout) {
                debugLog("[GAME OVER] Timeout - auto quitting");
                if (isCoordinator) {
                    mpScreenCoordinator.dismissScreen();
                }
                return "quit";
            ...
            var key = console.inkey(K_NONE, 100);
            if (!key) {
                // Keep coordinator state sync active
            ...
            // ENTER or Q = quit to lobby (not SPACE - it's used for gameplay)
            if (key === '\r' || key === '\n' || key.toUpperCase() === 'Q') {
                debugLog("[GAME OVER] Player pressed quit key");
                if (isMultiplayer) {
                    mpScreenCoordinator.setReady(myPlayerId, "quit");
                }
                // Wait briefly for coordinator to handle, or just quit
                if (!isCoordinator) {
                    mswait(500);
                }
                return "quit";
        ...
        // Single-player: wait for ENTER to proceed (not SPACE - mashed during gameplay)
        while (true) {
...
function renderGameOverDisplay(isDemoMode, isMultiplayer, systems) {
    ...
    if (isDemoMode) {
        ...
        // Multiplayer: Simple quit to lobby - no voting needed
        courtFrame.center("\r\n\1h[ENTER]\1n Quit to Lobby\r\n");
    } else {
        courtFrame.center("\r\n\1h[ENTER]\1n Play Again  \1h[T]\1n New Teams  \1h[Q]\1n Quit to Menu\r\n");
    }
