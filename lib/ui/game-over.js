...
function renderTeamBoxScore(teamKey, teamLabel, options) {
    var players = teamKey === "teamA" ? getRedTeam() : getBlueTeam();
...
/**
 * Display game over screen with final score and box scores
 * Handles both demo mode and player mode with different UI options
 * 
...
function showGameOver(isDemoMode) {
    ...
    var teamAName = gameState.teamNames && gameState.teamNames.teamA ? gameState.teamNames.teamA : "TEAM A";
    var teamBName = gameState.teamNames && gameState.teamNames.teamB ? gameState.teamNames.teamB : "TEAM B";
    ...
    if (gameState.score.teamA > gameState.score.teamB) {
        courtFrame.center(teamAColorCode + " " + teamAName.toUpperCase() + " WIN!\1n\r\n");
    } else if (gameState.score.teamB > gameState.score.teamA) {
    ...
    renderTeamBoxScore("teamA", teamAName, { halftime: false });
    courtFrame.center("\r\n");
    renderTeamBoxScore("teamB", teamBName, { halftime: false });
