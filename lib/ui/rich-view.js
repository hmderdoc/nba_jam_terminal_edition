...
// Load helpers for Frame utilities (drawBorder, cycleFrame, buryCursor)
try {
    var _helpersPath = _richViewLibPath.replace(/lib\/ui\/?$/, "lib/utils/helpers.js")
                                       .replace(/lib\\ui\\?$/, "lib\\utils\\helpers.js");
    load(_helpersPath);
} catch (e) {
    // Helpers may not be available, but RichView will still work
}
...
RichView.prototype.render = function() {
    if (this._mainFrame) {
        // Use cycleFrame helper if available (buries cursor after cycle for cleaner animations)
        if (typeof cycleFrame === "function") {
            cycleFrame(this._mainFrame);
        } else {
...
/**
 * Render without burying the cursor - use this before console.getstr() or 
 * other direct console input calls that need the cursor positioned correctly.
 * The cursor will remain at the frame's last written position.
 */
RichView.prototype.renderForInput = function() {
    if (this._mainFrame) {
        this._mainFrame.cycle();
    }
};
...
/**
 * Position the console cursor at a specific location in a zone.
 * Use this after render() and before console.getstr() when you need
 * to input at a specific position. 
 * @param {string} zoneName - Optional zone name (defaults to content zone)
 * @param {number} x - X position within zone (1-based)
 * @param {number} y - Y position within zone (1-based)
 */
RichView.prototype.positionCursor = function(zoneName, x, y) {
    var frame = this.getZone(zoneName || this._contentZoneName);
    if (!frame) return;
    
    // Calculate absolute screen position
    var absX = frame.x + (x || 1) - 1;
    var absY = frame.y + (y || 1) - 1;
...
/**
 * Write a line without auto-rendering. Use this when you need to write
 * multiple lines before rendering, or when preparing for input.
 * Call renderForInput() after to display without burying cursor.
 */
RichView.prototype.lineNoRender = function(text) {
    var frame = this.getContentFrame();
    if (!frame) return;
...
RichView.prototype.menu = function(items, options) {
    ...
    var zone = this._zones[zoneName];
    if (!zone || !zone.frame) return null;
    
    // Use inner frame if zone has a border, otherwise use zone frame
    var frame = zone._innerFrame || zone.frame;
    var zoneConfig = zone.config;
    
    // If using inner frame, coordinates are relative to it (start at 1,1)
    // If using zone frame, use traditional offsets
    var hasInnerFrame = !!zone._innerFrame;
    var menuX = options.x !== undefined ? options.x : (hasInnerFrame ? 1 : 2);
    var menuY = options.y !== undefined ? options.y : (this._cursorY + 1);
    var menuWidth = options.width || (hasInnerFrame ? frame.width - 2 : zoneConfig.width - 4);
    
    // Calculate available height for the menu viewport
    // If not explicitly provided, use remaining space in the frame
    var menuHeight;
    if (options.height !== undefined) {
        menuHeight = options.height;
    } else {
        // Calculate remaining height from menuY to bottom of frame
        var frameHeight = hasInnerFrame ? frame.height : zoneConfig.height;
        menuHeight = frameHeight - menuY + 1;
    }
    ...
    var menu = new FrameLightbar({
        ...
        onIdle: options.onIdle ? function(lb) {
            options.onIdle(self, lb);
        } : null
    });
...
RichView.prototype.prompt = function(text, maxLen) {
    ...
    // Calculate absolute cursor position for input
    // Frame position is screen-absolute; add the text length to position cursor after prompt
    var plainText = text.replace(/\x01[^\x01]/g, ""); // Strip Ctrl-A color codes
    var absX = frame.x + plainText.length;
    var absY = frame.y + this._cursorY - 1; // _cursorY was just incremented, so -1
    console.gotoxy(absX, absY);
...
RichView.prototype.confirm = function(text) {
    ...
    var plainText = text.replace(/\x01[^\x01]/g, ""); // Strip Ctrl-A color codes
    var absX = frame.x + plainText.length;
    var absY = frame.y + this._cursorY; // Cursor not incremented yet here
    console.gotoxy(absX, absY);
...
/**
 * Draw a border around a zone and create an inner content frame
 * Returns the inner frame for writing content with natural positioning
 * 
 * @param {string} zoneName - Name of the zone to add border to
 * @param {Object} options - Border options
 * @param {number|Array} options.color - Border color(s) for drawBorder
 * @param {string} options.title - Optional title for border top
 * @param {number} options.titleAttr - Title color attribute
 * @param {number} options.padding - Extra padding inside border (default 0)
 * @param {boolean} options.wordWrap - Enable word wrapping (default true)
 * @returns {Frame} The inner content frame - use gotoxy(1,1) naturally
 */
RichView.prototype.drawBorder = function(zoneName, options) {
    options = options || {};
    var zone = this._zones[zoneName];
    if (!zone || !zone.frame) return null;
    
    var outerFrame = zone.frame;
    var padding = options.padding || 0;
    
    // Draw the border on the outer frame
    if (typeof outerFrame.drawBorder === "function") {
        outerFrame.drawBorder(options.color || this._theme.fg, {
            title: options.title,
            titleAttr: options.titleAttr
        });
    }
    
    // Calculate inner frame dimensions (1 for border on each side + padding)
    var inset = 1 + padding;
    var innerX = outerFrame.x + inset;  // Absolute screen coords based on parent position
    var innerY = outerFrame.y + inset;
    var innerWidth = outerFrame.width - (inset * 2);
    var innerHeight = outerFrame.height - (inset * 2);
    ...
    // Close any existing inner frame
    if (zone._innerFrame) {
        try { zone._innerFrame.close(); } catch (e) {}
    }
    
    // Create child frame inside the bordered area
    // Child frame position is relative to parent's interior
    var innerFrame = new Frame(innerX, innerY, innerWidth, innerHeight, BG_BLACK, outerFrame);
    innerFrame.open();
    
    // Enable word wrap by default for clean text display
    if (options.wordWrap !== false) {
        innerFrame.word_wrap = true;
    }
    
    zone._innerFrame = innerFrame;
...
/**
 * Get border inset for a zone (0 if no border)
 */
RichView.prototype.getBorderInset = function(zoneName) {
    var zone = this._zones[zoneName];
    return zone && zone._borderInset ? zone._borderInset : 0;
};
...
RichView.prototype.close = function() {
    for (var name in this._zones) {
        if (this._zones.hasOwnProperty(name)) {
            // Close inner frame first if it exists (from drawBorder)
            if (this._zones[name]._innerFrame) {
                try { this._zones[name]._innerFrame.close(); } catch (e) {}
            }
            if (this._zones[name].frame) {
