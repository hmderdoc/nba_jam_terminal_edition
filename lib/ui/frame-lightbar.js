...
// Only load dependencies if not already present
...
function FrameLightbar(options) {
    ...
    this.onLeftRight = options.onLeftRight || null;  // For horizontal cycling in vertical menus
    this.onIdle = options.onIdle || null;  // Called each iteration when no key pressed
...
FrameLightbar.prototype._drawItemAt = function(index, viewY) {
    ...
    if (hasCtrlCodes) {
        // Text has embedded color codes
        // If this is the current/selected item, highlight only the LABEL portion (up to colon)
        // and preserve original formatting for the value portion after the colon
        if (isCurrent && !isDisabled) {
            // Find the colon position to determine where label ends
            var colonInfo = findColonPosition(displayText);
            if (colonInfo.colonPos >= 0) {
                // Has a colon - highlight label (up to and including colon), preserve value formatting
                var labelPart = stripCtrlCodes(displayText.substring(0, colonInfo.rawColonPos + 1));
                var valuePart = displayText.substring(colonInfo.rawColonPos + 1);
                frame.putmsg(attrToCtrl(textFg, textBg) + labelPart + "\1n" + valuePart);
            } else {
                // No colon - highlight entire text (strip codes and apply highlight)
                var strippedText = stripCtrlCodes(displayText);
                frame.putmsg(attrToCtrl(textFg, textBg) + strippedText);
            }
        } else {
            // Not selected - use embedded colors directly
            frame.putmsg(displayText);
        }
        // Always normalize after embedded color code text before right padding
        frame.putmsg("\1n");
...
FrameLightbar.prototype.draw = function() {
    ...
    if (this.parentFrame && this._ownsFrame && this.parentFrame.cycle()) {
        console.gotoxy(console.screen_columns, console.screen_rows);
    }
};
...
/**
 * Fire the onLeftRight callback for horizontal cycling in vertical menus
 * direction: -1 for left, 1 for right
 * Returns: true to stay in menu and redraw, false to ignore, or a value to return from getval
 */
FrameLightbar.prototype._fireLeftRight = function(direction) {
    if (this.onLeftRight && typeof this.onLeftRight === "function") {
        var item = this.items[this.current];
        try {
            return this.onLeftRight(item, this.current, direction, this);
        } catch (e) {
            log(LOG_WARNING, "[FrameLightbar] onLeftRight callback error: " + e);
        }
    }
    return false;
};
...
FrameLightbar.prototype.getval = function() {
    ...
    while (bbs.online) {
        ...
        if (!key) {
            // Fire idle callback for background processing (e.g., network updates)
            if (this.onIdle) {
                try { this.onIdle(this); } catch (e) {}
            }
            continue;
        }
        ...
        switch (key) {
            ...
            case KEY_LEFT:
                if (this.direction === 1) {
                    this._movePrev();
                } else if (this.direction === 0 && this.onLeftRight) {
                    // Vertical menu with left/right handler - fire callback
                    var lrResult = this._fireLeftRight(-1);
                    if (lrResult === true) {
                        // Callback handled it and wants to stay in menu - redraw
                        this.draw();
                        continue;
                    } else if (lrResult !== false && lrResult !== undefined) {
                        // Callback returned a value to exit with
                        this.close();
                        return lrResult;
                    }
                }
...
/**
 * Update a single menu item's text and redraw it
 * @param {number} index - The item index to update
 * @param {string} newText - The new text for the item
 */
FrameLightbar.prototype.updateItemText = function(index, newText) {
    if (index < 0 || index >= this.items.length) return;
    
    this.items[index].text = newText;
    
    // Redraw just this item
    var viewY = this._getItemViewY(index);
    if (viewY >= 1 && viewY <= this._getViewportHeight()) {
        this._drawItemAt(index, viewY);
...
/**
 * Strip all Ctrl-A color codes from a string, returning plain text.
...
function stripCtrlCodes(str) {
    ...
    return str.replace(/\x01[^\x01]/g, "");
}

/**
 * Find the position of a colon in a string, accounting for color codes.
 * Returns { colonPos: visible position, rawColonPos: raw string position }
 * If no colon found, returns { colonPos: -1, rawColonPos: -1 }
 */
function findColonPosition(str) {
    if (!str) return { colonPos: -1, rawColonPos: -1 };
    var visiblePos = 0;
    var i = 0;
    while (i < str.length) {
        if (str.charAt(i) === "\x01" && i + 1 < str.length) {
            // Skip color code (2 chars)
            i += 2;
        } else {
            if (str.charAt(i) === ":") {
                return { colonPos: visiblePos, rawColonPos: i };
            }
            visiblePos++;
            i++;
        }
    }
    return { colonPos: -1, rawColonPos: -1 };
}
