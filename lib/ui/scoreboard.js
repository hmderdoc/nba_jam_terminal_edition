...
function setFrontcourtEstablished(teamName, systems) {
    ...
    if (frontcourtEstablished) return;
    stateManager.set('frontcourtEstablished', true, 'frontcourt_established');
    var scoreFlash = stateManager.get('scoreFlash');
    if (scoreFlash && scoreFlash.active && scoreFlash.stopTeam === teamName) {
        stopScoreFlash(scoreFlash.activeTeam, systems);
    }
}
...
function drawTurboBar(player) {
    ...
    var capacity = (typeof player.playerData.turboCapacity === "number" && player.playerData.turboCapacity > 0)
        ? player.playerData.turboCapacity
        : MAX_TURBO;
    var jerseyDisplay = getJerseyDisplayValue(player);
    ...
    var highlightThreshold = Math.min(capacity, SHOE_TURBO_THRESHOLD);
    var displayColor = (turbo >= highlightThreshold) ? highColor : lowColor;

    var barLength = GAMEPLAY_CONSTANTS.SCOREBOARD.TURBO_BAR_LENGTH;
    var filled = (capacity > 0) ? Math.floor((turbo / capacity) * barLength) : 0;
...
function drawScore(systems) {
    ...
    // Wave 23D: State Manager migration
    var stateManager = systems.stateManager;
    ...
    var scoreboardLayout = GAMEPLAY_CONSTANTS.SCOREBOARD;
    var frameWidth = scoreFrame.width || scoreboardLayout.DEFAULT_WIDTH;
    var centerColumn = Math.floor(frameWidth / 2);
    var sideMargin = scoreboardLayout.SIDE_MARGIN;
    var turboGap = scoreboardLayout.TURBO_GAP;
    var clockGap = scoreboardLayout.CLOCK_GAP;
    var shotClockRow = scoreboardLayout.ROWS.SHOT_CLOCK;
    var turboRow = scoreboardLayout.ROWS.TURBO;
    var playerRow = scoreboardLayout.ROWS.PLAYER;
    var controllerRow = scoreboardLayout.ROWS.CONTROLLER;
    var scorePanelAttr = WHITE | BG_BLACK;

    var teamNames = stateManager.get('teamNames');
    var teamBTeamName = teamNames && teamNames.teamB ? teamNames.teamB.toUpperCase() : "TEAM B";
    var teamATeamName = teamNames && teamNames.teamA ? teamNames.teamA.toUpperCase() : "TEAM A";
    var resolvedAbbr = resolveTeamAbbreviations(stateManager.get('teamAbbrs'));
    var teamBTeamAbbr = resolvedAbbr.teamB;
    var teamATeamAbbr = resolvedAbbr.teamA;

    var score = stateManager.get('score');
    var teamBScoreValue = String(score.teamB);
    var teamAScoreValue = String(score.teamA);
    ...
    var teamBBg = (teamColors.teamB && teamColors.teamB.bg !== undefined) ? teamColors.teamB.bg : BG_BLACK;
    var teamABg = (teamColors.teamA && teamColors.teamA.bg !== undefined) ? teamColors.teamA.bg : BG_BLACK;
    var teamBNameColor = (teamColors.teamB ? teamColors.teamB.fg : WHITE) | teamBBg;
    var teamANameColor = (teamColors.teamA ? teamColors.teamA.fg : WHITE) | teamABg;
    var panelBgMask = scorePanelAttr & BG_MASK;
    var teamBScoreFg = (teamColors.teamB ? teamColors.teamB.fg : WHITE) & FG_MASK;
    var teamAScoreFg = (teamColors.teamA ? teamColors.teamA.fg : WHITE) & FG_MASK;
    ...
    if (flashInfo.active && flashInfo.regainCheckEnabled && !inbounding && currentTeam === flashInfo.activeTeam) {
        stopScoreFlash(flashInfo.activeTeam, systems);
        flashInfo = getScoreFlashState(systems);
    }

    var flashActive = !!(flashInfo.active && flashInfo.activeTeam);
    var tickCounter = stateManager.get('tickCounter');
    var flashTick = tickCounter || 0;
    var flashPeriod = 8;
    ...
    var clockInfo = calculateClockDisplay(systems);
    var clockText = clockInfo.halfLabel + " " + clockInfo.minutes + ":" + padStart(clockInfo.seconds, 2, '0');
    ...
    var clockAttr = clockInfo.isOvertime ? (YELLOW | BG_BLACK) : (LIGHTGREEN | BG_BLACK);
    scoreFrame.putmsg(clockText, clockAttr);
    ...
    var shotClockValue = Math.max(0, shotClock);
    ...
    function getNameAttr(teamKey, baseFg, baseBg, offset, playerSprite) {
        if (playerSprite && playerSprite.playerData && playerSprite.playerData.onFire) {
            var currentTick = stateManager.get('tickCounter');
            var idx = Math.floor((currentTick + (offset || 0)) / 2) % firePalette.length;
    ...
    function renderPlayerSlot(x, player, teamKey, offset) {
        ...
        var teamColorsAll = stateManager.get('teamColors');
        var baseBg = (teamColorsAll[teamKey] && teamColorsAll[teamKey].bg !== undefined)
            ? teamColorsAll[teamKey].bg
            : BG_BLACK;
        var baseFg = teamColorsAll[teamKey].fg;
    ...
    function renderControllerSlot(x, player, teamKey) {
        ...
        var isMyCoordinatorSprite = false;
        ...
        if (player && player.playerData) {
            ...
            if (player.playerData.controllerIsHuman) {
                var teamColorsAll = stateManager.get('teamColors');
                var teamColors = teamColorsAll[teamKey];
                var fg = teamColors ? teamColors.fg : WHITE;
                attr = fg | BG_BLACK;
                
                // Check if this is the coordinator's own sprite
                // The coordinator's sprite has controllerType="local" while remote players have "remote"
                if (typeof mpCoordinator !== "undefined" && mpCoordinator && mpCoordinator.isCoordinator) {
                    // Coordinator's own sprite is marked with controllerType="local"
                    // controllerType is on the sprite (player) not playerData
                    if (player.controllerType === "local") {
                        isMyCoordinatorSprite = true;
                    }
                }
            }
        }

        // If this is the coordinator's own sprite, render with cyan brackets
        if (isMyCoordinatorSprite && label.length >= 2 && label.charAt(0) === '<' && label.charAt(label.length - 1) === '>') {
            var truncated = label;
            if (truncated.length > width)
                truncated = truncated.substring(0, width);
            var innerText = truncated.substring(1, truncated.length - 1);
            var padLen = width - truncated.length;
            var padding = "";
            for (var p = 0; p < padLen; p++) padding += " ";
            
            scoreFrame.gotoxy(x, controllerRow);
            scoreFrame.putmsg("<", CYAN | BG_BLACK);
            scoreFrame.putmsg(innerText, attr);
            scoreFrame.putmsg(">", CYAN | BG_BLACK);
            scoreFrame.putmsg(padding, BG_BLACK);
        } else {
            var truncated = label;
            if (truncated.length > width)
                truncated = truncated.substring(0, width);
    ...
    if ((scoreFrame.height || 0) >= abbrRow) {
        var teamColorsForAbbr = stateManager.get('teamColors');
        var blueAbbrAttr = (teamColorsForAbbr.teamB ? teamColorsForAbbr.teamB.fg : WHITE) | BG_BLACK;
        var redAbbrAttr = (teamColorsForAbbr.teamA ? teamColorsForAbbr.teamA.fg : WHITE) | BG_BLACK;
