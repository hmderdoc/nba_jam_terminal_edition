...
function enforceBackcourtViolation(message, systems) {
    ...
    // setupInbound() expects the team that "scored" (or in this case, violated)
    // It will give the ball to the OTHER team internally
    // So pass the VIOLATING team, not the opposing team
    debugLog(">>> Calling setupInbound with violatingTeam=" + violatingTeam);

    if (violatingTeam === "teamA" || violatingTeam === "teamB") {
        setupInbound(violatingTeam, systems);
        
        // Transition to INBOUND_SETUP phase for natural inbound animation
        if (systems.phaseHandler && systems.phaseHandler.setPhase) {
            systems.phaseHandler.setPhase("INBOUND_SETUP", {}, 0);
        }

        var newTeam = stateManager.get('currentTeam');
        var newCarrier = stateManager.get('ballCarrier');
        debugLog(">>> After setupInbound: currentTeam=" + newTeam + ", ballCarrier=" + (newCarrier ? (newCarrier.playerData ? newCarrier.playerData.name : "no-name") : "NULL"));
    } else {
