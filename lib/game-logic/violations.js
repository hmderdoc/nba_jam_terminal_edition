...
/**
 * Setup inbound after violation (immediate, no animation)
 * For violations, we immediately award ball to opposing team
 */
function setupViolationInbound(violatingTeam, systems) {
    ...
    stateManager.set("inbounding", false, "violation_inbound");
    ...
    var ballHandler = teamPlayers[inboundIndex] || teamPlayers[0];
    var teammate = teamPlayers[1 - inboundIndex] || teamPlayers[1] || ballHandler;
    
    // Position for sideline inbound (violations inbound from sideline near midcourt)
    var attackDir = inboundTeam === "teamA" ? 1 : -1;
    var midX = Math.floor(COURT_WIDTH / 2);
    var midY = Math.floor(COURT_HEIGHT / 2);
    
    // Ball handler at sideline, slightly in backcourt
    var handlerX = clampToCourtX(midX - attackDir * 18);
    var handlerY = midY;
    
    // Teammate positioned nearby
    var teammateX = clampToCourtX(midX - attackDir * 12);
    var teammateY = midY + 3;
    
    // Move players directly (no animation for violations)
    if (ballHandler && ballHandler.moveTo) {
        ballHandler.moveTo(handlerX, handlerY);
    }
    if (teammate && teammate.moveTo) {
        teammate.moveTo(teammateX, teammateY);
    }
    
    // Position defenders at their defensive positions
    ...
    if (defenderSprites[0] && defenderSprites[0].moveTo) {
        defenderSprites[0].moveTo(defenderBaseX, midY - 3);
    }
    if (defenderSprites[1] && defenderSprites[1].moveTo) {
        defenderSprites[1].moveTo(clampToCourtX(defenderBaseX + attackDir * 4), midY + 3);
    }
    
    // Give ball to ball handler immediately
    stateManager.set("ballCarrier", ballHandler, "violation_inbound");
    stateManager.set("ballHandlerLastX", ballHandler.x, "violation_inbound");
    stateManager.set("ballHandlerLastY", ballHandler.y, "violation_inbound");
    stateManager.set("ballHandlerFrontcourtStartX", ballHandler.x, "violation_inbound");
    stateManager.set("ballHandlerProgressOwner", ballHandler, "violation_inbound");
    
    // Alternate ball handler for next inbound
    ...
    debugLog("[VIOLATION INBOUND] Complete: " + inboundTeam + " has ball at (" + ballHandler.x + "," + ballHandler.y + ")");
}
