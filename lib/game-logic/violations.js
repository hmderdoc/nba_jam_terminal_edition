...
function enforceBackcourtViolation(message, systems) {
    ...
    if (ballCarrier) {
        recordTurnover(ballCarrier, message || "backcourt", systems);
    }
...
function enforceFiveSecondViolation(systems) {
    ...
    var violatingTeam = currentTeam;
    if (ballCarrier) {
        recordTurnover(ballCarrier, "five_seconds", systems);
    }
...
function maybeEnforceBackcourtViolation(message, systems) {
    if (!ENFORCE_BACKCOURT_VIOLATIONS) {
        var stateManager = systems.stateManager;
        if (typeof debugLog === "function") {
            debugLog("[BACKCOURT] Enforcement disabled; skipping whistle ('" + (message || "backcourt") + "')");
        }
        stateManager.set("backcourtTimer", 0, "backcourt_enforcement_disabled");
        resetBackcourtState(systems);
        return false;
    }
        ...
        if (!frontcourtEstablished) {
            ...
            if (clearlyInFrontcourt && inboundGracePeriod === 0) {
                ...
                if (backcourtTimer + 1 >= 210 && !nearHalfCourt) {  // 10.5 seconds at 20 FPS
                    if (maybeEnforceBackcourtViolation("10-SECOND BACKCOURT VIOLATION!", systems)) {
            ...
            if (inboundGracePeriod > 0) {
                ...
                if (maybeEnforceBackcourtViolation("OVER AND BACK!", systems)) {
                    violationTriggeredThisFrame = true;
                }
            }
        }
