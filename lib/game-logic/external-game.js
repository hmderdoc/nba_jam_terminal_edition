 ...
 * 3. Return comprehensive results including stats and events
 * 
 * Game Modes:
 * - "play": Human vs AI (one human player controls a team)
 * - "spectate": Pure AI vs AI (for betting/watching)
 * - "multiplayer": Future - Human(s) with LORB characters
 */
...
var EXTERNAL_GAME_DEFAULTS = {
    ...
    recordEvents: true          // Track scoring/possession events
};
...
/**
 * Initialize game event tracking
 * @param {Object} systems - Game systems
 */
function initEventTracking(systems) {
    var stateManager = systems.stateManager;
    stateManager.set("externalGameEvents", [], "external_game_init");
}

/**
 * Record a game event for results
 * @param {string} type - Event type: "score", "possession"
 * @param {Object} data - Event data
 * @param {Object} systems - Game systems
 */
function recordGameEvent(type, data, systems) {
    var stateManager = systems.stateManager;
    var events = stateManager.get("externalGameEvents");
    if (!events) return;
    ...
    // Keep events array bounded (last 100 events max)
    if (events.length > 100) {
        events.shift();
    }
    
    stateManager.set("externalGameEvents", events, "external_game_event");
}
...
function runExternalGame(config) {
    ...
    try {
        ...
        if (options.recordEvents) {
            initEventTracking(systems);
            
            // Store reference to record function for event hooks
            stateManager.set("externalGameRecordEvent", function(type, data) {
                recordGameEvent(type, data, systems);
            }, "external_game_init");
        }
