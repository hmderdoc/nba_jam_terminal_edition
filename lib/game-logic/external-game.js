...
var EXTERNAL_GAME_DEFAULTS = {
    ...
    shotClock: 24,
    clockSpeedMultiplier: 1,   // Clock speed multiplier (2 = 2x faster)
...
function validateExternalConfig(config) {
    ...
    normalized.options = {
        ...
        clockSpeedMultiplier: (typeof opts.clockSpeedMultiplier === "number" && opts.clockSpeedMultiplier > 0) ? opts.clockSpeedMultiplier : EXTERNAL_GAME_DEFAULTS.clockSpeedMultiplier,
        ...
        isBettingGame: (typeof opts.isBettingGame === "boolean") ? opts.isBettingGame : false  // Wave 24: Enables pause/quit confirmation
    };
...
function normalizePlayerConfig(playerDef, label) {
    ...
    // Compute jersey number first so we can use it for jerseyString fallback
    var jerseyNum = (typeof p.jersey === "number") ? p.jersey : 
            (typeof p.jerseyNumber === "string" || typeof p.jerseyNumber === "number") ? parseInt(p.jerseyNumber, 10) : 
            Math.floor(Math.random() * 99);
    
    return {
        name: p.name || "Street Baller",
        jersey: jerseyNum,
        jerseyString: p.jerseyString || p.jerseyNumber || String(jerseyNum),
        skin: p.skin || "brown",
        ...
        // Boss/special buffs - passed through to sprite-init for buff system
        buffs: p.buffs || null
    };
}
...
function collectExternalGameResults(systems, config) {
    ...
    // Wave 24: Check if quit was a betting forfeit
    var quitForfeited = stateManager.get("quitForfeited") || false;
    ...
    return {
        ...
        quitForfeited: quitForfeited,  // Wave 24: True if player confirmed quit during betting game
...
function runExternalGame(config) {
    ...
    try {
        ...
        // Pass lorbContext through options for bench population
        var spritesOptions = options;
        if (normalizedConfig.lorbContext) {
            spritesOptions = {};
            for (var optKey in options) {
                if (options.hasOwnProperty(optKey)) {
                    spritesOptions[optKey] = options[optKey];
                }
            }
            spritesOptions.lorbContext = normalizedConfig.lorbContext;
        }
        initSpritesFromDynamic(normalizedConfig.teamA, normalizedConfig.teamB, spritesOptions, systems);
        ...
        stateManager.set("clockSpeedMultiplier", options.clockSpeedMultiplier || 1, "external_game_init");
        stateManager.set("isBettingGame", options.isBettingGame || false, "external_game_init");
