...
function attemptShot() {
    ...
    if (attemptType === "dunk") {
        ...
        made = roll < chance;
        debugLog("[DUNK ROLL] Chance: " + chance.toFixed(1) + "%, Roll: " + roll.toFixed(1) + ", Made: " + made);
        ...
        debugLog("[SHOT PATH] mpCoordinator: " + (mpCoordinator ? "exists" : "null") +
            ", isCoordinator: " + (mpCoordinator && mpCoordinator.isCoordinator ? "true" : "false"));

        // Coordinator: Use non-blocking executeShot()
        if (mpCoordinator && mpCoordinator.isCoordinator) {
            debugLog("[SHOT PATH] Taking MULTIPLAYER COORDINATOR path");
            ...
            if (made) {
                // Wait for shot animation to complete
                var animDuration = shotResult.animDuration || 800;
                var waited = 0;
                var waitInterval = 16; // ~60fps
                while (waited < animDuration && animationSystem && animationSystem.isBallAnimating()) {
                    mswait(waitInterval);
                    waited += waitInterval;
                }
                // Brief additional pause after animation
                mswait(100);
                drawScore();
                mswait(200);
                ...
                debugLog("[SHOT PATH] Miss - calling createRebound(), reboundActive before: " + gameState.reboundActive);
                createRebound(targetX, targetY);
                debugLog("[SHOT PATH] After createRebound(), reboundScramble.active: " + gameState.reboundScramble.active);
            }
            return;  // Early return - executeShot() already handled all game logic
        } else {
            // Single-player: Use blocking animation (backward compatible)
            debugLog("[SHOT PATH] Taking SINGLE-PLAYER path");
    ...
    if (made) {
        ...
        debugLog("[SHOT PATH] Single-player miss handling - attemptType: " + attemptType + ", blocked: " + blocked);

        if (attemptType === "dunk" && blocked) {
            if (!gameState.reboundActive) {
                debugLog("[SHOT PATH] Blocked dunk - calling createRebound()");
                createRebound(targetX, targetY);
            }
            ...
            debugLog("[SHOT PATH] Blocked shot - calling createRebound() at deflection point");
            ...
            debugLog("[SHOT PATH] Normal miss - calling createRebound() at basket");
            createRebound(targetX, targetY);
        }
    }
}
