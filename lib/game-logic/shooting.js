...
function attemptShot() {
    ...
    // Wave 23: Delegate to shooting system if available
    if (typeof globalThis !== 'undefined' && globalThis.shootingSystem) {
        var result = globalThis.shootingSystem.attemptShot(player);
        return result;
    }

    // Legacy path (fallback if system not initialized)
    // Wave 22B: Prevent multiple simultaneous shot attempts
    // Check shotInProgress FIRST (set immediately when shot starts)
    if (gameState.shotInProgress) {
        return;
    }

    // Also check phase state (defensive check in case shotInProgress not set yet)
    if (gameState.phase && gameState.phase.current) {
        ...
        if (currentPhase === PHASE_SHOT_QUEUED ||
            ...
            // Shot already in progress - ignore duplicate attempt
            return;
        }
    }
    ...
    var opposingTeamName = gameState.currentTeam === "teamA" ? "teamB" : "teamA";
    var opposingTeamPlayers = getTeamSprites(opposingTeamName);
    var closestDefender = getClosestPlayer(player.x, player.y, opposingTeamPlayers);
    if (!closestDefender) {
        closestDefender = {
    ...
    if (attemptType === "dunk") {
        ...
        // Wave 22B: Use non-blocking phase system for dunk animation
        debugLog("[DUNK PATH] About to setPhase for dunk, made=" + made);

        // CRITICAL: Set shotInProgress BEFORE setPhase to prevent race conditions
        gameState.shotInProgress = true;

        // Queue the dunk animation and let game loop handle it
        // CRITICAL: Capture shooterTeam NOW, not later when phase runs
        setPhase(PHASE_SHOT_QUEUED, {
            shooter: player,
            shooterTeam: shooterTeam,  // Capture shooting team for possession logic
            shotStartX: shotStartX,
            shotStartY: shotStartY,
            targetX: targetX,
            targetY: targetY,
            made: made,
            blocked: false,  // Will be determined during animation
            is3Pointer: false,  // Dunks are never 3-pointers
            attemptType: "dunk",
            dunkInfo: dunkInfo,
            animDuration: 900,
            reboundBounces: []
        ...
        debugLog("[DUNK PATH] setPhase completed, returning early");

        // Early return - phase handler will manage the dunk lifecycle
        return;
    } else {
        ...
        // Wave 22B: Use non-blocking phase system for shot animation
        // CRITICAL: Set shotInProgress BEFORE setPhase to prevent race conditions
        gameState.shotInProgress = true;

        // Queue the shot animation and let game loop handle it
        // CRITICAL: Capture shooterTeam NOW for correct possession switching
        setPhase(PHASE_SHOT_QUEUED, {
            shooter: player,
            shooterTeam: shooterTeam,  // Capture shooting team for possession logic
            shotStartX: shotStartX,
        ...
        // Early return - phase handler will manage the shot lifecycle
        return;
    }
}
