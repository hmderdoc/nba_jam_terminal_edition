...
/**
 * Attempt steal - human player (teamAPlayer1) steals from ball carrier
 * 
 * Success factors:
...
function attemptSteal() {
    ...
    if (distance < 6) {
        ...
        if (Math.random() < chance) {
            ...
            gameState.ballHandlerLastX = teamAPlayer1.x;
            gameState.ballHandlerLastY = teamAPlayer1.y;
            gameState.ballHandlerFrontcourtStartX = teamAPlayer1.x;
            gameState.ballHandlerProgressOwner = teamAPlayer1;
            teamAPlayer1.playerData.hasDribble = true;
...
function attemptUserSteal(defender) {
    ...
    if (Math.random() < chance) {
        // Steal successful!
        recordTurnover(ballCarrier, "steal");
        var defenderTeam = (defender === teamAPlayer1 || defender === teamAPlayer2) ? "teamA" : "teamB";
