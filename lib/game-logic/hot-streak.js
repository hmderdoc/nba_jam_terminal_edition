 ...
 * Respects fire immunity buff - players with immunity keep their fire
 */
function clearTeamOnFire(teamKey, systems) {
    ...
    var anyStillOnFire = false;
    
    for (var i = 0; i < sprites.length; i++) {
        var sprite = sprites[i];
        if (sprite && sprite.playerData) {
            // Check for fire immunity or permanent fire buff
            var hasImmunity = false;
            if (typeof BuffSystem !== "undefined") {
                hasImmunity = (BuffSystem.hasFireImmunity && BuffSystem.hasFireImmunity(sprite)) ||
                              (BuffSystem.hasPermanentFire && BuffSystem.hasPermanentFire(sprite));
            }
            
            if (hasImmunity) {
                // Keep fire active for immune players
                anyStillOnFire = true;
            } else {
