...
function createJumpBallSystem(deps) {
    ...
    function normalizeTurbo(value, capacity) {
        var cap = (typeof capacity === "number" && capacity > 0) ? capacity : MAX_TURBO;
        var turbo = (typeof value === "number") ? value : cap;
        if (turbo < 0) turbo = 0;
        if (turbo > cap) turbo = cap;
        return cap > 0 ? turbo / cap : 0;
    }
    ...
    function selectPrimaryJumper(teamKey) {
        ...
        for (var i = 0; i < sprites.length; i++) {
            ...
            var turboNorm = normalizeTurbo(playerData.turbo, playerData.turboCapacity);
