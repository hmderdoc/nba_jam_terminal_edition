...
function resetDeadDribbleTimer(systems) {
    ...
    } else if (ballCarrier && ballCarrier.playerData) {
        ballCarrier.playerData.hasDribble = true;
    }
}
...
function isBallHandlerDribbleDead(systems) {
    var stateManager = systems.stateManager;
    var handler = stateManager.get('ballCarrier');
...
function getBallHandlerDeadElapsed(systems) {
    ...
    if (!ballHandlerDeadSince) return 0;
    var now = getTimeMs();
    return Math.max(0, now - ballHandlerDeadSince);
}
