...
function createLooseBall(defender, victim, systems) {
    ...
    if (!skipAnimation) {
        ...
        for (var i = 0; i <= steps; i++) {
            ...
            drawScore(systems);  // Update score display to show shoved visual effect
            mswait(25);
        }
    }
    ...
    createRebound(endX, endY, systems);
    
    // Mark this as a loose ball, not an actual rebound
    var reboundScramble = stateManager.get('reboundScramble');
    reboundScramble.isLooseBall = true;
 ...
 */
function evaluateOffensiveShoveOpportunity(player, teamName, systems) {
    var stateManager = systems.stateManager;
    ...
    if (!isBallHandler && teammate === ballCarrier && teammate.playerData) {
        var teammateDefender = getClosestPlayer(teammate.x, teammate.y, getOpposingTeam(teamName));
        if (teammateDefender) {
            ...
            var teammateStruggling = teammateDefDist < 3.5 || ballHandlerStuckTimer >= 2;

            if (teammateStruggling && distToTeammateDefender <= 4.0) {
                var score = 60; // Base 60% priority
                if (ballHandlerStuckTimer >= 3) score += 15; // Bonus for very stuck
    ...
    if (!isBallHandler && ballCarrier) {
        ...
        if (myDefender) {
            ...
            if (defenderBlockingLane && distToMyDef <= 4.0) {
                ...
                if (shotClock <= 10) score += 10; // Urgency bonus
                if (score > bestScore) {
                    bestScore = score;
    ...
    if (reboundActive && reboundX && reboundY) {
 ...
 * Returns: { target, score, reason } or null
 */
function evaluateDefensiveShoveOpportunity(player, teamName, systems) {
    var stateManager = systems.stateManager;
    if (!player || !player.playerData) return null;
    if (player.playerData.shoveAttemptCooldown > 0) return null;
    ...
    for (var i = 0; i < opponents.length; i++) {
        ...
        var myBasket = teamName === "teamA" ? BASKET_LEFT_X : BASKET_RIGHT_X;
    ...
    var reboundActive = stateManager.get('reboundActive');
    var reboundXForDef = stateManager.get('reboundX');
    var reboundYForDef = stateManager.get('reboundY');
    if (reboundActive && reboundXForDef && reboundYForDef) {
        for (var j = 0; j < opponents.length; j++) {
            var opp2 = opponents[j];
            if (!opp2) continue;

            var oppDistToRebound = distanceBetweenPoints(opp2.x, opp2.y, reboundXForDef, reboundYForDef);
            var myDistToRebound = distanceBetweenPoints(player.x, player.y, reboundXForDef, reboundYForDef);
...
/**
 * Trigger defensive rotation after successful shove
 * Help defender rotates to cover ball carrier
 * If both defenders shoved, switch to zone defense temporarily
 */
function triggerDefensiveRotation(shovedDefender, defensiveTeam, systems) {
    var stateManager = systems.stateManager;
    ...
    if (rotationTarget) {
        ...
        var defensiveAssignments = stateManager.get('defensiveAssignments');
        if (!defensiveAssignments) {
            stateManager.set("defensiveAssignments", {}, "defensive_rotation_init");
            defensiveAssignments = stateManager.get('defensiveAssignments');
        }
        defensiveAssignments[helpDefenderKey] = rotationTarget;
...
function attemptShove(defender, targetOverride, systems) {
    ...
    if (distance > GAME_BALANCE.PHYSICAL.SHOVE_MAX_DISTANCE) return;  // Increased from 2.5 to 4.0 for more frequent shoves

    // Ball-handler shake limitation: once per possession OR 2+ seconds of dead dribble
    var currentBallCarrier = stateManager.get('ballCarrier');
    if (victim === currentBallCarrier && defender === currentBallCarrier) {
    ...
    if (rng < successChance) {
        ...
        var ballCarrier = stateManager.get('ballCarrier');
        if (victim === ballCarrier && defender === ballCarrier) {
        ...
        if (victimTeam !== defenderTeam && victimTeam === currentTeam) {
            // Offensive player shoved a defender - trigger rotation
            triggerDefensiveRotation(victim, victimTeam, systems);
        }
        ...
        // CRITICAL: If victim is ball carrier with dead dribble, flag for decision evaluation
        // Shove failed but carrier still needs to make a choice (shot or pass)
        if (victim === ballCarrier && victim.playerData.hasDribble === false && systems && systems.stateManager) {
            systems.stateManager.set("ballCarrierNeedsDecision", true, "shove_failed_decision_trigger");
            systems.stateManager.set("ballCarrierDecisionTime", Date.now(), "decision_timestamp");
        }
