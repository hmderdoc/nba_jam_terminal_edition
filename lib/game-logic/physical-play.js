...
function createLooseBall(defender, victim, systems) {
    ...
    stateManager.set("reboundActive", true, "loose_ball_created");
    stateManager.set("shotInProgress", false, "loose_ball_created");
    stateManager.set("ballCarrier", null, "loose_ball_created");
    stateManager.set("reboundX", endX, "loose_ball_position");
    stateManager.set("reboundY", endY, "loose_ball_position");
    clearPotentialAssist(systems);
    // Note: "shove" announcement already fired in attemptShove(), so skip "loose_ball" here
    resolveReboundScramble(systems);
}
