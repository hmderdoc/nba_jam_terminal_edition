...
function attemptShake(player, systems) {
    ...
    for (var i = 0; i < touching.length; i++) {
        ...
        knockBack(defender, player, push, systems);
...
function attemptShove(defender, targetOverride, systems) {
    ...
    var rbSystem = systems && systems.rubberBandingSystem;
    var rbModifiers = (rbSystem && typeof rbSystem.getTeamModifiers === "function")
        ? rbSystem.getTeamModifiers(defenderTeam)
        : null;
    
    // Check if victim has shove immunity buff
    if (typeof BuffSystem !== "undefined" && BuffSystem.isShoveImmune && BuffSystem.isShoveImmune(victim)) {
        defender.playerData.shoveAttemptCooldown = 30;
        return false;  // Shove fails against immune players
    }
    ...
    var baseMinChance = 0.15;
    var baseMaxChance = 0.75;
    var shoveBonus = (rbModifiers && typeof rbModifiers.shoveBonus === "number") ? rbModifiers.shoveBonus : 0;
    
    // Add buff-based shove bonus for the defender
    if (typeof BuffSystem !== "undefined" && BuffSystem.getShoveBonus) {
        shoveBonus += BuffSystem.getShoveBonus(defender);
    }
    ...
    if (shoveBonus !== 0) {
        successChance += shoveBonus;
        baseMaxChance += shoveBonus;
    }
    if (baseMaxChance > 0.95) baseMaxChance = 0.95;
    if (baseMaxChance < baseMinChance) baseMaxChance = baseMinChance;

    successChance = clamp(successChance, baseMinChance, baseMaxChance);
    ...
    if (Math.random() < successChance) {
        ...
        knockBack(victim, defender, push, systems);
