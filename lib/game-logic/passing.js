...
function executePass(passer, receiver, leadTarget) {
    ...
    if (interceptor) {
        ...
        var interceptorTeam = getPlayerTeamName(interceptor);
    ...
    if (mpCoordinator && mpCoordinator.isCoordinator) {
        ...
        // Multiplayer coordinator has already updated state, so no callback needed
        if (animationSystem) {
            animationSystem.queuePassAnimation(
                ...
                endX,
                endY,
                { interceptor: interceptor },  // Wave 22B: stateData param
                passTiming.durationMs,
                null  // Wave 22B: No callback - state already updated above
            );
        }
    }
 ...
 * Wave 23: ADAPTER - Delegates to new passing system when available
 * State changes happen in animation completion callback, not immediately
...
function animatePass(passer, receiver, leadTarget, inboundContext) {
    // Wave 23: Use new passing system if available
    if (typeof passingSystem !== 'undefined' && passingSystem) {
    ...
    // Fallback: old implementation (will be removed after full migration)
    _animatePassLegacy(passer, receiver, leadTarget, inboundContext);
}

/**
 * LEGACY implementation - Will be removed after migration complete
 * @private
 */
function _animatePassLegacy(passer, receiver, leadTarget, inboundContext) {
    ...
    if (!receiverInBounds) {
        // Receiver out of bounds - treat as turnover
        // This is immediate (not deferred) because no animation occurs
        recordTurnover(passer, "pass_oob");
        var opposingTeam = getOpposingTeamName(passer);
        gameState.currentTeam = opposingTeam;
        gameState.ballCarrier = null;
    ...
    // Wave 22B: Coordinator uses executePass (separate path)
    if (mpCoordinator && mpCoordinator.isCoordinator) {
    ...
    // Check for interception (determine outcome now, but don't mutate state yet)
    var interceptor = checkPassInterception(passer, receiver, targetPoint);
    ...
    // Wave 22B: Queue animation with ALL state data and completion callback
    if (animationSystem && typeof animationSystem.queuePassAnimation === "function") {
        if (typeof log === "function") {
            log(LOG_DEBUG, "[PASS] Queuing pass animation from (" + startX + "," + startY + ") to (" + endX + "," + endY + ") with callback");
        }
        ...
        animationSystem.queuePassAnimation(
            ...
            endY,
            {
                // Pass all data needed for state mutations
                passer: passer,
                receiver: receiver,
                interceptor: interceptor,
                targetPoint: targetPoint,
                inboundContext: inboundContext  // Include inbound-specific data if present
            },
            passTiming.durationMs,
            function (stateData) {
                // === STATE MUTATIONS HAPPEN HERE, AFTER ANIMATION COMPLETES ===
                if (typeof log === "function") {
                    log(LOG_DEBUG, "[PASS] Animation callback fired! Interceptor: " + (stateData.interceptor ? "YES" : "NO") + ", inboundContext: " + (stateData.inboundContext ? "YES" : "NO"));
                }
                ...
                if (stateData.interceptor) {
                    // === INTERCEPTION PATH ===
                    if (typeof recordTurnover === "function") {
                        recordTurnover(stateData.passer, "steal_pass");
                    }

                    gameState.ballCarrier = stateData.interceptor;
                    var interceptorTeam = getPlayerTeamName(stateData.interceptor);
                    gameState.currentTeam = interceptorTeam;
                    ...
                    gameState.ballHandlerLastX = stateData.interceptor.x;
                    gameState.ballHandlerLastY = stateData.interceptor.y;
                    gameState.ballHandlerFrontcourtStartX = stateData.interceptor.x;
                    gameState.ballHandlerProgressOwner = stateData.interceptor;

                    if (stateData.interceptor.playerData && stateData.interceptor.playerData.stats) {
                        stateData.interceptor.playerData.stats.steals++;
                    }

                    if (stateData.interceptor.playerData) {
                        stateData.interceptor.playerData.hasDribble = true;
                        if (typeof announceEvent === "function") {
                            announceEvent("steal", {
                                playerName: stateData.interceptor.playerData.name,
                                player: stateData.interceptor,
                                team: interceptorTeam
                            });
                        }
                    }

                    if (stateData.receiver && stateData.receiver.playerData) {
                        stateData.receiver.playerData.hasDribble = true;
                    }
                    if (stateData.passer && stateData.passer.playerData) {
                        stateData.passer.playerData.hasDribble = false;
                    }
                } else {
                    // === SUCCESSFUL PASS PATH ===
                    gameState.ballCarrier = stateData.receiver;
                    gameState.ballHandlerAdvanceTimer = 0;
                    gameState.ballHandlerFrontcourtStartX = stateData.receiver.x;
                    gameState.ballHandlerProgressOwner = stateData.receiver;
                    ...
                    if (typeof setPotentialAssist === "function") {
                        setPotentialAssist(stateData.passer, stateData.receiver);
                    }

                    // === INBOUND-SPECIFIC POST-COMPLETION LOGIC ===
                    if (stateData.inboundContext) {
                        ...
                        if (ctx.inbounder) {
                            ...
                            if (ctx.inbounder.playerData) {
                                ctx.inbounder.playerData.hasDribble = true;
                            }
                        }

                        // Clear inbounding flag now that pass is complete
                        gameState.inbounding = false;
                        gameState.inboundPasser = null;

                        // Reset shot clock and court state after inbound
                        gameState.shotClock = 24;
                        ...
                        // Enable score flash regain check
                        if (typeof enableScoreFlashRegainCheck === "function") {
                            enableScoreFlashRegainCheck(ctx.team);
                        }

                        // Set up offense formation
                        if (typeof primeInboundOffense === "function") {
                            primeInboundOffense(ctx.team);
                        }

                        // Set up defensive matchups
                        if (typeof assignDefensiveMatchups === "function") {
                            assignDefensiveMatchups();
                        }

                        // Announce inbound complete
                        if (typeof announceEvent === "function") {
                            announceEvent("inbounds", { team: ctx.team });
                        }
                    }
                }
                ...
                if (typeof resetDeadDribbleTimer === "function") {
                    resetDeadDribbleTimer();
                }
            }
        );
    } else {
        // FALLBACK: Animation system not available - do immediate state mutations
        // This ensures the game doesn't freeze when animations can't be queued
        ...
        if (interceptor) {
            ...
            if (inboundContext) {
                ...
                gameState.shotClock = 24;
                gameState.frontcourtEstablished = false;
                gameState.inboundGracePeriod = 30;
    ...
    // Wave 22B: State mutations handled either in callback (if animation queued) or immediately above (fallback)
}
...
/**
 * Entry point for human player pass
