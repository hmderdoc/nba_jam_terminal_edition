 ...
 */
function handleActionButton(player, systems) {
    ...
    if (currentTeam === playerTeam && ballCarrier === player) {
        ...
        attemptBlock(player, { direction: defenderDir }, systems);
    }
}
...
function handleSecondaryButton(player, systems) {
    ...
    if (currentTeam === playerTeam) {
        ...
        if (ballCarrier === player && teammate) {
            ...
            animatePass(teammate, player, null, null, systems);
        }
    } else {
...
function handleInput(key, systems) {
    ...
    if (keyUpper === 'S') {
        ...
        if (currentTeam === "teamA" && ballCarrier === teamAPlayer1) {
            ...
            animatePass(teamAPlayer1, teamAPlayer2, null, null, systems);
        } else if (currentTeam === "teamA" && ballCarrier === teamAPlayer2) {
            // Teammate has ball - command them to pass back
            animatePass(teamAPlayer2, teamAPlayer1, null, null, systems);
    ...
    if (teamAPlayer1 && isArrowKey) {
        ...
        if (budget.moves > 0) {
            ...
            var pressedKeys = stateManager.get('pressedKeys');
            if (!pressedKeys) {
                pressedKeys = {};
                stateManager.set("pressedKeys", pressedKeys, "input_init");
            }
            pressedKeys[key] = now;

            // Clean up old key presses (keys released over 100ms ago)
            for (var k in pressedKeys) {
                if (pressedKeys.hasOwnProperty(k)) {
                    if (now - pressedKeys[k] > 100) {
            ...
            var hasHorizontal = pressedKeys[KEY_LEFT] || pressedKeys[KEY_RIGHT];
            var hasVertical = pressedKeys[KEY_UP] || pressedKeys[KEY_DOWN];
            ...
            if (isDiagonal) {
                ...
                var horizKey = pressedKeys[KEY_LEFT] ? KEY_LEFT : KEY_RIGHT;
                var vertKey = pressedKeys[KEY_UP] ? KEY_UP : KEY_DOWN;
