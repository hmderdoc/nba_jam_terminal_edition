...
/**
 * PHASE: INBOUND_SETUP
 * Players move to inbound positions, then pass queues
 * Wave 23D: Extended to 4000ms (80 frames) for natural movement with easing and variation - doubled for better pacing
 */
function handleInboundSetup(frameDelayMs, systems) {
    ...
    // Animate player positioning (frames 0-69 of 80 total for natural movement - doubled duration)
    var positioning = stateManager.get("inboundPositioning");
    if (positioning && phase.frameCounter < 70) {
        var rawProgress = phase.frameCounter / 69; // 0.0 to 1.0

        // Ease-in-out function for natural acceleration/deceleration
        // Uses sine curve for smooth start and stop
        ...
        if (phase.frameCounter % 10 === 0) {
            debugLog("[INBOUND POSITIONING] frame " + phase.frameCounter + "/70, easeProgress=" + easeProgress.toFixed(2));
        }
        return;
    }

    // Frames 70-75: Pause with everyone in position (6 frames = 300ms pause - doubled)
    if (phase.frameCounter >= 70 && phase.frameCounter < 76) {
        phase.frameCounter++;
        if (phase.frameCounter === 73) {
            debugLog("[INBOUND] Players positioned, pausing before pass");
        }
        return;
    }

    // Frame 76: Queue the inbound pass
    if (phase.frameCounter === 76 && !phaseData.passQueued) {
        var passData = stateManager.get("inboundPassData");
        if (passData && passData.inbounder && passData.receiver) {
            debugLog("[INBOUND] Queuing pass animation at frame 76");
            var inboundContext = {
    ...
    // Frames 76+: Wait for pass animation
