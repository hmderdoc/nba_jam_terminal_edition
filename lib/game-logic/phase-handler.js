...
function updateGamePhase(frameDelayMs, systems) {
    ...
    switch (currentPhase) {
        ...
        case PHASE_INBOUND_SETUP:
            ...
            case PHASE_OVERTIME_INTRO:
                handleOvertimeIntro(frameDelayMs, systems);
                break;
    }
}
...
function handleShotAnimating(frameDelayMs, systems) {
    ...
    if (!systems.animationSystem || !systems.animationSystem.isBallAnimating()) {
        ...
        // Check if shot was blocked during animation (set by animation-system.js)
        var wasBlocked = stateManager.get("shotBlocked") === true;
        if (wasBlocked) {
            phaseData.blocked = true;
            phaseData.made = false;
            stateManager.set("shotBlocked", false, "block_consumed");
            debugLog(">>> PHASE: Shot was blocked during animation");
        }
...
function handleShotScored(frameDelayMs, systems) {
    ...
    // On the first frame of the phase, execute all the scoring logic.
    if (phase.frameCounter === 0) {
        debugLog("[SHOT_SCORED] Initializing score event.");
        var player = phaseData.shooter;
        var playerData = player ? player.playerData : null;

        if (!playerData) {
                setPhase(PHASE_INBOUND_SETUP, phaseData, INBOUND_SETUP_DURATION_MS, frameDelayMs, systems);
            return;
        }

        var scoringTeamKey = phaseData.shooterTeam || stateManager.get('currentTeam');
        var inboundTeamKey = (scoringTeamKey === "teamA") ? "teamB" : "teamA";
        debugLog("[SHOT_SCORED] scoringTeamKey=" + scoringTeamKey + ", inboundTeamKey=" + inboundTeamKey);
        ...
        if (player) player.hasBall = false;
        debugLog("[SHOT_SCORED] Cleared ballCarrier.");
        ...
        recordStatDelta(player, "points", points, systems);
        recordStatDelta(player, "fgm", 1, systems, { enableTrail: false });
        if (phaseData.is3Pointer) {
            recordStatDelta(player, "tpm", 1, systems, { enableTrail: false });
        }
        if (phaseData.attemptType === "dunk") {
            recordStatDelta(player, "dunks", 1, systems, { enableTrail: false });
        }

        if (systems.statTrailSystem && typeof systems.statTrailSystem.queueStatTrail === "function") {
            var statLabel = phaseData.attemptType === "dunk" ? "JAM" : "PTS";
            var statText = "+" + points + " " + statLabel;
            systems.statTrailSystem.queueStatTrail({
                text: statText,
                player: player,
                teamKey: scoringTeamKey,
                statType: phaseData.attemptType === "dunk" ? "dunks" : "points"
            });
        }

        if (typeof maybeAwardAssist === "function") maybeAwardAssist(player, systems);
        if (typeof clearPotentialAssist === "function") clearPotentialAssist(systems);

        // Refill turbo, announce events, check for "on fire", etc.
        ...
        for (var i = 0; i < scoringSprites.length; i++) {
            if (scoringSprites[i] && scoringSprites[i].playerData) {
                var scoringData = scoringSprites[i].playerData;
                var scoringCapacity = (typeof scoringData.turboCapacity === "number" && scoringData.turboCapacity > 0)
                    ? scoringData.turboCapacity
                    : MAX_TURBO;
                scoringData.turbo = scoringCapacity;
            }
        }
        ...
        for (var j = 0; j < inboundSprites.length; j++) {
            if (inboundSprites[j] && inboundSprites[j].playerData) {
                var inboundData = inboundSprites[j].playerData;
                var inboundCapacity = (typeof inboundData.turboCapacity === "number" && inboundData.turboCapacity > 0)
                    ? inboundData.turboCapacity
                    : MAX_TURBO;
                inboundData.turbo = inboundCapacity;
            }
        }

        if (phaseData.attemptType === "dunk") announceEvent("dunk", { playerName: playerData.name, player: player, team: scoringTeamKey, style: phaseData.dunkStyle || null }, systems);
        else if (phaseData.is3Pointer) announceEvent("three_pointer", { playerName: playerData.name, player: player, team: scoringTeamKey }, systems);
        else announceEvent("shot_made", { playerName: playerData.name, player: player, team: scoringTeamKey }, systems);

        if (playerData.fireMakeStreak >= 3 && typeof setPlayerOnFire === "function") {
            setPlayerOnFire(player, systems);
            announceEvent("on_fire", { playerName: playerData.name, player: player, team: scoringTeamKey }, systems);
        }

        stateManager.set("consecutivePoints." + inboundTeamKey, 0, "opponent_scored_reset");
        if (typeof clearTeamOnFire === "function") clearTeamOnFire(inboundTeamKey, systems);

        if (typeof triggerPossessionBeep === "function") triggerPossessionBeep(systems);
        if (typeof startScoreFlash === "function") startScoreFlash(scoringTeamKey, inboundTeamKey, systems);
        if (typeof drawScore === "function") drawScore(systems);
    ...
    if (advancePhaseTimer(systems)) {
        debugLog("[SHOT_SCORED HANDLER] Phase complete, transitioning to INBOUND_SETUP");
        ...
        var transitionData = getPhaseData(systems);
            setPhase(PHASE_INBOUND_SETUP, transitionData, INBOUND_SETUP_DURATION_MS, frameDelayMs, systems);
    }
}

function handleOvertimeIntro(frameDelayMs, systems) {
    var stateManager = systems.stateManager;
    var phaseData = getPhaseData(systems) || {};
    var endsAt = stateManager.get("overtimeIntroEndsAt");
    var now = Date.now();

    if (typeof renderOvertimeIntro === "function") {
        renderOvertimeIntro(systems, phaseData);
    }

    if (typeof endsAt === "number") {
        var remainingMs = endsAt - now;
        if (remainingMs <= 0) {
            stateManager.set("overtimeIntroRemainingSeconds", 0, "overtime_intro_countdown_complete");
            if (typeof finalizePendingOvertimeIntro === "function") {
        ...
        stateManager.set("overtimeIntroRemainingSeconds", remainingSeconds, "overtime_intro_countdown_tick");
    } else {
        if (typeof finalizePendingOvertimeIntro === "function") {
            finalizePendingOvertimeIntro(systems);
        }
...
/**
 * Transition to inbound setup phase after a violation
 * Called by violations.js when a violation is enforced
 * 
 * @param {string} inboundTeam - Team that will inbound the ball
 * @param {Object} systems - Systems object
 */
function transitionToViolationInbound(inboundTeam, systems) {
...
function handleInboundSetup(frameDelayMs, systems) {
    ...
    // UNLESS this is a violation inbound (already configured by setupViolationInbound)
    if (phase.frameCounter === 0) {
        var isViolation = phaseData.isViolation;

        if (isViolation) {
            debugLog("[INBOUND] Violation inbound - skipping setupInbound() call, using pre-configured positions");
        } else {
            ...
            if (typeof setupInbound === "function") {
                setupInbound(scoringTeamKey, systems, phaseData.reason);
            }
    ...
    if (advancePhaseTimer(systems) || (phaseData.passQueued && !isBallAnimating)) {
        ...
        // HARD RESET of client prediction state right before returning to normal play
        if (systems.playerClient && typeof systems.playerClient.resetPredictionState === 'function') {
            debugLog("[INBOUND] Performing final prediction state reset before returning to NORMAL.");
            systems.playerClient.resetPredictionState("phase_inbound_complete");
        }

        stateManager.set("inboundPositioning", null, "cleanup");
        stateManager.set("inboundPassData", null, "cleanup");
        if (phaseData && phaseData.reason === "second_half_start") {
            stateManager.set("secondHalfPositioningOverride", null, "second_half_positioning_cleared");
        }
        setPhase(PHASE_NORMAL, {}, 0, null, systems);
    }
}
