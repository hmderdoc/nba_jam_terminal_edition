...
function handleShotQueued(systems) {
    ...
    if (phaseData.attemptType === "dunk") {
        ...
        // Fallback to old blocking code if animation system not available
        debugLog(">>> PHASE: Falling back to blocking dunk animation");
        var dunkResult = animateDunk(player, dunkInfo, phaseData.targetX, phaseData.targetY, phaseData.made, systems);
...
function handleShotScored(frameDelayMs, systems) {
    ...
    if (phase.frameCounter === 0) {
        ...
        var currentScore = stateManager.get("score." + currentTeam);
        stateManager.set("score." + currentTeam, currentScore + points, "basket_scored");

        var currentConsecutive = stateManager.get("consecutivePoints." + currentTeam);
        stateManager.set("consecutivePoints." + currentTeam, currentConsecutive + 1, "consecutive_increment");
        ...
        // currentTeam may have changed between shot attempt and scoring
...
function handleShotMissed(frameDelayMs, systems) {
    ...
    if (phase.frameCounter === 0) {
        ...
        if (playerData) {
            ...
            consecutivePoints[currentTeam] = 0;
