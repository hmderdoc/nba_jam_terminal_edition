...
/**
 * Translate numpad key to movement info.
 * Used for both single-player input-handler and multiplayer client/coordinator.
 * 
 * Numpad layout:
 *   7 8 9   (7=up-left, 8=up, 9=up-right)
 *   4 5 6   (4=left, 5=center, 6=right)
 *   1 2 3   (1=down-left, 2=down, 3=down-right)
 * 
 * @param {string} key - The raw key pressed
 * @returns {Object} Translation result:
 *   - isDiagonal: true if this is a diagonal key (1,3,7,9)
 *   - isCardinal: true if this is a cardinal numpad key (2,4,6,8)
 *   - horizKey: KEY_LEFT or KEY_RIGHT for diagonals
 *   - vertKey: KEY_UP or KEY_DOWN for diagonals
 *   - cardinalKey: KEY_* constant for cardinal numpad keys
 *   - effectiveKey: The key to use for movement (cardinalKey or original key)
 */
function translateNumpadKey(key) {
    var result = {
        isDiagonal: false,
        isCardinal: false,
        horizKey: null,
    ...
    if (key === '7') {
        ...
        result.horizKey = KEY_LEFT;
        result.vertKey = KEY_UP;
    } else if (key === '9') {
        result.isDiagonal = true;
        result.horizKey = KEY_RIGHT;
        result.vertKey = KEY_UP;
    } else if (key === '1') {
        result.isDiagonal = true;
        result.horizKey = KEY_LEFT;
        result.vertKey = KEY_DOWN;
    } else if (key === '3') {
        result.isDiagonal = true;
        result.horizKey = KEY_RIGHT;
        result.vertKey = KEY_DOWN;
    ...
    // Cardinal numpad keys (map to arrow keys)
        ...
        result.isCardinal = true;
        result.cardinalKey = KEY_UP;
        result.effectiveKey = KEY_UP;
    } else if (key === '2') {
        result.isCardinal = true;
        result.cardinalKey = KEY_DOWN;
        result.effectiveKey = KEY_DOWN;
    } else if (key === '4') {
        result.isCardinal = true;
        result.cardinalKey = KEY_LEFT;
        result.effectiveKey = KEY_LEFT;
    } else if (key === '6') {
        result.isCardinal = true;
        result.cardinalKey = KEY_RIGHT;
        result.effectiveKey = KEY_RIGHT;
    }
...
/**
 * Convert movement delta to bearing string
 * Used to update sprite facing direction after diagonal movement
 * 
 * @param {number} dx - Horizontal delta (-1, 0, or 1)
 * @param {number} dy - Vertical delta (-1, 0, or 1)
 * @returns {string|null} Bearing string (n, ne, e, se, s, sw, w, nw) or null if no movement
 */
function getBearingFromDelta(dx, dy) {
...
function applyDiagonalMovement(sprite, horizontalKey, verticalKey, counters) {
    ...
    // Update sprite bearing to face the direction of movement
    var newBearing = getBearingFromDelta(dx, dy);
    if (newBearing) {
        sprite.bearing = newBearing;
    }
