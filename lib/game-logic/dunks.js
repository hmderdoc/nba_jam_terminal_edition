...
debugLog("[DUNKS.JS] Loaded version 1.1 - constants.js and game-balance.js loaded");
debugLog("[DUNKS.JS] DUNK_DISTANCE_BASE=" + (typeof DUNK_DISTANCE_BASE !== "undefined" ? DUNK_DISTANCE_BASE : "UNDEFINED"));
debugLog("[DUNKS.JS] GAME_BALANCE.DUNKS=" + (typeof GAME_BALANCE !== "undefined" && GAME_BALANCE.DUNKS ? "defined" : "UNDEFINED"));
...
function calculateDunkChance(playerData, dunkInfo, closestDefender, teamName) {
    ...
    debugLog("[DUNK CALC] Initial baseChance: " + baseChance + ", effectiveSkill: " + effectiveDunkSkill +
        ", rawSkill: " + rawDunkSkill + ", dunkRange: " + (dunkInfo ? dunkInfo.dunkRange : "undefined"));

    if (!playerData.onFire && rawDunkSkill <= 2) {
        baseChance -= (3 - rawDunkSkill) * 6;
    }
    debugLog("[DUNK CALC] After skill penalty, baseChance: " + baseChance);

    if (dunkInfo.adjustedDistance < GAME_BALANCE.DUNKS.CLOSE_DUNK_DISTANCE) {
        debugLog("[DUNK CALC] Close dunk bonus: adjustedDistance=" + dunkInfo.adjustedDistance + " < CLOSE_DUNK_DISTANCE=" + GAME_BALANCE.DUNKS.CLOSE_DUNK_DISTANCE);
        baseChance += 6;
    }
    debugLog("[DUNK CALC] After close bonus, baseChance: " + baseChance);

    if (!dunkInfo.insideKey) baseChance -= 4;
    debugLog("[DUNK CALC] After insideKey check, baseChance: " + baseChance);

    if (dunkInfo.adjustedDistance > (dunkInfo.dunkRange - 0.5)) {
        var penalty = (dunkInfo.adjustedDistance - (dunkInfo.dunkRange - 0.5)) * 8;
        debugLog("[DUNK CALC] Distance penalty: " + penalty + " (distance: " + dunkInfo.adjustedDistance + ", range: " + dunkInfo.dunkRange + ")");
        baseChance -= penalty;
    }
    debugLog("[DUNK CALC] After distance penalty, baseChance: " + baseChance);
    ...
    if (closestDefender && typeof closestDefender.x === "number" && typeof closestDefender.y === "number") {
        ...
        debugLog("[DUNK CALC] Defender coords: x=" + defenderCenterX + ", y=" + defenderCenterY +
            ", dunkInfo.centerX=" + dunkInfo.centerX + ", dunkInfo.centerY=" + dunkInfo.centerY);
        ...
        debugLog("[DUNK CALC] Defender penalty: " + defensePenalty + " (separation: " + separation + ", blockAttr: " + blockAttr + ")");
        baseChance -= defensePenalty;
    }
    debugLog("[DUNK CALC] After defender penalty, baseChance: " + baseChance);

    baseChance += (playerData.heatStreak || 0) * 4;
    debugLog("[DUNK CALC] After heat streak, baseChance: " + baseChance);
    if (playerData && playerData.onFire) {
        baseChance += 10;
    }
    debugLog("[DUNK CALC] After onFire bonus, baseChance: " + baseChance);

    if (baseChance < 25) baseChance = 25;
    if (baseChance > 98) baseChance = 98;

    debugLog("[DUNK CHANCE] Player: " + (playerData ? playerData.name : "unknown") +
        ", Base: " + baseChance + "%, Skill: " + effectiveDunkSkill +
        ", Distance: " + (dunkInfo ? dunkInfo.adjustedDistance.toFixed(1) : "?") +
        ", InKey: " + (dunkInfo ? dunkInfo.insideKey : "?"));
...
function animateDunk(player, dunkInfo, targetX, targetY, made) {
    ...
    if (!made) {
        debugLog("[DUNK] Dunk missed - clearing ballCarrier");
        gameState.ballCarrier = null;
    } else {
        debugLog("[DUNK] Dunk made - ballCarrier remains: " + (gameState.ballCarrier ? gameState.ballCarrier.playerData.name : "null"));
    }
...
function executeShot(shooter, shotStartX, shotStartY, targetX, targetY) {
    ...
    if (made && !blocked) {
        ...
        if (mpCoordinator && mpCoordinator.isCoordinator) {
            debugLog("[SHOT ANIMATION] Broadcasting shot_executed for made shot");
            ...
            debugLog("[SHOT ANIMATION] animationSystem exists: " + (typeof animationSystem !== "undefined" && animationSystem !== null));
            if (animationSystem) {
                debugLog("[SHOT ANIMATION] Queueing shot animation: startX=" + shotStartX + ", startY=" + shotStartY + ", targetX=" + targetX + ", targetY=" + targetY);
                ...
                debugLog("[SHOT ANIMATION] ERROR: animationSystem is not available!");
            }
        }
