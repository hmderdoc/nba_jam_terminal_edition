 ...
 * - animateDunk() with mswait() is DEAD CODE - shooting-system.js uses phase queue for dunks
 * - executeShot() is DEAD CODE - coordinator uses phase system
 * 
 * This file is kept for:
 * - evaluateDunkOpportunity() - used by shooting system
 * - calculateDunkChance() - used by shooting system
 * - selectDunkStyle() - used by shooting system
 * - generateDunkFlight() - used by shooting system
 * - autoContestDunk() - used by shooting system
 * - Helper functions for dunk flight calculations
 * 
 * TODO Wave 25: Extract active functions to utils, delete blocking animations
 */
 ...
 */
function animateDunk(player, dunkInfo, targetX, targetY, made, systems) {
    var stateManager = systems.stateManager;

    stateManager.set("shotInProgress", true, "dunk_started");
    stateManager.set("shotStartX", player.x, "dunk_tracking");
    stateManager.set("shotStartY", player.y, "dunk_tracking");

    var playerData = player.playerData || {};
    var style = selectDunkStyle(playerData, dunkInfo);
    var flightPlan = generateDunkFlight(player, dunkInfo, targetX, targetY, style);
    autoContestDunk(player, dunkInfo, targetX, targetY, style, systems);
    ...
    for (var i = 0; i < flightPlan.frames.length; i++) {
        ...
        var flashPalette = getDunkFlashPalette(player, systems);
        var flashText = getDunkLabelText(style, tickCounter + i);
        renderPlayerLabel(player, {
            highlightCarrier: false,
            forcedText: flashText,
            flashPalette: flashPalette,
            flashTick: tickCounter + i,
            forceTop: true
        }, systems);
        ...
        stateManager.set("ballX", handX, "dunk_animation");
        stateManager.set("ballY", handY, "dunk_animation");
    ...
    if (blocked) {
        ...
        renderPlayerLabel(player, { highlightCarrier: true, forceTop: true }, systems);
        ...
        stateManager.set("ballX", deflectX, "dunk_blocked");
        stateManager.set("ballY", deflectY, "dunk_blocked");
        Sprite.cycle();
        cycleFrame(courtFrame);
        mswait(70);

        stateManager.set("reboundActive", true, "dunk_blocked");
        stateManager.set("ballCarrier", null, "dunk_blocked");
        stateManager.set("reboundX", deflectX, "dunk_blocked");
        stateManager.set("reboundY", deflectY, "dunk_blocked");
        stateManager.set("shotInProgress", false, "dunk_blocked");
    ...
    if (made) {
        for (var drop = 0; drop < 3; drop++) {
            ...
            stateManager.set("ballX", targetX, "dunk_made_animation");
            stateManager.set("ballY", dropY, "dunk_made_animation");
            Sprite.cycle();
            cycleFrame(courtFrame);
            mswait(45);
        }
        ...
        stateManager.set("ballX", targetX, "dunk_made_animation");
        stateManager.set("ballY", clamp(targetY + 3, 1, COURT_HEIGHT), "dunk_made_animation");
        mswait(80);
    } else {
        moveBallFrameTo(targetX, targetY);
        stateManager.set("ballX", targetX, "dunk_missed_animation");
        stateManager.set("ballY", targetY, "dunk_missed_animation");
        Sprite.cycle();
        cycleFrame(courtFrame);
        mswait(60);
        ...
        stateManager.set("ballX", ricochetX, "dunk_missed_animation");
        stateManager.set("ballY", ricochetY, "dunk_missed_animation");
        Sprite.cycle();
        cycleFrame(courtFrame);
        mswait(80);
        moveBallFrameTo(targetX, targetY + 1);
        stateManager.set("ballX", targetX, "dunk_missed_animation");
        stateManager.set("ballY", targetY + 1, "dunk_missed_animation");
    }

    renderPlayerLabel(player, { highlightCarrier: true, forceTop: true }, systems);

    stateManager.set("shotInProgress", false, "dunk_completed");

    // Clear ball carrier for missed shots so rebound scramble can start
    if (!made) {
        stateManager.set("ballCarrier", null, "dunk_missed");
    }
 ...
 */
function executeShot(shooter, shotStartX, shotStartY, targetX, targetY, systems) {
    var stateManager = systems.stateManager;
    ...
    var shooterTeam = getPlayerTeamName(shooter) || currentTeam;
    ...
    var closestDefender = getClosestPlayer(shooter.x, shooter.y, getOpposingTeam(shooterTeam));
    if (!closestDefender) {
        closestDefender = {
    ...
    reboundScramble.anticipating = true;
    reboundScramble.anticipatedX = estimatedReboundX;
    reboundScramble.anticipatedY = estimatedReboundY;
    ...
    // Update game state
    stateManager.set("shotInProgress", false, "shot_executed");
    stateManager.set("ballCarrier", null, "shot_executed");

    if (made && !blocked) {
        // Score!
        var points = is3Pointer ? 3 : 2;

        // Update score using state manager
        var score = stateManager.get('score') || { teamA: 0, teamB: 0 };
        var currentTeam = stateManager.get('currentTeam');
        score[currentTeam] = (score[currentTeam] || 0) + points;
        stateManager.set("score", score, "shot_scored");

        // Update consecutive points
        var consecutivePoints = stateManager.get('consecutivePoints') || { teamA: 0, teamB: 0 };
        consecutivePoints[currentTeam] = (consecutivePoints[currentTeam] || 0) + 1;
        stateManager.set("consecutivePoints", consecutivePoints, "shot_scored");
        ...
        maybeAwardAssist(shooter, systems);
        clearPotentialAssist(systems);
        ...
        if (!playerData.onFire && playerData.fireMakeStreak >= 3) {
            setPlayerOnFire(shooter, systems);
        ...
        var consecutivePointsReset = stateManager.get('consecutivePoints') || { teamA: 0, teamB: 0 };
        consecutivePointsReset[inboundTeam] = 0;
        stateManager.set("consecutivePoints", consecutivePointsReset, "shot_scored");
        clearTeamOnFire(inboundTeam, systems);
        var reboundScrambleReset = stateManager.get('reboundScramble');
        reboundScrambleReset.anticipating = false;

        triggerPossessionBeep(systems);
        startScoreFlash(shooterTeam, inboundTeam, systems);
        ...
        return { made: true, blocked: false, points: points, is3Pointer: is3Pointer, animDuration: shotTiming.durationMs };
    } else {
        // Miss
        var currentTeamForMiss = stateManager.get('currentTeam');
        var consecutivePointsMiss = stateManager.get('consecutivePoints') || { teamA: 0, teamB: 0 };
        consecutivePointsMiss[currentTeamForMiss] = 0;
        stateManager.set("consecutivePoints", consecutivePointsMiss, "shot_missed");
        playerData.heatStreak = 0;
        stateManager.set("reboundActive", true, "shot_missed");
        ...
        if (mpCoordinator && mpCoordinator.isCoordinator) {
            // Start rebound scramble state
            var reboundScrambleForMiss = stateManager.get('reboundScramble');
            reboundScrambleForMiss.active = true;
            reboundScrambleForMiss.startTime = Date.now();
            reboundScrambleForMiss.anticipating = false;
            ...
            stateManager.set("reboundX", reboundX, "shot_rebound_calculated");
            stateManager.set("reboundY", reboundY, "shot_rebound_calculated");
            var reboundScramble = stateManager.get('reboundScramble');
            reboundScramble.reboundX = reboundX;
            reboundScramble.reboundY = reboundY;
            stateManager.set('reboundScramble', reboundScramble, 'shot_rebound_position');
