...
function animateDunk(player, dunkInfo, targetX, targetY, made) {
    ...
    if (!made) {
        try {
            log(LOG_DEBUG, "[DUNK] Dunk missed - clearing ballCarrier");
        } catch(e) {}
        gameState.ballCarrier = null;
    } else {
        try {
            log(LOG_DEBUG, "[DUNK] Dunk made - ballCarrier remains: " + (gameState.ballCarrier ? gameState.ballCarrier.playerData.name : "null"));
        } catch(e) {}
    }
