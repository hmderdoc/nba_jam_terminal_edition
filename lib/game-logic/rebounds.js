 ...
 * 
 * Dependencies:
 * - Game state (gameState with reboundActive, reboundScramble state)
 * - Player utilities (getAllPlayers, getPlayerTeamName, getPlayerGlobalId, getEffectiveAttribute)
 * - Court utilities (distanceBetweenPoints, getSpriteDistance, clamp)
 * - Game flow (switchPossession, triggerPossessionBeep, resetBackcourtState)
 * - Stats tracking (stats.rebounds)
 * - Assist tracking (clearPotentialAssist)
 * - Physical play (attemptShove for scramble shoving)
 * - Animation system (queueReboundAnimation for non-blocking animation)
 * - Multiplayer coordinator (event broadcasting)
 * - Input (flushKeyboardBuffer on possession change)
 * - Announcements (announceEvent for rebound awards)
 * - Defense (assignDefensiveMatchups)
 * - Constants (COURT_WIDTH, COURT_HEIGHT, ATTR_POWER)
 */
...
function createRebound(x, y) {
    ...
    try {
        log(LOG_DEBUG, "[REBOUND] createRebound() called at (" + x + "," + y + "), ballCarrier: " + 
    ...
    try {
        log(LOG_DEBUG, "[REBOUND] Scramble activated - reboundX: " + gameState.reboundX + ", reboundY: " + gameState.reboundY);
    } catch (e) { }
