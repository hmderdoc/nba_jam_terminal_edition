...
function secureRebound(player, systems) {
    ...
    // Defensive rebound (change of possession): Reset backcourt, player needs to advance
    // Offensive rebound (same team): Keep frontcourt established
    ...
    if (isDefensiveRebound) {
        ...
        // Player is in frontcourt (near basket) but hasn't "established" it yet
        resetBackcourtState(systems);
        // CRITICAL: Set grace period so player can advance without over-and-back violation
        // 50 frames = 2.5 seconds at 20 FPS
        stateManager.set("inboundGracePeriod", 50, "defensive_rebound_grace");
        debugLog("[REBOUND] Defensive rebound by " + teamName + " - backcourt state reset, grace period set");
    } else {
        // Offensive rebound: Same team keeps possession, keep existing frontcourt status
        // Don't reset backcourt state - team already had established position
