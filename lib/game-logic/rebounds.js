...
/**
 * Create rebound scramble at specified location
 * Calculates bounce physics and activates non-blocking scramble state
 * 
 * @param {number} x - X coordinate of rebound
 * @param {number} y - Y coordinate of rebound
 * @param {Object} systems - Systems object
 * @param {boolean} isLooseBall - Optional: true if this is from a shove/steal, false if from missed shot
 */
function createRebound(x, y, systems, isLooseBall) {
    ...
    // Defensive: Reinitialize if null (can happen after halftime reset)
    if (!reboundScramble) {
        reboundScramble = {
            active: false,
            startTime: 0,
        ...
        stateManager.set('reboundScramble', reboundScramble, 'rebound_reinit');
    }
    ...
    // Default to actual rebound from missed shot, unless explicitly marked as loose ball
    reboundScramble.isLooseBall = (typeof isLooseBall === 'boolean') ? isLooseBall : false;

    debugLog("[REBOUND] Scramble activated - reboundX: " + finalReboundX + ", reboundY: " + finalReboundY + ", isLooseBall: " + reboundScramble.isLooseBall);
...
function updateReboundScramble(systems) {
    ...
    if (!reboundScramble || !reboundScramble.active) return;
...
function secureRebound(player, systems) {
    ...
    recordStatDelta(player, "rebounds", 1, systems);
    clearPotentialAssist(systems);

    // Clear block trail from screen (SWAT! text persists until rebound secured)
    if (typeof animationSystem !== "undefined" && animationSystem && 
        typeof animationSystem.clearBlockTrail === "function") {
        animationSystem.clearBlockTrail();
    }

    // Repaint court after scramble artifacts or rim flashes
    stateManager.set("courtNeedsRedraw", true, "rebound_possession_redraw");
    ...
    if (!reboundScramble || !reboundScramble.isLooseBall) {
        ...
        // Loose ball recovery - different announcement
