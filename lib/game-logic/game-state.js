...
var PHASE_PAUSED = "PAUSED";               // Game paused (betting/spectate quit confirm)
...
function createDefaultGameState() {
    ...
    return {
        ...
        rubberBanding: {
            enabled: !!(typeof RUBBER_BANDING_CONFIG === "object" && RUBBER_BANDING_CONFIG && RUBBER_BANDING_CONFIG.enabled === true),
            profileId: (typeof RUBBER_BANDING_DEFAULT_PROFILE === "string" && RUBBER_BANDING_PROFILES && RUBBER_BANDING_PROFILES[RUBBER_BANDING_DEFAULT_PROFILE])
                ? RUBBER_BANDING_DEFAULT_PROFILE
                : null,
            activeTierId: null,
            trailingTeamId: null,
...
function resetGameState(options, systems) {
    ...
    // Wave 24: Support custom game time from options (e.g., LORB quick matches)
    if (options && typeof options.gameTime === "number" && options.gameTime > 0) {
        stateManager.set("timeRemaining", options.gameTime, "game_reset_custom_time");
        stateManager.set("totalGameTime", options.gameTime, "game_reset_custom_time");
        stateManager.set("regulationPeriodSeconds", options.gameTime, "game_reset_custom_time");
    }

    // Wave 24: Clock speed multiplier for accelerated games (e.g., betting sims)
    var clockMultiplier = (options && typeof options.clockSpeedMultiplier === "number" && options.clockSpeedMultiplier > 0)
        ? options.clockSpeedMultiplier
        : (typeof DEFAULT_CLOCK_SPEED_MULTIPLIER === "number" ? DEFAULT_CLOCK_SPEED_MULTIPLIER : 1);
    stateManager.set("clockSpeedMultiplier", clockMultiplier, "game_reset");
    ...
    if (systems && systems.rubberBandingSystem && typeof systems.rubberBandingSystem.reset === "function") {
        systems.rubberBandingSystem.reset();
    }
    resetPhase(systems);
}
