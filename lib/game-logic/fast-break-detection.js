...
function detectFastBreak(ballCarrier, offenseTeam) {
    ...
    var isFastBreak = (defendersAhead < GAME_BALANCE.FAST_BREAK.MAX_DEFENDERS_AHEAD && closestDefenderDist > GAME_BALANCE.FAST_BREAK.MIN_DEFENDER_DISTANCE);
    ...
    } else if (closestDefenderDist > GAME_BALANCE.FAST_BREAK.WIDE_OPEN_DISTANCE) {
...
function shouldUseTurboOnFastBreak(fastBreakInfo, player) {
    ...
    if (player.playerData.turbo > GAME_BALANCE.FAST_BREAK.TURBO_THRESHOLD && fastBreakInfo.closestDefenderDistance > GAME_BALANCE.FAST_BREAK.TURBO_USE_DISTANCE) {
        return true;
    }
