...
function runGameFrame(systems, config) {
    ...
    if (authorityActive) {
        ...
        var skipPhases = ["SHOT_SCORED", "INBOUND_SETUP", "SHOT_MISSED", "REBOUND_SCRAMBLE", "JUMP_BALL"];
    ...
    if (authorityActive) {
        ...
        var phaseType = currentPhase ? currentPhase.current : "NORMAL";
        var clocksShouldRun = (phaseType !== "INBOUND_SETUP" && phaseType !== "SHOT_SCORED" && phaseType !== "JUMP_BALL");
        if (violationPauseActive) {
            clocksShouldRun = false;
        }

        var lastSecondTime = stateManager.get("lastSecondTime");
        if (clocksShouldRun && now - lastSecondTime >= GAME_CLOCK_TICK_MS) {
    ...
    if (authorityActive) {
        ...
        var skipAIPhases = ["SHOT_SCORED", "INBOUND_SETUP", "SHOT_MISSED", "JUMP_BALL"];
    ...
    if (timeSinceLastUpdate >= COURT_RENDER_THROTTLE_MS) {
        ...
        // Draw hoops on dedicated hoop frames (includes basket flash animations)
        if (typeof drawHoops === "function") {
            drawHoops(systems);
        }

        // WAVE 24 REGRESSION FIX: The court is a Frame, not a Sprite. It needs its own cycle call.
        // This must happen in the main render block to ensure the background is drawn.
        if (courtFrame && typeof cycleFrame === "function") {
            cycleFrame(courtFrame);
        }

        // Cycle hoop frames to display hoops and basket flash
        ...
        // Ensure trail overlays render above the court/background layers
        ...
        // WAVE 24 REGRESSION FIX: A cycle is needed within the throttled render block
        // to prevent a black screen. This ensures the screen is refreshed.
        Sprite.cycle();
