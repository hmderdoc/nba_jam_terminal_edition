...
function initializeSystems(deps) {
    ...
    var passingSystem = createPassingSystem({
        ...
        helpers: {
            ...
            clampSpriteFeetToCourt: helpers.clampSpriteFeetToCourt,
    ...
    var rubberBandingConfig = constants.RUBBER_BANDING_CONFIG || {};
    var rubberBandingSystem = createRubberBandingSystem({
        state: stateManager,
        events: eventBus,
        helpers: {
            announceEvent: helpers.announceEvent,
        ...
        config: {
            enabled: !!rubberBandingConfig.enabled,
            showCue: !!rubberBandingConfig.showCue,
            defaultProfile: constants.RUBBER_BANDING_DEFAULT_PROFILE || null,
            profiles: constants.RUBBER_BANDING_PROFILES || {},
            probabilityCaps: constants.RUBBER_BANDING_PROBABILITY_CAPS || {}
        },
        constants: {
            maxTurbo: (typeof constants.MAX_TURBO === 'number')
                ? constants.MAX_TURBO
                : (typeof MAX_TURBO !== 'undefined' ? MAX_TURBO : 100)
        }
    });
    ...
    var shootingSystem = createShootingSystem({
        ...
        helpers: {
            ...
            getRubberBandingModifiersForTeam: function (teamId) {
                if (!rubberBandingSystem || typeof rubberBandingSystem.getTeamModifiers !== 'function') {
                    return null;
                }
                return rubberBandingSystem.getTeamModifiers(teamId);
            }
        }
    });

    var statTrailSystem = createStatTrailSystem({
        state: stateManager,
        constants: STAT_TRAIL_CONFIG,
        helpers: {
            clamp: clamp
        }
    });
    ...
    var jumpBallSystem = createJumpBallSystem({
        ...
        helpers: {
            ...
            announce: helpers.announce || (typeof announce === "function" ? announce : null),
            announceEvent: helpers.announceEvent || (typeof announceEvent === "function" ? announceEvent : null),
            drawCourt: helpers.drawCourt || (typeof drawCourt === "function" ? drawCourt : null),
            drawScore: helpers.drawScore || (typeof drawScore === "function" ? drawScore : null),
            ensureBallFrame: (typeof ensureBallFrame === "function") ? ensureBallFrame : null,
            moveBallFrameTo: (typeof moveBallFrameTo === "function") ? moveBallFrameTo : null,
            updateJumpIndicator: (typeof updateJumpIndicator === "function") ? updateJumpIndicator : null,
            clearJumpIndicator: (typeof clearJumpIndicator === "function") ? clearJumpIndicator : null,
            setPhase: helpers.setPhase || null,
            statTrailSystem: statTrailSystem,
            broadcastMultiplayerEvent: helpers.broadcastMultiplayerEvent || function (eventType, data) {
                if (typeof mpCoordinator !== "undefined" && mpCoordinator && mpCoordinator.isCoordinator && typeof mpCoordinator.broadcastEvent === "function") {
                    mpCoordinator.broadcastEvent(eventType, data);
                }
            }
        },
        constants: {
            layout: (typeof JUMP_BALL_LAYOUT !== "undefined") ? JUMP_BALL_LAYOUT : {},
            rules: (typeof JUMP_BALL_RULES !== "undefined") ? JUMP_BALL_RULES : {},
            countdownMs: (typeof JUMP_BALL_COUNTDOWN_MS === "number") ? JUMP_BALL_COUNTDOWN_MS : 800,
            dropDurationFrames: (typeof JUMP_BALL_DROP_DURATION_FRAMES === "number") ? JUMP_BALL_DROP_DURATION_FRAMES : 24,
            dropStartY: (typeof JUMP_BALL_DROP_START_Y === "number") ? JUMP_BALL_DROP_START_Y : -4,
            contestWindowFrames: (typeof JUMP_BALL_CONTEST_WINDOW_FRAMES === "number") ? JUMP_BALL_CONTEST_WINDOW_FRAMES : 6,
            frameIntervalMs: (typeof HUD_RENDER_INTERVAL_MS === "number") ? HUD_RENDER_INTERVAL_MS : 50,
            handoffDurationMs: (typeof JUMP_BALL_HANDOFF_DURATION_MS === "number") ? JUMP_BALL_HANDOFF_DURATION_MS : 400,
            cpuOffsetMaxRatio: (typeof JUMP_BALL_CPU_OFFSET_MAX_RATIO === "number") ? JUMP_BALL_CPU_OFFSET_MAX_RATIO : 0.6,
            cpuOffsetEarlyRatio: (typeof JUMP_BALL_CPU_OFFSET_EARLY_RATIO === "number") ? JUMP_BALL_CPU_OFFSET_EARLY_RATIO : 0.3,
            jumpAnimationDurationRatio: (typeof JUMP_BALL_JUMP_ANIMATION_DURATION_RATIO === "number") ? JUMP_BALL_JUMP_ANIMATION_DURATION_RATIO : 0.6,
            jumpAnimationMinMs: (typeof JUMP_BALL_JUMP_ANIMATION_MIN_MS === "number") ? JUMP_BALL_JUMP_ANIMATION_MIN_MS : 350,
            jumpAnimationMaxMs: (typeof JUMP_BALL_JUMP_ANIMATION_MAX_MS === "number") ? JUMP_BALL_JUMP_ANIMATION_MAX_MS : 700,
            arcMinDurationMs: (typeof JUMP_BALL_ARC_MIN_DURATION_MS === "number") ? JUMP_BALL_ARC_MIN_DURATION_MS : 400
        }
    });
    ...
    systems.statTrailSystem = statTrailSystem;
    systems.jumpBallSystem = jumpBallSystem;
    systems.rubberBandingSystem = rubberBandingSystem;
    ...
    if (rubberBandingSystem && typeof rubberBandingSystem.reset === "function") {
        rubberBandingSystem.reset();
    }
