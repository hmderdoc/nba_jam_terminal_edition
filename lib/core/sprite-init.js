...
var NO_JERSEY_SKINS = ["barney", "shrek", "airbud", "sonic", "donatello", "satan", "iceman", "vader"];
...
function initSpritesFromDynamic(teamADef, teamBDef, options, systems) {
    ...
    // Check for crew/contacts - support multiple LORB config structures:
    // 1. Single-player LORB: lorbContext.hydratedCrew (array of hydrated contacts)
    // 2. Multiplayer LORB: lorbContext.challengerCrew / lorbContext.challengeeCrew (separate per team)
    // 3. Legacy: lorbContext.playerCtx.crew / lorbContext.playerCtx.contacts
    
    var teamACrewData = null;
    var teamBCrewData = null;
    
    if (lorbContext) {
        // Multiplayer LORB: separate crews per team
        if (lorbContext.challengerCrew && Array.isArray(lorbContext.challengerCrew)) {
            teamACrewData = lorbContext.challengerCrew;
            debugLog("[SPRITE-INIT] Found challengerCrew for teamA: " + teamACrewData.length + " members");
        }
        if (lorbContext.challengeeCrew && Array.isArray(lorbContext.challengeeCrew)) {
            teamBCrewData = lorbContext.challengeeCrew;
            debugLog("[SPRITE-INIT] Found challengeeCrew for teamB: " + teamBCrewData.length + " members");
        }
        
        // Single-player LORB fallbacks (only set teamA if not already set from MP)
        if (!teamACrewData) {
            if (lorbContext.hydratedCrew && Array.isArray(lorbContext.hydratedCrew) && lorbContext.hydratedCrew.length > 0) {
                teamACrewData = lorbContext.hydratedCrew;
                debugLog("[SPRITE-INIT] Using pre-hydrated crew for teamA: " + teamACrewData.length + " members");
            } else if (lorbContext.playerCtx && lorbContext.playerCtx.crew) {
                // Need to hydrate from raw crew + contacts
            ...
            } else if (lorbContext.crew && Array.isArray(lorbContext.crew)) {
    ...
    // Build teamA bench
    if (teamACrewData && Array.isArray(teamACrewData)) {
        // Pre-hydrated crew - use directly
        var activeTeammateLorbId = (redInfo2 && redInfo2.lorbId) ? redInfo2.lorbId : null;
        debugLog("[SPRITE-INIT] TeamA active teammate lorbId to exclude: " + activeTeammateLorbId);
        
        for (var k = 0; k < teamACrewData.length; k++) {
            var crewMember = teamACrewData[k];
            if (crewMember && crewMember.id !== activeTeammateLorbId) {
                teamABench.push({
                    contactId: crewMember.id,
                    playerInfo: {
                        name: crewMember.name || "Crew Member",
                        jersey: crewMember.jersey || Math.floor(Math.random() * 99),
                        jerseyString: crewMember.jerseyString || String(crewMember.jersey || 0),
                        skin: crewMember.skin || "brown",
                        shortNick: crewMember.shortNick || null,
                        position: crewMember.position || "",
                        attributes: crewMember.attributes || [6, 6, 6, 6, 6, 6],
                        customSprite: crewMember.customSprite || null,
                        lorbId: crewMember.id,
                        lorbData: crewMember
                    },
                    inGame: false
                });
    ...
    } else if (crewData && Array.isArray(crewData)) {
        // Legacy path: raw crew entries needing lookup
        var activeTeammateLorbId = (redInfo2 && redInfo2.lorbId) ? redInfo2.lorbId : null;
    ...
    // Build teamB bench (for multiplayer LORB)
    if (teamBCrewData && Array.isArray(teamBCrewData)) {
        var blueActiveTeammateLorbId = (blueInfo2 && blueInfo2.lorbId) ? blueInfo2.lorbId : null;
        debugLog("[SPRITE-INIT] TeamB active teammate lorbId to exclude: " + blueActiveTeammateLorbId);
        
        for (var j = 0; j < teamBCrewData.length; j++) {
            var blueCrewMember = teamBCrewData[j];
            if (blueCrewMember && blueCrewMember.id !== blueActiveTeammateLorbId) {
                teamBBench.push({
                    contactId: blueCrewMember.id,
                    playerInfo: {
                        name: blueCrewMember.name || "Crew Member",
                        jersey: blueCrewMember.jersey || Math.floor(Math.random() * 99),
                        jerseyString: blueCrewMember.jerseyString || String(blueCrewMember.jersey || 0),
                        skin: blueCrewMember.skin || "brown",
                        shortNick: blueCrewMember.shortNick || null,
                        position: blueCrewMember.position || "",
                        attributes: blueCrewMember.attributes || [6, 6, 6, 6, 6, 6],
                        customSprite: blueCrewMember.customSprite || null,
                        lorbId: blueCrewMember.id,
                        lorbData: blueCrewMember
                    },
                    inGame: false
                });
            }
    ...
    debugLog("[SPRITE-INIT] Final bench: teamA=" + teamABench.length + ", teamB=" + teamBBench.length);
...
function performSubstitution(teamKey, activeSlot, benchIndex, systems) {
    ...
    if (!usingCustomSprite && !skipJersey) {
        // Use explicit undefined checks since BG_BLACK = 0 is falsy but valid
        ...
        log(LOG_DEBUG, "[SUBSTITUTION] Final jersey config: jerseyBg=" + jerseyBgColor + 
                 ", accentFg=" + accentColor + ", jerseyNumber='" + jerseyDigits + "'");
    ...
    // Record substitution for multiplayer sync
    // Coordinator will broadcast this; client will apply it
    var pendingSubstitutions = stateManager.get("pendingSubstitutions") || [];
    pendingSubstitutions.push({
        teamKey: teamKey,
        slot: activeSlot,
        playerInfo: {
            name: newPlayerInfo.name,
            jersey: newPlayerInfo.jersey || 0,
            jerseyString: newPlayerInfo.jerseyString || String(newPlayerInfo.jersey || 0),
            skin: newPlayerInfo.skin || "brown",
            shortNick: newPlayerInfo.shortNick || null,
            position: newPlayerInfo.position || "",
            attributes: newPlayerInfo.attributes || [6, 6, 6, 6, 6, 6],
            customSprite: newPlayerInfo.customSprite || null,
            lorbId: newPlayerInfo.lorbId || null
        },
        turbo: newPlayerData.turbo
    });
    stateManager.set("pendingSubstitutions", pendingSubstitutions, "substitution_recorded");
    debugLog("[SUBSTITUTION] Recorded for MP sync: " + teamKey + " slot " + activeSlot);
