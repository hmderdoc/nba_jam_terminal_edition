...
function electNewCoordinator(session, client) {
    if (!session || !session.playerList) {
        log(LOG_WARNING, "NBA JAM: electNewCoordinator() - Invalid session");
        return null;
    }

    var players = session.playerList || [];
    if (players.length === 0) {
        log(LOG_WARNING, "NBA JAM: electNewCoordinator() - No players in session");
        return null;
    }
    ...
    log(LOG_INFO, "NBA JAM: Elected new coordinator: " + newCoordinator);
...
function promoteToCoordinator(sessionId, newCoordinatorId, client) {
    if (!sessionId || !newCoordinatorId || !client) {
        log(LOG_WARNING, "NBA JAM: promoteToCoordinator() - Missing parameters");
        return false;
    }
    ...
    try {
        ...
        log(LOG_INFO, "NBA JAM: Promoted " + newCoordinatorId + " to coordinator");
        ...
        log(LOG_ERROR, "NBA JAM: Failed to promote coordinator: " + e);
        return false;
    }
}
...
function attemptCoordinatorFailover(sessionId, client) {
    if (!sessionId || !client) {
        log(LOG_ERROR, "NBA JAM: attemptCoordinatorFailover() - Missing parameters");
        return null;
    }
    ...
    try {
        ...
        if (!session) {
            log(LOG_ERROR, "NBA JAM: attemptCoordinatorFailover() - Session not found");
            return null;
        }
        ...
        if (!newCoordinator) {
            log(LOG_ERROR, "NBA JAM: attemptCoordinatorFailover() - No players available");
            return null;
        }
        ...
        if (success) {
            log(LOG_INFO, "NBA JAM: Coordinator failover successful - new coordinator: " + newCoordinator);
            return newCoordinator;
        } else {
            log(LOG_ERROR, "NBA JAM: Coordinator failover failed");
            return null;
        }

    } catch (e) {
        log(LOG_ERROR, "NBA JAM: Coordinator failover error: " + e);
        return null;
    }
}
...
function checkCoordinatorHealth(sessionId, myId, client) {
    ...
    try {
        ...
        if (!isCoordinatorActive(currentCoordinator, session)) {
            ...
            result.needsFailover = true;
            log(LOG_WARNING, "NBA JAM: Coordinator " + currentCoordinator + " disconnected - failover needed");
        }

        return result;

    } catch (e) {
        log(LOG_ERROR, "NBA JAM: checkCoordinatorHealth() error: " + e);
        return result;
    }
}
