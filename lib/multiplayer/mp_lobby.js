    ...
    this.connectToServer = function () {
        ...
        try {
            ...
            if (typeof NBA_JAM !== "undefined" && NBA_JAM.JsonClient && NBA_JAM.JsonClient.get) {
                NBA_JAM.JsonClient.configure({
                    addr: this.serverConfig.server.addr,
                    port: this.serverConfig.server.port
                });
                this.client = NBA_JAM.JsonClient.get();
            } else {
        ...
        while (running && !js.terminated) {
            ...
            switch (this.state) {
                ...
                case LOBBY_STATE.STARTING:
                    ...
                    if (result) {
                        // Disconnect chat before entering game - it can interfere with rendering
                        if (this.chat) {
                            try { this.chat.disconnect(); } catch (e) { }
                            this.chat = null;
                        }
