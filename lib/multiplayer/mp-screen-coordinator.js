...
function MPScreenCoordinator(systems, mpCoordinator, playerClient) {
    ...
    // Prefer constants from MP_CONSTANTS if available, fallback to defaults
    var screenTimeouts = (typeof MP_CONSTANTS === "object" && MP_CONSTANTS.SCREEN_TIMEOUTS) 
        ? MP_CONSTANTS.SCREEN_TIMEOUTS 
        : null;
    
    this.TIMEOUTS = {
        splash: (screenTimeouts && screenTimeouts.SPLASH_MS) || 10000,
        matchup: (screenTimeouts && screenTimeouts.MATCHUP_MS) || 15000,
        halftime: (screenTimeouts && screenTimeouts.HALFTIME_MS) || 20000,  // Reduced from 60s
        game_over: (screenTimeouts && screenTimeouts.GAME_OVER_MS) || 120000
    };
...
MPScreenCoordinator.prototype.getActivePlayers = function () {
    ...
    // During gameplay, all players in playerList are considered active
    // (Queue setup happens early, playerInputQueues may not exist yet during screen entry)
    for (var i = 0; i < session.playerList.length; i++) {
        active.push(session.playerList[i]);
    }
...
MPScreenCoordinator.prototype.handleScreenState = function (screenState) {
    ...
    // Screen changed without explicit dismissal (coordinator override)
    if (this.activeScreen && screenState.active && this.activeScreen !== screenState.active) {
        var previousScreen = this.activeScreen;
        debugLog("[MP SCREEN] Non-coordinator switching screen: " + previousScreen + " -> " + screenState.active);
