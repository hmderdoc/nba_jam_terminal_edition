...
function createAnimationHintTracker(options) {
    ...
    return {
        ...
        recordEvent: function (eventType, payload, frameNumber) {
            ...
            // WAVE 24: Handle stat trail hints for multiplayer sync
            if (eventType === 'stat_trail') {
                var trailTargetId = payload.targetId || null;
                if (trailTargetId) {
                    ensureHint('stat_trail', trailTargetId, frameNumber, {
                        text: payload.text || '',
                        teamKey: payload.teamKey || null,
                        statType: payload.statType || null,
                        fg: (typeof payload.fg === 'number') ? payload.fg : undefined,
                        bg: (typeof payload.bg === 'number') ? payload.bg : undefined
                    });
                }
                return;
            }
