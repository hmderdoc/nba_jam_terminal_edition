...
function PlayerClient(sessionId, client, myPlayerId, serverConfig, systems) {
    ...
    this.handleInput = function (key, frameNumber) {
        ...
        var isMovementKey = !(key === ' ' || upperKey === 'S' || upperKey === 'D');

        if (!this.disablePrediction && key === ' ' && typeof handleActionButton === "function" && this.systems && this.systems.stateManager) {
            ...
            if (stateManager && typeof stateManager.get === "function") {
                ...
                var playerTeam = getPlayerTeamName(this.mySprite);
                if (playerTeam && currentTeam !== playerTeam) {
                    // Apply defensive block locally so the non-authority client sees the animation immediately
                    handleActionButton(this.mySprite, this.systems);
                }
            }
        }
    ...
    this.updateOtherPlayers = function (positions, indexMap) {
        ...
        for (var globalId in indexMap) {
            ...
            if (this.isCoordinator) {
                ...
                // Non-coordinator relies on local prediction for their own sprite's movement
