...
// Calculate the next coordinates for a movement command so we can
// perform a local collision guard before applying the actual movement.
function calculateNextCoords(sprite, cmd) {
    // This mirrors the lightweight movement logic used by applyMovementCommand
    // but avoids side-effects. It attempts to compute where the sprite would
    // end up after applying `cmd` for one step/frame.
    try {
        var nx = sprite.x;
        var ny = sprite.y;

        if (!cmd) return { x: nx, y: ny };

        // Basic movement commands expected: { dx, dy, speed }
        if (typeof cmd.dx === 'number' && typeof cmd.dy === 'number') {
            nx += cmd.dx || 0;
            ny += cmd.dy || 0;
        } else if (cmd.direction) {
            // Handle both string directions and actual key codes
            var direction = cmd.direction;
            var speed = cmd.speed || 1;

            // Convert key codes to movements (assuming KEY_* constants are available)
            if (typeof KEY_LEFT !== 'undefined' && direction === KEY_LEFT) {
                nx -= speed;
            } else if (typeof KEY_RIGHT !== 'undefined' && direction === KEY_RIGHT) {
                nx += speed;
            } else if (typeof KEY_UP !== 'undefined' && direction === KEY_UP) {
                ny -= speed;
            } else if (typeof KEY_DOWN !== 'undefined' && direction === KEY_DOWN) {
                ny += speed;
            } else {
                // Fallback for string directions
                switch (direction) {
        ...
        // If anything goes wrong, return current position to avoid incorrect
...
function wouldCollideWithAuthoritativeOthers(systems, localSprite, nextPos) {
    try {
        ...
        for (var i = 0; i < allPlayers.length; i++) {
            ...
            // Use a slightly tighter threshold than authority (1.5 vs 2) to only block when definitely overlapping
...
function PlayerClient(sessionId, client, myPlayerId, serverConfig, systems) {
    ...
    this.handleInput = function (key, frameNumber) {
        ...
        if (isMovementKey && typeof calculateNextCoords === "function") {
            var nextCoords = calculateNextCoords(this.mySprite, { direction: key, speed: 1 });
    ...
    this.reconcileMyPosition = function (serverState) {
        ...
        if (myServerPos.fp) {
            ...
            this.pendingInputs = []; // CRITICAL: Clear pending inputs to prevent re-applying old movements

            // Also update bearing if provided
            if (myServerPos.b) {
        ...
        if (!skipAuthority) {
            ...
            if (deltaMagnitude < 0.75) {
                ...
                this.mySprite.x += (myServerPos.x - this.mySprite.x) * blendStrength;
                this.mySprite.y += (myServerPos.y - this.mySprite.y) * blendStrength;
                if (typeof debugLog === "function") {
