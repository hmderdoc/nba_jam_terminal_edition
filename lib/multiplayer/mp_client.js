...
function PlayerClient(sessionId, client, myPlayerId, serverConfig, systems) {
    ...
    this.updateGameState = function (state) {
        ...
        // Apply court redraw hint if coordinator requested it
        if (typeof state.cr !== 'undefined') {
            try {
                if (state.cr) {
                    this.systems.stateManager.set('courtNeedsRedraw', true, 'mp_coordinator_cr');
                }
            } catch (e) {
                if (typeof log === 'function') log(LOG_WARNING, 'MP CLIENT: failed to apply court redraw flag: ' + e);
            }
        }
