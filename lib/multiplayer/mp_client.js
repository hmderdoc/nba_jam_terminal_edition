...
function PlayerClient(sessionId, client, myPlayerId, serverConfig, systems) {
    ...
    this.lastAppliedSubsJson = null; // Debounce duplicate sub broadcasts
    ...
    // Post-snap prediction dampening (throttle replayed inputs after a reset)
    this.predictionDampenActive = false;
    this.predictionDampenFramesRemaining = 0;
    this.predictionDampenFactor = 1.0;
    this.predictionDampenAccumulator = 0;

    // Network monitor
    this.networkMonitor = new NetworkMonitor(client, sessionId);

    this._getDefaultDampenConfig = function () {
        if (typeof MP_CONSTANTS === "object" && MP_CONSTANTS.GAME_PHASES && MP_CONSTANTS.GAME_PHASES.POST_SNAP_RECOVERY) {
            var cfg = MP_CONSTANTS.GAME_PHASES.POST_SNAP_RECOVERY;
            return {
                frames: cfg.taperingFrames || 5,
                factor: cfg.taperingFactor || 0.5
            };
        }
        return { frames: 5, factor: 0.5 };
    };

    this._getPredictionTurboConfig = function () {
        if (typeof MP_CONSTANTS === "object" && MP_CONSTANTS.PREDICTION && MP_CONSTANTS.PREDICTION.TURBO) {
            return MP_CONSTANTS.PREDICTION.TURBO;
        }
        return null;
    };

    this._getPredictiveTurboDrainFactor = function () {
        // Coordinators or disabled prediction paths should always drain at the authoritative rate
        if (this.isCoordinator || this.disablePrediction) {
            return 1;
        }

        var config = this._getPredictionTurboConfig();
        var baseFactor = (config && typeof config.DRAIN_FACTOR === "number") ? config.DRAIN_FACTOR : 1;
        if (this.authoritativeCatchupFrames > 0) {
            var catchupFactor = (config && typeof config.CATCHUP_FACTOR === "number") ? config.CATCHUP_FACTOR : 0;
            baseFactor = catchupFactor;
        }

        if (baseFactor < 0) baseFactor = 0;
        if (baseFactor > 1) baseFactor = 1;
        return baseFactor;
    };

    this._getPredictiveTurboDrainAmount = function () {
        return TURBO_DRAIN_RATE * this._getPredictiveTurboDrainFactor();
    };

    this._startPredictionDampen = function (frames, factor, reason) {
        var cfg = this._getDefaultDampenConfig();
        var useFrames = (typeof frames === "number" && frames > 0) ? Math.floor(frames) : cfg.frames;
        var useFactor = (typeof factor === "number" && factor > 0) ? factor : cfg.factor;
        if (!(useFrames > 0) || !(useFactor > 0)) {
            return;
        }
        this.predictionDampenActive = true;
        this.predictionDampenFramesRemaining = useFrames;
        this.predictionDampenFactor = useFactor;
        this.predictionDampenAccumulator = 0;
        if (typeof debugLog === "function" && reason) {
            debugLog("[MP CLIENT] Prediction dampening start (" + reason + ") frames=" + useFrames + " factor=" + useFactor);
        }
    };

    this._tickPredictionDampen = function () {
        if (!this.predictionDampenActive) return;
        if (this.predictionDampenFramesRemaining > 0) {
            this.predictionDampenFramesRemaining--;
        }
        if (this.predictionDampenFramesRemaining <= 0) {
            this.predictionDampenActive = false;
            this.predictionDampenFactor = 1.0;
            this.predictionDampenAccumulator = 0;
            if (typeof debugLog === "function") {
                debugLog("[MP CLIENT] Prediction dampening complete");
            }
        }
    };
    ...
    this.requestAuthoritativeCatchup = function (frames, reason) {
        ...
        if (frames > this.authoritativeCatchupFrames) {
            ...
            this._startPredictionDampen(null, null, "authoritative_catchup");
        }
    };
    ...
    this._raiseSpriteFrame = function (sprite) {
        ...
        try {
            // WAVE 24 FLICKER FIX: Invalidate frame to ensure redraw after position change
            // This was missing - other sprites get invalidate() but controlled sprite only got top()
            if (typeof sprite.frame.invalidate === "function") {
                sprite.frame.invalidate();
            }
            if (typeof sprite.frame.top === "function") {
                sprite.frame.top();
            }
        } catch (e) {
            if (typeof debugLog === "function") {
    ...
    this._applyStagedSpriteCommit = function () {
        ...
        var roundedX = Math.round(commit.x);
        var roundedY = Math.round(commit.y);
        
        // WAVE 24 FLICKER FIX: Use moveTo if available (same as updateOtherPlayers)
        // This ensures consistent frame handling across all sprites
        if (typeof this.mySprite.moveTo === "function") {
            if (roundedX !== this.mySprite.x || roundedY !== this.mySprite.y) {
                this.mySprite.moveTo(roundedX, roundedY);
            }
        } else {
            this.mySprite.x = roundedX;
            this.mySprite.y = roundedY;
        }
    ...
    this.resetPredictionState = function (reason, options) {
        ...
        // Apply a short dampening window so replayed inputs don't reintroduce drift immediately.
        this._startPredictionDampen(null, null, "prediction_reset");
    };
    ...
    this.handleInput = function (key, frameNumber) {
        ...
        // Translate numpad keys to movement info
        ...
        if (turboIntent && this.mySprite.playerData && typeof this.mySprite.playerData.useTurbo === "function") {
            var predictiveDrain = this._getPredictiveTurboDrainAmount();
            if (predictiveDrain > 0) {
                this.mySprite.playerData.useTurbo(predictiveDrain);
            }
        }

        var upperKey = (typeof key === "string") ? key.toUpperCase() : key;
        // Check if this is a movement key (including numpad)
        var isActionKey = (upperKey === ' ' || upperKey === 'S' || upperKey === 'D');
        var isMovementKey = !isActionKey && (
            numpadInfo.isDiagonal || 
            numpadInfo.isCardinal || 
            key === KEY_UP || key === KEY_DOWN || key === KEY_LEFT || key === KEY_RIGHT
        );

        // WAVE 24 FIX: Predictively execute the CORRECT action for non-coordinators
        // Previously all action keys called handleActionButton which caused steal→jump bug
        // Now we call the same handler the coordinator uses for each key type
        if (!this.disablePrediction && isActionKey) {
            if (upperKey === ' ' && typeof handleActionButton === "function") {
                // Space bar - shoot or block
                handleActionButton(this.mySprite, this.systems, { predictive: true });
            } else if (upperKey === 'S' && typeof handleSecondaryButton === "function") {
                // S key - pass or steal
                handleSecondaryButton(this.mySprite, this.systems, { predictive: true });
            } else if (upperKey === 'D' && typeof handleDribbleButton === "function") {
                // D key - pick up dribble or shake
                handleDribbleButton(this.mySprite, this.systems, { predictive: true });
            }
        }
        ...
        // Collision guard for NON-COORDINATORS ONLY
        // Coordinator handles collision authoritatively in applyInput(), so we must not
        // block coordinator inputs here (would cause coordinator to be unable to move)
        var inputBlocked = false;
        var previewResult = null;
        if (!this.disablePrediction && isMovementKey && typeof previewMovementCommand === "function") {
            // For diagonal movement, check both directions
            if (numpadInfo.isDiagonal) {
                var previewH = previewMovementCommand(this.mySprite, numpadInfo.horizKey);
                var previewV = previewMovementCommand(this.mySprite, numpadInfo.vertKey);
                // Use combined attempted position for collision check
                var nextCoords = {
                    x: previewH ? previewH.attemptedX : this.mySprite.x,
                    y: previewV ? previewV.attemptedY : this.mySprite.y
                };
                var wouldCollide = wouldCollideWithAuthoritativeOthers(this.systems, this.mySprite, nextCoords);
                if (wouldCollide) {
                    debugLog("[MP CLIENT] NON-COORDINATOR diagonal input blocked by collision guard (key=" + key + ")");
                    return;
                }
            } else {
                // Cardinal movement
                var effectiveKey = numpadInfo.isCardinal ? numpadInfo.cardinalKey : key;
                previewResult = previewMovementCommand(this.mySprite, effectiveKey);
                ...
                if (wouldCollide) {
                    debugLog("[MP CLIENT] NON-COORDINATOR input blocked by collision guard (key=" + key + ")");
                    inputBlocked = true;
        ...
        if (!this.disablePrediction && isMovementKey) {
            ...
            if (budget && budget.moves > 0) {
                ...
                if (numpadInfo.isDiagonal && typeof applyDiagonalMovement === "function") {
                    ...
                    for (var m = 0; m < budget.moves; m++) {
                        var moved = applyDiagonalMovement(this.mySprite, numpadInfo.horizKey, numpadInfo.vertKey, counters);
                    ...
                    var effectiveKey = numpadInfo.isCardinal ? numpadInfo.cardinalKey : key;
                    for (var m = 0; m < budget.moves; m++) {
                        var moved = applyMovementCommand(this.mySprite, effectiveKey, counters);
                        if (moved) predictionApplied = true;
                        if (!moved) break;
                    }
                }
                ...
                if (numpadInfo.isDiagonal && typeof applyDiagonalMovement === "function") {
                    var moved = applyDiagonalMovement(this.mySprite, numpadInfo.horizKey, numpadInfo.vertKey);
                    if (moved) predictionApplied = true;
                } else if (typeof applyMovementCommand === "function") {
                    var effectiveKey = numpadInfo.isCardinal ? numpadInfo.cardinalKey : key;
                    var moved = applyMovementCommand(this.mySprite, effectiveKey);
                    if (moved) predictionApplied = true;
                }
            }
        }
        ...
        // 3. Buffer for sending to server (send raw key - coordinator will translate too)
    ...
    this.reconcile = function () {
        ...
        // Apply remote substitutions immediately when received - debounce duplicates
        if (!this.isCoordinator && serverState.g && serverState.g.subs && serverState.g.subs.length > 0) {
            var subsJson = JSON.stringify(serverState.g.subs);
            if (subsJson !== this.lastAppliedSubsJson) {
                this.lastAppliedSubsJson = subsJson;
                this.applyRemoteSubstitutions(serverState.g.subs);
            }
        }
        ...
        if (typeof replayInputsSince === "function" && !this.disablePrediction) {
            ...
            var dampenFactor = (this.predictionDampenActive && this.predictionDampenFramesRemaining > 0)
                ? this.predictionDampenFactor
                : 1.0;

            function shouldApplySingleStep() {
                if (dampenFactor >= 1.0) {
                    return true;
                }
                self.predictionDampenAccumulator += dampenFactor;
                if (self.predictionDampenAccumulator < 1.0) {
                    return false;
                }
                self.predictionDampenAccumulator -= 1.0;
                return true;
            }
            ...
            replayInputsSince(serverState.f, function (inputRecord) {
                ...
                if (inputRecord && inputRecord.key && self.mySprite) {
                    ...
                    if (turboActive && self.mySprite.playerData && typeof self.mySprite.playerData.useTurbo === "function") {
                        var replayDrain = self._getPredictiveTurboDrainAmount();
                        if (replayDrain > 0) {
                            self.mySprite.playerData.useTurbo(replayDrain);
                        }
                    }

                    // Translate numpad keys for replay (same as handleInput)
                    var numpadInfo = (typeof translateNumpadKey === "function") 
                        ? translateNumpadKey(inputRecord.key) 
                        : { isDiagonal: false, isCardinal: false, effectiveKey: inputRecord.key };

                    // Handle diagonal numpad replay
                    if (numpadInfo.isDiagonal && typeof applyDiagonalMovement === "function") {
                        var budget = (typeof createMovementCounters === "function")
                        ...
                        if (budget && budget.moves > 0) {
                            if (dampenFactor < 1.0) {
                                budget.moves = Math.max(1, Math.min(budget.moves, Math.round(budget.moves * dampenFactor)));
                                budget.horizontal = Math.max(0, Math.min(budget.horizontal, Math.round(budget.horizontal * dampenFactor)));
                                budget.vertical = Math.max(0, Math.min(budget.vertical, Math.round(budget.vertical * dampenFactor)));
                            }
                            var counters = {
                                horizontal: Math.max(0, budget.horizontal),
                                vertical: Math.max(0, budget.vertical)
                            ...
                            for (var m = 0; m < budget.moves; m++) {
                                ...
                                var diagNextX = self.mySprite.x;
                                var diagNextY = self.mySprite.y;
                                if (numpadInfo.horizKey === KEY_LEFT) diagNextX -= 1;
                                else if (numpadInfo.horizKey === KEY_RIGHT) diagNextX += 1;
                                if (numpadInfo.vertKey === KEY_UP) diagNextY -= 1;
                                else if (numpadInfo.vertKey === KEY_DOWN) diagNextY += 1;

                                var blocked = wouldCollideWithAuthoritativeOthers(self.systems, self.mySprite, { x: diagNextX, y: diagNextY });
                                if (blocked) {
                                    if (typeof debugLog === "function") {
                                        debugLog("[MP CLIENT] Diagonal replay blocked by collision guard (key=" + inputRecord.key + ")");
                                    }
                                    break;
                                }

                                var moved = applyDiagonalMovement(self.mySprite, numpadInfo.horizKey, numpadInfo.vertKey, counters);
                                if (moved) replayPredictionApplied = true;
                                if (!moved) break;
                            }
                        } else if (!budget) {
                            // Fallback without budget
                            var diagNextXb = self.mySprite.x;
                            var diagNextYb = self.mySprite.y;
                            if (numpadInfo.horizKey === KEY_LEFT) diagNextXb -= 1;
                            else if (numpadInfo.horizKey === KEY_RIGHT) diagNextXb += 1;
                            if (numpadInfo.vertKey === KEY_UP) diagNextYb -= 1;
                            else if (numpadInfo.vertKey === KEY_DOWN) diagNextYb += 1;

                            var blockedDiag = wouldCollideWithAuthoritativeOthers(self.systems, self.mySprite, { x: diagNextXb, y: diagNextYb });
                            if (!blockedDiag && shouldApplySingleStep()) {
                                var movedDiag = applyDiagonalMovement(self.mySprite, numpadInfo.horizKey, numpadInfo.vertKey);
                                if (movedDiag) replayPredictionApplied = true;
                            }
                        }
                        return; // Done with this input
                    }

                    // Cardinal movement (arrow keys or numpad 2,4,6,8)
                    var effectiveKey = numpadInfo.isCardinal ? numpadInfo.cardinalKey : inputRecord.key;
                    ...
                    if (typeof applyMovementCommand === "function") {
                        ...
                        if (budget && budget.moves > 0) {
                            ...
                            for (var m = 0; m < budget.moves; m++) {
                                ...
                                if (blocked) {
                                    if (typeof debugLog === "function") {
                                        debugLog("[MP CLIENT] Local prediction blocked by collision guard (key=" + effectiveKey + ", nextX=" + nextX + ", nextY=" + nextY + ")");
                                    }
                                    // Do not apply movement; treat as not moved and stop
                                    break;
                                }

                                var moved = applyMovementCommand(self.mySprite, effectiveKey, counters);
                                if (moved) {
                            ...
                            if (!blockedBasic && shouldApplySingleStep()) {
                                var movedBasic = applyMovementCommand(self.mySprite, effectiveKey);
                                if (movedBasic) {
                                    replayPredictionApplied = true;
                                }
                            } else {
                                if (typeof debugLog === "function") {
                                    debugLog("[MP CLIENT] Local prediction blocked by collision guard (key=" + effectiveKey + ", nextX=" + nextXb + ", nextY=" + nextYb + ")");
                                }
                            }
                        }
                    }
    ...
    this.updateOtherPlayers = function (positions, indexMap) {
        ...
        for (var globalId in indexMap) {
            ...
            if (sprite.playerData) {
                ...
                if (typeof pos.sr === "number") {
                    sprite.playerData.shoveRecoveryFrames = pos.sr;
                }
                if (typeof pos.td === "number") {
                    sprite.playerData.turboDisabledFrames = pos.td;
                }
    ...
    this.updateGameState = function (state) {
        ...
        if (this.isCoordinator) {
            debugLog("[MP CLIENT] updateGameState skipped - isCoordinator=true");
            return;
        }
        ...
        // Handle game end signal from coordinator
        // WAVE 24 FIX: Only accept gameRunning=false if we're actually in a running game.
        // During pre-game (matchup/splash screens), coordinator broadcasts gameRunning=false
        // but we shouldn't interpret that as "game over". Only transition to false when:
        // 1. The game was running AND
        // 2. Time has actually elapsed (tick > 0) indicating gameplay occurred
        if (typeof state.gr === "boolean") {
            var wasRunning = stateManager.get("gameRunning");
            var currentTick = stateManager.get("tickCounter") || 0;
            
            if (state.gr === true) {
                // Coordinator says game is running - accept it
                stateManager.set("gameRunning", true, "mp_game_running_update");
            } else if (wasRunning && !state.gr && currentTick > 10) {
                // Game was running, coordinator now says stopped, and we've had actual gameplay
                // This is a legitimate game end signal
                debugLog("[MP CLIENT] Game ended - coordinator signaled gameRunning=false at tick " + currentTick);
                stateManager.set("gameRunning", false, "mp_game_running_update");
            } else if (!state.gr && currentTick <= 10) {
                // Pre-game state packet with gameRunning=false - ignore it
                debugLog("[MP CLIENT] Ignoring pre-game gameRunning=false (tick=" + currentTick + ")");
            }
        }
        ...
        // Handle overtime state sync (for scoreboard period display and intro graphic)
        if (typeof state.ot === "boolean") {
            var wasOvertime = stateManager.get("isOvertime");
            if (!wasOvertime && state.ot) {
                debugLog("[MP CLIENT] Overtime started via state sync");
            }
            stateManager.set("isOvertime", state.ot, "mp_overtime_update");
        }

        if (typeof state.otp === "number") {
            stateManager.set("currentOvertimePeriod", state.otp, "mp_overtime_period_update");
        }

        // Handle overtime intro active flag (for displaying overtime graphic)
        if (typeof state.oia === "boolean") {
            var wasIntroActive = stateManager.get("overtimeIntroActive");
            if (!wasIntroActive && state.oia) {
                debugLog("[MP CLIENT] Overtime intro active via state sync");
                stateManager.set("courtNeedsRedraw", true, "mp_overtime_intro_start");
            } else if (wasIntroActive && !state.oia) {
                debugLog("[MP CLIENT] Overtime intro ended via state sync");
                stateManager.set("courtNeedsRedraw", true, "mp_overtime_intro_end");
            }
            stateManager.set("overtimeIntroActive", state.oia, "mp_overtime_intro_update");
        }
        ...
        if (state.bf) {
            try {
                ...
                // Swallow basketFlash sync errors to avoid disrupting client updates
            }
        }
    ...
    this._executeAnimationHint = function (record) {
        ...
        switch (record.type) {
            ...
            case 'shove_knockback':
                ...
                // Client uses null for systems - direction already computed by coordinator
                knockBack(victimSprite, attackerSprite, pushDistance, null);
                return true;
            case 'stat_trail':
                // WAVE 24: Sync stat trail animations from coordinator to non-coordinator
                if (!this.systems || !this.systems.statTrailSystem || 
                    typeof this.systems.statTrailSystem.queueStatTrail !== 'function') {
                    return false;
                }
                this.systems.statTrailSystem.queueStatTrail({
                    text: meta.text || '',
                    player: record.sprite || null,
                    teamKey: meta.teamKey || null,
                    statType: meta.statType || null,
                    fg: (typeof meta.fg === 'number') ? meta.fg : undefined,
                    bg: (typeof meta.bg === 'number') ? meta.bg : undefined
                });
                debugLog("[MP CLIENT] Applied stat_trail hint: " + (meta.text || ''));
                return true;
            default:
                return false;
        }
    ...
    this.applyAnimationHints = function (entries, serverFrame) {
        ...
        for (var j = 0; j < cache.order.length; j++) {
            ...
            // WAVE 24: stat_trail hints can work without a sprite (fallback to center of court)
            if (!item.sprite && item.type !== 'stat_trail') {
                continue;
            }
    ...
    this.applyPhaseSettings = function (phaseName) {
        ...
        if (phaseConfig.inputTapering) {
            ...
            this._startPredictionDampen(this.inputTaperingFramesRemaining, this.inputTaperingFactor, "phase_" + phaseName);
    ...
    /**
     * Apply substitutions that occurred on the coordinator side
     * Called when halftime ends and we receive pending substitutions
     * 
     * @param {Array} subs - Array of substitution data from coordinator
     */
    this.applyRemoteSubstitutions = function (subs) {
        ...
        for (var i = 0; i < subs.length; i++) {
            ...
            if (!sub || !sub.teamKey || sub.slot === undefined || !sub.playerInfo) {
                debugLog("[MP CLIENT] Invalid substitution data at index " + i);
                continue;
            }
            ...
            try {
                ...
                if (!currentSprite) {
                    debugLog("[MP CLIENT] Cannot find sprite " + spriteId + " for substitution");
                    continue;
                }
                ...
                if (typeof Player === "function") {
                    ...
                    newPlayerData.turbo = sub.turbo || 135;  // Use turbo from coordinator
                } else {
                    // Fallback minimal playerData
                    newPlayerData = {
                ...
                debugLog("[MP CLIENT] Applied remote substitution: " + teamKey + " slot " + slot + " -> " + playerInfo.name + " (skin=" + skinName + ", spriteBase=" + spriteBase + ")");
                
                // Refresh sprite map entry
                if (this.spriteMap) {
                    for (var playerId in this.spriteMap) {
                        if (this.spriteMap[playerId] === currentSprite) {
                            this.spriteMap[playerId] = newSprite;
                            debugLog("[MP CLIENT] Updated sprite map for player " + playerId);
                            break;
                        }
                    }
                }
                
                // If this was my sprite, update reference
                if (currentSprite === this.mySprite) {
                    this.mySprite = newSprite;
                    debugLog("[MP CLIENT] Updated mySprite reference after substitution");
                }
        ...
        // Request court redraw to show new sprites
        stateManager.set("courtNeedsRedraw", true, "mp_substitution_applied");
    };
