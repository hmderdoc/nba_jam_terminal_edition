    ...
    this.updateGameState = function (state) {
        ...
        if (typeof state.ht === "boolean") {
            ...
            if (!wasHalftime && isHalftime) {
                debugLog("[MP CLIENT] Halftime detected via state sync, showing halftime screen");
                stateManager.set("isHalftime", true, "mp_halftime_start");

                // Show halftime screen for non-coordinator
                if (typeof showHalftimeScreen === "function") {
                    showHalftimeScreen(this.systems);
                } else {
                    debugLog("[MP CLIENT] ERROR: showHalftimeScreen function not available!");
                }
