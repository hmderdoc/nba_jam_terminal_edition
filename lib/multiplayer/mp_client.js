...
function PlayerClient(sessionId, client, myPlayerId, serverConfig, systems) {
    ...
    this.handleInput = function (key, frameNumber) {
        ...
        if (!this.disablePrediction && isActionKey && typeof handleActionButton === "function") {
    ...
    this.updateGameState = function (state) {
        ...
        if (typeof state.gr === "boolean") {
            ...
            if (wasRunning && !state.gr) {
                debugLog("[MP CLIENT] Game ended - coordinator signaled gameRunning=false");
            }
            stateManager.set("gameRunning", state.gr, "mp_game_running_update");
        }
