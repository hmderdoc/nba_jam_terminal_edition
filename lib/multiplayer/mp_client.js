...
function PlayerClient(sessionId, client, myPlayerId, serverConfig, systems) {
    ...
    // State reconciliation timing (reduced to 10 Hz to eliminate flicker)
    // Lower frequency reduces phase misalignment oscillation between broadcast/reconcile/render
    this.stateCheckInterval = 100; // Check every 100ms (10 Hz) - was 50ms (20 Hz)
    ...
    this.handleHalftimeEvent = function (data) {
        if (!data || !this.systems || !this.systems.stateManager) return;
