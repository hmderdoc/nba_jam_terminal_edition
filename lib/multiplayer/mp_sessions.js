...
function ensureTeamContainers(session) {
    ...
    if (!session.teams.teamA)
        session.teams.teamA = { name: null, players: [], abbr: null, roster: {}, cpuIndex: null };
    if (!session.teams.teamB)
        session.teams.teamB = { name: null, players: [], abbr: null, roster: {}, cpuIndex: null };
...
function createGameSession(client, options) {
    ...
    var initialRedKey = options.teamATeam ? MPTeamData.findTeamKey(options.teamATeam) : null;
    var initialBlueKey = options.teamBTeam ? MPTeamData.findTeamKey(options.teamBTeam) : null;
...
function selectTeamAndPlayer(client, sessionId, teamInput, rosterIndex) {
    ...
    try {
        ...
        var redData = session.teams.teamA;
        var blueData = session.teams.teamB;

        var targetSide = null;
        if (redData.name && redData.name === teamKey)
            targetSide = "teamA";
        if (blueData.name && blueData.name === teamKey)
            targetSide = targetSide || "teamB";
        ...
        var opponentSide = targetSide === "teamA" ? "teamB" : "teamA";
...
function checkAllPlayersReady(session) {
    ...
    if (!session.teams.teamA.name || !session.teams.teamB.name)
        return false;
    if (session.teams.teamA.name === session.teams.teamB.name)
        return false;
    if (session.teams.teamA.players.length === 0 || session.teams.teamB.players.length === 0)
        return false;
...
function assignPlayerToTeam(client, sessionId, playerId, team) {
    if (team !== "teamA" && team !== "teamB") return false;
    ...
    var redIndex = session.teams.teamA.players.indexOf(playerId);
    if (redIndex >= 0) {
        session.teams.teamA.players.splice(redIndex, 1);
    }

    var blueIndex = session.teams.teamB.players.indexOf(playerId);
    if (blueIndex >= 0) {
        session.teams.teamB.players.splice(blueIndex, 1);
    }
