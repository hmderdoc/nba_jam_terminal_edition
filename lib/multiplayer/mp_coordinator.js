...
function GameCoordinator(sessionId, client, serverConfig, systems) {
    ...
    this.serializeGameState = function () {
        ...
        var state = {
            ...
            subs: stateManager.get('pendingSubstitutions') || null  // Substitution sync for halftime
        };
        
        // Debug: Log when subs are being broadcast during halftime
        if (state.ht && state.subs && state.subs.length > 0) {
            debugLog("[MP COORD] Broadcasting " + state.subs.length + " substitution(s) during halftime");
        }
