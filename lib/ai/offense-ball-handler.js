...
function aiOffenseBall(player, teamName, context, systems) {
    ...
    if (stateManager && needsDecision && dribbleDead) {
        ...
        if (timeSinceDecision < 200) { // Within 200ms of shove, make quick decision
            ...
            if (teammate) {
                ...
                var passQuality = 0.5;
                if (passDistance < 25) passQuality += 0.2;
                if (teammateDistToBasket < passerDistToBasket - 5) passQuality += 0.3;
                if (teammate.playerData && teammate.playerData.openForPass) passQuality += 0.2;
                ...
                if (passQuality > 0.45 && Math.random() < 0.60) {
