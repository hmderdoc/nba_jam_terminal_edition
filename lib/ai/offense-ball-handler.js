...
function aiOffenseBall(player, teamName, context, systems) {
    ...
    // STUCK WITHOUT PRESSURE: AI has ball, still dribbling, but hasn't moved
    // Force them to DO something - drive, pass, or shoot
    if (stateManager && needsDecision && !dribbleDead) {
        debugLog("[AI DECISION] " + playerName + " stuck without pressure - forcing action (still has dribble)");
        stateManager.set("ballCarrierNeedsDecision", false, "stuck_no_pressure_action");

        // Options: drive to basket, pass to teammate, or pull up for shot
        var stuckTeammate = teammate || getTeammate(player, teamName);
        var canDrive = distToBasket > 8 && playerData.turbo > 20;
        var canPass = stuckTeammate && getSpriteDistance(player, stuckTeammate) < 30;
        
        // If close to basket, shoot
        if (distToBasket < 12) {
            debugLog("[AI DECISION] Stuck - taking shot (close to basket)");
            playerData.aiLastAction = "stuck_shot_close";
            attemptShot(systems);
            return;
        }
        
        // Randomly choose: 40% drive, 40% pass, 20% pullup shot
        var actionRoll = Math.random();
        if (canDrive && actionRoll < 0.4) {
            debugLog("[AI DECISION] Stuck - driving to basket");
            playerData.aiLastAction = "stuck_drive";
            playerData.turboActive = true;
            playerData.useTurbo(TURBO_DRAIN_RATE);
            steerToward(player, basket.x, basket.y, 4, systems);
            return;
        } else if (canPass && actionRoll < 0.8) {
            debugLog("[AI DECISION] Stuck - passing to teammate");
            playerData.aiLastAction = "stuck_pass";
            var stuckLeadTarget = getSmartPassTarget(player, stuckTeammate);
            animatePass(player, stuckTeammate, stuckLeadTarget, null, systems);
            return;
        } else {
            debugLog("[AI DECISION] Stuck - taking pullup shot");
            playerData.aiLastAction = "stuck_pullup";
            attemptShot(systems);
            return;
        }
    }
    ...
    // PRIORITY 1.5: BUNCHING DETECTION - Defender blocking direct path to basket
    // When close to basket and a defender is directly in the path, shove to clear the lane
    var bunchingLaneBlockedDist = aiOffenseValue("BUNCHING.LANE_BLOCKED_DISTANCE", 12);
    var bunchingDefBlockThreshold = aiOffenseValue("BUNCHING.DEFENDER_BLOCKING_THRESHOLD", 3.5);
    var bunchingDefCloseDist = aiOffenseValue("BUNCHING.DEFENDER_CLOSE_DISTANCE", 4.5);
    
    if (!inBackcourt && distToBasket <= bunchingLaneBlockedDist && playerData.shoveAttemptCooldown <= 0) {
        // Check if defender is blocking the direct line to basket
        var toBasketDx = basket.x - player.x;
        var toBasketDy = basket.y - player.y;
        var toBasketLen = Math.sqrt(toBasketDx * toBasketDx + toBasketDy * toBasketDy);
        
        if (toBasketLen > 0.1) {
            toBasketDx /= toBasketLen;
            toBasketDy /= toBasketLen;
            
            // Find defenders blocking the lane
            var blockingDefender = null;
            var bestBlockScore = -999;
            var allPlayers = spriteRegistry ? spriteRegistry.getAllPlayers() : getAllPlayers();
            
            for (var bi = 0; bi < allPlayers.length; bi++) {
                var potentialBlocker = allPlayers[bi];
                if (!potentialBlocker || !potentialBlocker.playerData) continue;
                var blockerTeam = getPlayerTeamName(potentialBlocker);
                if (blockerTeam === teamName) continue; // Skip teammates
                
                var distToBlocker = getSpriteDistance(player, potentialBlocker);
                if (distToBlocker > bunchingDefCloseDist) continue; // Too far to be blocking
                
                // Project blocker position onto line from player to basket
                var toBlockerX = potentialBlocker.x - player.x;
                var toBlockerY = potentialBlocker.y - player.y;
                var projection = toBlockerX * toBasketDx + toBlockerY * toBasketDy;
                
                // Defender must be between player and basket (positive projection, less than distance)
                if (projection <= 0 || projection >= toBasketLen) continue;
                
                // Calculate perpendicular distance from defender to line
                var closestPointX = player.x + toBasketDx * projection;
                var closestPointY = player.y + toBasketDy * projection;
                var perpDist = Math.sqrt(
                    (potentialBlocker.x - closestPointX) * (potentialBlocker.x - closestPointX) +
                    (potentialBlocker.y - closestPointY) * (potentialBlocker.y - closestPointY)
                );
                
                // Is this defender blocking the lane?
                if (perpDist <= bunchingDefBlockThreshold) {
                    var blockScore = (bunchingDefBlockThreshold - perpDist) + (bunchingDefCloseDist - distToBlocker);
                    if (blockScore > bestBlockScore) {
                        bestBlockScore = blockScore;
                        blockingDefender = potentialBlocker;
                    }
                }
            }
            
            // If we found a blocking defender, consider shoving
            if (blockingDefender) {
                var shoveChanceBlocked = aiOffenseValue("BUNCHING.SHOVE_CHANCE_BLOCKED", 0.70);
                var minTurboToShove = aiOffenseValue("BUNCHING.MIN_TURBO_TO_SHOVE", 10);
                
                if (playerData.turbo >= minTurboToShove && Math.random() < shoveChanceBlocked) {
                    debugLog("[AI BUNCHING] " + playerName + " detected lane blocked by " + 
                        (blockingDefender.playerData.name || "defender") + " - attempting shove");
                    
                    if (attemptShove(player, blockingDefender, systems)) {
                        playerData.aiLastAction = "bunching_clear_lane";
                        return;
                    }
                }
            }
            
            // Check for general congestion (multiple opponents clustered nearby)
            var clusterRadius = aiOffenseValue("BUNCHING.CLUSTER_RADIUS", 5);
            var clusterMinOpponents = aiOffenseValue("BUNCHING.CLUSTER_MIN_OPPONENTS", 2);
            var nearbyOpponents = 0;
            var closestClusterOpponent = null;
            var closestClusterDist = 999;
            
            for (var ci = 0; ci < allPlayers.length; ci++) {
                var clusterCandidate = allPlayers[ci];
                if (!clusterCandidate || !clusterCandidate.playerData) continue;
                var candTeam = getPlayerTeamName(clusterCandidate);
                if (candTeam === teamName) continue;
                
                var distToCandidate = getSpriteDistance(player, clusterCandidate);
                if (distToCandidate <= clusterRadius) {
                    nearbyOpponents++;
                    if (distToCandidate < closestClusterDist) {
                        closestClusterDist = distToCandidate;
                        closestClusterOpponent = clusterCandidate;
                    }
                }
            }
            
            // Congested - multiple defenders nearby
            if (nearbyOpponents >= clusterMinOpponents && closestClusterOpponent && playerData.shoveAttemptCooldown <= 0) {
                var shoveChanceCongested = aiOffenseValue("BUNCHING.SHOVE_CHANCE_CONGESTED", 0.50);
                var minTurbo = aiOffenseValue("BUNCHING.MIN_TURBO_TO_SHOVE", 10);
                
                if (playerData.turbo >= minTurbo && Math.random() < shoveChanceCongested) {
                    debugLog("[AI BUNCHING] " + playerName + " detected congestion (" + 
                        nearbyOpponents + " opponents) - attempting shove on " + 
                        (closestClusterOpponent.playerData.name || "defender"));
                    
                    if (attemptShove(player, closestClusterOpponent, systems)) {
                        playerData.aiLastAction = "bunching_clear_congestion";
                        return;
                    }
                }
            }
