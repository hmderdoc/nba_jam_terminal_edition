...
function aiOffenseBall(player, teamName, context, systems) {
    ...
    debugLog("[AI OFFENSE BALL] teamName=" + teamName + ", player.x=" + player.x + ", playerData.name=" + (playerData.name || "unknown"));

    var ballCarrier = context.getBallCarrier();
    if (ballCarrier && ballCarrier !== player) {
        debugLog("[AI MISMATCH!] AI operating on " + playerData.name + " at x=" + player.x + ", but ballCarrier is " + ballCarrier.playerData.name + " at x=" + ballCarrier.x);
    }
    ...
    if (inBackcourt && !context.isFrontcourtEstablished()) {
        debugLog("[AI BACKCOURT] teamName=" + teamName + ", player.x=" + player.x + ", targetX=" + spots.frontcourt_entry.x);
        ...
        if (isStuck) {
            ...
            if (teammate && teammate.playerData) {
                ...
                if (teammateInFrontcourt || teammateDistToMid < myDistToMid) {
                    ...
                    animatePass(player, teammate, null, null, systems);
                    return;
                }
            }
        }
    ...
    if (isHelpCollapsing(player, teamName) && Math.random() < GAME_BALANCE.AI.PASS_ON_HELP_COLLAPSE_PROB) {
        var teammate = getTeammate(player);
        if (teammate && teammate.playerData) {
    ...
    if (teammate && teammate.playerData && teammate.playerData.openForPass) {
        ...
        if (passDistance < GAME_BALANCE.AI.MAX_CLOSE_PASS_DISTANCE && teammateDistToBasket < GAME_BALANCE.AI.TEAMMATE_TO_BASKET_DISTANCE && Math.random() < GAME_BALANCE.AI.PASS_CLOSE_TO_BASKET_PROB) {
        ...
        if (closestDefender && defenderDist < 3) {
            ...
            if (playerData.shakeCooldown <= 0 && Math.random() < shakeChance) {
                if (attemptShake(player, systems)) {
                    ...
                    if (shotQuality > SHOT_PROBABILITY_THRESHOLD - 10 && Math.random() < GAME_BALANCE.AI.SHOOT_AFTER_SHAKE_PROB) {
                        attemptShot(systems);
                    }
                    return;
                }
