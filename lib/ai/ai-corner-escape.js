...
/**
 * Get target position to escape from corner
 * @param {Object} player - Player sprite
 ...
 */
function getCornerEscapeTarget(player, teamName, systems) {
    ...
    var isBallHandler = (ballCarrier === player);
 ...
 * @param {Object} systems - Systems object for dependency injection
 */
function executeCornerEscape(player, teamName, systems) {
    if (!player || !player.playerData) return;

    var escapeTarget = getCornerEscapeTarget(player, teamName, systems);
    ...
    moveAITowards(player, escapeTarget.x, escapeTarget.y, systems);
}
