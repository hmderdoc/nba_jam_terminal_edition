...
function aiDefenseOnBall(player, teamName, ballCarrier, context, systems) {
    ...
    // WAVE 24: Press vs Retreat Decision
    // Evaluate whether to pursue full court press or fall back to halfcourt
    var pressDecision = evaluatePressDecision(player, ballCarrier, teamName, systems);
    
    if (!pressDecision.shouldPress && pressDecision.retreatTarget) {
        // Don't press - retreat to safer position near halfcourt
        playerData.aiLastAction = "retreat_halfcourt";
        
        // Use momentum smoothing for retreat movement too
        var retreatResponsiveness = 0.3; // Moderate speed to retreat position
        var smoothedRetreat = applyDefenderMomentum(player, pressDecision.retreatTarget.x, 
                                                     pressDecision.retreatTarget.y, 
                                                     retreatResponsiveness, false);
        steerToward(player, smoothedRetreat.x, smoothedRetreat.y, 
                   aiDefenseOnBallValue("containSpeed", 2.5), systems);
        return; // Skip normal containment logic
    }
