...
/**
 * Calculate defender's responsiveness based on court position.
 * Defenders far from their own basket (full court press) react slower,
 * giving offense an advantage to get by them.
 * 
 * @param {Object} defender - Defender sprite
 * @param {string} defenderTeam - "teamA" or "teamB"
 * @returns {number} Responsiveness value (0.05 - 0.85, lower = slower reaction)
 */
function calculateDefenderResponsiveness(defender, defenderTeam) {
    if (!defender) return 0.3; // Default fallback
    
    var courtPos = (typeof AI_CONSTANTS === "object" && AI_CONSTANTS.DEFENSE_ON_BALL && 
                    AI_CONSTANTS.DEFENSE_ON_BALL.COURT_POSITION) 
        ? AI_CONSTANTS.DEFENSE_ON_BALL.COURT_POSITION 
        : null;
    
    if (!courtPos) {
        // Fallback defaults if constants not loaded
        return 0.25;
    }
    
    // Get defender's own basket (the one they're protecting)
    var ownBasket = defenderTeam === "teamA"
        ? { x: BASKET_LEFT_X, y: BASKET_LEFT_Y }
        : { x: BASKET_RIGHT_X, y: BASKET_RIGHT_Y };
    
    // Calculate distance from defender to their own basket
    var distToOwnBasket = distanceBetweenPoints(defender.x, defender.y, ownBasket.x, ownBasket.y);
    
    // Determine court zone and responsiveness
    var frontcourtMax = courtPos.frontcourtMaxDistance || 20;
    var midcourtMax = courtPos.midcourtMaxDistance || 40;
    var frontResp = courtPos.frontcourtResponsiveness || 0.35;
    var midResp = courtPos.midcourtResponsiveness || 0.25;
    var backResp = courtPos.backcourtResponsiveness || 0.12;
    
    var responsiveness;
    if (distToOwnBasket <= frontcourtMax) {
        // Frontcourt - quick reactions (protecting the paint)
        responsiveness = frontResp;
    } else if (distToOwnBasket <= midcourtMax) {
        // Midcourt - interpolate between front and mid
        var midProgress = (distToOwnBasket - frontcourtMax) / (midcourtMax - frontcourtMax);
        responsiveness = frontResp - (frontResp - midResp) * midProgress;
    } else {
        // Backcourt (full court press territory) - sluggish reactions
        // Further interpolate toward backResp the deeper they go
        var backProgress = Math.min(1, (distToOwnBasket - midcourtMax) / 20);
        responsiveness = midResp - (midResp - backResp) * backProgress;
    }
    
    return clamp(responsiveness, 0.05, 0.85);
}

/**
 * Track ball handler direction changes and return penalty info for defender.
 * When ball handler changes direction, defender gets a reaction delay penalty.
 * 
 * @param {Object} defender - Defender sprite
 * @param {Object} ballCarrier - Ball carrier sprite
 * @returns {Object} { inPenalty: boolean, penaltyResponsiveness: number }
 */
function trackDirectionChangePenalty(defender, ballCarrier) {
    if (!defender || !defender.playerData || !ballCarrier) {
        return { inPenalty: false, penaltyResponsiveness: 0.3 };
    }
    
    var data = defender.playerData;
    var courtPos = (typeof AI_CONSTANTS === "object" && AI_CONSTANTS.DEFENSE_ON_BALL && 
                    AI_CONSTANTS.DEFENSE_ON_BALL.COURT_POSITION) 
        ? AI_CONSTANTS.DEFENSE_ON_BALL.COURT_POSITION 
        : null;
    
    var penaltyFrames = (courtPos && courtPos.directionChangePenaltyFrames) || 6;
    var penaltyResp = (courtPos && courtPos.directionChangeReducedResponse) || 0.08;
    
    // Initialize tracking state if needed
    if (data.trackedBallCarrierLastX === undefined) {
        data.trackedBallCarrierLastX = ballCarrier.x;
        data.trackedBallCarrierLastY = ballCarrier.y;
        data.trackedBallCarrierDirX = 0;
        data.trackedBallCarrierDirY = 0;
        data.directionChangePenaltyTimer = 0;
        return { inPenalty: false, penaltyResponsiveness: penaltyResp };
    }
    
    // Calculate current ball carrier movement direction
    var currDirX = ballCarrier.x - data.trackedBallCarrierLastX;
    var currDirY = ballCarrier.y - data.trackedBallCarrierLastY;
    
    // Detect direction change (sign flip in either axis with movement)
    var xFlip = (data.trackedBallCarrierDirX !== 0 && currDirX !== 0 && 
                 Math.sign(currDirX) !== Math.sign(data.trackedBallCarrierDirX));
    var yFlip = (data.trackedBallCarrierDirY !== 0 && currDirY !== 0 && 
                 Math.sign(currDirY) !== Math.sign(data.trackedBallCarrierDirY));
    
    // Trigger penalty on direction change
    if (xFlip || yFlip) {
        data.directionChangePenaltyTimer = penaltyFrames;
        debugLog("[DEF REACTION] Direction change detected for defender " + 
            (data.name || "?") + ", penalty=" + penaltyFrames + " frames");
    }
    
    // Update tracking state
    if (currDirX !== 0) data.trackedBallCarrierDirX = currDirX;
    if (currDirY !== 0) data.trackedBallCarrierDirY = currDirY;
    data.trackedBallCarrierLastX = ballCarrier.x;
    data.trackedBallCarrierLastY = ballCarrier.y;
    
    // Decrement penalty timer
    var inPenalty = false;
    if (data.directionChangePenaltyTimer > 0) {
        data.directionChangePenaltyTimer--;
        inPenalty = true;
    }
    
    return { inPenalty: inPenalty, penaltyResponsiveness: penaltyResp };
}

/**
 * Check if ball handler has a blowby opportunity against defender.
 * Returns true if ball handler should get separation based on momentum advantage.
 * 
 * @param {Object} defender - Defender sprite
 * @param {Object} ballCarrier - Ball carrier sprite  
 * @param {string} defenderTeam - "teamA" or "teamB"
 * @returns {boolean} True if blowby should occur
 */
function checkBlowbyOpportunity(defender, ballCarrier, defenderTeam) {
    if (!defender || !ballCarrier || !defender.playerData) {
        return false;
    }
    ...
    if (!courtPos) return false;
    
    // Calculate ball carrier speed (using prevX/prevY if available)
    var carrierSpeed = 0;
    if (ballCarrier.prevX !== undefined && ballCarrier.prevY !== undefined) {
        var dx = ballCarrier.x - ballCarrier.prevX;
        var dy = ballCarrier.y - ballCarrier.prevY;
        carrierSpeed = Math.sqrt(dx * dx + dy * dy);
    }
    
    // Need minimum speed for blowby
    var speedThreshold = courtPos.blowbySpeedThreshold || 2.5;
    if (carrierSpeed < speedThreshold) {
        return false;
    }
    
    // Get defender's distance from own basket to determine blowby chance
    var ownBasket = defenderTeam === "teamA"
        ? { x: BASKET_LEFT_X, y: BASKET_LEFT_Y }
        : { x: BASKET_RIGHT_X, y: BASKET_RIGHT_Y };
    var distToOwnBasket = distanceBetweenPoints(defender.x, defender.y, ownBasket.x, ownBasket.y);
    ...
    var blowbyChance;
    if (distToOwnBasket <= frontcourtMax) {
        blowbyChance = courtPos.blowbyChanceFrontcourt || 0.05;
    } else if (distToOwnBasket <= midcourtMax) {
        blowbyChance = courtPos.blowbyChanceMidcourt || 0.15;
    } else {
        blowbyChance = courtPos.blowbyChanceBackcourt || 0.35;
    }
    
    // Turbo bonus for ball carrier
    if (ballCarrier.playerData && ballCarrier.playerData.turboActive) {
        blowbyChance += 0.10;
    }
    
    // Roll for blowby
    if (Math.random() < blowbyChance) {
        debugLog("[DEF BLOWBY] Ball carrier got blowby! chance=" + blowbyChance.toFixed(2) + 
            " speed=" + carrierSpeed.toFixed(1) + " defDistToBasket=" + distToOwnBasket.toFixed(1));
        return true;
    }
...
/**
 * Evaluate whether defender should press (full court) or retreat (fall back to halfcourt).
 * Considers: catchup potential (speed/turbo), game situation (score/time), threat assessment,
 * and rubber banding status.
 * 
 * @param {Object} defender - Defender sprite
 * @param {Object} ballCarrier - Ball carrier sprite
 * @param {string} defenderTeam - "teamA" or "teamB"
 * @param {Object} systems - Systems object for state access
 * @returns {Object} { shouldPress: boolean, pressScore: number, retreatTarget: {x, y} or null }
 */
function evaluatePressDecision(defender, ballCarrier, defenderTeam, systems) {
    if (!defender || !ballCarrier || !systems || !systems.stateManager) {
        return { shouldPress: true, pressScore: 1.0, retreatTarget: null };
    }
    
    var stateManager = systems.stateManager;
    var pressConfig = (typeof AI_CONSTANTS === "object" && AI_CONSTANTS.DEFENSE_ON_BALL && 
                       AI_CONSTANTS.DEFENSE_ON_BALL.PRESS_DECISION) 
        ? AI_CONSTANTS.DEFENSE_ON_BALL.PRESS_DECISION 
        : null;
    ...
    // Get defender's own basket
    var ownBasket = defenderTeam === "teamA"
        ? { x: BASKET_LEFT_X, y: BASKET_LEFT_Y }
        : { x: BASKET_RIGHT_X, y: BASKET_RIGHT_Y };
    
    // Check minimum press distance - don't evaluate press if ball handler is close to defender's basket
    var ballCarrierDistToDefBasket = distanceBetweenPoints(ballCarrier.x, ballCarrier.y, ownBasket.x, ownBasket.y);
    var minPressDistance = pressConfig.minPressDistance || 30;
    
    if (ballCarrierDistToDefBasket < minPressDistance) {
        // Ball handler is in defender's frontcourt - always defend normally, no retreat needed
        return { shouldPress: true, pressScore: 1.0, retreatTarget: null };
    }
    
    // Start calculating press score
    var pressScore = 0;
    
    // === 1. CATCHUP POTENTIAL ===
    // Compare defender's speed+turbo vs ball carrier's speed+turbo
    var defenderData = defender.playerData;
    var carrierData = ballCarrier.playerData;
    
    var defSpeed = defenderData ? (getEffectiveAttribute(defenderData, ATTR_SPEED) || 5) : 5;
    var defTurbo = defenderData ? (defenderData.turbo || 0) : 0;
    var carrierSpeed = carrierData ? (getEffectiveAttribute(carrierData, ATTR_SPEED) || 5) : 5;
    var carrierTurbo = carrierData ? (carrierData.turbo || 0) : 0;
    
    var speedDiff = (defSpeed - carrierSpeed) * (pressConfig.catchupSpeedScale || 0.08);
    var turboDiff = (defTurbo - carrierTurbo) * (pressConfig.catchupTurboScale || 0.01);
    var catchupBonus = (speedDiff + turboDiff) * (pressConfig.catchupWeight || 0.15);
    pressScore += catchupBonus;
    
    // === 2. GAME SITUATION ===
    var score = stateManager.get("score") || { teamA: 0, teamB: 0 };
    var defenderScore = (defenderTeam === "teamA") ? score.teamA : score.teamB;
    var opponentScore = (defenderTeam === "teamA") ? score.teamB : score.teamA;
    var scoreDiff = defenderScore - opponentScore; // Positive = defender's team leading
    
    var timeRemaining = stateManager.get("timeRemaining") || 999;
    
    // Trailing urgency - more likely to press when behind
    if (scoreDiff < 0) {
        var pointsBehind = Math.abs(scoreDiff);
        pressScore += pressConfig.trailingUrgencyBonus || 0.25;
        
        // Desperation mode when trailing by a lot
        if (pointsBehind >= (pressConfig.desperationThreshold || 10)) {
            pressScore += pressConfig.desperationBonus || 0.35;
        }
        
        // Time urgency when trailing - must create turnovers
        if (timeRemaining <= (pressConfig.timeUrgencyThreshold || 30)) {
            var timeUrgency = ((pressConfig.timeUrgencyThreshold || 30) - timeRemaining) * 
                              (pressConfig.timeUrgencyWeight || 0.02);
            pressScore += timeUrgency;
        }
        
        // Critical time + trailing = maximum urgency
        if (timeRemaining <= (pressConfig.criticalTimeThreshold || 15)) {
            pressScore += pressConfig.criticalTimeBonus || 0.20;
        }
    } else if (scoreDiff >= (pressConfig.leadingComfortThreshold || 15)) {
        // Leading comfortably - can afford to gamble (low risk)
        pressScore += pressConfig.leadingComfortBonus || 0.10;
    }
    
    // === 3. THREAT ASSESSMENT ===
    // Good shooters are more dangerous if they get an open look from a blowby
    var shooter3pt = carrierData ? (getEffectiveAttribute(carrierData, ATTR_3PT) || 5) : 5;
    var shooterThreshold = pressConfig.shooterThreatThreshold || 7;
    
    if (shooter3pt > shooterThreshold) {
        var threatPenalty = (shooter3pt - shooterThreshold) * (pressConfig.shooterThreatWeight || 0.04);
        pressScore -= threatPenalty; // Reduce press score against good shooters
    }
    
    // === 4. RUBBER BANDING ===
    var rubberBandActive = stateManager.get("rubberBandingActive");
    var rubberBandTeam = stateManager.get("rubberBandingTeam");
    
    if (rubberBandActive && rubberBandTeam === defenderTeam) {
        pressScore += pressConfig.rubberBandBonus || 0.15;
    }
    
    // === DECISION ===
    var threshold = pressConfig.pressThreshold || 0.4;
    var shouldPress = pressScore >= threshold;
    
    // Calculate retreat target if not pressing
    var retreatTarget = null;
    if (!shouldPress) {
        var retreatDistance = pressConfig.retreatDistance || 25;
        var retreatJitter = pressConfig.retreatJitter || 4;
        
        // Calculate retreat position: defender's basket + retreatDistance toward midcourt
        var midcourtX = Math.floor(COURT_WIDTH / 2);
        var dirToMidcourt = (ownBasket.x < midcourtX) ? 1 : -1;
        
        retreatTarget = {
            x: clampToCourtX(ownBasket.x + dirToMidcourt * retreatDistance + (Math.random() - 0.5) * retreatJitter),
            y: clampToCourtY(BASKET_LEFT_Y + (Math.random() - 0.5) * retreatJitter * 2) // Center vertically with jitter
        };
    }
    
    // Debug logging (throttled - only when decision changes or periodically)
    if (!defender.playerData.lastPressDecision || defender.playerData.lastPressDecision !== shouldPress) {
        debugLog("[PRESS DECISION] " + (defenderData ? defenderData.name : "?") + 
            " shouldPress=" + shouldPress + " score=" + pressScore.toFixed(2) +
            " (catchup=" + catchupBonus.toFixed(2) + 
            ", scoreDiff=" + scoreDiff + 
            ", time=" + timeRemaining +
            ", shooter3pt=" + shooter3pt + 
            ", rubberBand=" + (rubberBandActive && rubberBandTeam === defenderTeam) + ")");
        defender.playerData.lastPressDecision = shouldPress;
    }
