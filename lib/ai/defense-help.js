...
function aiDefenseHelp(player, teamName, ballCarrier, context, systems) {
    ...
    if (distBallToBasket < aiDefenseHelpValue("ballCloseDistance", 15)) {
        ...
        // BUNCHING DETECTION: Help defender shoves ball handler in paint if very close
        var distToCarrier = distanceBetweenPoints(player.x, player.y, ballCarrier.x, ballCarrier.y);
        var helpShoveDistance = aiDefenseHelpValue("helpShoveDistance", 3.5);
        var helpShoveChance = aiDefenseHelpValue("helpShoveChance", 0.45);
        var helpMinTurbo = aiDefenseHelpValue("helpMinTurbo", 10);
        
        if (distToCarrier <= helpShoveDistance && playerData.shoveAttemptCooldown <= 0 && 
            playerData.turbo >= helpMinTurbo) {
            var powerSkill = getEffectiveAttribute(playerData, ATTR_POWER) || 5;
            var adjustedChance = helpShoveChance + (powerSkill - 5) * 0.03;
            
            // Higher chance when ball handler has picked up dribble
            if (dribbleDead) {
                adjustedChance += 0.15;
            }
            
            if (Math.random() < adjustedChance) {
                debugLog("[AI DEFENSE HELP] " + playerData.name + " help defender shoving ball handler in paint");
                if (attemptShove(player, ballCarrier, systems)) {
                    playerData.aiLastAction = "help_paint_shove";
                    return;
                }
            }
        }
