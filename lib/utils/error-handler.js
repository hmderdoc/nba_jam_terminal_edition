...
// Use absolute paths to nba_jam data directory
var GAME_DATA_DIR = js.exec_dir + "data/";
var ERROR_LOG_PATH = GAME_DATA_DIR + "error.log";
var STATE_SNAPSHOT_DIR = GAME_DATA_DIR + "error-snapshots/";
...
function captureGameStateSnapshot(stateManager) {
    ...
    var snapshot = {
        phase: phase ? JSON.parse(safeStringify(phase)) : null,
        ...
        score: {
            teamA: stateManager.get('teamAScore'),
            teamB: stateManager.get('teamBScore')
        },
        ballCarrier: null,
        ballPosition: {
            x: stateManager.get('ballX'),
            y: stateManager.get('ballY')
        },
        shotInProgress: stateManager.get('shotInProgress'),
        reboundActive: stateManager.get('reboundActive'),
        inbounding: stateManager.get('inbounding'),
        consecutivePoints: stateManager.get('consecutivePoints')
    };
    ...
    if (ballCarrier) {
        try {
            snapshot.ballCarrier = {
                ...
                x: ballCarrier.x,
                y: ballCarrier.y,
                team: (typeof getPlayerTeamName === "function") ? getPlayerTeamName(ballCarrier) : "unknown"
            };
        } catch (e) {
...
function writeErrorLog(logEntry) {
    try {
        ...
        if (!logFile.open("a")) {
            ...
            print("Tried path: " + ERROR_LOG_PATH);
            print("Current dir: " + js.exec_dir);
            return;
        }

        // Format log entry with circular reference handling
        var contextStr = "{}";
        try {
            contextStr = safeStringify(logEntry.context);
        } catch (e) {
        ...
        logFile.close();
        print("Successfully wrote to error.log");
    } catch (e) {
        print("EXCEPTION writing error log: " + e.toString());
    }
}
...
function writeSnapshotFile(logEntry) {
    try {
        ...
        if (!snapshotFile.open("w")) {
            print("WARNING: Could not write snapshot file: " + filename);
            return;
        }

        // Use safe stringify for circular references
        snapshotFile.write(safeStringify(logEntry));
        snapshotFile.close();

        print("Snapshot saved to: " + filename);
    } catch (e) {
        print("EXCEPTION writing snapshot: " + e.toString());
    }
}
...
function generateReproductionTest(logEntry) {
    try {
        ...
        // Safe snapshot stringify
        var snapshotStr = "{}";
        try {
            snapshotStr = safeStringify(logEntry.gameStateSnapshot);
        } catch (e) {
            snapshotStr = "{error: 'Could not capture snapshot'}";
        }

        // Safe context stringify
        var contextStr = "{}";
        try {
            contextStr = safeStringify(logEntry.context);
        } catch (e) {
            contextStr = "{error: 'Could not capture context'}";
        }
        ...
        var testCode = [
            ...
            "    // TODO: Add assertions",
            "    ",
            "    print('Context: " + contextStr.replace(/'/g, "\\'") + "');",
            "}",
            "",
        ...
        if (testFile.open("w")) {
            ...
            print("WARNING: Could not write test file: " + testFilename);
        }
    } catch (e) {
        print("EXCEPTION generating reproduction test: " + e.toString());
    }
}
...
function setupGlobalErrorHandler() {
    ...
    // Silently initialized - logs go to ERROR_LOG_PATH
}
