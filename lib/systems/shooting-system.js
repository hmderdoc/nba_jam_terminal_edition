...
function createShootingSystem(deps) {
    ...
    function calculateShotProbability(shooter, targetX, targetY, closestDefender) {
        ...
        var shooterTeamName = (typeof helpers.getPlayerTeamName === "function")
            ? helpers.getPlayerTeamName(shooter)
            : null;
        var rubberBandModifiers = (typeof helpers.getRubberBandingModifiersForTeam === "function")
            ? helpers.getRubberBandingModifiersForTeam(shooterTeamName)
            : null;
        var shotMultiplier = 1;
        var probabilityCap = 95;
        var contestBonus = 0;
        if (rubberBandModifiers) {
            if (typeof rubberBandModifiers.shotMultiplier === "number") {
                shotMultiplier = rubberBandModifiers.shotMultiplier;
            }
            if (typeof rubberBandModifiers.probabilityCap === "number") {
                probabilityCap = rubberBandModifiers.probabilityCap;
            }
            if (typeof rubberBandModifiers.contestBonus === "number") {
                contestBonus = rubberBandModifiers.contestBonus;
            }
        }
        if (probabilityCap > 100) probabilityCap = 100;
        if (probabilityCap < 0) probabilityCap = 0;

        // Add buff-based shot multiplier (Satan/boss special abilities)
        if (typeof BuffSystem !== "undefined" && BuffSystem.getShotMultiplier && shooter) {
            var buffShotMult = BuffSystem.getShotMultiplier(shooter);
            if (buffShotMult && buffShotMult !== 1) {
                shotMultiplier *= buffShotMult;
            }
        }
        ...
        if (defenderPenalty > 0 && contestBonus !== 0) {
            defenderPenalty = Math.max(0, defenderPenalty - contestBonus);
        }
        ...
        finalProbability *= shotMultiplier;

        if (finalProbability < 0) finalProbability = 0;
        if (finalProbability > probabilityCap) finalProbability = probabilityCap;
    ...
    function attemptShot(shooter, options) {
        ...
        // Wave 24 Predictive Action: If this is a predictive client-side call,
        // we only want to start the animation, not determine the outcome.
        if (options.predictive) {
            log("PREDICTIVE SHOT: Client is starting shot animation locally.");
            // Here, we can trigger a generic shooting animation on the player sprite
            // without involving the ball or the phase system yet. This provides
            // immediate visual feedback. The authoritative animation will come
            // from the coordinator's broadcast.
            if (shooter.playerData) {
                shooter.playerData.isShooting = true; // A simple state for rendering
            }
            // We don't calculate outcome, just show the player is shooting.
            return { success: true, reason: 'predictive_client_start' };
        }
        ...
        // CRITICAL: Don't allow shots while ball is animating (pass/dunk in progress)
        // This prevents the nonsensical situation where a player shoots while passing
        if (animationSystem && typeof animationSystem.isBallAnimating === "function" && animationSystem.isBallAnimating()) {
            log("Shot BLOCKED: ball is animating (pass/shot/dunk in progress)");
        ...
        // Reset stuck timer - player is taking a shot, no longer stuck
        state.set('ballHandlerStuckTimer', 0, 'shot_attempt');
        ...
        // OUT-OF-BOUNDS CHECK
        var margin = 2;
        var COURT_WIDTH = rules.COURT_WIDTH || 80;
        ...
        // ARCHITECTURAL FIX: Calculate ball position directly from shooter sprite
        // Don't read from state - that's stale/dependent on render order
        // Ball is held at shooter's position + offset (same as rendering logic)
        var ballOffsetX = 2;
        var ballOffsetY = 0;
        var shotStartX = shooter.x + ballOffsetX;
        var shotStartY = shooter.y + ballOffsetY + 2;

        log("SHOT DEBUG: shotStartX=" + shotStartX + " (calculated from shooter.x=" + shooter.x + ")" +
            ", shotStartY=" + shotStartY + " (calculated from shooter.y=" + shooter.y + ")");

        // Determine target basket
        var shooterTeam = helpers.getPlayerTeamName(shooter) || state.get("currentTeam");
        var BASKET_RIGHT_X = rules.BASKET_RIGHT_X || 77;
        var BASKET_RIGHT_Y = rules.BASKET_RIGHT_Y || 9;
        var BASKET_LEFT_X = rules.BASKET_LEFT_X || 4;
        var BASKET_LEFT_Y = rules.BASKET_LEFT_Y || 9;
        ...
        var rubberBandModifiersForTeam = (typeof helpers.getRubberBandingModifiersForTeam === "function")
            ? helpers.getRubberBandingModifiersForTeam(shooterTeam)
            : null;

        log("SHOT DEBUG: shooterTeam=" + shooterTeam + ", targetX=" + targetX + ", targetY=" + targetY);
        ...
        if (attemptType === "dunk") {
            ...
            var dunkProbabilityCap = 98;
            if (rubberBandModifiersForTeam) {
                if (typeof rubberBandModifiersForTeam.shotMultiplier === "number") {
                    chance *= rubberBandModifiersForTeam.shotMultiplier;
                }
                if (typeof rubberBandModifiersForTeam.probabilityCap === "number") {
                    dunkProbabilityCap = rubberBandModifiersForTeam.probabilityCap;
                }
            }
            if (dunkProbabilityCap > 100) dunkProbabilityCap = 100;
            if (dunkProbabilityCap < 0) dunkProbabilityCap = 0;
            if (chance < 0) chance = 0;
            if (chance > dunkProbabilityCap) chance = dunkProbabilityCap;

            var roll = Math.random() * 100;
            made = roll < chance;

            // Select dunk style and build flight plan
            var style = "default";
            var flightPlan = null;
            if (typeof selectDunkStyle === "function") {
                style = selectDunkStyle(playerData, dunkInfo);
            }
            if (typeof generateDunkFlight === "function") {
                flightPlan = generateDunkFlight(shooter, dunkInfo, targetX, targetY, style);
            }
            ...
            if (helpers.setPhase) {
                helpers.setPhase("SHOT_QUEUED", {
                    ...
                    style: style,
                    animDuration: flightPlan ? (flightPlan.totalDurationMs || 600) : 600,
                    reboundBounces: []
                }, 0);
            }
        ...
        // Buff-based three-point bonus (Satan/boss special abilities)
        if (is3Pointer && typeof BuffSystem !== "undefined" && BuffSystem.getThreePointBonus && shooter) {
            var threePointBuff = BuffSystem.getThreePointBonus(shooter);
            if (threePointBuff) {
                baseChance += threePointBuff * 100; // Convert 0.10 to +10
            }
        }
        ...
        // Apply buff-based shot multiplier (Satan/boss special abilities)
        if (typeof BuffSystem !== "undefined" && BuffSystem.getShotMultiplier && shooter) {
            var buffShotMult = BuffSystem.getShotMultiplier(shooter);
            if (buffShotMult && buffShotMult !== 1) {
                chance *= buffShotMult;
