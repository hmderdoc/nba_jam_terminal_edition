...
function getOnFireTrailAttr(player, stepOffset, defaultAttr, systems) {
    ...
    if (!player || !player.playerData || !player.playerData.onFire)
        return baseAttr;
    var fireFg = getFireFg(stepOffset, systems);
...
function getTeamBaselineColors(teamKey, systems) {
    // No color assumptions based on teamA/teamB - colors come from rosters.ini
    // Use neutral fallbacks that make missing colors obvious
    var fallbackBg = BG_BLACK;
    var fallbackFg = WHITE;
    var teamColors = null;
    if (systems && systems.stateManager) {
        var allTeamColors = systems.stateManager.get('teamColors');
        debugLog("[GET BASELINE COLORS] teamKey=" + teamKey + ", allTeamColors=" + (allTeamColors ? "FOUND" : "NULL"));
        if (allTeamColors) {
            debugLog("[GET BASELINE COLORS] allTeamColors.teamA=" + (allTeamColors.teamA ? "EXISTS" : "MISSING") + ", allTeamColors.teamB=" + (allTeamColors.teamB ? "EXISTS" : "MISSING"));
        }
        teamColors = (allTeamColors && allTeamColors[teamKey]) ? allTeamColors[teamKey] : null;
        if (teamColors) {
            debugLog("[GET BASELINE COLORS] " + teamKey + " colors: fg=" + teamColors.fg + ", bg=" + teamColors.bg);
        } else {
            debugLog("[GET BASELINE COLORS] " + teamKey + " colors: NONE FOUND, using fallbacks");
        }
    } else {
        debugLog("[GET BASELINE COLORS] No stateManager available!");
    }
