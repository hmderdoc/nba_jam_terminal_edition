...
function renderPlayerLabel(player, options, systems) {
    ...
    // Start with team colors as the default (same as single player)
    var baseFg = (teamColors && typeof teamColors.fg === "number") ? teamColors.fg : WHITE;
    var baseBg = (teamColors && typeof teamColors.bg === "number") ? teamColors.bg : WAS_BROWN;
    
    // Check for LORB custom nametag colors - override team colors if player has set custom values
    var lorbData = player.playerData.lorbData;
    if (lorbData) {
        if (typeof lorbData.nametagFg === "number") {
            baseFg = lorbData.nametagFg;
        }
        if (typeof lorbData.nametagBg === "number") {
            baseBg = lorbData.nametagBg;
        }
    }
    ...
    if (highlightCarrier && isCarrier && (teamKey === "teamA" || teamKey === "teamB")) {
        // For ball carrier, check for LORB highlight colors first
        if (lorbData && typeof lorbData.nametagHiFg === "number") {
            textFg = lorbData.nametagHiFg;
            textBg = (typeof lorbData.nametagHiBg === "number") ? lorbData.nametagHiBg : baseBg;
            baselineColors = { fg: textFg, bg: textBg };  // Mark as handled
        } else {
            // For carrier highlight, use bg_alt (highlight color) if available, otherwise fall back to bg
            var carrierBg = (teamColors && typeof teamColors.bg_alt === "number") ? teamColors.bg_alt : baseBg;
            textFg = baseFg;  // Keep same foreground
            textBg = carrierBg;  // Use highlight background
            baselineColors = { fg: textFg, bg: textBg };
        }
    }
