...
function AnimationSystem() {
                ...
                if (anim.type === "rebound" && anim.currentBounce < anim.bounces.length - 1) {
                    ...
                    if (this.systems && this.systems.stateManager) {
                        ...
                        if (reboundActive) {
                            ...
                            anim.idleBounceHeight = 1.5;  // Gentle bounce height
                            anim.msPerStep = 50;          // Slightly slower cadence
                            anim.maxSteps = 8;            // 8 steps per idle bounce (400ms cycle)
                            anim.step = 0;
                            anim.nextStepTime = now + anim.msPerStep;
                            anim.trailPositions = [];     // Reset trail tracking
                            debugLog("[ANIM] Rebound converted to idle bounce at (" + anim.centerX + "," + anim.centerY + ")");
                        } else {
                            debugLog("[ANIM] Completing " + anim.type + " animation, affectsBall=" + anim.affectsBall);
                            this.completeAnimation(anim);
                            completedIndices.push(i);
    ...
    this.updateIdleBounceAnimation = function (anim) {
        // Gentle bouncing at rest position while scramble is active
        var t = anim.step / anim.maxSteps;
        var bounceHeight = anim.idleBounceHeight || 1.5;
        ...
        // Very light trail for idle bounces (just a dot)
        if (anim.step > 0 && anim.step % 2 === 0 && trailFrame) {
            var prevT = (anim.step - 1) / anim.maxSteps;
            var prevY = Math.round(anim.centerY - Math.sin(prevT * Math.PI) * bounceHeight);
            prevY = clamp(prevY, 1, COURT_HEIGHT);

            trailFrame.setData(x - 1, prevY - 1, ".", DARKGRAY | WAS_BROWN, false);

            if (anim.trailPositions) {
                anim.trailPositions.push({ x: x - 1, y: prevY - 1 });
            }
        }
    };
