...
function AnimationSystem() {
    ...
    this.updateShotAnimation = function (anim) {
                ...
                if (blockDist < 4) { // Blocker must be very close
                    ...
                    if (typeof getEffectiveAttribute === "function" && blocker.playerData) {
                        ...
                        if (roll < blockChance) {
                            ...
                            var deflectionFrames = 10;  // More frames for visible SWAT! trail
        ...
        if (anim.step > 0 && trailFrame) {
            ...
            var trailChar = ".";
            if (anim.blocked && typeof anim.deflectionStartStep === "number") {
                // Blocked shot: bright red trail with SWAT text
                trailAttr = LIGHTRED | BG_BLACK;
                // Draw SWAT letters along the deflection path
                var swatText = "SWAT!";
                var deflectStep = anim.step - anim.deflectionStartStep;
                if (deflectStep >= 0 && deflectStep < swatText.length) {
                    trailChar = swatText.charAt(deflectStep);
                }
            } else {
            ...
            if (typeof debugLog === "function" && anim.step === 1) {
                debugLog("[TRAIL SHOT] Drew trail at x=" + (prevBallX - 1) + " y=" + (prevBallY - 1) + " attr=" + trailAttr + (anim.blocked ? " (BLOCKED)" : ""));
            }

            // Track position for cleanup
            if (anim.trailPositions) {
                anim.trailPositions.push({ x: prevBallX - 1, y: prevBallY - 1 });
            }
        }
    };
