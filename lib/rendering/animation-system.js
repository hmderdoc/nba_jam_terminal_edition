...
function AnimationSystem() {
    ...
    this.updateShotAnimation = function (anim) {
                ...
                if (blockDist < 4) { // Blocker must be very close
                    ...
                    if (typeof getEffectiveAttribute === "function" && blocker.playerData) {
                        ...
                        if (roll < blockChance) {
                            ...
                            // Deflection distance: 6-10 units away from block point
                            var deflectDistance = 6 + Math.round(Math.random() * 4);
                            ...
                            // Stop shot animation early (like dunks) - add frames for deflection visual
                            var deflectionFrames = 20;  // More frames for visible SWAT! trail
        ...
        if (anim.step > 0 && trailFrame) {
            ...
            // For blocked shots during deflection, draw at current ball position with SWAT text
            if (anim.blocked && typeof anim.deflectionStartStep === "number" && anim.step >= anim.deflectionStartStep) {
                // Draw SWAT at current ball position during deflection
                trailX = ballX;
                trailY = ballY;
            } else if (anim.blocked && typeof anim.deflectionStartStep === "number" && anim.step > anim.deflectionStartStep) {
                // Deflection trail - calculate previous deflection position
            ...
            if (anim.blocked && typeof anim.deflectionStartStep === "number") {
                ...
                // Cycle through SWAT letters for entire trail
                var swatText = "SWAT";
                var deflectStep = anim.step - anim.deflectionStartStep;
                trailChar = swatText.charAt(deflectStep % swatText.length);
                if (typeof debugLog === "function") {
                    debugLog("[SWAT TRAIL] step=" + anim.step + ", deflectStep=" + deflectStep + ", char='" + trailChar + "', pos=(" + trailX + "," + trailY + ")");
                }
            } else {
