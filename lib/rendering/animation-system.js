...
function AnimationSystem() {
        ...
        for (var i = 0; i < this.animations.length; i++) {
            ...
            if (anim.step < anim.maxSteps && now >= anim.nextStepTime) {
                ...
                if (anim.type === "shot") {
                    ...
                    // Use slower timing during deflection phase
                    if (anim.blocked && anim.deflectionMsPerStep) {
                        anim.msPerStep = anim.deflectionMsPerStep;
                    }
    ...
    this.updateShotAnimation = function (anim) {
            ...
            if (activeBlock && blockJumpTimer > 0) {
                ...
                if (blockDist < 4) { // Blocker must be very close
                    ...
                    if (typeof getEffectiveAttribute === "function" && blocker.playerData) {
                        ...
                        var baseMinChance = 20;
                        var baseMaxChance = 95;
                        var blockChance = getEffectiveAttribute(blocker.playerData, ATTR_BLOCK) * 8 + baseMinChance;
                        var blockerTeam = (typeof getPlayerTeamName === "function") ? getPlayerTeamName(blocker) : null;
                        var rbSystem = this.systems && this.systems.rubberBandingSystem;
                        var rbModifiers = (rbSystem && typeof rbSystem.getTeamModifiers === "function")
                            ? rbSystem.getTeamModifiers(blockerTeam)
                            : null;
                        var blockBonus = (rbModifiers && typeof rbModifiers.blockBonus === "number") ? rbModifiers.blockBonus : 0;

                        // Add buff-based block bonus (Satan/Iceman special abilities)
                        if (typeof BuffSystem !== "undefined" && BuffSystem.getBlockBonus && blocker) {
                            blockBonus += BuffSystem.getBlockBonus(blocker);
                        }

                        if (blockChance < baseMinChance) blockChance = baseMinChance;
                        if (blockChance > baseMaxChance) blockChance = baseMaxChance;

                        if (blockBonus !== 0) {
                            var bonusPoints = blockBonus * 100;
                            blockChance += bonusPoints;
                            baseMaxChance += bonusPoints;
                        }

                        if (baseMaxChance > 99) baseMaxChance = 99;
                        if (blockChance > baseMaxChance) blockChance = baseMaxChance;
                        if (blockChance < baseMinChance) blockChance = baseMinChance;

                        var roll = Math.random() * 100;
                        if (typeof debugLog === "function") {
                            debugLog("[BLOCK CHECK] blockChance=" + blockChance + ", roll=" + roll + (blockBonus ? ", blockBonus=" + blockBonus.toFixed(3) : ""));
                        }
                        if (roll < blockChance) {
                            if (typeof debugLog === "function") {
                            ...
                            anim.blocked = true;
                            anim.made = false;
                            anim.blocker = blocker;
                            anim.blockPointX = clampedX;
                            anim.blockPointY = clampedY;

                            // Calculate deflection direction - ball bounces away from blocker
                            var deflectDirX = clampedX - blocker.x;
                            var deflectDirY = clampedY - blocker.y;
                            var deflectMag = Math.sqrt(deflectDirX * deflectDirX + deflectDirY * deflectDirY);
                            if (deflectMag < 0.1) {
                                // Fallback: random deflection if directly on blocker
                                deflectDirX = (Math.random() - 0.5) * 2;
                                deflectDirY = Math.random() * 0.5 + 0.5; // Bias downward
                                deflectMag = Math.sqrt(deflectDirX * deflectDirX + deflectDirY * deflectDirY);
                            }
                            deflectDirX /= deflectMag;
                            deflectDirY /= deflectMag;

                            // Deflection distance: 12-18 units away from block point for visible trail
                            var deflectDistance = 12 + Math.round(Math.random() * 6);
                            var deflectEndX = clamp(Math.round(clampedX + deflectDirX * deflectDistance), 2, COURT_WIDTH - 2);
                            var deflectEndY = clamp(Math.round(clampedY + deflectDirY * deflectDistance), 2, COURT_HEIGHT - 2);

                            // Store deflection point for phase-handler to create rebound at correct location
                            if (this.systems && this.systems.stateManager) {
                                this.systems.stateManager.set("blockDeflectionX", deflectEndX, "block_deflection");
                                this.systems.stateManager.set("blockDeflectionY", deflectEndY, "block_deflection");
                                this.systems.stateManager.set("shotBlocked", true, "block_detected");
                            }

                            // Deflection frames based on distance to ensure ~1 unit per frame
                            var deflectionFrames = deflectDistance;
                            anim.maxSteps = anim.step + deflectionFrames;
                            anim.deflectionStartStep = anim.step;
                            anim.deflectionEndX = deflectEndX;
                            anim.deflectionEndY = deflectEndY;
                            anim.deflectionMsPerStep = 80;  // Slow down deflection (3x normal shot speed)
                            ...
                            if (typeof debugLog === "function") {
                                ...
                                debugLog("[BLOCK DEFLECT] Ball deflecting from (" + clampedX + "," + clampedY + ") to (" + deflectEndX + "," + deflectEndY + ")");
                            }
                            if (typeof announceEvent === "function" && blockerTeam) {
                                announceEvent("block", {
                            ...
                            // Record for multiplayer sync
                            recordMultiplayerAnimation("block_deflect", {
                                blockPointX: clampedX,
                                blockPointY: clampedY,
                                deflectEndX: deflectEndX,
                                deflectEndY: deflectEndY,
                                blocker: getSpriteGlobalId(blocker)
                            });
                        }
                    }
                }
        ...
        // If blocked, animate deflection path instead of continuing to basket
        var ballX = clampedX;
        var ballY = clampedY;
        if (anim.blocked && typeof anim.deflectionStartStep === "number") {
            var deflectProgress = (anim.step - anim.deflectionStartStep) / (anim.maxSteps - anim.deflectionStartStep);
            deflectProgress = clamp(deflectProgress, 0, 1);
            var deflectDx = anim.deflectionEndX - anim.blockPointX;
            var deflectDy = anim.deflectionEndY - anim.blockPointY;
            // Small downward arc on deflection
            var deflectArc = Math.sin(deflectProgress * Math.PI) * 1.5;
            ballX = clamp(Math.round(anim.blockPointX + deflectDx * deflectProgress), 1, COURT_WIDTH);
            ballY = clamp(Math.round(anim.blockPointY + deflectDy * deflectProgress - deflectArc), 1, COURT_HEIGHT);
        }
        ...
        if (anim.step > 0 && trailFrame) {
            var trailX, trailY;
            var shouldDrawTrail = true;
            
            // For blocked shots during deflection, draw trail BEHIND the ball (at previous position)
            if (anim.blocked && typeof anim.deflectionStartStep === "number" && anim.step > anim.deflectionStartStep) {
                // Calculate PREVIOUS deflection position (one step behind current)
                var prevDeflectProgress = (anim.step - 1 - anim.deflectionStartStep) / (anim.maxSteps - anim.deflectionStartStep);
                prevDeflectProgress = clamp(prevDeflectProgress, 0, 1);
                var prevDeflectDx = anim.deflectionEndX - anim.blockPointX;
                var prevDeflectDy = anim.deflectionEndY - anim.blockPointY;
                var prevDeflectArc = Math.sin(prevDeflectProgress * Math.PI) * 1.5;
                trailX = clamp(Math.round(anim.blockPointX + prevDeflectDx * prevDeflectProgress), 1, COURT_WIDTH);
                trailY = clamp(Math.round(anim.blockPointY + prevDeflectDy * prevDeflectProgress - prevDeflectArc), 1, COURT_HEIGHT);
                
                // Only draw if position differs from last drawn position (avoid stacking letters)
                if (typeof anim.lastTrailX === "number" && trailX === anim.lastTrailX && trailY === anim.lastTrailY) {
                    shouldDrawTrail = false;
                }
            } else if (anim.blocked && typeof anim.deflectionStartStep === "number" && anim.step === anim.deflectionStartStep) {
                // First deflection frame - draw at block point
                trailX = anim.blockPointX;
                trailY = anim.blockPointY;
            } else {
                // Normal shot trail - draw at previous position
                var prevT = (anim.step - 1) / anim.maxSteps;
                trailX = Math.round(anim.startX + (dx * prevT));
                trailY = Math.round(anim.startY + (dy * prevT) - (Math.sin(prevT * Math.PI) * arcHeight));
                trailX = clamp(trailX, 1, COURT_WIDTH);
                trailY = clamp(trailY, 1, COURT_HEIGHT);
            }

            // Draw trail - RED for blocked shots with SWAT text, normal for regular shots
            var trailAttr;
            var trailChar = ".";
            if (anim.blocked && typeof anim.deflectionStartStep === "number" && shouldDrawTrail) {
                // Blocked shot: bright red trail with repeating SWAT text
                trailAttr = LIGHTRED | BG_BLACK;
                // Cycle through SWAT letters - use swatCharIndex to only advance when we actually draw
                var swatText = "SWAT";
                if (typeof anim.swatCharIndex !== "number") {
                    anim.swatCharIndex = 0;
                }
                trailChar = swatText.charAt(anim.swatCharIndex % swatText.length);
                anim.swatCharIndex++;
                // Track last position to avoid stacking letters
                anim.lastTrailX = trailX;
                anim.lastTrailY = trailY;
            } else if (!anim.blocked) {
                trailAttr = getOnFireTrailAttr(anim.shooter, anim.step, LIGHTGRAY | WAS_BROWN);
            }
            
            // Only draw if we should (skip when blocked and position unchanged)
            if (shouldDrawTrail) {
                trailFrame.setData(trailX - 1, trailY - 1, trailChar, trailAttr, false);
                if (typeof debugLog === "function" && anim.step === 1) {
                    debugLog("[TRAIL SHOT] Drew trail at x=" + (trailX - 1) + " y=" + (trailY - 1) + " attr=" + trailAttr + (anim.blocked ? " (BLOCKED)" : ""));
                }

                // Track position for cleanup - but for blocked shots, store separately so trail persists
                if (anim.trailPositions) {
                    if (anim.blocked) {
                        // Store block trail positions in state so they persist until rebound secured
                        if (this.systems && this.systems.stateManager) {
                            var blockTrail = this.systems.stateManager.get("blockTrailPositions") || [];
                            blockTrail.push({ x: trailX - 1, y: trailY - 1 });
                            this.systems.stateManager.set("blockTrailPositions", blockTrail, "block_trail_add");
                        }
                    } else {
                        anim.trailPositions.push({ x: trailX - 1, y: trailY - 1 });
                    }
                }
            }
        }
    ...
    /**
     * Clear block trail from trail frame
     * Called when rebound is secured after a blocked shot
     */
    this.clearBlockTrail = function () {
        if (!this.systems || !this.systems.stateManager) return;
        
        var blockTrail = this.systems.stateManager.get("blockTrailPositions");
        if (!blockTrail || !blockTrail.length) return;
        
        if (trailFrame) {
            var hasClearData = typeof trailFrame.clearData === "function";
            var hasSetData = typeof trailFrame.setData === "function";
            for (var i = 0; i < blockTrail.length; i++) {
                var pos = blockTrail[i];
                if (!pos) continue;
                if (hasClearData) {
                    trailFrame.clearData(pos.x, pos.y, false);
                } else if (hasSetData) {
        ...
        this.systems.stateManager.set("blockTrailPositions", [], "block_trail_cleared");
        if (typeof debugLog === "function") {
            debugLog("[ANIM] Cleared " + blockTrail.length + " block trail positions");
        }
    };
