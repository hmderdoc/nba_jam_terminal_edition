...
function AnimationSystem() {
        ...
        for (var i = 0; i < this.animations.length; i++) {
            ...
            if (anim.step < anim.maxSteps && now >= anim.nextStepTime) {
                ...
                if (anim.type === "shot") {
                    ...
                    // CRITICAL: Check if scramble was resolved AFTER each bounce step
                    // Don't wait for maxSteps - check immediately
                    if (this.systems && this.systems.stateManager) {
                        var reboundActive = this.systems.stateManager.get('reboundActive');
                        if (!reboundActive) {
                            debugLog("[ANIM] Idle bounce: scramble resolved, completing animation");
                            this.completeAnimation(anim);
                            completedIndices.push(i);
                            continue; // Skip to next animation
                        }
                ...
                if (anim.type === "rebound" && anim.currentBounce < anim.bounces.length - 1) {
                    ...
                    if (this.systems && this.systems.stateManager) {
                        ...
                        if (reboundActive) {
                            ...
                            anim.lastBounce = { vx: vx, vy: vy };  // Pass velocity to idle animation
                            anim.msPerStep = 50;          // Slightly slower cadence
                            anim.maxSteps = 999999;       // Unlimited - runs until secured
                            anim.step = 0;
                            anim.nextStepTime = now + anim.msPerStep;
                            anim.trailPositions = [];     // Reset trail tracking
                            debugLog("[ANIM] Rebound converted to idle bounce at (" + anim.centerX + "," + anim.centerY + ") with velocity (" + vx.toFixed(2) + "," + vy.toFixed(2) + ")");
                        } else {
                            debugLog("[ANIM] Completing " + anim.type + " animation, affectsBall=" + anim.affectsBall);
                            this.completeAnimation(anim);
                            completedIndices.push(i);
    ...
    this.updateIdleBounceAnimation = function (anim) {
        // Physics-based bouncing ball with momentum decay
        // Initialize velocity on first step
        if (anim.step === 0) {
            // Get initial velocity from last rebound bounce if available
            var lastBounce = anim.lastBounce || { vx: 0, vy: 0 };

            // Start with some random velocity if none provided
            anim.vx = lastBounce.vx || (Math.random() * 4 - 2); // -2 to +2
            anim.vy = lastBounce.vy || (Math.random() * 4 - 2);

            // Ensure minimum velocity
            if (Math.abs(anim.vx) < 0.5 && Math.abs(anim.vy) < 0.5) {
                anim.vx = Math.random() > 0.5 ? 1.5 : -1.5;
                anim.vy = Math.random() > 0.5 ? 1.5 : -1.5;
            }
        ...
        // Bounce off court boundaries (like billiard ball)
        var bounced = false;
        if (anim.x <= 2) {
            anim.x = 2;
            anim.vx = Math.abs(anim.vx) * 0.8; // Reverse and dampen
            bounced = true;
        } else if (anim.x >= COURT_WIDTH - 1) {
            anim.x = COURT_WIDTH - 1;
        ...
        if (anim.y <= 2) {
            ...
            bounced = true;
        } else if (anim.y >= COURT_HEIGHT - 1) {
            anim.y = COURT_HEIGHT - 1;
            anim.vy = -Math.abs(anim.vy) * 0.8;
            bounced = true;
        }
        ...
        if (!bounced) {
            anim.vx *= 0.95; // 5% decay per frame
            anim.vy *= 0.95;
        }

        // Stop if velocity too low (ball has settled)
        var speed = Math.sqrt(anim.vx * anim.vx + anim.vy * anim.vy);
        if (speed < 0.1) {
            anim.vx = 0;
            anim.vy = 0;
        }

        var x = Math.round(anim.x);
        var y = Math.round(anim.y);
        ...
        // Trail for moving ball
        if (speed > 0.5 && anim.step > 0 && anim.step % 2 === 0 && trailFrame) {
    ...
    this.getIdleBouncePosition = function () {
        for (var i = 0; i < this.animations.length; i++) {
            var anim = this.animations[i];
            if (anim && anim.type === "rebound_idle" && anim.x !== undefined && anim.y !== undefined) {
                return {
                    x: Math.round(anim.x),
                    y: Math.round(anim.y)
                };
