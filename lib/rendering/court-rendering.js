...
function drawBaselineTeamNames() {
    // Draw team names vertically on left and right baselines
    var teamBTeamName = gameState.teamNames.teamB || "BLUE";
    var teamATeamName = gameState.teamNames.teamA || "RED";
    ...
    var teamBBaseline = getTeamBaselineColors("teamB");
    var teamABaseline = getTeamBaselineColors("teamA");
...
function updateBallPosition() {
    ...
    // DEBUG: Log what ballCarrier is when rendering
    try {
        var f = new File("/sbbs/repo/xtrn/nba_jam/pass-debug.txt");
        f.open("a");
        f.writeln("updateBallPosition called:");
        f.writeln("  ballCarrier: " + (player && player.playerData ? player.playerData.name : "NULL"));
        f.writeln("  position: (" + (player ? player.x : "?") + "," + (player ? player.y : "?") + ")");
        f.close();
    } catch (e) { }
    ...
    var targetBasket = gameState.currentTeam === "teamA" ? BASKET_RIGHT_X : BASKET_LEFT_X;

    // Get player's bearing or default towards their basket
    var bearing = player.bearing || (gameState.currentTeam === "teamA" ? "e" : "w");
