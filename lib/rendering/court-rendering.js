...
function updateSpriteRendering(systems) {
    ...
    // Bring each sprite to top in order and ensure frame is marked for redraw
    // WAVE 24 FLICKER FIX: invalidate() ensures frame data is pushed on next cycle
    for (var i = 0; i < validPlayers.length; i++) {
        var frame = validPlayers[i].frame;
        if (typeof frame.invalidate === "function") {
            frame.invalidate();
        }
        frame.top();
    }
