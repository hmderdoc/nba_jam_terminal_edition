...
function applyUniformMask(sprite, options) {
    ...
    for (var bearingIndex = 0; bearingIndex < bearingCount; bearingIndex++) {
        ...
        for (var positionIndex = 0; positionIndex < positionCount; positionIndex++) {
            ...
            // Scan row 0 only for eye/eyebrow overrides (eyes are on the top row)
            // Row 0 (0-indexed) is the first row of the sprite where eyes appear
            var eyeRowY = 0;  // 0-indexed: row 0 is the eye row
            for (var localX = 0; localX < width; localX++) {
                var globalX = xOffset + localX;
                var globalY = yOffset + eyeRowY;
                ...
                if (ch === 'O' || ch === 'o' || ch === '0') {
                    ...
                    recordRowEye(eyeRowY, globalX, finalAttr);
                }
            }
