...
load(js.exec_dir + "mp_team_data.js");
...
function NBAJamLobby() {
    ...
    this.refreshTeamSelectSession = function(forceDefaults) {
        ...
        var mySide = null;
        if (session.teams.red.players.indexOf(myId) !== -1)
            mySide = "red";
        else if (session.teams.blue.players.indexOf(myId) !== -1)
            mySide = "blue";
        ...
        if (forceDefaults) {
            var defaultKey = null;
            var defaultRoster = 0;
            if (mySide === "red" && session.teams.red.name) {
                defaultKey = session.teams.red.name;
                var redChoice = session.teams.red.roster[myId];
                if (redChoice && typeof redChoice.index === "number")
                    defaultRoster = redChoice.index;
            } else if (mySide === "blue" && session.teams.blue.name) {
                defaultKey = session.teams.blue.name;
                var blueChoice = session.teams.blue.roster[myId];
            ...
            if (!defaultKey && this.teamSelect.teamList.length > 0) {
                var opponentSide = mySide === "red" ? "blue" : (mySide === "blue" ? "red" : null);
                for (var i = 0; i < this.teamSelect.teamList.length; i++) {
                    var entry = this.teamSelect.teamList[i];
                    var key = entry.key;
                    var redCount = (session.teams.red.name === key) ? session.teams.red.players.length : 0;
                    var blueCount = (session.teams.blue.name === key) ? session.teams.blue.players.length : 0;
    ...
    this.ensureTeamSelectIndices = function(options) {
        ...
        var myId = this.myId.globalId;
        var red = session.teams.red;
        var blue = session.teams.blue;
        var newSide = null;
    ...
    this.showSplash = function() {
        ...
        // Default splash
        console.center("\1h\1rN\1yB\1wA \1cJ\1gA\1mM\1n");
        console.crlf();
        console.center("\1n\1wMultiplayer Lobby");
        console.crlf();
        console.center("\1n\1kVersion " + LOBBY_VERSION);
        console.crlf(2);
        mswait(1500);
    };
    ...
    this.selectServerInteractive = function() {
        ...
        if (config) {
            console.clear();
            console.print("\1h\1wUse saved server: \1c" + config.server.name + "\1w? (Y/n): \1n");
    ...
    this.connectToServer = function() {
        console.clear();
        console.print("\1h\1cConnecting to \1w" + this.serverConfig.server.name + "\1c...\1n\r\n");
        ...
        try {
            ...
            if (!this.client.connected) {
                console.print("\1r\1hConnection failed!\1n\r\n\r\n");
            ...
            console.print("\1g\1hConnected!\1n\r\n");
    ...
    this.updateChat = function() {
        ...
        if (!this.chat || !this.chat.channels || !this.chat.channels["#NBA_JAM"]) {
            return;
        }

        var channel = this.chat.channels["#NBA_JAM"];
        ...
        for (var i = startIndex; i < messages.length; i++) {
            ...
            if (!msg.nick || !msg.nick.name) {
                // System message
                this.chatFrame.putmsg("\1n\1k[" + msg.msg + "]\1n\r\n");
            } else {
                // Player message
                ...
                this.chatFrame.putmsg(format("\1h\1c%-12s\1n\1w %s\1n\r\n",
                    displayName,
                    msg.msg));
            }
        }
    ...
    this.renderMainLobby = function() {
        ...
        // Header
        this.listFrame.putmsg(format("\1h\1w Available Games: \1c%d\1w  Players Online: \1c%d\1n\r\n",
            this.sessions.length, totalPlayers), WHITE | BG_BLACK);
        this.listFrame.putmsg(format("\1n\1w%-4s %-20s %-10s %-8s\1n\r\n",
            "##", "Host", "Players", "Age"), LIGHTGRAY | BG_BLACK);

        if (this.sessions.length === 0) {
            this.listFrame.putmsg("\1n\1k  No games available. Press \1h\1wC\1n\1k to create one.\1n\r\n");
        } else {
    ...
    this.renderSessionLobby = function() {
        ...
        // Session info
        this.listFrame.putmsg(format("\1h\1wGame: \1c%s\1n\r\n", this.currentSession.substring(0, 40)), WHITE | BG_BLACK);
        this.listFrame.putmsg(format("\1n\1wHost: \1c%s\1n\r\n", session.hostPlayer.displayName), LIGHTGRAY | BG_BLACK);
        this.listFrame.putmsg("\1n\1w-------------------------------------------------\1n\r\n", LIGHTGRAY | BG_BLACK);

        var redTeamData = (session.teams && session.teams.red) || { name: null, players: [], roster: {} };
        var blueTeamData = (session.teams && session.teams.blue) || { name: null, players: [], roster: {} };
        var redTeamDef = redTeamData.name ? MPTeamData.getTeam(redTeamData.name) : null;
        var blueTeamDef = blueTeamData.name ? MPTeamData.getTeam(blueTeamData.name) : null;
        var redTeamName = redTeamDef ? redTeamDef.name : (redTeamData.name ? redTeamData.name.toUpperCase() : "(Select team)");
        var blueTeamName = blueTeamDef ? blueTeamDef.name : (blueTeamData.name ? blueTeamData.name.toUpperCase() : "(Select team)");

        this.listFrame.putmsg(format("\1n\1wTeams:\1n Team 1=%s  Team 2=%s\r\n", redTeamName, blueTeamName), LIGHTGRAY | BG_BLACK);
        this.listFrame.putmsg("\1n\1w-------------------------------------------------\1n\r\n", LIGHTGRAY | BG_BLACK);

        this.listFrame.putmsg(format("\1h\1w%-20s %-24s %-10s\1n\r\n", "Player", "Selection", "Status"), WHITE | BG_BLACK);
        ...
        for (var i = 0; i < session.playerList.length; i++) {
            ...
            var readyStr = ready ? "\1g\1h[READY]\1n" : "\1r\1h[NOT READY]\1n";
        ...
        renderCpuPartner.call(this, "Team 1", redTeamData, redTeamDef);
        renderCpuPartner.call(this, "Team 2", blueTeamData, blueTeamDef);
    ...
    this.renderTeamSelect = function() {
        ...
        if (!this.teamSelect.active) {
            this.listFrame.clear();
            this.listFrame.home();
            this.listFrame.putmsg("\1h\1cTeam Selection Unavailable\1n\r\n", WHITE | BG_BLACK);
            this.listFrame.invalidate();
            return;
        }
        ...
        if (!teamList || teamList.length === 0) {
            this.listFrame.clear();
            this.listFrame.home();
            this.listFrame.putmsg("\1h\1cNo Teams Available\1n\r\n", WHITE | BG_BLACK);
            this.listFrame.invalidate();
            return;
        }
        ...
        var opponentSide = mySide === "red" ? "blue" : (mySide === "blue" ? "red" : null);
        var redTeamData = session.teams.red;
        var blueTeamData = session.teams.blue;
        ...
        this.listFrame.clear();
        this.listFrame.home();
        this.listFrame.putmsg("\1h\1wTEAM & PLAYER SELECT\1n\r\n", WHITE | BG_BLACK);
        ...
        for (var i = 0; i < visibleCount; i++) {
            ...
            var key = entry.key;
            var redCount = (redTeamData.name === key) ? redTeamData.players.length : 0;
            var blueCount = (blueTeamData.name === key) ? blueTeamData.players.length : 0;
            var total = redCount + blueCount;
            var pointer = (idx === this.teamSelect.index) ? "\1h\1c>\1n" : " ";
            var status;
        ...
        if (selectedEntry && selectedTeamDef) {
            this.listFrame.putmsg(format("\1h\1w%s Roster\1n\r\n", selectedTeamDef.name), WHITE | BG_BLACK);
            ...
            if (roster.length === 0) {
                ...
                for (var r = 0; r < roster.length; r++) {
                    ...
                    var redOwner = findOwner(redTeamData, r);
                    var blueOwner = findOwner(blueTeamData, r);
                    if (redOwner) {
                        occupantId = redOwner;
                        occupantSide = "red";
                    } else if (blueOwner) {
                        occupantId = blueOwner;
                        occupantSide = "blue";
                    }
                    ...
                    } else if (redTeamData.name === selectedEntry.key && redTeamData.cpuIndex !== null && clampRosterIndexLocal(selectedTeamDef, redTeamData.cpuIndex) === r) {
                        occupantLabel = "(CPU Partner)";
                        occupantAttr = LIGHTCYAN | BG_BLACK;
                    } else if (blueTeamData.name === selectedEntry.key && blueTeamData.cpuIndex !== null && clampRosterIndexLocal(selectedTeamDef, blueTeamData.cpuIndex) === r) {
                        occupantLabel = "(CPU Partner)";
                        occupantAttr = LIGHTCYAN | BG_BLACK;
                    }

                    var pointer = (selectedIndex === r) ? "\1h\1c>\1n" : " ";
    ...
    this.renderStatusBar = function() {
        ...
        if (this.networkMonitor) {
            var display = this.networkMonitor.getQualityDisplay();
            this.statusFrame.putmsg(format("Network: %s%s %dms\1n  ",
                display.color,
                display.bars,
        ...
        switch (this.state) {
            case LOBBY_STATE.MAIN_LOBBY:
                helpText = "\1h\1wC\1n=Create  \1h\1wJ\1n=Join  \1h\1wR\1n=Refresh  \1h\1wQ\1n=Quit  \1h\1wArrows\1n=Select";
                break;
            case LOBBY_STATE.SESSION_LOBBY:
                helpText = "\1h\1wT\1n=Team Select  \1h\1wR\1n=Ready  \1h\1wL\1n=Leave  \1h\1wEnter\1n=Chat";
                break;
            case LOBBY_STATE.TEAM_SELECT:
                helpText = "\1h\1wArrows\1n=Team  \1h\1w1-6\1n=Player  \1h\1wEnter\1n=Confirm  \1h\1wC\1n=Clear  \1h\1wEsc\1n=Cancel";
                break;
        }
    ...
    this.renderInputLine = function() {
        ...
        console.print("\1n" + BG_BLUE + "\1h\1w> \1n\1h" + this.inputLine + "\1n");
    };
    ...
    this.sendChatMessage = function(message) {
        if (!message)
            return;
        if (!this.chat) return;
        this.chat.submit("#nba_jam", message);
    };
